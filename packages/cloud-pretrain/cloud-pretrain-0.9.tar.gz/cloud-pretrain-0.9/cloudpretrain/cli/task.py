...
def train(model_type, job_name, dataset_name, dataset_version, dataset_group, dataset_public, \
    seq_length, num_epochs, batch_size, learning_rate, num_gpus, cp_filter, enable_tensorboard):

    model_type = Bert_size_enum[model_type]

    dataset_folder = gen_dataset_folder(dataset_name, dataset_version, dataset_public, dataset_group)
    ...
    if not is_dataset_exists(dataset_name, dataset_version, dataset_public, dataset_group):
    ...
    dataset_content = load_dataset_des(dataset_name, dataset_version, dataset_public, dataset_group)

    problem = json.loads(dataset_content)["problem"]

    click.echo(info("we will use your best-effort v100-32g gpu for the tasks (2 gpu for each sub-tasks)"))

    if model_type == Bert_size_enum.LARGE and batch_size > 32:
        click.echo(error("for bert large, fix the batch size to 32 (for 2 gpus) to avoid the oom"))
        batch_size = 32
        num_gpus = 2
    ...
    for model_name in tqdm(model_names, desc="Submitting"):
        cloudml_task_names.append(_submit_single_train_job(job_name, model_name, problem, dataset_folder, seq_length, \
            batch_size, learning_rate, num_epochs, bucket_name, enable_tensorboard))
...
def predict(job_name, gpu_type, gpu_memory, gpu_number, priority_class):
    ...
    for cloud_ml_task in tqdm(cloud_ml_tasks, desc="Submiting"):
        ...
        pretrained_models = "/fds/pretrained_models/{}".format(model_name)
...
def export(job_name, metric_stage, metric, fast, fp16, num_labels, use_all_tokens):
    ...
    for cloudml_task in tqdm(cloudml_tasks, "Exporting"):
        ...
        bert_config_path = "pretrained_models/{}/bert_config.json".format(model_name)
...
def serving(job_name, saved_model_folder, deploy_type, replicas, gpu_type, gpu_memory, \
    ...
    model = ConfiguredModelService(model_name=serving_name, model_version='1', model_uri=to_mount_uri(saved_model_folder))
    if online:
    ...
    if use_server_batching:
        batching_params = " --enable_batching=true --batching_parameters_file={}".format(to_mount_uri(batching_config))
...
def _submit_single_train_job(job_name, base_model, problem, dataset_folder, seq_length, \
    ...
    with create_temp_dir() as trainer_uri:
        ...
        bert_job.config_quota(gpu_limit=num_gpus)
