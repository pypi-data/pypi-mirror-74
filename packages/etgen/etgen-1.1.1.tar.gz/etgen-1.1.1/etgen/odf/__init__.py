...
It uses `lxml <http://lxml.de/>`__ and a copy of the original RelaxNG
schema
(:srcref:`/etgen/odf/relaxng/OpenDocument-v1.2-os-schema.rng`)
...
if False:
    for prefix, url in rng_tree.getroot().nsmap.items():
        if prefix is not None:
            ...
            globals()[prefix] = Namespace(prefix, url)
...
def render_to_odt(target_file, startfile=False, **context):
    ...
    #~ context = dict(body=body,self="etgen.odf example")
...
def unused_table2odt(headers, fields, widths, rows):
    ...
    t = table.table(
        table.makeattribs(name="Mytable", style_name="Mytable"),
        table.table_column(
            table.makeattribs(style_name="A", number_columns_repeated="2")),
        table.table_row(
            table.table_cell(text.p("First"),
                             table.makeattribs(value_type="string")),
            table.table_cell(text.p("Second"),
                             table.makeattribs(value_type="string")),
        ),
        table.table_row(
            table.table_cell(text.p("Third"),
                             table.makeattribs(value_type="string")),
            table.table_cell(text.p("Fourth"),
                             table.makeattribs(value_type="string")),
        ),
    )
    validate_chunks(t)
