...
def remove_html_style_tag(page_content: str) -> str:
    page_content_cleaned = re.sub(
        r'/\* vertical_spacer_plugin \*/.*/\* vertical_spacer_plugin_end \*/',
        '',
        page_content,
    )
    return page_content_cleaned
...
class AlgoliaPageDataProxy(Title):
    ...
    def page_content(self) -> str:
        ...
        page_content = remove_html_style_tag(page_content)
        is_below_limit = len(page_content) < 10000
        if is_below_limit:
            return page_content
        else:
            return ''
