    ...
    def resolve(self, service, path=""):
        if service not in self.services:
            raise UnknownServiceError(
                ...
                known_service_names=", ".join(sorted(self.services.keys())),
            )

        if self.alfa_env not in self.services[service]:
            raise ServiceEnvironmentError(
                service_name=service, environment=self.alfa_env
            )

        service_name = self.services[service][self.alfa_env]
        base_url = self.hostname.format(service=service_name, domain=self.domain)
...
class AlfaConfigHelper:
        ...
        try:
            with zipfile.ZipFile(file_path) as zip:
                if "alfa.yml" not in zip.namelist():
                    raise AlfaConfigError(
                        message=message, error="alfa.yml not found in package"
                    )
    ...
    def load(file_path, is_package=False):
        ...
        try:
            ...
            raise AlfaConfigError(
                message="Failed to read file from path", error=str(err)
            )
