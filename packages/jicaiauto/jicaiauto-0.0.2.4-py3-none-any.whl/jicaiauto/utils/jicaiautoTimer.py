...
from tkinter import Tk, Label, Entry, Button, Frame, Listbox, Scrollbar, END, LEFT, RIGHT, Y, StringVar
from tkinter.messagebox import showerror
from jicaiauto.config.config import TIMERCONFIG
...
def _jiexi(_type='min', _timer='* * * * *'):
    msg = '''
        ...
        week    1 ... 7
        表达式：
        * or 1,2,3-7/2 or 1-7/2 or 1,2,3-7 or */2 or 1-7 or 1,2,3
    '''
    _min = list(range(60))
    _hour = list(range(24))
...
def _check_title():
    while 1:
        if IS_RUN:
            root.title('吉彩自动化定时器 v0.1 【运行中】' + str(datetime.now())[:-7])
        else:
            root.title('吉彩自动化定时器 v0.1 ' + str(datetime.now())[:-7])
        sleep(1)
...
Entry(master=frame00, textvariable=_t).pack()
Label(master=frame01, text='设置当前路径：').pack()
Entry(master=frame01, textvariable=_p).pack()
frame0.pack()
frame00.pack(side=LEFT, fill=Y)
frame01.pack(side=RIGHT, fill=Y)
Label(master=root, text='执行命令：').pack()
Entry(master=root, textvariable=_c, width=50).pack()
