...
async def statusdata2log(station, **data):
    ...
    try:
        ...
        if installation:
            this_time = datetime.fromtimestamp(
                data.get('BATT_MEM').get('TIMESTAMP')).astimezone(pytz.utc)
            data_log = {
        ...
        print("No existe el atributo para el elemento", data, type(data), ae)
...
class DjangoDataManager:
    ...
    async def cycle_read_from_rmq(self, *args):
        engine = args[0]
        v = args[1]
        channels = args[2]
        if v == 0:
