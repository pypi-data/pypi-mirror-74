...
def featurize_conllu(conllu, feature_spec, freq_dict=None):
    ...
    class Token:
        def __init__(self, word="_", lemma="_", pos="_", func="_", parent="_", label="O", idx=1,descendents_len=0,
                     freq=0.0, sent_len=1):
            self.word = word
            ...
            self.label = label
            self.idx = int(idx)
            self.percentile = int(idx)/sent_len
    ...
    def get_feats(prev_tok, tok, next_tok, specs, freq_dict=None):
        feat_dict = {"bias": 1.0}
        ...
        for key in specs:
            ...
            elif attr in ["pos","lemma","func","parent","idx","descendents_len","freq","sent_len","percentile","length"]:
    ...
    for sent in sents:
        ...
        for line in lines:
            if "\t" in line:
                ...
                freq = freq_dict[word]
                sent_len = len(lines)
                toks.append(Token(word=word, lemma=lemma, pos=xpos, func=func, parent=parent, label=label, idx=tid,
