    ...
    def save(self, yoc_file=None):
        ...
        with codecs.open(yoc_file, 'w', 'UTF-8') as f:
            for k, v in self.__dict__.items():
                ...
                if k not in ['yoc_path', 'init'] and v:
                    f.write("{}: {}\n".format(k, v))
        self.init = True
...
class YoC:
    def __init__(self):
        ...
        if not self.conf.yoc_version:
            put_string("Please add yoc_version filed in .yoc file! e.g.\nyoc_version: v7.3.0")
            exit(-1)
            ...
            if component.path == os.getcwd():
                if board_name:
                    if self.components.get(board_name):
                        ...
                        exit(-1)
                components, lost = self.components.get_depend(component)
                if lost:
                    exit(-1)
    ...
    def upload(self, name):
        ...
        if component:
            ...
            if not os.path.isdir(os.path.join(component.path, '.git')):
                ...
                if self.occ.conf.yoc_version:
                    version = self.occ.conf.yoc_version
