...
class Component:
    def __init__(self, conf, filename=''):
        ...
        self.repo = None
        self.branch = self.conf.yoc_version
        self.source_files = []
        self.defconfig = {}
    ...
    def download(self, yoc_path=''):
        if True:
            ...
            # put_string("%s(%s), clone %s %s ..." % (self.name, self.version, self.repo_url, self.path))
            # put_string("%s(%s) ..." % (self.name, self.version))
            prj.pull(self.branch, None)
    ...
    def upload(self):
        ...
        try:
            ...
            prj.pull(self.branch)
            prj.import_path(self.path, self.branch)
