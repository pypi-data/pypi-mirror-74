...
# @Last Modified time: 2020-07-23 21:30:14
...
class LocalServer(object):
	...
	def upzipJar(self,jar_list="",jarDir="",specialServiceList=[],special_deal=False):
		"""
		...
		if self.alreadyRecord.service_name in specialServiceList and not special_deal:
			...
			os.popen("rm -rf /home/maoyongfan10020/jc/{service_name}/all/{service_name}".format(
										service_name=self.alreadyRecord.service_name
										)).read()
	...
	def jacocoReport(self):
		...
		if (len(eval(self.alreadyRecord.server_ip)))==1:
			...
			report_command = ("cd {jacocoDir};java -jar jacococli.jar report " +\
					self.jacocoIt + " --classfiles {classfile} --sourcefiles {sourcefiles} --html {reportDir} --xml {xml}").format(
