...
def add_data_logger(name, signals, data_file, use_suffix=True):
    """
    Adds one logger to the model that will collect ``signals`` data and store them to the
    specified ``data`` file.

    .. note::
        Only streaming analog and digital signals can be continuously logged to the data file.

    .. note::
        Data logger will start to collect data only after simulation and data logger are started.

    .. note::
        Each time data logger is started it will overwrite old data file in case ``use_suffix=False`` otherwise it will
        create new file with the current data and time suffix.
    ...
    Args:
        ...
        signals (list): list of streaming analog or digital signals' names
            ...
            .. note::
                Only streaming signals that works on the same execution rates are supported to be logged in one data logger.

        data_file (string): file path to the data file where collected data are going to be saved.

            .. note::
                For now only Comma Separated Values (.csv) and HDF5 (.h5) data files dormats are supported.

            .. note::
                It is not recommended to use .csv data format for high data transfer rates.

        use_suffix (bool): suffix with date and time will be added to the end of given data file name

    Returns:
        status (bool): ``True`` if data logger is successfully added, otherwise returns ``False``
    ...
    Example::
        ...
        # to open and parse saved data files you can use excellent Python Pandas library
        import pandas as pd
...
def set_source_scaling(name, scaling, executeAt=None, ramp_time=0, ramp_type='lin'):
    ...
    .. note::
        This function is no longer supported on HIL400 devices!
    ...
    """
        ...
        Args:
            cpSettings (list): list[``decimation``, ``numberOfChannels``, ``numberOfSamples``, ``enableDigitalCapture``]
                ...
                * decimation (int): capture the decimation value (int value).
...
def get_device_features(device=None, conf_id=None, feature=None):
    ...
    Args:
        device (str): HIL device name (in format 'HIL602', 'HIL402'...)
...
def get_hw_info():
    ...
    Returns:
        ...
        If an error occurred ``None`` will be returned instead.
            * product ID (str): product ID in the format "HIL402, HIL602...".
