...
class Graph(object):
    ...
    def astar(self, start: Node, end: Node, g_func: Callable[[Node, Node], float] = None, h_func: Callable[[Node, Node], float] = None) -> List[Node]:
        ...
        while len(open_set) > 0:
            ...
            for new_graph_node in self._graph_dict[current_node.graph_node]:  # Adjacent nodes
                if self._edge_at(current_node.graph_node.pos, new_graph_node.pos).enabled():
                    # Create new node
