...
#from dsnd_probability_14072020.Generaldistribution import Distribution
...
class Binomial(Distribution):
 ...
 # a distribution. Hence you are going to write a method that calculates n, p, mean and
    ...
    def replace_stats_with_data(self):
        ...
        success = np.count_nonzero(arr == 1)
            
        self.p= success / len(self.data)
    ...
    def pdf(self,k):
        ...
        Numerator = math.factorial(self.n) * math.pow(self.p,k) * math.pow((1-self.p),(self.n -k))
        Denominator = math.factorial(self.n-k) * math.factorial(k)
        pdf_output = Numerator / Denominator
        return pdf_output
    ...
    def plot_pdf():
        ...
        pdf_list=[]
        k_list =  [k for k in range (0, self.n+1)]
        
        for k in range (0, self.n+1):
            pdf_list.append(pdf(k))
    ...
    def __repr__(self):
        ...
        print("Test: mean {}, standard deviation {}, p {}, n {}".\
        ...
        """
        obj_dict = Binomial.__dict__
        #sorted_dict =   collections.OrderedDict(sorted(obj_dict.items()))
        returned_string =""
        for k,v in obj_dict.items():
            if k == 'mean':
                returned_string = returned_string + 'mean ' + str(v) + ','
            if k == 'stdev':
                returned_string = returned_string + 'standard deviation ' + str(v) + ','
            if k == 'p':
                returned_string = returned_string + 'p ' + str(v) + ','
            if k == 'n':
                returned_string = returned_string + 'n ' + str(v)
