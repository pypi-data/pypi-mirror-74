...
class Graph(ctypes.Structure):
	...
	def drawWithGraphviz(self,filename,nodenames=None):
		if self.destroyed:
			return
		import os
	...
	for t in range(columns):
		...
		if t in targets:# and distinct > num_states:
			...
			if progress_hook is not None:
				progress_hook(t,columns,"DISCRETIZE")

			if discretization is None and num_states is not None:
				...
				lib.discretize_precomputed(ctypes.c_uint64(result.ctypes.data), ctypes.c_uint64(col_data.ctypes.data), ctypes.c_uint64(len(data)), disc_info)
...
def train(data=None, graph=None, mode = ModelType.mrf, inference = InferenceType.belief_propagation, iters = 1000, seed = 0, k = 3, input_model = None, initial_stepsize = 0.1, opt_progress_hook = 0, progress_hook = 0, regularization = 0, proximal_operator = 0, T = 1, threshold = 0.1, lambda_proximal = 0, lambda_regularization = 0, shared_states = False, optimizer = Optimizer.accelerated_proximal_gradient, zero_init=False, clique_size = 3):
	...
	if input_model is None:
		...
		if not isinstance(graph, Graph) and (graph == GraphType.auto or graph == GraphType.elemgm):
