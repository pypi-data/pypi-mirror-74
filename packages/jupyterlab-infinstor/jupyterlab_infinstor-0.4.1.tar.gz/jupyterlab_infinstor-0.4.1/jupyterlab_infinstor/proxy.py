...
class ProxyHandler(WebSocketHandlerMixin, IPythonHandler):
    ...
    async def proxy(self, host, port, proxied_path):
        ...
        if (proxied_path == '/' or proxied_path.startswith('/static-files')):
            host = 'mlflowstatic.' + builtins.service + '.com'
            print('Proxying static-files call. after mods: host=' + str(host) + ', port=' + str(port) + ', proxied_path=' + str(proxied_path));
            rest_call = False
        elif (proxied_path.startswith('/ajax-api/2.0/preview')):
            # print('Proxying REST call. host=' + str(host) + ', port=' + str(port) + ', proxied_path=' + str(proxied_path));
            host = 'mlflow.' + builtins.service + '.com'
            proxied_path = '/Prod/2.0' + proxied_path[len('/ajax-api/2.0/preview'):]
            print('Proxying REST call. after mods: host=' + str(host) + ', port=' + str(port) + ', proxied_path=' + str(proxied_path));
            rest_call = True
        else:
            host = 'mlflowstatic.' + builtins.service + '.com'
