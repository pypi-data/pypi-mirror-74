...
from ...ranges import Range, Excluded, Included
...
DEFAULT_MAX_BASES = 5
st.register_type_strategy(IntervalSet, float_intervals())
st.register_type_strategy(IntervalSet[float], float_intervals())
st.register_type_strategy(IntervalSet[datetime.datetime], date_intervals())
...
class DateRangeHelper(_RangeHelper):
    ...
    BASE_STRATEGY = staticmethod(
        lambda **kwargs: st.datetimes(
            timezones=st.just(pytz.UTC), allow_imaginary=False, **kwargs
        ).map(lambda d: d.replace(tzinfo=None))
    )
...
st.register_type_strategy(Range[datetime.datetime], date_ranges())
st.register_type_strategy(Range[Union[int, float]], float_ranges())
