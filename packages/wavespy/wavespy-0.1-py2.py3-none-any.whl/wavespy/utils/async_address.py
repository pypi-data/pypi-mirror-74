...
from wavespy.utils.address import (
    BaseWavesAddress, sign_required, DEFAULT_BURN_TRANSACTION_FEE,
...
class WavesAsyncAddress(BaseWavesAddress):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._api_client = WavesAsyncClient(
    ...
    async def get_address_info(self):
        ...
        """
        address_info = dict()
        address_info["valid"] = await self.validate()
        if not address_info["valid"]:
            return address_info

        address_info["balance"] = await self.get_balance()
        address_info["effective_balance"] = await self.get_effective_balance()
        address_info["assets"] = await self.get_assets()
        address_info["address_data"] = await self.get_address_data()
        return address_info
    ...
    @sign_required
    async def transfer_waves(self, recipient, amount, attachment=None, transaction_fee=DEFAULT_TRANSFER_TRANSACTION_FEE,
                             timestamp=0):
        """
        Send waves to address

        :param recipient: recipient address in base58
        :type recipient: str or WavesAddress or WavesAsyncAddress
        :param amount: amount of waves
        :type amount: int
    ...
    @sign_required
    async def mass_transfer_waves(self, transfer_data, attachment=None, timestamp=None):
        """
        Mass transfer waves
    ...
    @sign_required
    async def lease_waves(self, recipient, amount, transaction_fee=DEFAULT_LEASE_TRANSACTION_FEE, timestamp=0):
        """
    ...
    def __repr__(self):
        return "WavesAsyncAddress({})".format(self.value)
