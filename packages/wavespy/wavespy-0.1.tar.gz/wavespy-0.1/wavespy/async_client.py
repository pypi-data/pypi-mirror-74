...
from wavespy.client import WavesClientException, WavesClientResponse, BaseClient


waves_client_logger = logging.getLogger('wavespy.WavesClient')


class WavesAsyncClientResponse(WavesClientResponse):
    """Async API client response"""


class WavesAsyncClientException(WavesClientException):
    """Exception for async waves client"""


class WavesAsyncClient(BaseClient):
    """
    ...
    async def address_data_key(self, address, key):
        ...
        :return:
        :rtype: WavesAsyncClientResponse
        """
    ...
    async def start_session(self):
        ...
        waves_client_logger.debug("Started session for {}".format(self))
    ...
    async def close_session(self):
        ...
        :rtype: None
        """
        waves_client_logger.debug("Closing session for {}".format(self))
        await self.session.close()
        self.session = None
        waves_client_logger.debug("Session closed for {}".format(self))
    ...
    async def request(self, method, endpoint, params=None, data=None, json_data=None, headers=None, matcher=False):
        ...
        :rtype: WavesAsyncClientResponse or dict
        """
        request_params = self._setup_request_params(endpoint, params, data, json_data, headers, matcher)
        if not self.online:
            waves_client_logger.debug("Offline request '{}' in {}".format(endpoint, self))
        ...
        try:
            ...
            waves_client_logger.debug("Requesting '{}' in {}".format(endpoint, self))
            async with session_method(**request_params) as response:
                waves_client_logger.debug("Finished request '{}' in {}".format(endpoint, self))
    ...
    async def _handle_response(self, response, endpoint):
        ...
        :rtype: WavesAsyncClientResponse
        :raises: WavesAsyncClientException
        """
        ...
        return WavesAsyncClientResponse(successful=True, endpoint=endpoint, response_data=result)
