...
DEFAULT_NODE_ADDRESS = 'https://nodes.wavesplatform.com'
DEFAULT_MATCHER_ADDRESS = 'https://matcher.waves.exchange'
DEFAULT_CHAIN_ID = "W"
CHAIN_ID_NAMES = (('W', 'mainnet'), ('T', 'testnet'), ('S', 'devnet'))
DEFAULT_HEADERS = (('user-agent', 'wavespy-client'),)

client_logger = logging.getLogger('wavespy.WavesClient')


class WavesClientException(Exception):
    """Exception for Waves client"""


class WavesClientResponse:
    """
    API client response. Any API method of `WavesClient` returns `WavesClientResponse` with response data or error
...
class BaseClient:
    ...
    :type api_key: str
    :param raise_exception: raise WavesClientException on request error
...
class WavesClient(BaseClient):
    """
    Waves API client class based on `requests` HTTP client
    ...
    def request(self, method, endpoint, params=None, data=None, json_data=None, headers=None, matcher=False):
        ...
        :rtype: WavesClientResponse or dict
        """
    ...
    def _handle_response(self, response, endpoint):
        ...
        return WavesClientResponse(successful=True, endpoint=endpoint, response_data=result)
