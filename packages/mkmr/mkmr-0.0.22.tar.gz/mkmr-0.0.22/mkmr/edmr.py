...
def main():
    ...
    if not mrnum.isdigit():
        try:
            ...
            print("branch name given, {}, has no corresponding cache file".format(mrnum))
            sys.exit(1)
        else:
            # This is executed in a try-catch if there are no exceptions raised
            if mrnum == "":
                print("cache file for {} is empty".format(mrnum))
        ...
        if (
            ...
            if strtobool(val) is None:
                if not options.quiet:
                    print("value of {} ({}), is invalid, should be True or False".format(key, val))
                continue
            ...
            if not val.isdigit():
                if not options.quiet:
                    print("value of {} ({}), is invalid, should be an integer".format(key, val))
            else:
            ...
            for value in val.split():
                try:
                    ...
                    if not options.quiet:
                        print(
                            "key {} has invalid sub-value {} in value {}".format(key, value, val)
                        )
                else:
                    list_v.append(int_v)
