...
from mkmr.instance import Instance
...
def main():
    ...
    try:
        origin = Instance(options, remote=options.origin)
    except (ValueError, AttributeError) as e:
        err(e)
        sys.exit(1)
    except Instance.ProjectIdReset:
        msg("Resetted project-id for remote '{}'".format(options.origin))
        sys.exit(0)
    except Instance.BranchCacheCleaned:
        msg("Removed dangling branch references for remote '{}'".format(options.origin))
        sys.exit(0)

    try:
        upstream = Instance(options, remote=options.upstream)
    except (ValueError, AttributeError) as e:
        err(e)
        sys.exit(1)
    except Instance.ProjectIdReset:
        msg("Reset project-id for remote '{}'".format(options.origin))
        sys.exit(0)
    except Instance.BranchCacheCleaned:
        msg("Removed dangling branch references for remote '{}'".format(options.origin))
        sys.exit(0)
    ...
    if "gitlab.alpinelinux.org/alpine/aports" in upstream.gitlab.api_url:
        aports = True
    ...
    try:
        ...
        (upstream.cache.branches / source_branch).write_text(str(mr.attributes["iid"]))
    except ValueError:
        err(
