...
class QubitOperator(BasicGate):
    ...
    def __init__(self, term=None, coefficient=1.):
        ...
        if not isinstance(coefficient, (Number, SympyBase)):
            raise ValueError('Coefficient must be a numeric type or a sympy '
                             'expression!')
        self.terms = {}
        if term is None:
            return
    ...
    def compress(self, abs_tol=1e-12):
        ...
        for term in self.terms:
            ...
            if (isinstance(coeff, (int, float, complex))
                    and abs(coeff.imag) <= abs_tol):
                coeff = coeff.real
            elif (isinstance(coeff, SympyBase)
                  and abs(sympy.im(coeff)) <= abs_tol):
    ...
    def isclose(self, other, rel_tol=1e-12, abs_tol=1e-12):
        ...
        for term in set(self.terms).intersection(set(other.terms)):
            ...
            a_is_param = isinstance(coeff_a,
                                    SympyBase) and not coeff_a.is_number
            b_is_param = isinstance(coeff_b,
                                    SympyBase) and not coeff_b.is_number
            ...
            if a_is_param and b_is_param:
                ...
                if not cmath.isclose(
                        coeff_a, coeff_b, rel_tol=rel_tol, abs_tol=abs_tol):
                    return False
    ...
    def __or__(self, qubits):
        ...
        if ((isinstance(coefficient, Number) or coefficient.is_number)
                and (abs(coefficient) < 1 - EQ_TOLERANCE
    ...
    def __mul__(self, multiplier):
        ...
        if isinstance(multiplier,
                      (int, float, complex, SympyBase, QubitOperator)):
    ...
    def __str__(self):
        ...
        string_rep = ''
        for term in self.terms:
            if isinstance(self.terms[term], Number):
...
class IsUnitaryCoeffOpSentinel(sympy.Function):
    """
    ...
    def _eval_evalf(self, prec):
        try:
            val = self.args[0]._eval_evalf(prec)
            if abs(float(abs(val)) - 1) > EQ_TOLERANCE:
                raise UnitaryInverseError(
                    'Unable to invert QubitOperator: '
                    'coefficient is not close to 1 ({})'.format(val))
            return val
        except TypeError:
            raise UnitaryIsSymbolicError(
                'Unable to invert QubitOperator: '
                'coefficient is symbolic! ({})'.format(val))
