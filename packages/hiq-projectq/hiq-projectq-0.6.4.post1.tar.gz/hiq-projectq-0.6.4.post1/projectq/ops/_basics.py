...
class _ClassDescriptor:
    ...
    def __get__(self, obj, objtype):
        if (issubclass(objtype, DispatchGateClass)
                and len(objtype.__bases__) > 1 and any(
                    not issubclass(obj, DispatchGateClass)
                    for obj in objtype.__bases__)):
...
class BasicGate:
    ...
    def is_identity(self):
        """
        Checks whether the gate instance represent the identity

        Returns:
            True if the gate is the identity, False otherwise.
        """
        # pylint: disable=no-self-use
        return False
...
class BasicAngleGate(BasicGate):
    ...
    The continuous parameter is modulo _mod_pi * pi, self.angle is in the
    interval [0, _mod_pi * pi).
    """
    ...
    def __init__(self, angle):
        ...
        super().__init__()
        rounded_angle = round(
            float(angle) % (self.__class__._mod_pi * math.pi), ANGLE_PRECISION)
    ...
    def is_identity(self):
        ...
        # pylint: disable=protected-access
        return (self.angle == 0.
                or self.angle == self.__class__._mod_pi * math.pi)
...
class BasicRotationGate(BasicAngleGate):
    ...
    def tex_str(self):
        ...
        .. code-block:: latex
            ...
            [CLASSNAME]$_[ANGLE]$
        """
        return '{}$_{{{}\\pi}}$'.format(self.klass.__name__,
                                        round(self.angle / math.pi, 3))
