...
def get_config(key_ids, serial_stream, timeout=TIMEOUT_DEF):
...
def set_config(key_value_dict, serial_stream, timeout=TIMEOUT_DEF):
    """
    ...
    for i,(key_id,value) in enumerate(sorted_dict.items()):
        
        n_transaction = 2 if i else 1
        payload = struct.pack('<BBBB', 1, 7, n_transaction, 0)

        logger.debug('set_config::key_id [ {} ] --> [ {} ] (transaction {})'.format(key_id, value, n_transaction))
        try:
    ...
    if n_items_sent:
        payload = struct.pack('<BBBB', 1, 7, 3, 0)
        send(PACKET_TYPE, payload, serial_stream)
        wait_for_ack_nak(PACKET_TYPE, serial_stream, timeout=TIMEOUT_DEF)
...
def __go_to_next_packet__(stream, timeout):
    ...
    while(True):
        ...
        if byte_1 != PREAMBLE[0]:
            ...
            #logger.debug(f'Byte 1 {byte_1} preamble {PREAMBLE[0]}')
        else:
            byte_2 = int.from_bytes(stream.read(1), "big")
            #logger.debug(f'Byte 2 {byte_2} preamble {PREAMBLE[1]}')
