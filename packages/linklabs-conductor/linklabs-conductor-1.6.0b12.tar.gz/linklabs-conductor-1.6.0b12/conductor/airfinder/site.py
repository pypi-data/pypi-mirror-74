...
class Site(AirfinderSubject):
    ...
    def get_gateway(self, address):
        """ """
        x = self._get_registered_assets_by_site('gateways', address)
        return Gateway(self.session, x.get('nodeAddress'),
    ...
    # TODO
    def bulk_add_tags(self, file_path):
    ...
    def get_locations(self):
        ...
        for x in self._get_registered_assets_by_site('locations', 'none'):
            ...
            if obj:
                ...
                dev = x['assetInfo']['metadata']['props'].get('deviceType')
                LOG.error("No device conversion for {}".format(dev))
                nodes.append(Location(self.session, subject_id,
                                      self.instance, x))
        return nodes
