...
class CliConfigManager(BaseConfigManager):
    ...
    def reset(
        ...
        if not any([current_version, server_installation, log_handler, last_check]):
            return
        cli_config = cls.get_config_or_default()
        if current_version is not None:
            cli_config.current_version = current_version
        if server_installation is not None:
            cli_config.server_installation = server_installation
