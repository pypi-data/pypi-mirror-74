...
def get_server_installation(polyaxon_client=None, sys_exit: bool = True):
    ...
    try:
        ...
        if e.status == 403:
            session_expired()
        handle_cli_error(e, message="Could not get server version.", sys_exit=sys_exit)
    except HTTPError:
        Printer.print_error(
...
def get_compatibility(key: str, service: str, version: str, sys_exit: bool = True):
    if not key:
        key = uuid.uuid4().hex
    polyaxon_client = PolyaxonClient(
        config=ClientConfig(), token=NO_AUTH
    )
    try:
        ...
        handle_cli_error(
            e, message="Could not reach the compatibility API.", sys_exit=sys_exit
        )
    except HTTPError:
        CliConfigManager.reset(last_check=now())
        Printer.print_error(
...
def get_log_handler(polyaxon_client=None, sys_exit: bool = True):
    polyaxon_client = polyaxon_client or PolyaxonClient()
    try:
        ...
        if e.status == 403:
            session_expired()
        CliConfigManager.reset(last_check=now())
        handle_cli_error(e, message="Could not get cli version.", sys_exit=sys_exit)
    except HTTPError:
        CliConfigManager.reset(last_check=now())
        Printer.print_error(
            "Could not connect to remote server to fetch log handler.", sys_exit
        )
...
def set_versions_config(
    ...
    if set_installation:
        ...
        if not key and server_installation.key:
