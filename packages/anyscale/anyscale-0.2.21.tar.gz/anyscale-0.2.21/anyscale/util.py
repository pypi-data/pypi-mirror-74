...
def get_endpoint(endpoint: str) -> str:
    return "{}{}".format(anyscale.conf.ANYSCALE_HOST, endpoint)
...
def check_use_snapshot_feature_flag() -> bool:
    use_snapshot_resp = send_json_request(
        "/api/v2/userinfo/check_is_feature_flag_on",
        {"flag_key": FLAG_KEY_USE_SNAPSHOT},
    )

    return cast(bool, use_snapshot_resp["result"]["is_on"])


def get_active_sessions(project_id: int, session_name: str) -> Any:
...
def wait_for_session_start(project_id: int, session_name: str) -> int:
    print(f"Waiting for session {session_name} to start.")
    while True:
        sessions = get_active_sessions(project_id, session_name)
        if len(sessions) > 0:
            if not sessions[0]["starting_up"]:
                return cast(int, sessions[0]["id"])
        time.sleep(5)
