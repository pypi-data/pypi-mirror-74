...
from anyscale.util import (
    _get_role,
    _resource,
    check_use_snapshot_feature_flag,
    confirm,
...
def setup_aws_cross_account_role(email: str, region: str, user_id: int) -> None:
...
def setup_aws(region: str) -> None:
    ...
    setup_aws_cross_account_role(
        response["result"]["email"], region, response["result"]["id"]
    )
...
def register_project(project_definition: Any) -> None:
    ...
    if os.path.exists(project_id_path) or os.path.exists(
        ...
        raw_resp = send_json_request_raw(
            "/api/v2/projects/{project_id}".format(project_id=project_id), {}
        )
        if raw_resp.status_code == 401:
            ...
            json_resp: Dict[str, Any] = raw_resp.json()
            project = json_resp["result"][0]
...
def anyscale_start(
    ...
    if cluster_config_file:
        project_definition.config["cluster"]["config"] = cluster_config_file
...
@click.option("--register", help="Register a gateway.", is_flag=True)
@click.option(
    "--provider-path",
    help="The path to the provider yaml file.",
    type=str,
    required=True,
)
@click.option("--gateway-id", type=str, required=False, default=None)
def anyscale_cloudgateway(
    register: bool, provider_path: str, gateway_id: Optional[str]
) -> None:
    # Make sure only registered users can start the gateway.
    user_info = send_json_request("/api/v2/userinfo/", {})["result"]
    if register:
        if not gateway_id:
            logger.warning(
                "No gateway-id was provided. Generating one automatically..."
            )
            gateway_id = str(uuid.uuid4())
        provider_config = yaml.safe_load(open(provider_path).read())
        if provider_config["provider"]["region"]:
            region = provider_config["provider"]["region"]
        else:
            region = "N/A"

        logger.info("Registering gateway-id: " + gateway_id + " in the database...")
        send_json_request(
            "/api/v2/clouds/",
            {
                "provider": "CLOUDGATEWAY",
                "region": region,
        ...
        logger.info(
            "Successfuly registered the cloud gateway. To run the gateway run: \n"
            + '"anyscale cloudgateway --provider-path {0} --gateway-id {1}".'.format(
                provider_path, gateway_id
            )
        )
    else:
        if not gateway_id:
            ValueError("Please specify --gateway-id when running the gateway.")
        found = False
        response = send_json_request("/api/v2/clouds/", {})
        clouds = response["results"]
        for cloud in clouds:
        ...
        if not found:
            logger.error(
                ...
                + str(gateway_id)
                + " in the database. You can register a new gateway by omitting the gateway-id flag."
            )
            sys.exit()
            logger.info("Connecting to Anyscale ...")

        logger.info(
            f"""Your gateway-id is: {gateway_id}. Store it in the provider section in the cluster
yaml file of the remote cluster that interacts with this gateway.
E.g., remote_config[\"provider\"][\"gateway_id\"]={gateway_id}.".format(gateway_id=gateway_id)"""
        )

        anyscale_address = f"/api/v2/cloudgateway/{gateway_id}"
        cloudgateway_runner = CloudGatewayRunner(provider_path, anyscale_address)
...
def anyscale_clone(project_name: str) -> None:
    ...
    if check_use_snapshot_feature_flag():
        ...
        with open("{}/{}".format(project_name, ANYSCALE_PROJECT_FILE), "w") as f:
        ...
        with open("{}/{}".format(project_name, ANYSCALE_AUTOSCALER_FILE), "w") as f:
...
def ray_cli(ctx: Any) -> None:
    ...
    if "--help" not in sys.argv and ctx.invoked_subcommand in ["up", "down"]:
        ...
        if ctx.invoked_subcommand == "up":
            ...
            new_command = "anyscale start --cluster-config-file {}".format(
                " ".join(args)
            )
        else:
