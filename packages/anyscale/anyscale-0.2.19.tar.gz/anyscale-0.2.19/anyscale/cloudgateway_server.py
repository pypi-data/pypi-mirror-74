# type: ignore
# TODO(ameer): importing backend code in frontend is not making mypy happy.
...
from anyscale.util import send_json_request, startup_log_name
...
from backend.server.services.gateway_router_service import GatewayID, GatewayRequest
from backend.server.util import (
    StdoutStderrRedirect,
    temporary_db_session,
)

logger = logging.getLogger(__name__)


class ServerNodeProvider(NodeProvider):
    ...
    Args:
        ...
        cluster_config (dict): the cluster config. Passed only in testing mode.
    """
    ...
    def __init__(
        ...
        self.gateway_id = self.provider_config["gateway_id"]
        self.session_id = self.provider_config["session_id"]
        ...
        if cluster_config:
            ...
            log_path = startup_log_name(self.session_id)
            with StdoutStderrRedirect(log_path), temporary_db_session() as db_session:
                session = db_session.query(Session).get(self.session_id)
                cluster_config = session.cluster_config
                session_params = json.loads(session.session_params)
                for key, val in session_params.items():
                    cluster_config = cluster_config.replace("{{" + key + "}}", str(val))
                self.cluster_config = fillout_defaults(json.loads(cluster_config))
    ...
    def get_response(self, request: GatewayRequest) -> Any:
        ...
        response = ray.get(
            ray_actor_handler.pull_response.remote(self.gateway_id, request)
        )
    ...
    def create_node(
        ...
        request = {
            "type": "create_node",
            "args": (node_config, tags, count, self.cluster_config),
        }
