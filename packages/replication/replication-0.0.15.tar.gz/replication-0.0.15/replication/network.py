...
from .data import (ReplicatedCommand,
                   ...
                   RepServerSnapshotCommand,
                   RepKickCommand)
from .graph import ReplicationGraph
from .constants import (STATE_ACTIVE, STATE_INITIAL,
                        STATE_SYNCING,
                        STATE_AUTH, STATE_WAITING, STATE_SRV_SYNC,
                        STATE_LOBBY, COMMITED, RP_COMMON,
                        RP_STRICT, RP_COMMON, CONNECTION_TIMEOUT)
from .utils import current_milli_time, assert_parent_process_running
...
class ClientNetService(Service):
    def __init__(
        ...
        self._publish.setsockopt(zmq.IDENTITY, self._uuid)
    ...
    def _push_waiting_items(self):
        while not self._items_to_push.empty() and self._state != STATE_INITIAL:
            ...
            if isinstance(node, (RepDeleteCommand, RepKickCommand, RepRightCommand,
                                 RepDisconnectCommand, RepUpdateUserMetadata)):
                node.push(self._command)
            else:
                node.push(self._publish)

                if self.state == STATE_SRV_SYNC:
                    # START TO PUSH TO THE SERVER
                    self._srv_snapshot_progress += 1

                    self.notify_state(
                        current=self._srv_snapshot_progress,
                        total=self._srv_snapshot_size
                    )

                    if self._srv_snapshot_progress == self._srv_snapshot_size:
                        snapshot_end = RepServerSnapshotCommand(
                            owner='server',
                            data='SNAPSHOT_END')
    ...
    def main(self, sockets):
        ...
        if self._command in sockets:
            command = ReplicatedCommand.fetch(
                self._command, self._command_factory)
        ...
        if self._subscriber in sockets:
            ...
            # SERVER-->CLIENT SNAPSHOT update
            if self.state == STATE_SYNCING:
                assert(datablock.uuid == self._current_snapshot)
...
class ServerNetService(threading.Thread):
    ...
    def disconnect_client(self, client):
        if client in self.clients:
            ...
            logging.info(f"Disconnecting {leaving_client['id']} from server")
            cleanup_commands = []
            ...
            if self._command in socks:
                ...
                if isinstance(command, RepDisconnectCommand):
                    self._ttl_pipe.send_multipart(
                        [b'STOP_WATCHING', command.sender])
                ...
                if isinstance(command, RepServerSnapshotCommand):
                    ...
                    if cli_snapshot_state == 'INIT':
                        ...
                        if self._state == STATE_WAITING:  # ACCEPT and CONFIGURE
                ...
                if self._state == STATE_ACTIVE:
                    ...
                    for cli_name in self.clients.keys():
                        if cli_name != datablock.sender:
                            datablock.push(self._publisher, identity=cli_name)
                elif self._state == STATE_SRV_SYNC:
                    self._client_snapshot_progress += 1
                    logging.info(
                        f"Receiving snapshot {self._client_snapshot_progress}/{self._client_snapshot_size}")
        ...
        while not self._exit_event.is_set():
            ...
            if self._command in socks:
                ...
                if frame == b'INIT':
                    ...
                    self._clients_state[identity]['last_update'] = current_time
                ...
                client['latency'] = (current_time-(client['last_update']))-50
            ...
            if current_time-last_update_time > 1000:
                ...
                for client in self._clients_state.keys():
                    ping = (current_time -
                            (self._clients_state[client]['last_update']))

                    if ping > self._timeout:
                        logging.error(
                            f"Lost connexion to {client} ({ping} ms)")
                ...
                self._pipe.send_multipart(
                    [b'STATE', pickle.dumps(self._clients_state)])
        self._command.close()
        self._pipe.close()
        self._state = STATE_INITIAL
