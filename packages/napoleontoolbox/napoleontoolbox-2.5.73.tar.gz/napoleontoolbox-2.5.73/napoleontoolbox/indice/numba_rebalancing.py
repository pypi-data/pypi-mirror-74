...
def numba_indice_rebalancing(early_rebal, rebal_data, price_data, previous_price_data, target_weights):
    ...
    for i in range(price_data.shape[0]):
        ...
        if early_rebal:
            drifted_weights = flat_numba_drift_rebal(weights[i - 1, :], price_data[i, :], previous_price_data[i, :])
            must_rebalance = must_rebalance or np.sum(drifted_weights > target_weights)>0
...
def pre_numba_indice_rebalancing(price_data= None, inception_date=None, target_weights=None, initial_value= 100., rebalancing_method = None , early_rebalancing = False, last = True):
