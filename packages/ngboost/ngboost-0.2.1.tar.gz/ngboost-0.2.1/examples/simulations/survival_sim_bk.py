...
if __name__ == '__main__':
    ...
    ngb = NGBSurvival(Dist=LogNormal,
    ...
    plt.hist(np.log(preds.mean()), range=(-10, 10), bins=30, alpha=0.5, label="Pred")
    plt.hist(Y_te, range=(-10, 10), bins=30, alpha=0.5, label="True")
    plt.legend()
    plt.show()

    # since we simulated the data we fully observe all outcomes
    # calibration assuming complete observations
    pctles, observed, slope, intercept = calibration_regression(preds, np.exp(Y_te))
    plot_calibration_curve(pctles, observed)
    plt.show()

    # calibration for partial observations
    pctles, observed, slope, intercept = calibration_time_to_event(preds, np.exp(np.minimum(Y_te, T_te).squeeze()), E_te.squeeze())
    plot_calibration_curve(pctles, observed)
    plt.show()
