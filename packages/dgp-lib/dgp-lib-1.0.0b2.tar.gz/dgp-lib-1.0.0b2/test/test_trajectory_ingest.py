...
from dgplib.ingest import trajectory as ti


class TestTrajectoryIngest(unittest.TestCase):

    def test_import_trajectory(self):
        fields = ['mdy', 'hms', 'lat', 'long', 'ell_ht', 'ortho_ht', 'num_sats', 'pdop']
        df = ti.import_trajectory(os.path.abspath('test/sample_trajectory.txt'),
    ...
    def test_import_trajectory_time_formats(self):
        ...
        df1 = ti.import_trajectory(os.path.abspath('test/sample_trajectory.txt'),
                                   columns=fields1, skiprows=1, timeformat='hms')

        fields2 = ['week', 'sow', 'lat', 'long', 'ell_ht', 'ortho_ht', 'num_sats', 'pdop']
        df2 = ti.import_trajectory(os.path.abspath('test/sample_trajectory_week-sow.txt'),
