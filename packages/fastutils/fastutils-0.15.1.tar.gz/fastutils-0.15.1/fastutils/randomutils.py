...
class UuidGenerator(object):
    ...
    def __init__(self, namespace=""):
        self.namespace = str(uuid.uuid4()) + "-" + namespace
        ...
        self.domain_template = ".".join(["{ts}", "{counter}", str(self.tid), str(self.pid), self.user, str(self.node), str(self.namespace)])
        
    def next(self):
        with self.counter_lock:
            self.counter += 1
        ts = int(time.time() * 1000000)
        domain = self.domain_template.format(ts=ts, counter=self.counter)
