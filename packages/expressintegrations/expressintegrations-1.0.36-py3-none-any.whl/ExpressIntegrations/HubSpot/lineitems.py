...
def search_lineitems(property_names, filters, after=None):
  post_url = f"{BASE_URL}crm/v3/objects/lineitems/search"
  if after:
...
def get_lineitem(lineitem_id, property_names=None, associations=None):
  post_url = f"{BASE_URL}crm/v3/objects/lineitems/{lineitem_id}?archived=false"
  if property_names:
...
def create_lineitem(properties):
  post_url = f"{BASE_URL}crm/v3/objects/lineitems"
  post_body = {
...
def update_lineitem(lineitem_id, properties):
  post_url = f"{BASE_URL}crm/v3/objects/lineitems/{lineitem_id}?"
  post_body = {
      'properties': properties
  }
