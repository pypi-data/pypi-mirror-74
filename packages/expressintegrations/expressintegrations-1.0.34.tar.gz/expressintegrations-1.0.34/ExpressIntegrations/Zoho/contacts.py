...
def create_contact(contact):
  post_url = f"{BASE_URL}/contacts"
  result = post(post_url, HEADERS, json.dumps(contact))
...
def update_contact(contact_id, contact):
  post_url = f"{BASE_URL}/contacts/{contact_id}"
...
def search_contacts(contact_name=None,
  ...
  post_url = f"{BASE_URL}/contacts?sort_column={sort_column}"
...
def create_contact_person(contact):
  post_url = f"{BASE_URL}/contacts"
  result = post(post_url, HEADERS, json.dumps(contact))
  if not Utils.is_success(result['status_code']):
    raise Exception(f"Failed to create contact. Result: {result}")
