...
) -> Tuple[bool, List[Tuple[Requirement, Optional[Requirement]]]]:
    working_set_by_key = working_set.by_key  # type: ignore
    incompat_list = []
    for expected_req in validated_requirements:
        if expected_req.name not in working_set_by_key:  # type: ignore
            continue
...
def _get_package_incompatibilities(packages: Set[str],
    ...
    with logging_utilities.log_activity(logger, activity_name=constants.TelemetryConstants.PACKAGES_CHECK):
        ...
        file_name = 'validated_{}_requirements.txt'.format(platform)
        ...
        if os.path.exists(file_name):
            ...
            for req_text in text:
                ...
                except pkgr.RequirementParseError:  # type: ignore
            ...
            logger.warn("No verified requirements file found. Unable to verify all requirements.")
            validated_requirements = []
        ...
        if do_incompatibilities_exist:
            ...
            for unverified_package in unverified_packages:
                message_template = 'We validated this package with {} but has {}.'
            ...
            raise OptionalDependencyMissingException.create_without_pii(
                msg="Unverified pacakge(s) found: {}".format(exception_message),
                reference_code=ReferenceCodes._UNVERIFIED_PACKAGES)

        logger.debug("No package incompatbilities found.")
