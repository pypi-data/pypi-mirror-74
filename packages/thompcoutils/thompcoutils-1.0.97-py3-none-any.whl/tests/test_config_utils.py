...
from thompcoutils.config_utils import ConfigManager, EmailConnectionConfig, HiLow, PiConfig, ConfigException
...
class TestConfigUtils(unittest.TestCase):
    ...
    def test_pi_config(self):
        ...
        default_filename = '{}{}'.format(config_filename, PiConfig._default_file_extension)
        ...
        pi_config.write()
        # test reading a real file (move any existing file to a backup for recovery later)
        if os.path.exists(config_filename):
            os.unlink(config_filename)
        f = open(config_filename, 'w')
        # fill the file
        f.write('[misc]\n')
        f.write('host_name = a_host_name\n')
        f.write('wifi_country_code = CA\n\n')
        for i in range(1, 3):
            f.write('[wifi_{}]\n'.format(i))
            f.write('ssid=testing wifi {}\n'.format(i))
            f.write('key = testing wifi key {}\n'.format(i))
            f.write('hidden = {}\n'.format(True if i == 1 else False))
            f.write('priority = {}\n\n'.format(i))
        f.close()
        # read it
        pi_config = PiConfig()
        print(pi_config.misc.host_name)
        for wifi in pi_config.wifi_sections:
            print(wifi.section)
            print(wifi.ssid)
            print(wifi.wifi_key)
        ...
        # test no actual file but have default file
        try:
            PiConfig()
            print('Should have thrown an exception as the actual file is missing and the default one exists')
            assert False
        except ConfigException as c:
            print(c)
            print('This is the correct exception')
