...
try:
    ...
    pio_loaded = False
    raise PioException('PIO NOT LOADED')
...
class ADC:
    ...
    def start(self):
        """
        Start the ADC._get_data_thread loop
        :return: None
        """
        if not self.running:
            self.running = True
            WorkerThread(callback_function=self._get_data_thread, parameters=self).start()

    def stop(self):
        """
        Stop the ADC._get_data_thread loop
        :return: None
        """
        self.running = False
...
class TemperatureHumidity:
    ...
    def _get_data_thread(temp_humidity):
        """
        This thread runs contiuously getting data from the sensor.  It fills the queue with Values
        :param temp_humidity: this is the calling object (self)
        :return: None
        """
        logger = get_logger()
        while temp_humidity.thread_running:
