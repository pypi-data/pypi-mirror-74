...
class ConfigManager:
    ...
    def _insert_note(lines, line_number, note):
        """
        This method inserts a note at a particular note in an array of lines that will ultimately be written to the
        config file
    ...
    def add_note(self, section, notes, entry=None):
        """
    ...
    def write(self, out_file, *args, **kwargs): #
...
class PiConfig(ConfigManager):
    ...
    class WiFi:
        ...
        def __init__(self, cfg_mgr=None, ssid='my wifi ap', wifi_key='my wifi key', hidden=False, section=section,
                     priority=1):
            """
            WifiConfig defines the data for the wpa_suplicant file
            :param cfg_mgr: the Configuration
    ...
    class Miscellaneous:
        ...
        def __init__(self, cfg_mgr=None, host_name='Pi', wifi_country_code='US'):
            """
            WifiConfig defines the data for the wpa_suplicant file
            :param cfg_mgr: the Configuration
            ...
            if cfg_mgr is None:
                ...
                self.wifi_country_code = wifi_country_code
            else:
    ...
    def __init__(self,
                 ...
                 default_filename='configuration.default',
                 remove_once_read=True):
        """
        ...
        specified default_filename in the boot_folder if file_name is missing and raise an exception.
        NOTE that this class can only be used on a raspberry pi!
        :param file_name: name of the file to read
        :param title:
        :param create_if_missing: creates the default file if the specified file is missing
        :param default_filename: filename to create if it is not already there
        :param remove_once_read: indicates the file should be removed once it has been read (presumably because the
        information is stored elsewhere.  This is a good de
        """
        if not os.path.isdir(PiConfig._boot_folder):
            raise RuntimeError('This can only be used on a Raspberry Pi')

        default_filename = os.path.join(PiConfig._boot_folder, default_filename)
        ...
        try:
            ...
            self.file_name = file_name
            self.remove_once_read = remove_once_read
            self.get_wifi_sections()
            self.misc = PiConfig.Miscellaneous(self)
        except FileNotFoundError:
            # file was not there
            if create_if_missing:
                # try to create the default one
                super().__init__(file_name=file_name, title=default_filename, create=True)
                ...
                self.wifi_sections.append(PiConfig.WiFi(self, section='{}_second'.format(PiConfig.WiFi.section),
                                                        ...
                                                        priority=2))
                self.remove_once_read = False
            else:
                # could not read the file and not specified to create one, so throw an exception
                raise ConfigException('Configuration file {} not found and create_if_missing is not specified'.
                                      format(file_name))

    def get_wifi_sections(self):
    ...
    def backup_original_file(file_name):
        ...
        if os.path.exists(original_backup):
            ...
            os.rename(file_name, original_backup)
    ...
    def write_host_files(self):
        ...
        f.close()
        # todo need to edit the /etc/hosts file

    def write(self, out_file=None, *args, **kwargs):
        """
        ...
        :return:
        """
        super(PiConfig, self).write(out_file=self.file_name, stop=False, overwrite=True)
        if self.remove_once_read:
