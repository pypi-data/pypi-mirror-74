...
class NovatiqClient(Client):
    ...
    def update_campaign(self, campaign_id, name=None, segments=None, start_date=None,
                        end_date=None):
        """Create a campaign.

        Only the name, segments, and start/end dates can be changed.
        ...
        if start_date is not None:
            payload['startDate'] = arrow.get(start_date).format(self.TIME_FORMAT)
