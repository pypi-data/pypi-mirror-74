...
SUFFIX_GENERATOR = CompanyNameSuffixGenerator()
...
class LegalEntityNameGenerator2:
    ...
    def make(self, sic, country_code=None):
        ...
        if not isinstance(sic, str):
            # if it's not a string assume it's a SIC object
            sic = sic.code
    ...
    def create_name(self, config, country_code):
        country_code = data_access.resolve_country(country_code).locale
        ...
        while True:
            ...
            if v not in NAMES:
                ...
                if country_code:
                    suffix = SUFFIX_GENERATOR.make(country_code)
                    return f"{v} {suffix}"
                return v
