...
from domauto import da_iolib as daio
from domauto.da_classlib import *
...
def plot_scp_DOM_hist( scp_datagroup,
    ...
    y_delta = y_max - y_min 
    y_bins  = np.ceil( YBINS if  y_delta / data_delta > YBINS else y_delta / data_delta )
...
def plot_DET_DOMs( scp_dataframe,
    ...
    for idu, du in enumerate( daio.get_live_DUs( scp_dataframe, clbm ) ):
        plt.cla()
        plt.clf()
        ...
        for (omkey, clb) in sorted( clbm.omkeys.items() ):
            if clb.du != du:
                continue
            try:
                scp_datagroup = daio.get_DOM_datagroup( scp_dataframe, clb )
...
def plot_DET_DUs( scp_dataframe,
    ...
    live_DUs = daio.get_live_DUs( scp_dataframe, clbm )
    ...
    for idu, du in enumerate( live_DUs ):
        ...
        if VERBOSE: print("\nDU:{0}".format(du), end='')                                                                          
        DU_omkeys = daio.get_DU_omkeys( [du], clbm.omkeys, ignore_base = False)
        
        try:
            scp_datagroup = daio.get_DU_datagroup( scp_dataframe, [du], DETECTOR )
        except ValueError as e: #no data
            if VERBOSE: print(" ERROR: DU {0}: {1}. Skipping this DU.".format(du, e))
            continue

        if scp_datagroup.size < 1 :
            print("ERROR: datagroup empty! Skipping this DU.")
            continue
        if VERBOSE:
            print( "Plotting datagroup of size {0}".format( scp_datagroup.size ) )
