...
from watson_machine_learning_client.experiment.autoai.autoai import PipelineTypes
from watson_machine_learning_client.tests.utils import get_wml_credentials, is_cp4d
...
class TestLale(unittest.TestCase):
    ...
    def test_20_deployment_setup_and_preparation(self):
        if is_cp4d():
            TestLale.service = WebService(wml_credentials=self.wml_credentials.copy(),
                                          project_id=self.project_id,
                                          space_id=self.space_id)
        else:
            TestLale.service = WebService(wml_credentials=self.wml_credentials)
