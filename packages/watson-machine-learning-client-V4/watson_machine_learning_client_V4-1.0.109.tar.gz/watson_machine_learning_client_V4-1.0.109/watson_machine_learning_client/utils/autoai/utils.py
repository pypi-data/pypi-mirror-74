...
from .errors import MissingPipeline, AutoAIComputeError, DataFormatNotSupported, LibraryNotCompatible, \
    CannotInstallLibrary, CannotDownloadTrainingDetails
...
def try_import_autoai_libs():
    ...
    try:
        ...
        try:
            check_call([executable, "-m", "pip", "install", "autoai_libs>=1.10.12"])

        except Exception as e:
            raise CannotInstallLibrary(value_name=e,
                                       reason="autoai_libs>=1.10.12 failed to install. Please install it manually.")
...
def try_load_dataset(buffer: 'BytesIO', sheet_name: str = 0, separator: str = ',') -> Union['DataFrame', 'OrderedDict']:
    """
    ...
    try:
        buffer.seek(0)
        data = read_csv(buffer, sep=separator)
