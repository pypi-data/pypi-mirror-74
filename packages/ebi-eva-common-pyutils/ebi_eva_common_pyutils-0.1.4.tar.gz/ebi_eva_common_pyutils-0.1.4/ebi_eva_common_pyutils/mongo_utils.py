...
def copy_db(mongo_source_config: MongoConfig, mongo_destination_config: MongoConfig, mongodump_args: dict,
            mongorestore_args: dict):
    mongodump_args_str = " ".join(["--{0} {1}".format(key, value) for key, value in
                                   merge_two_dicts(mongo_source_config, mongodump_args).items()])
    mongorestore_args_str = " ".join(["--{0} {1}".format(key, value) for key, value in
                                      merge_two_dicts(mongo_destination_config, mongorestore_args).items()])
    run_command_with_output("Running mongodump", "mongodump " + mongodump_args_str)
