...
def _review(args: argparse.Namespace) -> int:
    ...
    unresolved_comments = set(
        _get_comments(source=sys.stdin, pattern=_get_pattern(args))
    )
    posted_comments = set(reviewer.get_existing_comments())
    for comment in unresolved_comments - posted_comments:
        reviewer.post_comment(comment)
        print(comment, file=sys.stderr)
    for comment in posted_comments - unresolved_comments:
        reviewer.resolve_comment(comment)
    return len(unresolved_comments)
