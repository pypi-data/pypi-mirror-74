...
class EstimQueue_Results:
    def __init__(self, colnames, sampleshape, estim, diag, crlb, 
    ...
    def Merge(result_list):
        ...
        for r in result_list:
            ...
            it.append(r.it)
            ids.append(r.ids)
            chisq.append(r.chisq)
            crlb.append(r.crlb)
            
        return EstimQueue_Results(result_list[0].colnames, 
                result_list[0].sampleshape,
...
class EstimQueue:
    def __init__(self, estim:Estimator, batchSize=256, maxQueueLenInBatches=5, numStreams=-1, keepSamples=True, ctx=None):
        if ctx is None:
            self.ctx = estim.ctx
        else:
            self.ctx = ctx
...
#        DLL_EXPORT int EstimQueue_GetResults(LocalizationQueue* q, int maxresults, float* estim, float* diag, float *fi);
    ...
    def Schedule(self, samples, roipos=None, ids=None, initial=None, constants=None):
        ...
        if constants is not None:
            ...
            if constants.size != numspots*self.estim.numconst:
                raise ValueError(f'Estimator is expecting constants array with size [{[numspots,self.estim.numconst]}]. Given: {constants.shape}')
        else:
    ...
    def GetResults(self,maxResults=None, getSampleData=False) -> EstimQueue_Results:  # 
        ...
        r = EstimQueue_Results(self.colnames, self.estim.sampleshape, 
