...
class SpotList:
    ...
    def __init__(self, r: loc.LocResultList, roi_source, pixelsize, outdir, IBg=None, IBg_crlb=None):
        """
    ...
    def plot_proj_fft(self):
        ...
        for k in range():
            fig = plt.figure()
            scale = len(self.proj[k]) / self.locs.imgshape[0]
    ...
    def render_sr_image_per_pattern(self, ep, frames, sr_factor, ctx:Context):
        indices = np.concatenate([self.locs.frames[i] for i in frames])
        if ep<0:
            xyI = self.locs.get_xyI()
        else:
            xyI = self.get_xyI(ep, normalize=True)[indices]
        w,h=self.locs.get_image_size()
        img = np.zeros((h*sr_factor,w*sr_factor))
        img = loc.draw_spots(img,xyI[:,0],xyI[:,1],xyI[:,2],2/sr_factor,2/sr_factor,sr_factor,ctx)
        return img
    ...
    def _estimate_angle_and_pitch_dft(self, pattern_indices, frames, ctx:Context, freq_minmax):
        ...
        z=6
        smpimg = self.render_sr_image_per_pattern(-1, frames, z,ctx)
        ...
        for ep in pattern_indices:
            img = self.render_sr_image_per_pattern(ep, frames, sr_factor=z,ctx=ctx)
...
#        plt.imsave(self.outdir + f"pattern-{pattern_indices}-FFT-mask.png", mask)
        ...
        xy = self._find_dft2_peak(yx[[1,0]], pattern_indices, frames, ctx)
        
        ang = np.arctan2(xy[1],xy[0])
        freq = np.sqrt(xy[0]**2+xy[1]**2)
        return ang,2*np.pi/freq
        
    def _find_dft2_peak(self, xy, pattern_indices, frames, ctx:Context):
        indices = np.concatenate([self.locs.frames[i] for i in frames])
        S = 0.02
...
#        plt.figure()
 #       plt.imshow(img)
    ...
    def estimate_angle_and_pitch(self, pattern_frames, frame_bins, ctx,freq_minmax):
    ...
    def _estimate_phase_and_depth(self, k, ep, frames):
        ...
        spotPhaseField = k[0] * xyI[:, 0] + k[1] * xyI[:, 1]
        ...
        relint = np.mean(xyI[:,2] / self.locs.get_xyI()[indices,2])
        return phase, depth, relint
    ...
    def plot_intensity_variations(self,mod,minfilter,pattern_frames):
        ...
        xyI = self.locs.get_xyI()[sel,np.newaxis]
        mod_ = mod[np.newaxis]
...
#        sumI = np.sum(self.IBg[indices][: , pattern_frames, 0],1)
        ...
        xyI = self.locs.get_xyI()[indices] * 1
...
#            x, residual, rank, s = np.linalg.lstsq(M,b,rcond=None)
            ...
            q = relint * (1+depth*np.sin(k[0]*xyI[:,0]+k[1]*xyI[:,1]-phase))
            xyQ = xyI_modulated*1
            xyQ[:,2] /= sumI
...
#        print(f"ep{ep}. depth={depth}, phase={phase}")
    ...
    def simflux_fit(self, mod, ctx:Context, indices):
        with Context(ctx.smlm) as ctx:
            sf = SIMFLUX(ctx)
            roisize = self.locs.cfg['roisize']
            
            psf = sf.CreateEstimator_Gauss2D(self.locs.cfg['sigma'], self.numPatterns, 
                                                 roisize, self.numPatterns,
                                                 simfluxEstim=True)
            ...
            queue = EstimQueue(psf, batchSize=2048, numStreams=2, keepSamples=False)

            idx = 0
            for roipos, pixels, block_indices in self.roi_source(indices):
                
                #roi_mod = np.repeat([mod[0].view(np.float32)],len(roipos),0)
                roi_mod = mod[block_indices]
            ...
            chisq = sf_results.chisq
            roipos = sf_results.roipos

        data = self.locs.data[indices, :] * 1
        data[:, [loc.DataColumns.X, loc.DataColumns.Y]] = est[:, [0, 1]] + roipos[:,[2,1]]
        data[:, [loc.DataColumns.ROIX, loc.DataColumns.ROIY]] = roipos[:,[2,1]]
        ...
        data[:, loc.DataColumns.CHISQ] = chisq
                
        return (loc.LocResultList(data,self.locs.cfg, self.locs.get_area(),src_indices=indices),
                loc.LocResultList(self.locs.data[indices,:]*1,self.locs.cfg, self.locs.get_area(),src_indices=indices))
    
    
    def simflux_compare_plot(self):
        import matplotlib.ticker as ticker
        # matplotlib gets really slow with large point counts
        draw_sel = sel[:50000 if len(sel)>50000 else len(sel)]*1
        gxy = self.locs.get_xyI()[draw_sel]
...
#        plt.tight_layout()
    ...
    def get_filtered_spots(self, spotfilters, mod, frames=None, reportfn=None):
        if frames is None:
            frames = np.arange(len(self.locs.frames))
    ...
    def excitation_matrix(self, mod, sel):
        xyI = self.locs.get_xyI()[sel,None]
        mod_ = mod[None]
        kx = mod_['k'][:,:,0]
        ky = mod_['k'][:,:,1]
    ...
    def plot_moderr_vs_intensity(self,mod, spotfilter):
        ...
        intensities = self.locs.get_xyI()[:,2]
...
#                ax[0].set_title(f"Axis {axis} modulation quality (Chi-Squared)")
    ...
    def plot_modulation_chisq_timebins(self, mod, pattern_indices, spotfilter, numbins, chisq=True):
        fr = np.arange(len(self.locs.frames))
