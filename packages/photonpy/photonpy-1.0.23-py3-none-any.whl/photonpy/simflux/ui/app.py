...
from .drift_correct_dlg import DriftCorrectionDialog
...
class Window(QDialog):
    ...
    def localize(self):
        ...
        def localize_thread():
            ...
            self.results, self.imgshape = process_movie.localize(tiff_path, cfg, locs_fn, progress_update, est_sigma)
            if not pbar.abortPressed:
    ...
    def showLocsInfo(self):
        ...
        m_I=np.median(self.results.estim[:,2])
        self.ui.labelLocsInfo.setText(f"#Spots: {len(self.results.estim)}. Imgsize:{self.imgshape[0]}x{self.imgshape[1]} pixels. Median CRLB X: {m_crlb_x:.2f} [pixels], bg:{m_bg:.1f}. I:{m_I:.1f}")
