...
from tank_forecaster.decomp import generic_yearly_decomp, generic_weekly_decomp, generic_daily_decomp
...
def forecast_far(sales_history,
    ...
    yearly_decomp = yearly_decomp or generic_yearly_decomp
    weekly_decomp = weekly_decomp or generic_weekly_decomp
    ...
    if type(sales_history) != pd.core.frame.DataFrame:
        ...
        data = {'ds': [pd.to_datetime(datetime.now()), pd.to_datetime(datetime.now())],
                'sales': [0, 0]}
...
def forecast_near(validated_tank_history,
    ...
    if type(validated_tank_history) != pd.core.frame.DataFrame:
        ...
        forecast = forecast_far(sales_history=[],
