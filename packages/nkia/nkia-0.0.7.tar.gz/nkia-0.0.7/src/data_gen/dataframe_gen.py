...
class dataframeGen(object):
    ...
    def get_products_dataframe(self, products_categories):
        ...
        query = ('''MATCH (i:Ingredient)<-[:HAS_INGREDIENT]-(p:Product)-[:FROM_CATEGORY]-(c:Category {name: $name})
    ...
    def create_products_dict(self, products_and_ingredients):
        ...
        for record in products_and_ingredients:
            if record['product']['name'] in products_and_ingredients_dict:
                products_and_ingredients_dict[record['product']['name']].append(
                    record['ingredient']['name'])
            else:
                products_and_ingredients_dict[record['product']['name']] = [
                    record['product'], record['ingredient']['name']]
