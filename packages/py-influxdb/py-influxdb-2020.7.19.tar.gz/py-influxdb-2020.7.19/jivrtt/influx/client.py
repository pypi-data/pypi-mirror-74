...
from jivrtt.influx.base import BaseClient, Query
...
class InfluxQLClient(BaseClient):
    ...
    def write(self, db, measure, field, value, tags=None, timestamp=None):
        ...
        data = f"{measure}{f',{tags}' if tags else ''} {field}={value} {timestamp or ''}".strip()
...
if __name__ == '__main__':
    ...
    client = InfluxQLClient('192.168.99.100', user='nkm', password='nkm')
    ...
    query = Query().measure('sample').select(['val', 'tag1', 'tag2'])
