...
class ProficloudIOMetrics():
    ...
    def __init__(self, staging=False):
        ...
        self._auth_host = 'auth.kaa.proficloud-production.io'
        if staging:
            self._host = 'tsd.proficloud-staging.io'
            self._auth_host = 'auth.kaa.proficloud-staging.io'
        self._client_id = 'kaa-frontend'
    ...
    def queryMetrics(self, uuid, metrics, start_time=None, end_time=None, createDf=True, fillNaMethod = None, dropTrailingNa=True, orderDesc=False):
        """
        ...
        if not createDf:
            ...
            if ProficloudIOMetrics.DEBUGTIME:
                start_t = datetime.now()
            dfres = ProficloudIOMetrics.convert_response(response, uuid, fillNaMethod=fillNaMethod, dropTrailingNa=dropTrailingNa, convertTimestamp=True)
            if ProficloudIOMetrics.DEBUGTIME:
                end_t = datetime.now()
    ...
    @staticmethod
    def convert_response(response, uuid, fillNaMethod=None, dropTrailingNa=True, convertTimestamp=False):#, orderDesc=False
        """
        ...
        if convertTimestamp and ("timestamp" in result):
            result["timestamp"] = result["timestamp"].map(ProficloudIOMetrics.dateparse)
