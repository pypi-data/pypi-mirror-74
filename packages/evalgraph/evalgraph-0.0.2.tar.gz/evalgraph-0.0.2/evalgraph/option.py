...
import math
from evalgraph.nodes import node, nodefn, GraphEvaluator
...
class Underlier(NodeObject):
    ...
    @node
    def Spot(self):
        return Market().Spot(self.Ticker())()
...
class Option(NodeObject):
    ...
    def Description(self):
        ...
        return f"{type}[K={self.Strike()},Mat={self.Maturity().strftime('%-m/%-d/%Y')}]"   
    
    @node
    def Strike(self):
        return Market().Spot(self.Underlier().Ticker())()
