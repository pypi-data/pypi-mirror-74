...
class AbstractProperty(object):
    ...
    def __init__(self, properties: Any=None, manager: str=None, contract: str=None):
        ...
        if isinstance(properties, list):
            ...
            self._create_properties(attributes=properties, prefix=prefix, path=None)
...
class AbstractPropertyManager(ABC):
    ...
    def report_info(self):
        """Reports on the current contract info"""
        return {'contract': self.manager_name(), 'task': self.task_name, 'desciption': self.description,
    ...
    def set_intent(self, intent_param: dict, level: [int, str]=None, order: int=None, replace_intent: bool=None,
        ...
        remove_duplicates = remove_duplicates if isinstance(remove_duplicates, bool) else True
        level = 'A' if not isinstance(level, (int, str)) else level
        order = 0 if not isinstance(order, int) else order
        # remove any repeats of this exact intent
        if remove_duplicates:
            self.remove_intent(intent_param=intent_param)
        for label in intent_param.keys():
            if order == -1:
                if not self.get_intent():
                    self.set(self.join(self.KEY.intent_key, level, 0), intent_param)
                    return
                order = 0
                while True:
                    if not self.is_key(self.join(self.KEY.intent_key, level, order, label)):
                        self.set(self.join(self.KEY.intent_key, level, order), intent_param)
                        break
                    order += 1
            else:
                if self.is_key(self.join(self.KEY.intent_key, level, order, label)):
                    if replace_intent:
                        self.remove(self.join(self.KEY.intent_key, level, order, label))
                        self.set(self.join(self.KEY.intent_key, level, order), intent_param)
                else:
                    self.set(self.join(self.KEY.intent_key, level, order), intent_param)
        return
    ...
    def set_connector_contract(self, connector_name: str, connector_contract: ConnectorContract, aligned: bool=None):
        ...
        self.set(self.join(base_key, 'modified'), 0)
    ...
    def modify_connector_aligned(self, connector_name: str, template_contract: ConnectorContract) -> bool:
        ...
        if self.get(self.join(self.KEY.connectors_key, connector_name, 'aligned'), False):
            self.remove_connector_handler(connector_name=connector_name)
            raw_uri = self.get(self.join(self.KEY.connectors_key, connector_name, 'raw_uri'), None)
    ...
    def set_modified(self, connector_name, modified: [str, int, float]):
        """ set modified """
        if self.has_connector(connector_name):
            self.set(self.join(self.KEY.connectors_key, connector_name, 'modified'), modified)
        return
    ...
    def has_connector(self, connector_name) -> bool:
        """Test if the contract has intent"""
        if self.get(self.join(self.KEY.connectors_key, connector_name)) is None:
            return False
        return True

    def has_connector_handler(self, connector_name) -> bool:
    ...
    def reset_connector_contracts(self):
        ...
        property_connector = self.get(self.join(self.KEY.connectors_key, self.CONNECTOR_PM_CONTRACT), None)
    ...
    def is_connector_modified(self, connector_name) -> bool:
        """test is the raw file has changed since last set. Returns True if modified has changed"""
        if not self.has_connector(connector_name):
            raise ModuleNotFoundError("The connector '{}' has not been set".format(connector_name))
        if self.get(self.join(self.KEY.connectors_key, connector_name, 'modified')) is None:
            return True
        modified = self.get_connector_handler(connector_name).get_modified()
        if modified == self.get(self.join(self.KEY.connectors_key, connector_name, 'modified')):
            return False
        return True

    def pm_file_pattern(self, file_type: str=None):
        ...
        return f"hadron_pm_{self.manager_name()}_{self.task_name}.{file_type}"

    def file_pattern(self, connector_name: str, path: [str, list]=None, prefix: str=None, suffix: str=None,
        ...
        :param connector_name: The name of the connector the file pattern is for
        :param path: (optional) a file path that precedes the prefix and file pattern. uses os.path.join so takes a list
        :param prefix: (optional) a prefix to put at the front of the file pattern
        :param suffix: (optional) a suffix to put at the end of the file pattern
        :param file_type: (optional) an alternative file extension to the default 'pickle' format
        ...
        file_type = file_type if isinstance(file_type, str) else 'pickle'
        _pattern = "_".join(["hadron", self.manager_name(), self.task_name, connector_name])
        _pattern = f"{prefix}{_pattern}"
    ...
    def set_canonical_schema(self, name: str, canonical_report: dict):
        """ add the dictionary """
        if self.is_key(self.join(self.KEY.knowledge.schema_key, name)):
            self.remove(self.join(self.KEY.knowledge.schema_key, name))
        return self.set(self.join(self.KEY.knowledge.schema_key, name), canonical_report)
