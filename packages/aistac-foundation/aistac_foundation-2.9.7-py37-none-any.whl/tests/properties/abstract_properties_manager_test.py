...
class AbstractPropertiesManagerTest(unittest.TestCase):
    ...
    def test_set_property(self):
        ...
        self.assertEqual('v00', result.version)
    ...
    def test_info(self):
        ...
        result = dpm.report_info()
        control = {'contract': 'control', 'task': 'test', 'desciption': 'This Task description', 'status': 'stable', 'version': 'v1.01'}
        self.assertEqual(control, result)
        dpm.reset_info()
        result = dpm.report_info()
        control = {'contract': 'control', 'task': 'test', 'desciption': '', 'status': 'discovery', 'version': 'v0.00'}
    ...
    def test_set_intent(self):
        ...
        control = {'A': {'0': {'clean_01': {'dtype': ['number', 'str'], 'headers': 'h1'}}}}
        dpm.set_intent(cleaner1, replace_intent=False)
        self.assertEqual(control, dpm.get_intent())
        control = {'A': {'0': {'clean_01': {'headers': 'h4'}}}}
        dpm.set_intent(cleaner1, replace_intent=True)
        self.assertEqual(control, dpm.get_intent())
        # second cleaner
        cleaner2 = {'clean_02': {'headers': ['h2', 'h3'], 'regex': 'text'}}
        dpm.set_intent(cleaner2)
        control = {'A': {'0': {'clean_01': {'headers': 'h4'},
    ...
    def test_get_intent(self):
        ...
        result = dpm.get_intent(level='A', order=0, intent='to_str')
        self.assertDictEqual({'to_str' : {'headers': ['five', 'six']}}, result)
        result = dpm.get_intent(level='A', order=0)
        self.assertEqual(['remove_head', 'to_str'], AistacCommons.list_formatter(result.keys()))
    ...
    def test_intent_report(self):
        ...
        self.assertEqual(['clean_01', 'clean_03', 'clean_01', 'clean_02'], report['intent'])
        self.assertEqual(['A', 'A', 'A', 'C'], report['level'])
        self.assertEqual(['0', '0', '1', '0'], report['order'])
        self.assertEqual(["headers='h1'"], report['parameters'][0])
        self.assertEqual(["headers='h3'"], report['parameters'][1])
    ...
    def test_file_pattern(self):
        pm = ControlPropertyManager('task')
        result = pm.file_pattern(connector_name='source')
        self.assertEqual('hadron_control_task_source.pickle', result)
        result = pm.file_pattern(connector_name='source',file_type='json')
        self.assertEqual('hadron_control_task_source.json', result)
        result = pm.file_pattern(connector_name='source', stamped="hours")
        self.assertEqual('hadron_control_task_source${TO_HOURS}.pickle', result)
        result = pm.file_pattern(connector_name='source', versioned=True, stamped="minutes")
        self.assertEqual('hadron_control_task_source${VERSION}${TO_MINUTES}.pickle', result)
        result = pm.file_pattern(connector_name='source', versioned=True)
        ...
        # test prefix & suffix
        result = pm.file_pattern(connector_name='source', prefix="prefix_")
        self.assertEqual('prefix_hadron_control_task_source.pickle', result)
        result = pm.file_pattern(connector_name='source', suffix="?name=fred&type=male")
        self.assertEqual('hadron_control_task_source.pickle?name=fred&type=male', result)
        result = pm.file_pattern(connector_name='source', prefix="prefix_", suffix="?name=fred&type=male")
        self.assertEqual('prefix_hadron_control_task_source.pickle?name=fred&type=male', result)
        # test path
        result = pm.file_pattern(connector_name='source', path="s3://bucket/path")
    ...
    def test_modify_connector_aligned(self):
        ...
        uri = "s3://bucket/path/hadron_control_task_connector.pickle?name=fred&value=45"
        cc = ConnectorContract(uri=uri, module_name='module.package', handler='Handler')
        template = ConnectorContract(uri="http://git.com/hadron/", module_name='module.pickle', handler='PickleHandler', sep= ',', encoder= 'latin1')
