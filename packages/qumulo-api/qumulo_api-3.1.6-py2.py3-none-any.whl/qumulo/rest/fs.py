...
@request.request
def get_file_attr(
    conninfo, credentials, id_=None, path=None, snapshot=None, stream_id=None
):
    method = 'GET'
...
def read_file(
    ...
    method = 'GET'
    return request.rest_request(
        conninfo, credentials, method, str(uri), response_file=file_
    )
...
def validate_major_minor_numbers(file_type, major_minor_numbers):
    if (
        file_type == 'FS_FILE_TYPE_UNIX_BLOCK_DEVICE'
        or file_type == 'FS_FILE_TYPE_UNIX_CHARACTER_DEVICE'
    ):
...
@request.request
def rename(
    conninfo, credentials, name, source, dir_path=None, dir_id=None, clobber=False
):
    """
...
def release_nlm_locks_by_client(
    conninfo, credentials, owner_name=None, owner_address=None
):
...
@request.request
def tree_walk_preorder(conninfo, credentials, path, max_depth=-1):
    path = str(path)

    def call_read_dir(conninfo, credentials, path, max_depth):
        if max_depth == 0:
            return
            ...
            if 'files' in result.data:
                for f in result.data['files']:
                    ...
                    if f['type'] == 'FS_FILE_TYPE_DIRECTORY':
                        for ff in call_read_dir(
                            conninfo, credentials, f['path'], max_depth
                        ):
                            yield ff
    ...
    for f in call_read_dir(conninfo, credentials, path, max_depth):
        yield f
    ...
    def call_read_dir(conninfo, credentials, path):
        for result in read_entire_directory_and_ignore_not_found(
            ...
            if 'files' in result.data:
                for f in result.data['files']:
                    if f['type'] == 'FS_FILE_TYPE_DIRECTORY':
                        for ff in call_read_dir(conninfo, credentials, f['path']):
                            yield ff
                    yield request.RestResponse(f, result.etag)
...
def acl_explain_chmod(conninfo, credentials, path=None, id_=None, mode=None):
    ...
    return request.rest_request(
        conninfo, credentials, method, str(uri), body={'mode': mode}
    )
...
@request.request
def create_stream(
    conninfo, credentials, stream_name, path=None, id_=None, if_match=None
):
    method = 'POST'
    uri = build_files_uri([ref(path, id_), 'streams']).append_slash()
    if_match = None if not if_match else str(if_match)
...
@request.request
def rename_stream(
    conninfo, credentials, old_id, new_name, path=None, id_=None, if_match=None
):
    method = 'POST'
