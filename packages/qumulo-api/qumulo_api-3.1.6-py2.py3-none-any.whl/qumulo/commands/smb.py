...
class SMBAddShareCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        # Deprecated, remove in 3.2.0
        parser.add_argument(
            '--bytes-per-sector', type=int, default=None, help=argparse.SUPPRESS
        )
    ...
    def main(conninfo, credentials, args, outfile=sys.stdout, smb_mod=smb):
        if args.bytes_per_sector is not None:
            warnings.warn('--bytes-per-sector is deprecated', DeprecationWarning)
...
class SMBModifySettingsCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            ...
            nargs='*',
            help='Space separated set of SMB dialects to allow clients to '
            'negotiate. Specify no dialects (eg. -d "") to turn off SMB '
            'for the cluster. Choose from the following: '
            'smb2_dialect_3_11, smb2_dialect_3_0, '
            'smb2_dialect_2_1, smb2_dialect_2_002.',
        )
...
class SMBCloseFileHandleCommand(qumulo.lib.opts.Subcommand):
    ...
    FORMATTED_DESCRIPTION = SYNOPSIS + textwrap.dedent(
        """\n
...
class SMBCloseSessionsCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args, smb_mod=smb, opts_mod=qumulo.lib.opts):
        ...
        smb_mod.close_smb_sessions(conninfo, credentials, sessions)
