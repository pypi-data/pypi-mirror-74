...
class ResolveIpAddresses(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        parser.add_argument(
            '--ips', required=True, nargs='+', help='IP addresses to resolve'
        )

    @staticmethod
    def main(conninfo, credentials, args):
        print(dns.resolve_ips_to_names(conninfo, credentials, args.ips))
...
class ResolveHostnames(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        parser.add_argument(
            '--hosts', required=True, nargs='+', help='Hostnames to resolve'
        )
