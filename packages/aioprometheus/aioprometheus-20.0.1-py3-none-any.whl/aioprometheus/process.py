...
class ProcessCollector(object):
    ...
    def __init__(self, namespace="", pid=lambda: "self", proc="/proc"):
    ...
    def observe(self):
        ...
        try:
            ...
            vmem = float(parts[20])
            rss = float(parts[21]) * _PAGESIZE
            start_time_secs = float(parts[19]) / self._ticks
            start_time = start_time_secs + self._btime
        ...
        try:
            max_fds = float(line.split()[3])
            open_fds = len(os.listdir(os.path.join(pid, "fd")))
