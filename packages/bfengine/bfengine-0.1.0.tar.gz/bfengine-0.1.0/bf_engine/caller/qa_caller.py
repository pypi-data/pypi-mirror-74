...
from bf_engine.entity.enums import QuestionType, QuestionMode
...
class QACaller(CallerBase):
    ...
    def upload_status(self, data_id: str) -> int:
        """
        查询上传状态
        :param data_id: 上传id
        :return 上传进度
        """
        resp = requests.get(self.upload_faq_upload_status_url + "/" + data_id, headers=self.header, json={})
    ...
    def train_status(self, train_id):
        ...
        :return 训练进度
        """
        resp = requests.get(self.upload_faq_train_status_url + "/" + train_id, headers=self.header, json={})
        progress = resp.json()["data"]["progress"]
