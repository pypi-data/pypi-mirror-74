...
class TaskEngine(Module):
    ...
    def load(self, path):
        ...
        if scenario_id is None:
            ...
            log.info("te: 导入scenario【" + scenario_name + "】完毕。")
            if self.enable_scenario(scenario_id):
                return self._public_scenario(scenario_id)
        return False
    ...
    def query(self, txt):
        ...
        try:
            ...
            if "TTSText" not in resp or resp["TTSText"] is None:
                Answer(Config.backfill_speech, 60)
            else:
                return Answer(resp["TTSText"], 100)
        except Exception:
        ...
        try:
            ...
            for scenario in scenario_lists:
                if 'scenarioID' in scenario and scenario['scenarioID'] == scenario_id:
                    log.info("te: 导入scenario【" + scenario_id + "】存在重复scenario，进行新增导入。")
                    metadata = self._create_scenario(scenario_name)
                    if metadata is None:
                        log.info("te: 导入scenario【" + scenario_id + "】存在重复scenario，新增失败")
                        return None, None
                    break
            ...
            if metadata is not None:
                ...
                log.info("te: 导入scenario【" + scenario_id + "】存在重复scenario，新增scenarioid为【" + scenario_id + "】")
            else:
                params['upload'] = 1
                params['type'] = 0
            ...
            if resp is None or resp.status_code != 200:
                return None, None
            log.info("te: 导入scenario【" + scenario_id + "】完毕。")
    ...
    def enable_scenario(self, scenario_id):
        ...
        """
        try:
            log.info("te: 启用scenario【" + scenario_id + "】开始。")
            ...
            if resp is None or resp.status_code != 200:
                log.info("te: 启用scenario【" + scenario_id + "】失败。")
                return False
            log.info("te: 启用scenario【" + scenario_id + "】完毕。")
            return True
        except Exception:
            msg = "Exception When POST '{}' : {}".format(self.query_scenarios_url, traceback.format_exc())
    ...
    def _disable_scenario(self, scenario_id):
        ...
        """
        try:
            log.info("te: 停用scenario【" + scenario_id + "】开始。")
            params = {'scenarioid': scenario_id, "appid": self.app_id, 'enable': 'false'}
            req = Request('POST', self.query_scenarios_url, headers=self.headers, params=params)
            resp = self.sess.send(req.prepare(), verify=False, timeout=2)
            if resp is None or resp.status_code != 200:
                log.info("te: 停用scenario【" + scenario_id + "】失败。")
                return False
            log.info("te: 停用scenario【" + scenario_id + "】完毕。")
            return True
        except Exception:
            msg = "Exception When POST '{}' : {}".format(self.query_scenarios_url, traceback.format_exc())
    ...
    def _public_scenario(self, scenario_id):
        ...
        """
        try:
            log.info("te: 发布scenario【" + scenario_id + "】开始。")
            ...
            resp = self.sess.send(req.prepare(), verify=False, timeout=2)
            if resp is None or resp.status_code != 200:
                log.info("te: 发布scenario【" + scenario_id + "】失败。")
                return False
            log.info("te: 发布scenario【" + scenario_id + "】完毕。")
            return True
        except Exception:
            msg = "Exception When POST '{}' : {}".format(self.query_scenario_info_url, traceback.format_exc())
    ...
    def _create_scenario(self, scenario_name):
        ...
        """
        try:
            log.info("te: 新建scenario【" + scenario_name + "】开始。")
            ...
            resp = self.sess.send(req.prepare(), verify=False, timeout=2).json()
            log.info("te: 新建scenario【" + scenario_name + "】完毕。")
    ...
    def _del_scenarios(self, scenario_id):
        ...
        """
        try:
            log.info("te: 删除scenario【" + scenario_id + "】开始。")
            params = {"scenarioid": scenario_id, "delete": 1}
            req = Request('PUT', self.query_scenario_info_url, headers=self.headers, params=params)
            resp = self.sess.send(req.prepare(), verify=False, timeout=2)
            if resp is None or resp.status_code != 200:
                log.info("te: 删除scenario【" + scenario_id + "】失败。")
                return False
            log.info("te: 删除scenario【" + scenario_id + "】完毕。")
            return True
        except Exception:
            msg = "Exception When POST '{}' : {}".format(self.query_scenario_info_url, traceback.format_exc())
