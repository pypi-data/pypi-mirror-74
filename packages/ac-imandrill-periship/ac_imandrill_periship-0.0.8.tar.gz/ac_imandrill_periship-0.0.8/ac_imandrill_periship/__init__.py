...
def template_merge_tags(api_key, template_name, merge_language):
    ...
    for tag in all_merge_tags:
        if 'MC' not in tag and 'END' not in tag:
            if 'if:' in tag.lower():
                tag = re.sub(substr1, '', tag.lower())
                if ' ' in tag:
                    tag = tag.split(' ', 1)[0]
    ...
    for tag in add_merge_tags:
        if tag.lower() not in merge_tags:
            merge_tags.append(tag.lower())
    return {'merge_tags': merge_tags, 'subject': subject_line}
...
def send_template(api_key, template_name, fields):
    ...
    try:
        ...
        msg_to_send = {
            ...
            "to": fields['to'],
            "headers": {
                "Reply-To": fields['reply-to'] if 'reply-to' in fields.keys() else None
            },
            "important": True,
            "track_opens": True,
            "track_clicks": True,
        ...
        return "Missing required field in message=" + str(e)
...
def send_template_attachment(api_key, template_name, fields):
    ...
    try:
        ...
        if fields['attachments'][0]['content']:
            ...
            for attach in attachment_list:
                entry = {'type': 'application/octet-stream', 'name': attach['name'],
                         'content': image_to_base64_encoder(attach['content'])}
                structured_attachments.append(entry)
