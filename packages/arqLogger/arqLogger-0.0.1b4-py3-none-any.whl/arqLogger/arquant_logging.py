...
class ArquantLogger():
    ...
    def __init__(self, url, strategy, strategy_id, param_list):
    ...
    def _on_error(self, exception):
        if self.retry < 5:
            self.strategy.logs("error during websocket connection: %s." % exception)
            if not self.ws_connection.sock.connected:
                self.strategy.logs("retring connecting nro: %s" % self.retry)
                self.retry += 1
                self.connect()
        else:
            self.strategy.logs("stop trying to connect to websocket. tried to connect %s times and fail. exception: %s" % (self.retry, exception))
    ...
    def new_limit_order(self, data, side, px, qty, additional=None):
        if side is Side.Buy.value:
            order = self.strategy.buy(data=data, price=px, size=qty, exectype=self.strategy.Order.Limit)
        elif side is Side.Sell.value:
            order = self.strategy.sell(data=data, price=px, size=qty, exectype=self.strategy.Order.Limit)
        else:
