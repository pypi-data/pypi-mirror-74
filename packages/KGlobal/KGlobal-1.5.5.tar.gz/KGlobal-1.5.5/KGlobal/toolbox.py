...
class Toolbox(TBoxBase, SQLQueue, LogHandle):
    ...
    def __init__(self, script_path, logging_dir=None, logging_base_name=None, max_pool_size=None,
                 *args, **kwargs):
    ...
    def config_sql_conn(self, sql_config, new_instance=False, conn_max_pool_size=DEFAULT_CONNECTION_SIZE):
        ...
        if new_instance or engine is None:
            self.create_sql_engine_to_pool(sql_config=sql_config, conn_max_pool_size=conn_max_pool_size)
            engine = self.__find_engine(sql_config)
