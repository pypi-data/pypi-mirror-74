...
class MDG(Tk):
    ...
    def __build(self):
        header = ['Welcome to main database setup!', 'Please fill out the below fields to setup']
        header2 = ['Welcome to existing main database setup!', 'Please fill out the below fields to setup']

        # Set GUI Geometry and GUI Title
        self.geometry('280x160+550+150')
        ...
        settings_dir_label = Label(new_settings_frame, text='DB Dir:')
        ...
        # Apply Widgets to existing_frame
        #   Apply Main Filepath Entry Box Widget
        settings_fp_label = Label(existing_main_frame, text='DB Fp:')
        ...
        # Apply Buttons to the existing_buttons_frame
        #     Generate button
        generate_button = Button(self, text='Link DB', width=15, command=self.__generate2)
    ...
    def __generate2(self):
        if not self.__main_db_path:
            ...
            if not self.__pointer:
                ...
                self.__pointer = DataConfig(file_dir=self.__local_db_dir, file_name_prefix='Script_Pointer',
