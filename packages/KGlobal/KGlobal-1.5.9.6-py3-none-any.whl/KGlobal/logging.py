...
class LogHandle(object):
    ...
    def __init__(self, file_dir=None, base_name=None):
        ...
        p = Thread(target=self.__pipe_listen)
        p.daemon = True
        p.start()
    ...
    def gui_console(self, title=None, destroy=False, turn_off=False, gui_obj=None):
        self.__write_pipe.send({'command': 'gui_console','title': title, 'destroy': destroy, 'turn_off': turn_off,
                                'gui_obj': gui_obj})

    def write_to_log(self, message, action='info', print_only=False):
        self.__write_pipe.send({'command': 'write_to_log', 'message': message, 'action': action,
                                'print_only': print_only})

    def __gui_console_set(self, title=None, destroy=False, turn_off=False, gui_obj=None, *args, **kwargs):
