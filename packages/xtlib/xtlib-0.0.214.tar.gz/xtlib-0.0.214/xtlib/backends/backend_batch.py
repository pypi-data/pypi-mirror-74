...
class AzureBatch(BackendBase):
    ...
    def adjust_run_commands(self, job_id, job_runs, using_hp, experiment, service_type, snapshot_dir, args):
        ...
        for i, box_runs in enumerate(job_runs):
            ...
            # NOTE: we pass the NODE_ID and RUN_NAME as parameters to the fn_wrapped script
            ...
            if merge_batch_logs:
                ...
                # create an STDBOTH.txt that merged stdout and stderr
                node_cmd = "{} > ../stdboth.txt 2>&1".format(" ".join(wrapped_parts))
                ...
                run_specs["cmd_parts"] = ["bash", constants.FN_BATCH_HELPER, node_id, run_name]
    ...
    def wrap_cmd_for_batch(self, user_cmd, node_id, store_data_dir, data_action, data_writable, 
        ...
        super().add_first_cmds(cmds, change_dir=True, args=args)
        ...
        store_path = "runs/$2/output"     # $2 is the run_name pass as 2nd arg to the script

        if docker_cmd:
            # blobfuse preinstalled on DSVM but necessarily on docker image
            self.append_install_blobfuse_cmds(cmds, sudo_available=True)

        self.append_title(cmds, "mount WORKSPACE for output dir")
        self.emit_mount_cmds(cmds, storage_name, storage_key, container=workspace, store_path=store_path, 
            ...
            sudo_available=False, use_username=False, use_allow_other=False, env_var_name="XT_OUTPUT_DIR", 
    ...
    def submit_job(self, job_id, job_runs, workspace, compute_def, resume_name, 
        ...
        for n, run_data_list in enumerate(job_runs):    # run_data_list_by_node):
            ...
            node_id = "node" + str(n)
            service_info_by_node[node_id] = {"job_id": job_id, "node_id": node_id, "task_index": n, "merge_batch_logs": merge_batch_logs}
    ...
    def read_log_file(self, service_node_info, log_name, start_offset=0, end_offset=None, 
        ...
        if log_name is None or log_name in ["primary", "stdout", "stdout.txt"]:
            file_path = "stdboth.txt" if merge_batch_logs else "stdout.txt"
        ...
        if start_offset == 0:
            ...
            node_index = utils.node_index(node_id)
            job_path = "node-{}/after/{}".format(node_index, file_path)
            if self.store.does_job_file_exist(job_id, job_path):
                new_text = self.store.read_job_file(job_id, job_path)
        ...
        return {"new_text": new_text, "simple_status": simple_status, "log_name": log_name, "next_offset": next_offset, 
            "service_status": batch_status, "file_path": file_path}
