...
def read_from_pipe(pipe, run_info, runner, stdout_is_text):
    ...
    try:
        ...
        self.store.log_job_event(self.job_id, "node_error", {"node_id": self.node_id, "run": self.parent_run_name, "exception": str(ex)})
...
class XTController(rpyc.Service):
    ...
    def __init__(self, concurrent=1, my_ip_addr=None, multi_run_context_fn=None, multi_run_hold_open=False, 
        ...
        if os.path.exists(fn):
            #console.print("removing previous CONTROLLER LOG file")
            os.remove(fn)
    ...
    def reset_state(self, start_queue_worker=False):
        ...
        log_info("rundir_parent", self.rundir_parent)
        log_info("PYTHONPATH",  os.getenv("PYTHONPATH"))
        log_info("current CONDA env",  os.getenv("CONDA_DEFAULT_ENV"))
    ...
    def process_multi_run_context(self, multi_run_context_fn):
        ...
        # cache store for later use (don't want storge/mongo traffic needed for subsequent creations)
        store = store_from_context(self.mrc_context)
        self.store = store
        mongo = store.mongo

        # log start of controller to JOB
        store.log_job_event(self.job_id, "node_start", {"node_id": self.node_id, "run": parent_run_name})

        self.use_simple_scheduling = self.mrc_context.use_simple_scheduling
        ...
        if not self.use_simple_scheduling:
    ...
    def bg_queue_worker(self):
        ...
        '''
        console.print("bg_queue_worker: background thread started")
    ...
    def on_shutdown(self, context):
        ...
        log_title("XT controller: mongo call stats")
        self.store.mongo.print_call_stats()

        # log end of controller to JOB
        self.store.log_job_event(self.job_id, "node_end", {"node_id": self.node_id, "run": self.parent_run_name})
    ...
    def idle_check(self):
        ...
        if alive_count == 0:
            ...
            if self.idle_check_count > MAX_IDLE_CHECK:
                ...
                console.print("idle_check: waiting for runs to wrap up: {}".format(not_wrapped_up))
            else:
                console.print("idle_check: ALL RUNS are wrapped up")
                self.on_idle()
    ...
    def process_queue_entry(self, run_info):
        if run_info.parent_prep_needed:
            ...
            if self.mri:
                ...
                # simple scheduler (static, with no run-level restart support)
                ri = self.first_run_index + self.next_child_id - 1
                if ri > self.last_run_index:
                    entry = None
                else:
                    run_name = self.parent_run_name + "." + str(self.next_child_id)
                    self.next_child_id += 1
                    entry = {"run_index": ri, "run_name": run_name, "node_id": self.node_id, "status": "unstarted", "job_id": self.job_id}

            # log run index to JOB
            store.log_job_event(self.job_id, "get_index", {"node_id": self.node_id, "entry": entry})
    ...
    def exit_handler(self, run_info, run_info_is_locked=False, called_from_thread_watcher=False):
        ...
        '''
        console.print("exit_handler: {}".format(run_info.run_name))
        ...
        if run_info.parent_prep_needed:
            ...
            # log run end to JOB
            self.store.log_job_event(self.job_id, "end_run", {"node_id": self.node_id, 
                "run": run_info.run_name, "status": run_info.status, "exit_code": run_info.exit_code})

            console.print("run_end logged to job: {}".format(run_info.run_name))
        ...
        console.print("end of exit handler")
    ...
    def queue_job_core(self, context, cmd_parts, previously_queue=False, aml_run=None):
        ...
        run_info = RunInfo(run_name, context.ws, cmd_parts, run_script, context.repeat, context, "queued", True, 
            ...
            node_id=self.node_id, run_index=None, store=self.store)
    ...
    def start_local_run_core(self, run_info, cmd_parts, rundir, rundir_index):
        ...
        # before we can turn this on, we need to work out issues with not copying batch-specific dirs
        # in the batch working dir (for batch, need to move to wd/xt/cwd (not ~/.xt/cwd))
        side_load_files = False
        ...
        if side_load_files:
            ...
            omit_list = ["rundirs", "mnt", "blobfusetmp*"]
        ...
        if mnt_path and parent_name:
            ...
            log_info("child XT_OUTPUT_MNT", child_mnt_path)
        ...
        store.log_job_event(self.job_id, "start_run", {"node_id": self.node_id, "run": run_name, "restart": context.restart})
