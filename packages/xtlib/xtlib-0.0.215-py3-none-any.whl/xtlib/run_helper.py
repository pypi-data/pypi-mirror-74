...
def get_filtered_sorted_limit_runs(store, config, show_gathering, col_dict=None, args=None, preserve_order=False):
...
def get_service_node_info(store, core, workspace, run_name):
    rr = get_run_record(store, workspace, run_name, {"job_id": 1, "node_index": 1})
    job_id = rr["job_id"]
    node_index = rr["node_index"]

    service_node_info, backend = job_helper.get_service_node_info(store, core, job_id, node_index)
...
def build_metrics_sets(records, steps=None, merge=False, metrics=None, timebase=None):
    '''
