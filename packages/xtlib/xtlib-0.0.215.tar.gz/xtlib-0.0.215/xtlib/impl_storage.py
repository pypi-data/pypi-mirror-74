...
class ImplStorage(ImplBase):
    ...
    @option(name="line-size", type=float, default=1, help="the line width used to draw line plots")
    ...
    def plot(self, runs, col_list, x, layout, break_on, title, show_legend, plot_titles, legend_titles, 
        ...
        x_label, legend_args, plot_args, colors, color_map, color_steps, x_min, x_max, y_min, y_max, 
        sort, reverse, alpha, line_size, timebase, x_format, y_format, buffer_size, plot_type="line"):
        ...
        args = {"run_list": runs, "workspace": workspace, "all": True, 
            ...
            "max_runs": max_runs, "columns": ["run", "metrics.*"], "buffer_size": buffer_size }
        ...
        pb = plot_builder.PlotBuilder(run_names, col_list, x_col, layout, break_on, title, show_legend, plot_titles,
            ...
            group_by, error_bars, show_plot, save_to, x_label, colors, color_map, color_steps, legend_args, 
            x_min, x_max, y_min, y_max, alpha, line_size, timebase, x_format, y_format, plot_args)
    ...
    @argument(name="run-name", type="str", help="the name of the run to get logs for", required=True)
    ...
    def extract_logs(self, run_name, dest_dir, workspace, response, browse):
        
        console.print("extracting files for: {}...".format(run_name))
        extract = True
        ...
        if extract:
            ...
            service_node_info, backend = run_helper.get_service_node_info(self.store, self.core, workspace, run_name)
            items = backend.download_logs(service_node_info, dest_dir)
            count = len(items)
    ...
    def get_hp_config_path(self, aggregate_dest, dest_name, hp_config_dir, last_chance=False):
        if aggregate_dest == "job":
            _, filenames = self.store.get_job_filenames(dest_name, hp_config_dir)
            if not filenames and last_chance:
                errors.store_error("Missing hp-config file for job={}".format(dest_name))
        else:
            _, filenames = self.store.get_experiment_filenames(workspace, dest_name, hp_config_dir)
            if not filenames and last_chance:
    ...
    def plot_summary(self, aggregate_name, workspace, timeout, primary_metric):

        dest_name = aggregate_name
        aggregate_dest = "job" if job_helper.is_job_id(dest_name) else "experiment"

        hp_config_path = self.get_hp_config_path(aggregate_dest, aggregate_name, constants.HP_CONFIG_DIR)
        if not hp_config_path:
            # try old config dir name
            hp_config_path = self.get_hp_config_path(aggregate_dest, aggregate_name, constants.OLD_HP_CONFIG_DIR, last_chance=True)

        search_history = SearchHistory(agg_name=aggregate_name, workspace=workspace, timeout=timeout, primary_metric=primary_metric, 
            hp_config_file_name=hp_config_path)
    ...
    def dump_mongo_doc(self, name, doc, prev_indent, show_log_records=False):
            ...
            if key == "log_records":
                if show_log_records:
                    ...
                    for i, lr in enumerate(value):
                        self.dump_mongo_doc("[{}]".format(i), lr, indent)
                else:
    ...
    def view_mongo(self, name, workspace, log_records):
        ...
        if name.startswith("run"):
            ...
            mongo_docs = self.store.get_ws_runs(ws, filter_dict, True)

            for doc in mongo_docs:
                #console.print("doc:")
                self.dump_mongo_doc("\nMONGO-DB for " + full_run_name, doc, "", log_records)
