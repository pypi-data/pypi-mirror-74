...
class Exchange(object):
    ...
    def fetch(self, url, method='GET', headers=None, body=None):
        ...
        try:
            if isinstance(self.proxies, list):
                _proxies = random.choice(self.proxies)
            else:
            ...
            error_string = str(e)
            if ('ECONNRESET' in error_string) or ('Connection aborted.' in error_string) or ('ProxyError' in error_string):
    ...
    def amount_to_precision_ceil(self, symbol, amount):
        result = self.decimal_to_precision(amount, ROUND, self.markets[symbol]['precision']['amount'], self.precisionMode)
        result = round(float(result) + 10**(-self.markets[symbol]['precision']['amount']),
            self.markets[symbol]['precision']['amount'])
        return result
