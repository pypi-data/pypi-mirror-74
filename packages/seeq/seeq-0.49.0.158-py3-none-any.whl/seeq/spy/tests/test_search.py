...
def test_search_with_url_wrong_url():
    ...
    workbook_id = uuid.uuid1()
    worksheet_id = uuid.uuid1()
...
def test_search_of_topic_url():
    ...
    topic = workbooks_api.create_workbook(body=WorkbookInputV1(name="test_Topic_search_url",
                                                               data=json.dumps(dict(isReportBinder=True))))
    topic_worksheet = workbooks_api.create_worksheet(workbook_id=topic.id, body=WorksheetInputV1(name='Doc1'))

    with pytest.raises(ValueError, match=f'URL must be for a valid Workbench Analysis. '
                                         f'You supplied a URL for a Topic.'):
        spy.search(f'{host}/workbook/{topic.id}/worksheet/{topic_worksheet.id}')
