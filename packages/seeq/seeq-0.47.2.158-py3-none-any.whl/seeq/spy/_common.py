...
HTML_EQUALS_REGEX = r'(?:=|&#61;)'
WORKSHEET_LINK_REGEX = r'links\?type%sworkstep&amp;workbook%s(%s)&amp;worksheet%s(%s)&amp;' % (
    HTML_EQUALS_REGEX, HTML_EQUALS_REGEX, GUID_REGEX, HTML_EQUALS_REGEX, GUID_REGEX)
WORKSTEP_LINK_REGEX = r'%sworkstep%s(%s)' % (
    WORKSHEET_LINK_REGEX, HTML_EQUALS_REGEX, GUID_REGEX)
...
ROLL_UP_FUNCTIONS = [
    ...
    RollUpFunction('counts', 'counts', 'Condition', 'Signal', 'vararg'),
    RollUpFunction('combine with', 'combineWith', 'Condition', 'Condition', 'fluent'),
...
def get_variable_number_statistic_function(statistic):
    match = re.match('^\w+\(([0-9]*.?[0-9]+|[0-9]+|[0-9]+.?[0-9]*)\)$', statistic)
...
def get_variable_time_unit_statistic_function(statistic):
    if re.match('^[\w ]+(\([\'"](s|min|h|day)[\'"]\))?$', statistic) is not None:
...
def get_analysis_worksheet_from_url(url, include_archived=False):
    ...
    worksheet_id = get_worksheet_id_from_url(url)
    ...
    if get(workbook.definition, 'Archived') and not include_archived:
