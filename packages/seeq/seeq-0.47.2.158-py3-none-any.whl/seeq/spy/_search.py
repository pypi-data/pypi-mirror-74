...
def search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
           include_archived=False, estimate_sample_period=None, quiet=False,
           status=None):
    """
    ...
    try:
        return _search(query, all_properties=all_properties, workbook=workbook, recursive=recursive,
                       include_archived=include_archived, estimate_sample_period=estimate_sample_period,
...
def _search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
            include_archived=False, estimate_sample_period=None, quiet=False, status=None):
    status = Status.validate(status, quiet)
    ...
    if isinstance(query, pd.DataFrame):
        ...
        worksheet = _common.get_analysis_worksheet_from_url(query, include_archived)
    ...
    for status_index in range(len(queries)):
        ...
        if _common.present(current_query, 'ID'):
            ...
            _add_to_metadata(_add_all_properties(current_query['ID'], dict(), _get_tree=all_properties))
