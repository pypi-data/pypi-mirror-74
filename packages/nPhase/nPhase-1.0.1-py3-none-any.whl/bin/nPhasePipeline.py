...
def nPhasePipeline(args):
    ...
    outputLog,systemMessage=longReadMapping(args.strainName,args.longReadFile,args.reference,mappedLongReadPath,splitReadFlag,args.longReadPlatform,args.threads)
    print(systemMessage)
    ...
    outputLog,systemMessage=shortReadMapping(args.strainName,args.shortReadFile_R1,args.shortReadFile_R2,args.reference,mappedShortReadPath)
    print(systemMessage)
    updateLog(fullLogPath,outputLog)
    ...
    getShortReadPositions(hetSNPVCFFilePath,shortReadPositionsOutputFilePath)
    ...
    assignLongReadToSNPs(cleanLongReadSamFile,shortReadSNPsBedFilePath,args.reference,minQ,minMQ,minAln,longReadPositionNTFile)
    ...
    longReadValidation(longReadPositionNTFile,minCov,minRatio,minTrioCov,validatedSNPAssignmentsFile,contextDepthsFile)

    #Run everything through nPhase
    nPhase(validatedSNPAssignmentsFile,args.strainName,contextDepthsFile,phasedPath,args.reference,args.minSim,args.minOvl,args.minLen,args.maxID)
    ...
    generateDataVis(simpleOutPath,datavisPath)
    ...
    generateLongReadFastQFiles(haplotigReadNameFile,args.longReadFile,fastQFilePrefix)#Needs to work more efficiently, shouldn't just load the entire fastq into memory...right? (why not?) Do I do that at any other point? #Make this last just in case.
...
def nPhaseAlgorithm(args):
    ...
    #Run everything through nPhase
    nPhase(args.validatedSNPAssignmentsFile,args.strainName,args.contextDepthsFile,phasedPath,args.reference,args.minSim,args.minOvl,args.minLen,args.maxID)
...
#subparsers = parser.add_subparsers(help="selecting 'pipeline' will run through all of the steps, whereas selecting 'nPhase' will only perform the phasing operation",required=True,dest='\n\nSelect pipeline or nphase mode. For example:\n\npython nPhasePipeline.py pipeline [args]\n\npython nPhasePipeline.py nphase [args]')
...
parser_b = subparsers.add_parser('nphase', help='Only run the nphase algorithm. NOTE: This will require files generated by running the pipeline mode',parents=[parser])
...
import os
import subprocess
from nPhasePipelineFunctions import *
from nPhase import *
