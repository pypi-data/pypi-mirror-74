...
class AppHealthCheckMonitor:
    ...
    def _start(self, identifier: AppInstanceIdentifier):
        ...
        if succeed:
            check_public = any([app for app in self._apps
                                if app.name == identifier.name and app.has_public_access]) and \
                           self._app_service.can_access_from_public_address(identifier=identifier)
    ...
    def _test_public_network(self, identifier: AppInstanceIdentifier):
        ...
        try:
            ...
            #                                          dns_name=address)
            healthy = self._app_service.public_healthcheck(identifier)
            if healthy:
