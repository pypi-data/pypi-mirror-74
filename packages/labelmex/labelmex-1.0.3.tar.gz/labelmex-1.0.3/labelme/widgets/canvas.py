...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setStyleSheet("""QToolTip img { 
                            display : block;
                            margin : auto;
                           }""")
    ...
    def mouseMoveEvent(self, ev):
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            if index is not None:
                ...
                product_dir = osp.join(here, '../products')
                icon_path = osp.join(':/', product_dir, '%s.jpg' % shape.label.lower())
                icon_path = osp.abspath(icon_path) # img scr does not take in relative path
                if not osp.exists(icon_path):
                    print('Product image (%s) does not exist' % shape.label)
                    icon_path = osp.join(':/', product_dir, '%s.png' % 'missing_file')
                    icon_path = osp.abspath(icon_path)

                # self.setToolTip('<center><b>%s</b><p><img src="%s" width="75"></center>' % (shape.label, icon_path))

                self.setToolTip(
                    self.tr('<center><b>%s</b><p><img src="%s" width="100"></center>' % (shape.label, icon_path)))
                # self.setStatusTip(self.toolTip())
                self.setStatusTip("Click & drag to move shape '%s'" % shape.label)
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        #JJ
            ...
            if not self.selectedVertex():
                if not (int(ev.modifiers()) == QtCore.Qt.ControlModifier):
                    if QT5:
                        pos = self.transformPos(ev.localPos())
                    else:
                        pos = self.transformPos(ev.posF())
