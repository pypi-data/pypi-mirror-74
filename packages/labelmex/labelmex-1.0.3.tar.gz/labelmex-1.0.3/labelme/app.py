...
from labelme.widgets import LabelDialogAuto
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        #JJ
        self.labelDialogAuto = LabelDialogAuto(
            parent=self,
            labels=self._config['labels'],
            sort_labels=self._config['sort_labels'],
            show_text_field=self._config['show_label_text_field'],
        ...
        self.canvas.viewShape.connect(self.editLabel_shape) #JJ
        ...
        #JJ
        trainModel = action(
            text=self.tr('&Train Model'),
            slot=self.trainModel,
            #shortcuts['annotate_file'],
            icon='annotate',
            tip=self.tr('Submit & train model'),
            enabled=False)
        
        #JJ
        autoAnnotate = action(
            text=self.tr('&Auto Annotate'),
            slot=self.autoAnnotate,
            #shortcuts['annotate_file'],
            icon='automate',
            tip=self.tr('Auto annotate products'),
            enabled=False)

        #JJ
        setTrainAddr = action(
            text=self.tr('&Set Training API'),
            slot=self.setTrainAddr,
            #shortcuts['annotate_file'],
            icon='server',
            tip=self.tr('Enter API address for training model.'),
            enabled=True)

        #JJ
        setPredictAddr = action(
            text=self.tr('&Set Predict API'),
            slot=self.setPredictAddr,
            #shortcuts['annotate_file'],
            icon='server',
            tip=self.tr('Enter API address for predicting images.'),
            enabled=True)
        ...
        self.actions = utils.struct(
            ...
            trainModel=trainModel, #JJ
            autoAnnotate=autoAnnotate, #JJ
            setTrainAddr=setTrainAddr, #JJ
            setPredictAddr=setPredictAddr, #JJ
    ...
    def editLabel(self, item=None):
        ...
        text, flags, group_id, code = self.labelDialogAuto.popUp(
            text=shape.label, flags=shape.flags, group_id=shape.group_id,  points=shape.points, image=self.image
        )
        if code == 2:
            # print('Cancel Validation')
            return
        if code == 0:
            # print('delete shape')
        ...
        if not self.uniqLabelList.findItemsByLabel(shape.label):
            new_item = self.uniqLabelList.createItemFromLabel(shape.label)
            self.uniqLabelList.addItem(new_item)
            rgb = self._get_rgb_by_label(shape.label)
            self.uniqLabelList.setItemLabel(new_item, shape.label, rgb)
            self.labelDialog.addLabelHistory(shape.label)
            self.labelDialogAuto.addLabelHistory(shape.label)
    ...
    #JJ
    def saveCocoLabels(self, filename):
        lf = LabelFile()
    ...
    def newShape(self):
        ...
        if self._config['display_label_popup'] or not text:
            ...
            if not text:
                ...
                self.labelDialogAuto.edit.setText(previous_text)
    ...
    #JJ
    def editLabel_shape(self, shapes):
        for shape in shapes:
            text, flags, group_id, code = self.labelDialogAuto.popUp(
                text=shape.label,move=False, flags=shape.flags, group_id=shape.group_id, points=shape.points, image=self.image
            )
            if code == 2:
                # print('Cancel Validation')
                break
    ...
    def trainModel(self):
        mb = QtWidgets.QMessageBox
        msg = self.tr('Submit labels to train model?')
        answer = mb.question(self, self.tr('Train Model'), msg, mb.Yes | mb.No)
        if answer == mb.No:
            return
        self.enableSaveImageWithData(True)
        basename = osp.basename(osp.splitext(self.filename)[0])
        labelfile_name = osp.join(self.currentPath(), basename + LabelFile.suffix)

        self.saveLabels(labelfile_name)

        try:
            with closing(open(labelfile_name, 'rb')) as f:
                requests.post(self._config['train_url'], data=f, stream=True)
        except requests.exceptions.RequestException as e:
            self.errorMessage(
                self.tr('Connection Error'),
                self.tr('Unable to reach server: <b>%s</b>') % self._config['train_url']
            )
            self.setTrainAddr()
            return
    ...
    def autoAnnotate(self):
        mb = QtWidgets.QMessageBox
        msg = self.tr('<p><b>%s</b></p>%s' % ('Current unsaved changes will be discarded', 'Submit?'))
        answer = mb.warning(self, self.tr('Auto-Annotate'), msg, mb.Yes | mb.No)
        if answer == mb.No:
            return
        self.progressdialog = QProgressDialog(self)
        self.progressdialog.setWindowTitle("Please wait ...")
        self.progressdialog.setLabelText("Downloading data ...")
        ...
        coco_labelfile_name = osp.join(self.currentPath(), "coco_" + basename  + LabelFile.suffix)
        self.saveCocoLabels(coco_labelfile_name)
        labelfile_name = osp.join(self.currentPath(), basename + LabelFile.suffix)

        self.saveLabels(labelfile_name)

        if 'auto_' in basename:
            auto_basename = basename
        else:
            auto_basename =  "auto_" + basename
        
        auto_labelfile_name = osp.join(self.currentPath(), auto_basename + LabelFile.suffix)
        try:
            data = bytearray()
            with closing(open(labelfile_name, 'rb')) as f:
                with closing(requests.post(self._config['predict_url'], data=f, stream=True)) as res:
                    for line in res.iter_content(chunk_size=1000):
                        data += line
            ...
            self.errorMessage(
                self.tr('Connection Error'),
                self.tr('Unable to reach server: <b>%s</b>') % self._config['predict_url']
            )
            self.setPredictAddr()
            return
        except OSError as e:
        ...
        ann_data = json.loads(data)
        self.progressdialog.reset()

        with open(auto_labelfile_name, 'wb' if PY2 else 'w') as f:
            json.dump(ann_data, f, ensure_ascii=False, indent=2)
        
        if ann_data['imageData'] is not None:
            imageData = base64.b64decode(ann_data['imageData'])
            with io.BytesIO() as f:
                f.write(imageData)
                img = Image.open(f)
                path = osp.join(self.currentPath(), auto_basename  + '.jpg')
                img.save(path)

        self.loadFile(path)
        self.setDirty()
        mb = QtWidgets.QMessageBox
        msg = self.tr('<p><b>%s</b></p>%s' % ('Auto annotation is completed', 'Review?'))
        answer = mb.question(self, self.tr('Sucessful!'), msg, mb.Yes | mb.No)
        if answer == mb.Yes:
            self.editLabel_auto()

    #JJ
    def setTrainAddr(self):
        text, okPressed = QtWidgets.QInputDialog.getText(self, "Train API", "API URL:", QtWidgets.QLineEdit.Normal, self._config['train_url'])
        if okPressed and text != '':
            self._config['train_url'] = text
            print(self._config['train_url'])

    #JJ
    def setPredictAddr(self):
        text, okPressed = QtWidgets.QInputDialog.getText(self, "Predict API", "API URL:", QtWidgets.QLineEdit.Normal, self._config['predict_url'])
        if okPressed and text != '':
            self._config['predict_url'] = text
            print(self._config['predict_url'])
