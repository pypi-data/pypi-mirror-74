...
if __name__ == '__main__':
    ...
    geofileops.buffer(
            ...
            force=force)
    '''
    sql_stmt = f"""SELECT geom /*ST_Buffer(geom, -1, 5) AS geom*/
                        ,code_obj
                        ,gwscod_h
                        ,gesp_pm
                    FROM "{input_prc_path.stem}"
                    WHERE GWSCOD_H not in ('1','2','6','7','8','9','81','85','99','100','895','962','999','9536','9573','9574','9823','9825','9829')
                        AND (GESP_PM is null
                            OR (GESP_PM not like '%SER%'
                                AND GESP_PM not like '%SGM%'
                                AND GESP_PM not like '%PLA%'
                                AND GESP_PM not like '%NPO%'
                                AND GESP_PM not like '%CON%'
                                AND GESP_PM not like '%CIV%'))
            """
    geofileops.select(
            input_path=input_prc_path,
            output_path=prc_m1m_path,
