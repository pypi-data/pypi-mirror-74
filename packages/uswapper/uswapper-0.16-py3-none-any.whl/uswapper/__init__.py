    ...
    def __init__(self):
        ...
        while True:
            try:
                ...
                self.ethprice = float(
                        json.loads( asyncio.run( self.client.execute( '{bundles{ethPrice}}' ) ) )['data']['bundles'][0][
                            'ethPrice'] )
            except:
        ...
        if symbol in self.getassets().values:  # check if the symbol is part of uniswap tokens
            while True:
                try:
                    ...
                    price = float(
                            json.loads( asyncio.run( self.client.execute( f'{{tokens(where: {{symbol: "{symbol}"}}){{'
                                                                          f'derivedETH}}}}' ) ) )['data']['tokens'][0][
                                'derivedETH'] )

                    if price == 0:
                        price = float( json.loads( asyncio.run(
