...
class DotNetBuildManager(BuildManager):
    ...
    def _set_package_version(self):
        ...
        version = self._version.number_str
        print(f'Updating {self._project} projects\' version to {version}:')
    ...
    def _run_msbuild(self):
        ...
        VisualStudio().build.run(
            [f'"{self._project}"', '/t:Clean,ReBuild', f'/p:Configuration={mode.name.title()}']
