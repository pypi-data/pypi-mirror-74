...
class TSNE(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    exaggeration : float, optional
        Value to be multiplied on pij before 250 iterations. The natural
        clusters would be more separated with larger value, which means
        there would be more empty space on the map.
        Default to 12.0.
    angle : float, optional
        The legal value should be between 0.0 to 1.0. Setting it to 0.0 means
        using the "exact" method which would run O(N^2) time, otherwise TSNE
        would employ Barnes-Hut approximation which would run O(NlogN). This
        value is a tradeoff between accuracy and training speed for Barnes-Hut
    ...
    object_frequency : int, optional
        ...
        This parameter value should not be larger than n_iter value.
        Default to 50.
    ...
    perplexity : float, optional
        ...
        above. Larger value is suitable for large dataset.
        Make sure PERPLEXITY * 3 < [no. of samples]
        Default to 30.0.
