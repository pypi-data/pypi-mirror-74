...
The following classes are available:
    ...
    * :class:`KNN`
"""
#pylint:disable=too-many-lines, too-few-public-methods, ungrouped-imports
import logging
import uuid
...
class _KNNBase(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    """
    Base class for k-nearest-neighbor algorithm, be able to do classification and regression.
    """
    algorithm_map = {'brute_force':0, 'kd_tree':1}
    func_map = {'classification': 0, 'regression': 1}
    voting_map = {'majority': 0, 'distance_weighted': 1}
    aggregate_map = {'average': 0, 'distance_weighted': 1}
    ...
    def __init__(self,#pylint:disable=too-many-branches, too-many-statements, too-many-locals, too-many-arguments
                 n_neighbors=None,
                 thread_ratio=None,
        ...
        if self.search_strategy is not None:
            ...
            if self.param_values is not None:
                for x in self.param_values:#pylint:disable=invalid-name
                    ...
                    if x[0] in set_param_list:
                        ...
                        logger.error(msg)
                        raise ValueError(msg)
                    if x[0] == 'n_neighbors' and not (isinstance(x[1], list) and all(isinstance(t, int) for t in x[1])):#pylint:disable=line-too-long
                    ...
                    #    logger.error(msg)
                    #    raise ValueError(msg)
                    if (x[0] in ('category_weights', 'minkowski_power')) and not (isinstance(x[1], list) and all(isinstance(t, (int, float)) for t in x[1])):#pylint:disable=line-too-long
                        msg = "Valid values of `{}` must be a list of numericals.".format(x[0])
                        logger.error(msg)
                        raise TypeError(msg)
                    if x[0] == 'aggregate_type' and self.functionality == self.func_map['classification']:#pylint:disable=line-too-long
                    ...
                    if x[0] == 'voting_type' and self.functionality == self.func_map['regression']:
                        ...
                        logger.error(msg)
                        raise ValueError(msg)
                    if (x[0] in ('metric', 'aggregate_type', 'voting_type')) and not (isinstance(x[1], list) and all(t in self.map_map[x[0]] for t in x[1])):#pylint:disable=line-too-long
            ...
            if self.param_range is not None:
                ...
                for x in self.param_range:#pylint:disable=invalid-name
                    ...
                    #    logger.error(msg)
                    #    raise ValueError(msg)
                    if x[0] == 'n_neighbors' and not (isinstance(x[1], list) and len(x[1]) in rsz and all(isinstance(t, int) for t in x[1])):#pylint:disable=line-too-long
                        msg = "Valid values of `{}` must be a list of int.".format(x[0])
                        logger.error(msg)
                        raise TypeError(msg)
                    if (x[0] in ('category_weights', 'minkowski_power')) and not (isinstance(x[1], list) and len(x[1]) in rsz and all(isinstance(t, (int, float)) for t in x[1])):#pylint:disable=line-too-long
                        msg = "Valid values of `{}` must be a list of numericals.".format(x[0])
                        logger.error(msg)
                        raise TypeError(msg)
                    set_param_list.append(x[0])
    ...
    @trace_sql
    def _fit(self, data, key=None, features=None, label=None, categorical_variable=None):#pylint: disable=too-many-locals, too-many-statements, too-many-branches, too-many-arguments
        ...
        if categorical_variable is not None:
            ...
            try:
                self.categorical_variable = self._arg('categorical_variable', categorical_variable, ListOfStrings)#pylint:disable=line-too-long, undefined-variable
            except:
                msg = ("`categorical_variable` must be list of strings or string.")
                logger.error(msg)
                raise TypeError(msg)
        if not data.dtypes([self.label])[0][1] in ('INT', 'DOUBLE') or self.categorical_variable is not None and self.label in self.categorical_variable:#pylint:disable=line-too-long
        ...
        self._training_set = data[[self.key]  + self.features + [self.label]]#pylint:disable=attribute-defined-outside-init
        if self.resampling_method is not None:
            ...
            try:
                call_pal_auto(conn, 'PAL_KNN_CV', self._training_set, ParameterTable().with_data(param_rows), *tables)#pylint:disable=line-too-long
    ...
    @trace_sql
    def _predict(self, data, key, features=None, categorical_variable=None):#pylint:disable=too-many-locals,too-many-branches,too-many-statements,inconsistent-return-statements
        conn = data.connection_context
        require_pal_usable(conn)
        ...
        if categorical_variable is not None:
            if isinstance(categorical_variable, str):
                categorical_variable = [categorical_variable]
            try:
                categorical_variable = self._arg('categorical_variable', categorical_variable, ListOfStrings)#pylint:disable=line-too-long, undefined-variable
            except:
                msg = ("`categorical_variable` must be list of strings or string.")
                logger.error(msg)
                raise TypeError(msg)
        ...
        try:
            call_pal_auto(conn, 'PAL_KNN', self._training_set, data_, ParameterTable().with_data(param_rows), res_tbl, stats_tbl)#pylint:disable=line-too-long
        except dbapi.Error as db_err:
            logger.exception(str(db_err))
            try_drop(conn, tables)
            raise
...
class KNNClassifier(_KNNBase):
    ...
    thread_ratio : float, optional
        ...
        1 means using at most all the currently available threads. Values outside this range
        are ignored and this function heuristically determines the number of threads to use.
        Default to 0.0.
    voting_type : {'majority', 'distance_weighted'}, optional
        Voting type.
        Default to 'distance_weighted'.
    stat_info : bool, optional
        Indicate if statistic information will be stored into the STATISTIC table.
        Only valid when model evaluation/parameter selection is not enabled.
        Default to True.
    ...
    minkowski_power : float, optional
        ...
        Only valid when metri` is set as 'minkowski'.
        Defaults to 3.0.
    ...
    param_values : ListOfTuples, optional
        ...
        Input should be the a list of tuples, with 1st element of each tuple
        being the target parameter name(in string format),
        while 2nd element being the a list of valued for selection.
    ...
    param_range : ListOfTuples, optional
        ...
        Input should be a list of tuples, with 1st element of each tuple being
        the name of the target parameter(in string format),
        while 2nd element being a list that specifies the range of parameters with the
    ...
    If not set as model evaluation or parameter selection:
        ...
        stats_tbl : DataFrame
            KNN prediction statistics infomation. Structured as following:
                -TEST_ + ID2 column name: Prediction data ID.
                -K: K number.
                -TRAIN_ + ID1 column name: Train data ID.
                -DISTANCE: Distance.
    ...
    """
    def __init__(self, n_neighbors=None, thread_ratio=None, stat_info=None,#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, line-too-long
                 voting_type=None, metric=None, minkowski_power=None, category_weights=None, algorithm=None, resampling_method=None,#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, line-too-long
                 evaluation_metric=None, fold_num=None, repeat_times=None, search_strategy=None, random_search_times=None,#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements,line-too-long
                 random_state=None, timeout=None, progress_indicator_id=None, param_values=None, param_range=None):#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements,line-too-long
    ...
    def fit(self, data, key=None, features=None, label=None):#pylint: disable=too-many-locals, too-many-statements, too-many-branches, too-many-arguments
        """
        ...
        key : str, optional
            ...
            Required if parameter selection/model evaluation is not specified.
    ...
    def  _predict(self, data=None, key=None, features=None, categorical_variable=None):#pylint: disable=too-many-locals, too-many-statements, too-many-branches, too-many-arguments
        """
...
class KNNRegressor(_KNNBase):
    ...
    param_values : ListOfTuples, optional
        Specifies values of parameters to be selected.
        Input should be the a list of tuples, with 1st element of each tuple being the target
    ...
    param_range : ListOfTuples, optional
        Specifies ranges of parameters to be selected.
        Input should be a list of tuples, with 1st element of each tuple being the name of the
        target parameter(in string format),
        while 2nd element being a list that specifies the range of parameters with the following
        format: [start, step, end] or [start, end].
        Only valid when `search_strategy` is specified.
        Valid parameter names include: 'minkowski_power', 'category_weights', 'n_neighbors'.
        No default value.
    ...
    9         3  1         4  14000.999849
    """
    def __init__(self, n_neighbors=None, thread_ratio=None, stat_info=None, aggregate_type=None,#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, line-too-long
                 metric=None, minkowski_power=None, category_weights=None, algorithm=None, resampling_method=None,#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, line-too-long
                 evaluation_metric=None, fold_num=None, repeat_times=None, search_strategy=None, random_search_times=None,#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, line-too-long
                 random_state=None, timeout=None, progress_indicator_id=None, param_values=None, param_range=None):#pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, line-too-long
    ...
    def predict(self, data=None, key=None, features=None, categorical_variable=None):#pylint: disable=too-many-locals, too-many-statements, too-many-branches, too-many-arguments
        """
    ...
    def predict(self, data, key, features=None):#pylint:disable=too-many-locals
        ...
        nearest_neighbors_df : DataFrame
            ...
            Structured as follows:
              - TEST_ + ``data`` 's ID name, with same type as ``data`` 's ID column,
                query data ID.
              - K, type INTEGER, K number.
              - TRAIN_ + training data's ID name, with same type as training
