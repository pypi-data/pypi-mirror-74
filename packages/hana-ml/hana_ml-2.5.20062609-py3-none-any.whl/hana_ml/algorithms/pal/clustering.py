...
class _ClusterAssignmentMixin(object):#pylint: disable=bad-option-value
...
class AffinityPropagation(PALBase):#pylint: disable=too-many-instance-attributes
    r"""
    ...
    seed_ratio : float, optional
        ...
        Value range: (0,1]. If ``seed_ratio`` is 1, all the input data will be the seed.
    ...
    def fit(self, data, key, features=None):#pylint: disable=too-many-locals
        ...
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        outputs = ['#AFFINITY_PROPAGATION_{}_TBL_{}_{}'.format(name, self.id, unique_id) for
                   name in ['RESULT', 'STAT']]
        result_tbl, stats_tbl = outputs#pylint: disable=unused-variable
        ...
        self.labels_ = conn.table(result_tbl)#pylint:disable=attribute-defined-outside-init
...
class AgglomerateHierarchicalClustering(PALBase):#pylint: disable=too-many-instance-attributes
    r"""
    ...
    def fit(self, data, key, features=None, categorical_variable=None):#pylint: disable=too-many-locals
        r"""
        Fit the model when given the training dataset.
        ...
        if features is None:
            features = cols
        exclude_feature = list(set(cols) - set(features))
        ...
        if exclude_feature is not None:
            param_rows.extend([('EXCLUDED_FEATURE', None, None, exc_feature)
                               for exc_feature in exclude_feature])
        ...
        self.combine_process_ = conn.table(combine_process_tbl)#pylint:disable=attribute-defined-outside-init
        self.labels_ = conn.table(result_tbl)#pylint:disable=attribute-defined-outside-init
...
class DBSCAN(PALBase, _ClusterAssignmentMixin):#pylint: disable=too-many-instance-attributes
    r"""
    ...
    save_model : bool, optional
        ...
        ``save_model`` must be True to call predict().
    ...
    def __init__(self, minpts=None, eps=None, thread_ratio=None,#pylint: disable=too-many-arguments
    ...
    def fit(self, data, key, features=None, categorical_variable=None):#pylint: disable=too-many-locals
        ...
        self.labels_ = conn.table(result_tbl)#pylint:disable=attribute-defined-outside-init
        self.model_ = conn.table(model_tbl) if self.save_model else None#pylint:disable=attribute-defined-outside-init
...
class GeometryDBSCAN(PALBase):#pylint: disable=too-many-instance-attributes
    """
...
class KMeans(PALBase, _ClusterAssignmentMixin):#pylint: disable=too-many-instance-attributes
    r"""
    ...
    thread_ratio : float, optional
        ...
        using at most all the currently available threads. Values outside the range will
        be ignored and this function heuristically determines the number of threads to use.
    ...
    def __init__(self, n_clusters=None, n_clusters_min=None, n_clusters_max=None,#pylint: disable=too-many-branches, too-many-locals, too-many-statements, too-many-arguments
    ...
    def fit(self, data, key, features=None, categorical_variable=None):#pylint: disable=too-many-locals
        ...
        result_tbl, centers_tbl, model_tbl, statistics_tbl, placeholder_tbl = outputs#pylint: disable=unused-variable
...
class _KClusteringBase(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    distance_map = {}
    normalization_map = {'no':0, 'l1_norm':1, 'min_max':2}
    init_map = {'first_k':1, 'no_replace':2, 'replace':3, 'patent':4}
    ...
    def fit_predict(self, data, key, features=None, categorical_variable=None):#pylint: disable=invalid-name
        """
...
class KMedoids(_KClusteringBase):
    ...
    thread_ratio : float, optional
        ...
        a single thread, and 1 indicates all available threads. Values
        between 0 and 1 will use up to that percentage of available
