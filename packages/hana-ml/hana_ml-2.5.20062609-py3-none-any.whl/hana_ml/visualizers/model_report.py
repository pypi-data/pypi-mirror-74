...
class TemplateUtil(object):
    ...
    __TAB_METADATA = {
        ...
        'nav_active_item': '<li class="active"><a href="#{}" role="tab" data-toggle="tab">{}</a></li>',
        'nav_item': '<li><a href="#{}" role="tab" data-toggle="tab">{}</a></li>',
        # {pane_id} {pane_items}
        'pane': '<div id="{}" class="tab-content">{}</div>',
        # {pane_item_id} {pane_item_content}
        'pane_active_item': '<div class="tab-pane active" id="{}">{}</div>',
        'pane_item': '<div class="tab-pane" id="{}">{}</div>'
    }
    ...
    @staticmethod
    def get_base_template(title, content, load_echarts_js):
        template = TemplateUtil.__ENV.get_template('base.html')
        return template.render(
            title=title,
            body=content,
            load_echarts_js=load_echarts_js)

    @staticmethod
    def get_iframe_base_template(title, content, load_echarts_js):
        template = TemplateUtil.__ENV.get_template('iframe_base.html')
        return template.render(
            title=title,
            body=content,
            load_echarts_js=load_echarts_js)
    ...
    def generate_html_file(filename, content):
        file = open('{}_model_report.html'.format(
...
class StatisticReportBuilder(object):
    def __init__(self):
        ...
        self.__html_table1_columns = ['CLASS NAME', 'PRECISION', 'RECALL', 'F1-SCORE', 'SUPPORT']
        self.__html_table1_data = []
        self.__html_table2_columns = ['STAT NAME', 'STAT VALUE', 'CLASS NAME']
    ...
    def build(self):
        ...
        section_html = TemplateUtil.generate_section(
            'Statistic', table1_html + table2_html)
...
class ParameterReportBuilder(object):
    ...
    def build(self):
        ...
        section_html = TemplateUtil.generate_section('Parameter', table_html)
...
class ConfusionMatrixReportBuilder(object):
    ...
    @staticmethod
    def plot_confusion_matrix(pandas_df, normalize=False):
    ...
    def build(self):
        ...
        img_html = PlotUtil.plot_to_str(
            ConfusionMatrixReportBuilder.plot_confusion_matrix(self.__confusion_matrix_table.collect()))
        section_html = TemplateUtil.generate_section('Confusion Matrix', img_html, True)
...
class VariableImportanceReportBuilder(object):
    ...
    def set_variable_importance_table(self, variable_importance_table: dataframe.DataFrame):
        self.__variable_importance_table = variable_importance_table
...
class MetricReportBuilder(object):
    ...
    def transform_roc_data_format(self, roc_sampling=None):
        ...
        roc_chart_data = self.__get_roc_chart_data(
            list(fpr_table.select('Y').collect()['Y']),
            list(tpr_table.select('Y').collect()['Y']))
    ...
    def build(self, sampling=False):
        ...
        tab_html, nav_id = TemplateUtil.generate_tab(data)
        section_html = TemplateUtil.generate_section('Metrics', tab_html)
...
class UnifiedClassificationReportBuilder(object):
    ...
    def build(self, metric_sampling=False):
        ...
        self.__add_html(self.__statistic_report_builder.get_generated_html())
        self.__add_html(self.__parameter_report_builder.get_generated_html())
        self.__add_html(self.__confusion_matrix_report_builder.get_generated_html())

        section_html, load_chart_js = self.__variable_importance_report_builder.get_generated_html_and_js()
        self.__add_html(section_html)
        self.__add_js(load_chart_js)

        if metric_sampling:
            section_html, load_chart_js = self.__metric_report_builder.build(sampling=True)
        else:
            section_html, load_chart_js = self.__metric_report_builder.get_generated_html_and_js()

        self.__add_html(section_html)
        self.__add_js(load_chart_js)

        if metric_sampling is False:
            self.__report_html = TemplateUtil.get_base_template(
                'Unified Classification Model Report', self.__all_html, self.__all_js)
            self.__iframe_report_html = TemplateUtil.get_iframe_base_template(
                'Unified Classification Model Report', self.__all_html, self.__all_js)
            self.__iframe_report_html = TemplateUtil.get_notebook_iframe(self.__iframe_report_html)
    ...
    def set_metric_samplings(self, roc_sampling: Sampling = None, other_samplings: dict = None):
        if roc_sampling or other_samplings:
            ...
            self.__report_sampling_html = TemplateUtil.get_base_template(
                'Unified Classification Model Report', self.__all_html, self.__all_js)
            self.__iframe_report_sampling_html = TemplateUtil.get_iframe_base_template(
                'Unified Classification Model Report', self.__all_html, self.__all_js)
            self.__iframe_report_sampling_html = TemplateUtil.get_notebook_iframe(self.__iframe_report_sampling_html)

    def generate_html_report(self, filename, metric_sampling=False):
        if metric_sampling:
            TemplateUtil.generate_html_file(filename, self.__report_sampling_html)
        else:
            TemplateUtil.generate_html_file(filename, self.__report_html)
