...
def hist(data, columns, bins, debrief=False, x_axis_fontsize=10,
         x_axis_rotation=0, title_fontproperties=None):
    """
    Plot histograms for HANA dataframe.
...
class EDAVisualizer(Visualizer): #pylint: disable=invalid-name
    ...
    def distribution_plot(self, data, column, bins, title=None, x_axis_fontsize=10, #pylint: disable= too-many-locals, too-many-arguments
        ...
        data_ = data.fillna(value=0, subset=[column])
        logger.warn("NULL values will be replaced by 0.")
        ...
        ax.bar(x=x_axis_ticks, height=bin_data['COUNT'], width=0.8 * bin_size, align='edge')
    ...
    def pie_plot(self, data, column, explode=0.03, title=None, legend=True,
                 title_fontproperties=None, legend_fontproperties=None): #pylint: disable=too-many-arguments
        """
        ...
        if title is not None:
            if title_fontproperties is not None:
                ax.set_title(title, title_fontproperties)
            else:
                ax.set_title(title)
        return ax, pie_data

    def correlation_plot(self, data, key=None, corr_cols=None, label=True, cmap="RdYlBu"): #pylint: disable=too-many-locals
        """
    ...
    def scatter_plot(self, data, x, y, x_bins, y_bins, title=None, label=True, #pylint: disable=too-many-locals, too-many-arguments, too-many-statements, invalid-name
                     cmap="Blues", debrief=True, rounding_precision=3, label_fontsize=12, title_fontproperties=None):
        """
        ...
        if debrief:
            ...
            ax.text(ax.figure.get_figwidth(), 0, 'Correlation: {:.2f}'.format(corr), style='italic',
                    bbox={'facecolor':'white', 'pad':0.65, 'boxstyle':'round'})
        else:
            pass
        if title is not None:
    ...
    def bar_plot(self, data, column, aggregation, title=None, label_fontsize=12, title_fontproperties=None): #pylint: disable=too-many-branches, too-many-statements
        """
    ...
    def box_plot(self,
        ...
        if not outliers and groupby is None:
            ...
            # Add legend
            ax.legend(loc='best', edgecolor='w', bbox_to_anchor=(1, 0.3, 0.8, 0.8))
            # Turn spines off
