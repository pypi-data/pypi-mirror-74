...
class DataFrame(object):#pylint: disable=too-many-public-methods
    ...
    def add_id(self, id_col):
        ...
        """
        select_statement = \
        "SELECT CAST(ROW_NUMBER() OVER() AS INTEGER) AS {}, * FROM ({})"\
        .format(id_col, self.select_statement)
    ...
    def diff(self, index, periods=1):
        ...
        for column in self.columns:
            if column not in index:
                ...
                .format(col=quotename(column), periods=periods, idx=index)
...
#        return DataFrame(self.connection_context, new_select_statement)
    ...
    def save(self, where, table_type=None, force=False):
        ...
        if table_type != 'VIEW':
            table_type += ' TABLE'
        with self.connection_context.connection.cursor() as cur:
            if force is True and table_type != 'VIEW':
                try:
...
def read_pickle(connection_context, path, table_name, compression='infer', schema=None, force=False, replace=True, object_type_as_bin=False, table_structure=None):
    """
...
def data_manipulation(connection_context, table_name, unload=True, schema=None):
    """
...
def create_dataframe_from_pandas(connection_context, pandas_df, table_name, schema=None, force=False, replace=True,\
object_type_as_bin=False, table_structure=None, drop_exist_tab=False, allow_bigint=False):
    """
    ...
    if (force == False) and (tab_exist == True):
        logger.error("Table already exists. Please set 'force=True'.")
        raise ValueError("Table already exists. Please set 'force=True'.")
    replace_subset = []
    if (tab_exist == False) or ((tab_exist == True) and (drop_exist_tab == True)):
        ...
        for col, dtype in zip(pandas_df.columns, dtypes_list):
            if table_structure is not None and col in table_structure:
                ...
                if ('int64' in dtype) and allow_bigint:
                ...
                elif 'object' in dtype and object_type_as_bin:
    ...
    rows = tuple(map(tuple, [[None if element is None or pd.isnull(element) else (str(element) if 'time' in str(type(element)) else (element.item() if 'numpy' in str(type(element)) else element)) for element in lines] for lines in pandas_df.values])) #pylint: disable=line-too-long
    if rows:
        parms = ("?," * len(rows[0]))[:-1]
    ...
    connection_context.connection.commit()
    if replace:
        return DataFrame(connection_context, 'SELECT * FROM {}'.format(table_reference)).fillna(0, subset=replace_subset)
    return DataFrame(connection_context, 'SELECT * FROM {}'.format(table_reference))
