...
object_type_as_bin=False, table_structure=None, drop_exist_tab=False, allow_bigint=False):
    ...
    rows = tuple(map(tuple, [[None if element is None or np.isnan(element) else (str(element) if 'time' in str(type(element)) else (element.item() if 'numpy' in str(type(element)) else element)) for element in lines] for lines in pandas_df.values])) #pylint: disable=line-too-long
    if rows:
        parms = ("?," * len(rows[0]))[:-1]
    ...
    connection_context.connection.commit()
    if replace:
        return DataFrame(connection_context, 'SELECT * FROM {}'.format(table_reference)).fillna(0)
