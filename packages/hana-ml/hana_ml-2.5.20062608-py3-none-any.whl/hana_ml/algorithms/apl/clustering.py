...
class _AutoClusteringBase(APLBase):
    ...
    def _fit(self, data, key=None, features=None, label=None):
        ...
        """
        if not features:
            features = [col for col in data.columns if col not in [key, label]]
...
class AutoSupervisedClustering(_AutoClusteringBase):
    ...
    def fit(self, data, key=None, label=None, features=None):
        """
        Fits the model.
    ...
    def fit_predict(self, data, key=None, label=None, features=None):
        """
        Fits a clustering model and uses it to generate prediction output on the training dataset.
        ...
        'extra_applyout_settings' parameter.
        """
        self.fit(data=data, key=key, label=label, features=features)
        return self.predict(data)
