...
def f_oneway(data, group=None, sample=None,
    ...
    try:
        ...
        #create(conn, Table(stats_tbl, stats_spec))
        #create(conn, Table(anova_tbl, anova_spec))
        #create(conn, Table(multi_comparison_tbl, multi_comparison_spec))
        call_pal_auto(conn,
                      'PAL_ONEWAY_ANOVA',
                      data_,
...
def median_test_1samp(data, col=None, mu=None, test_type=None,#pylint: disable=invalid-name
    ...
    thread_ratio : float, optional
        ...
        this function. The value range is from 0 to 1, where 0 means only
        using 1 thread, and 1 means using at most all the currently available
        threads. Values outside the range will be ignored and this function
    ...
    col = arg('col', col, str)
    if col is None and len(data.columns) != 1:
        msg = ("If 'col' is not specified, the input dataframe " +
               "must only have one column.")
        logger.error(msg)
        raise ValueError(msg)
...
def grubbs_test(data, key, col=None, method=None, alpha=None):
    ...
    col : str, optional
        Name of the data column that needs to be tested.
        If not given, the input dataframe must only have two column.
        The non-ID column will be `col`.
...
def entropy(data, col=None,
    ...
    col : str/ListofStrings, optional
        ...
        If not given, all columns will be processed.
    ...
    thread_ratio : flaot, optional
        ...
        at most all the currently available threads. Values outside the range are ignored and
        this function heuristically determines the number of threads to use.
        Default to 0.
...
def condition_index(data, key=None, col=None, scaling=True,
    ...
    col : str/ListofStrings, optional
        ...
        If not given, all columns except the ID column will be processed.
...
def factor_analysis(data, key, factor_num, col=None,
    ...
    -------
    eigen_tbl : DataFrame
    var_explanation_tbl : DataFrame
    communalities_tbl : DataFrame
    loadings_tbl : DataFrame
    rotated_loadings_tbl : DataFrame
    structure_tbl : DataFrame
    rotation_tbl : DataFrame
    factor_correlation_tbl : DataFrame
    score_model_tbl : DataFrame
    scores_tbl : DataFrame
    stats_tbl : DataFrame
...
def kaplan_meier_survival_analysis(data, event_indicator=None, conf_level=None):
    ...
    res_tbl : DataFrame
        Statistics result, structured as follows:
            - GROUP, group.
    ...
    >>> df.collect()
        ...
        16   101       0            1      2
        17     8       0            1      1
    Apply the kaplan_meier_survival_analysis function:
...
def cdf(data, distr_info, col=None, complementary=False):
    ...
    complementary : bool, optional
        False: 'cdf'
        True: 'ccdf'
        Default to False('cdf').

    Returns
    -------
    res_tbl : DataFrame
        CDF results, structured as follows:
          - INPUT_DATA column name, input data.
...
def distribution_fit(data, distr_type, optimal_method=None, censored=False):
    ...
    optimal_method : {'maximum_lielihood', 'median_rank'}, optional
        Estimation method:
        Default to 'median_rank' for weibull, 'maximum_likelihood' for others.
    censored : bool, optional
        Specify if the data is censored of not. True only valid for weibull.
        Default to un-censored.
