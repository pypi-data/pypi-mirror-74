...
class _MLPBase(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def __init__(self,#pylint: disable=too-many-arguments, too-many-branches, too-many-statements, too-many-locals
        ...
        if self.resampling_method is not None:
            ...
            if self.resampling_method not in self.resampling_method_list:#pylint:disable=line-too-long, bad-option-value
                ...
                logger.error(msg)
                raise ValueError(msg)
            elif self.functionality == 1 and self.resampling_method in ('cv', 'boostrap'):
        ...
        if self.evaluation_metric is not None:
            if self.functionality == 0 and self.evaluation_metric == 'RMSE':#pylint:disable=line-too-long, bad-option-value
                ...
                logger.error(msg)
                raise ValueError(msg)
            elif self.functionality == 1 and self.evaluation_metric in ('ACCURACY', 'F1_SCORE', 'AUC_1VSRest', 'AUC_pairwise'):#pylint:disable=line-too-long, bad-option-value
        ...
        if self.param_values is not None and self.search_strategy is not None:
            for x in self.param_values:#pylint:disable=invalid-name
                if len(x) != 2:#pylint:disable=bad-option-value
                    ...
                    logger.error(msg)
                    raise ValueError(msg)
                elif x[0] not in list(self.range_params_map.keys()):
                    msg = ("Specifying the values of '{}' for ".format(x[0])+
                           "parameter selection is invalid.")
                    logger.error(msg)
                    raise ValueError(msg)
                elif self.training_style == 1 and  x[0] == 'batch_size':
    ...
    def _fit(self, data, key=None, features=None, label=None, categorical_variable=None):#pylint: disable=too-many-locals, too-many-statements, too-many-branches
        ...
        outputs = model_tbl, log_tbl, stat_tbl, optimal_tbl = [
            '#MLP_{}_TBL_{}'.format(name, self.id) for name in ['MODEL',
                                                                'TRAINING_LOG',
                                                                'STATISTICS',
                                                                'OPTIMAL_PARAM']
            ]
...
class MLPClassifier(_MLPBase):
    ...
    activation : {'tanh', 'linear', 'sigmoid_asymmetric', 'sigmoid_symmetric', \
    'gaussian_asymmetric', 'gaussian_symmetric', 'elliot_asymmetric', \
    'elliot_symmetric', 'sin_asymmetric', 'sin_symmetric', 'cos_asymmetric', \
    'cos_symmetric', 'relu'}, conditionally mandatory

        Activation function for the hidden layer.
        Mandatory if ``activation_options`` is not provided.
    ...
    output_activation : {'tanh', 'linear', 'sigmoid_asymmetric', 'sigmoid_symmetric', \
    'gaussian_asymmetric', 'gaussian_symmetric', 'elliot_asymmetric', 'elliot_symmetric', \
    'sin_asymmetric', 'sin_symmetric', 'cos_asymmetric', 'cos_symmetric', 'relu'},
...
class MLPRegressor(_MLPBase):
    ...
    param_values : list of tuple, optional
        ...
        Each tuple contains two elements - 1st element is the parameter name(str type), \
        2nd element is a list of valid values for that parameter.
