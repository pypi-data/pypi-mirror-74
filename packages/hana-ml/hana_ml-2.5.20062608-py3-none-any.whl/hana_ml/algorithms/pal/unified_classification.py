...
class UnifiedClassification(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    **kwargs : keyword arguments
        ...
        **Note that some parameters are disabled in the classification algorithm!**
            ...
            - **'RandomForest'** : :class:`~hana_ml.algorithms.pal.trees.RandomForestClassifier`
        ...
        >>> dt_params = dict(algorithm='c45',
                             ...
                             parameter_values = dict(split_threshold=[1e-3 , 1e-4, 1e-5])))
    ...
    map_dict = {
        ...
        'HGBT' : {
            ...
            'min_sample_weight_leaf' : ('MIN_SAMPLE_WEIGHT_LEAF', float),
        ...
        'MLP' : {
            ...
            'hidden_layer_size' : ('HIDDEN_LAYER_SIZE', tuple),
    ...
    def fit(self,#pylint: disable=too-many-branches, too-many-statements
        ...
        purpose : str, optional
            ...
            If ``partition_method`` is set to be 'user_defined', this column is mandatory.
        ...
        **kwargs : keyword arguments
        ...
        training_size = self._arg('training_percent', training_size, float)
        check = False
        if partition_method is not None:
            if partition_method == 'stratified':#pylint:disable=line-too-long
                check = True
        if check is False:
            purpose = None
        ...
        outputs = ['MODEL', 'IMPORTANCE', 'STATS', 'OPT_PARAM', 'CONFUSION_MATRIX',
                   'METRICS', 'PLACE_HOLDER1', 'PLACE_HOLDER2']
        outputs = ['#PAL_UNIFIED_CLASSIFICATION_{}_{}_{}'.format(tbl, self.id, unique_id)
                   for tbl in outputs]
        model_tbl, imp_tbl, stats_tbl, opt_param_tbl, cm_tbl, metrics_tbl, _, _ = outputs
        try:
            call_pal_auto(conn,
        ...
        # add report builder for model
        if self.__report_builder is None:
            self.__report_builder = UnifiedClassificationReportBuilder()

        parameter_df = pd.DataFrame(data=param_rows, columns=ParameterReportBuilder.get_table_columns())
    ...
    def generate_html_report(self, filename, metric_sampling=False):
        ...
        metric_sampling : bool, optional
            Whether the metric table needs to sample.
        """
        if self.__report_builder is None:
            raise Exception('To generate a report, you must call the fit method firstly.')
