...
class _RandomForestBase(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def __init__(self,
        ...
        super(_RandomForestBase, self).__init__()
...
class RandomForestClassifier(_RandomForestBase):#pylint: disable=too-many-instance-attributes
    ...
    def __init__(self,
                 ...
                 priors=None):
        #pylint:disable=too-many-locals
        super(RandomForestClassifier, self).__init__(n_estimators,
...
class _DecisionTreeBase(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    def _fit(self, data, key=None, features=None, label=None, categorical_variable=None):
        #pylint:disable=too-many-locals, too-many-branches, too-many-statements, too-many-locals
        conn = data.connection_context
        require_pal_usable(conn)
        key = self._arg('key', key, str)
        features = self._arg('features', features, ListOfStrings)
...
class DecisionTreeClassifier(_DecisionTreeBase):
    ...
    timeout : int, optional
        The time allocated (seconds) for program running.
...
class DecisionTreeRegressor(_DecisionTreeBase):
    ...
    param_values : ListOfTuples, optional
        ...
        while 2nd element being the a list of valued for selection.
        Only valid when ``search_strategy`` is specified.
        Valid Parametr names include: 'split_threshold', 'max_depth',
                                      'min_records_of_leaf', 'min_records_of_parent'
...
@deprecated(version='1.0.8', reason="This method is deprecated. Please use HybridGradientBoostingClassifier instead.")
...
@deprecated(version='1.0.8', reason="This method is deprecated. Please use HybridGradientBoostingRegressor instead.")
...
#pylint: disable=too-many-instance-attributes
    ...
    param.search.strategy: {'grid', 'random'}, optional
    ...
    param_range : list of tuples, optional

        Indicates the range of parameters involded for parameter selection. \
        Effective only when ``resampling_method`` and ``param_search_strategy`` are both not None. \
        Each tuple is a pair, with the first being parameter name of str type, and \
        the second being the a list of numbers with the following strcture: \
        [<begin-value>, <step-size>, <end-value>].\
        The <step-size> value can be omitted if ``param_search_strategy`` is 'random'.
     ...
     param_values : list of tuples, optional

        Indicates the values of parameters involded for parameter selection. \
        Effective only when ``resampling_method`` and ``param_search_strategy`` are both not None. \
        Each tuple is a pair, with the first being parameter name of str type, and \
        the second being the a list of numbers with the following strcture: \
...
.
    ...
    >>> ghc = HybridGradientBoostingClassifier(
    ...           n_estimators = 4, split_threshold=0,
    ...           learning_rate=0.5, fold_num=5, max_depth=6,
...
#pylint: disable=too-many-instance-attributes
    ...
    param_range : list of tuples, optional
        ...
        Indicates the range of parameters involded for parameter selection. \
        Valid only when ``resampling_method`` and ``param_search_strategy`` are both not None.\
        Each tuple is a pair, with the first being parameter name of str type, and \
        the second being the a list of numbers with the following strcture: \
        [<begin-value>, <step-size>, <end-value>].\
        The <step-size> value can be omitted if ``param_search_strategy`` is set to 'random'.\
        Suppported parameters for cross-validation: n_estimators, max_depth, learning_rate, \
        min_sample_weight_leaf, max_w_in_split, col_subsample_split, col_subsample_tree,
        lamb, alpha, scale_pos_w, base_score.
    ...
    param_values : list of tuples, optional
        ...
        Indicates the values set of parameters for parameter selection. \
        Valid only when ``resampling_method`` and ``param_search_strategy`` are both not None.\
        Each tuple is a pair, with the first being parameter name of str type, and \
        the second being the a list of numbers with the following strcture: \
        [<value1>, <value2>, <value3>,...].

        Suppported parameters for values specification as as those for range specification, see ``param_range``.
