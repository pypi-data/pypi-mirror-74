...
def dev_config():
    return {
        ...
        "move_finished_extra_wait_sec": 0.01,
        "acceleration": 10,
...
def test_start(com_config, dev_config):
    ...
    for param in config:
        ...
        val = config[param] + 1  # not matching config values
...
def test_move_to_relative_position(started_newport):
    ...
    mot.position = 20
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
    com.put_text(f"1TP{mot.position + pos}")
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
...
def test_wait_until_move_finished(started_newport):
    com, mot = started_newport
    pos = -1
    duration = 0.02
    mot.target_position = -3
    # normal case
    state = dev.NewportSMC100PP.StateMessages.MOVING.value
    com.put_text(f"1TS0000{state}")
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
    com.put_text(f"1TP{pos - mot.config.user_position_offset}")
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
    com.put_text(f"1PT{duration}")
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
    state = dev.NewportSMC100PP.StateMessages.READY_FROM_MOVING.value
    com.put_text("1TS0000" + state)
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
    mot.wait_until_move_finished()
    assert com.get_written() == "1TS"
    ...
    assert com.get_written() == "1TP"
    assert com.get_written() == "1TE"
    assert com.get_written() == f"1PT{abs(mot.target_position-mot.position)}"
    assert com.get_written() == "1TE"
    assert com.get_written() == "1TS"
    assert com.get_written() == "1TE"
    # not MOVING case
