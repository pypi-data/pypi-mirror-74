...
def test_dev_config_invalid(wrong_config_dict):
    with pytest.raises(ValueError):
        SupercubeConfiguration(**wrong_config_dict)
...
def test_status_poller(
    com_config, dev_config, demo_opcua_server: DemoServer, mocker: MockFixture,
):
    ...
    poller.stop_polling()
    assert spy_stop_polling.spy_return == (False, None)
    ...
    assert spy_stop_polling.spy_return == (True, None)
