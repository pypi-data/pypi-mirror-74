...
def test_correct_config_value(com_config, dev_config, config_key, correct_value):
    correct_config = dict(dev_config)
    correct_config[config_key] = correct_value
    dev = ILS2T(com_config, correct_config)
    assert getattr(dev.config, config_key) == correct_value
