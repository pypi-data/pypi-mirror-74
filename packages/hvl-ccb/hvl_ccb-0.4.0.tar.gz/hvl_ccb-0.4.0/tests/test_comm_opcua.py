...
def test_com_config_invalid(com_config, wrong_config_dict):
    for key in OpcUaCommunicationConfig.required_keys():
        if key not in wrong_config_dict:
            wrong_config_dict[key] = com_config[key]
    with pytest.raises(ValueError):
        OpcUaCommunicationConfig(**wrong_config_dict)
...
def _test_write_client_error(
    ...
    com_config, demo_opcua_server: DemoServer, mocker: MockFixture,
):
    comm_protocol = OpcUaCommunication(com_config)
