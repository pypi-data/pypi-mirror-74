...
def test_wrong_config_value(com_config, dev_config, config_key, wrong_value):
    ...
    with pytest.raises(ValueError):
        Luminox(com_config, wrong_config)
...
def test_read_stream(testdev):
    ...
    readout = lumi.read_stream()
    expected_measurement_values = {measurement: float(value)
                                   for (measurement, value) in
                                   expected_measurement_values.items()}
...
def test_query_single_measurement(testdev, measurement_type, value_str):
    ...
    com.put_text(
        f"{measurement_type.command} {value_str}{com.config.terminator_str()}"
    )

    lumi.output = LuminoxOutputMode.polling
    value = lumi.query_single_measurement(measurement_type)
...
def test_query_single_measurement_error_streaming_mode(testdev):
    ...
    com.put_text(
        f"{measurement_type} {expected_value:06.1f}"
        f"{com.config.terminator_str()}"
    )

    lumi.output = LuminoxOutputMode.streaming
    with pytest.raises(LuminoxOutputModeError):
        lumi.query_single_measurement(LuminoxMeasurementType.partial_pressure_o2)
...
def test_query_single_measurement_warning(testdev):
    ...
    lumi.output = LuminoxOutputMode.polling
    value = lumi.query_single_measurement(LuminoxMeasurementType.partial_pressure_o2)
    assert value == expected_value
...
def test_query_single_measurement_str_enum_name(testdev):
    ...
    lumi.output = LuminoxOutputMode.polling
    value = lumi.query_single_measurement(measurement_type.name)
    assert value == expected_value

    with pytest.raises(ValueError):
        lumi.query_single_measurement(measurement_type.name.replace("_", " "))
...
def test_query_single_measurement_str_enum_value(testdev):
    ...
    lumi.output = LuminoxOutputMode.polling
    value = lumi.query_single_measurement(measurement_type.value)
    assert value == expected_value

    with pytest.raises(ValueError):
        lumi.query_single_measurement(measurement_type.command + " ?")
