...
class CryLasLaser(SingleCommDevice):
    ...
    class LaserStatus(Enum):
        ...
        @property
        def is_ready(self):
            return self is not self.UNREADY_INACTIVE

        @property
        def is_inactive(self):
            return self is not self.READ_ACTIVE
    ...
    def __init__(self, com, dev_config=None):
        ...
        # concurrent future that polls the laser status until it is ready
        self._polling_future = None

        # event to stop the polling thread, e.g. when device is stopped
    ...
    def _start_polling(self):
        ...
        # Note: using concurrent.futures.Future over threading.Thread to seamlessly
        #       pass errors raised in a thread
    ...
    def _poll_until_ready(self, stop_event) -> None:
        ...
        stop_time = time.time() + self.config.polling_timeout
        logging.info(
            f"[{datetime.now()}] Start polling status: "
            f"{self.laser_status.value} ..."
        )
        while (
            not self.laser_status.is_ready
            and not stop_event.wait(self.config.polling_period)
            and time.time() < stop_time
        ):
            self.update_laser_status()
            logging.info(
                f"[{datetime.now()}] Polling status: {self.laser_status.value} ..."
            )

        if stop_event.wait(0):
            logging.info(f"[{datetime.now()}] Polling status: ... STOPPED")
            return
        elif not self.laser_status.is_ready:
        ...
        if not answer_list:
            ...
            for line in answer_list:
                ...
                if answer in (
                    ...
                    new_status = self.LaserStatus.READ_ACTIVE
    ...
    def laser_on(self) -> None:
        ...
        if not self.laser_status.is_ready:
            ...
            if not answer or self.AnswersStatus(answer) != self.AnswersStatus.ACTIVE:
                ...
                self.laser_status = self.LaserStatus.READ_ACTIVE
