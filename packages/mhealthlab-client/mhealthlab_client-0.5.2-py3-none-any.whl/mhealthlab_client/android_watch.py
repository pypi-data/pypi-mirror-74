...
class AndroidWatch:
    ...
    def _parse_session(self):
        ...
        folders = list(filter(lambda path: os.path.isdir(
            path) and 'signaligner' not in path, folders))
    ...
    def convert_to_actigraph(self, date_range=None, sr=50):
        ...
        filepaths = list(filter(lambda f: 'signaligner' not in f, filepaths))
        ...
        for i in range(len(sub_session_markers) - 1):
            ...
            output_path = os.path.join(
                self._root, 'signaligner', f'{st_display}_{et_display}_sensors', f'{self._pid}_Android_{data_id}_{st_display}_{et_display}.sensor.csv')
