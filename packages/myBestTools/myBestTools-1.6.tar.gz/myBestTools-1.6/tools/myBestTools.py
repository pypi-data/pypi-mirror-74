...
try:
    redis_cache = Redis(host='127.0.0.1', port=6379, decode_responses=True)
except:
    pass
L = threading.Lock()
Q = queue.Queue()
...
def cache(f):
    ...
    def wrapper(*args, **kwargs):
        md5 = hashlib.md5()
        md5.update(bytes("%s" % [i for i in args if '__main__' not in str(type(i))], encoding='utf-8'))
        key = "%s_%s" % (f.__name__, md5.hexdigest())
        res = redis_cache.get(key)
        if res:
            return json.loads(res)
        else:
            res = f(*args, **kwargs)
            redis_cache.set(key, json.dumps(res, cls=CJsonEncoder))
            return res
...
class Tool:
    ...
    def get_html(self, url, headers={}, proxy={}, timeout=5, debug=False):

        for i in range(5):
            try:
                res = self.session.get(url, headers=headers, timeout=timeout, proxies=proxy)
                return res
    ...
    def post_html(self, url, headers={}, data={}, proxy={}, timeout=5, debug=False):

        for i in range(5):
            try:
                res = self.session.post(url, data=data, headers=headers, timeout=timeout, proxies=proxy)
                return res
            except Exception as e:
                if debug:
                    logging.info(e)
    ...
    def get_proxy(self):
        proxy = self.con('select * from proxy order by rand() limit 1', update_cursor=True)[0]
    ...
    def update_proxy_pool(self):
        while True:
            proxies = self.con('select * from proxy', update_cursor=True)
            L.acquire()
            self.proxy_pool = []
