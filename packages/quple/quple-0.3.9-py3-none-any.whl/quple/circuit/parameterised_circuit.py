...
from cirq import ParamResolver, protocols
...
class ParameterisedCircuit(QuantumCircuit, ABC):
    def __init__(self, n_qubit:int, copies:int=1,
        ...
        self._measure_qubit = measure_qubit
        if auto_assemble:
            self.assemble()
    ...
    def _parse_blocks(blocks) -> List:
        ...
        blocks = [QuantumCircuit._parse_gate_operation(block) if
                  isinstance(block, str) else block for block in blocks]
        return blocks
    ...
    def get_parameter_resolver(self, vals:np.ndarray) -> Union['cirq.ParamResolver',List['cirq.ParamResolver']]:
        ...
        resolver = cirq.ListSweep([ParamResolver({param:value for param, value in 
                                   np.column_stack((self.parameters,val))}) for val in vals])
        if self.expr_map:
    ...
    def resolve_parameters(self, vals:np.ndarray)-> Union['cirq.Circuit', List['cirq.Circuit']]:
        ...
        param_resolver = self.get_parameter_resolver(vals)
        if ndim == 1:
            return protocols.resolve_parameters(self, param_resolver)
        elif ndim == 2:
            return [protocols.resolve_parameters(self, params) for params in param_resolver]
        else:
            return None
    ...
    def assemble(self):
        ...
        if self.measure_qubit is not None:
            self.measure(self.measure_qubit)
    ...
    @staticmethod
    def get_common_parameters(a:'ParametersisedCircuit', b:'ParametersisedCircuit'):
        '''Returns the common parameters of two circuits
        Args:
            a: A parameterised circuit whose parameters are to be compared
            b: Another parameterised circuit whose parameters are to be compared
        '''
        a_params = set(a.parameter_table.params.keys())
        b_params = set(b.parameter_table.params.keys())
        return set.intersection(a_params, b_params)
