...
class PylintIgnoreDecorator(object):
    ...
    def _init_from_args(self, args):
        ...
        while arg_i < len(args):
            ...
            if arg == '--update-ignorefile':
                ...
                if num_jobs != '1':
                    if '=' in arg:
                        sys.stderr.write('Invalid argument {0}\n'.format(arg))
                    else:
                        sys.stderr.write('Invalid argument {0} {1}\n'.
                            format(arg, num_jobs))
                    sys.stderr.write(
    ...
    def _new_entry(self, key, old_entry, msg_text, srctxt):
        if old_entry:
        ...
        return ignorefile.Entry(key.msg_id, key.path, key.symbol, msg_text,
            author, date, srctxt)

    def is_enabled_entry(self, msg_id, path, symbol, msg_text, srctxt):
    ...
    def _is_message_enabled_wrapper(self):
        ...
        def is_any_message_def_enabled(linter, msgid, line):
            ...
            for msg_def in _pylint_msg_defs(linter, msgid):
                ...
                _is_enabled = self.is_enabled_entry(msgid, linter.
                    current_file, msg_def.symbol, msg_text, srctxt)
                if not _is_enabled:
                    return False
            return True
        ...
        def is_message_enabled(linter, msg_descr, line=None, confidence=None):
            try:
                ...
                if not is_enabled:
                    return False
                is_always_enabled = re.match('\\w\\d{1,5}', msg_descr
                    ) is None or msg_descr[:1
                    ] == 'E' or linter.current_file is None
