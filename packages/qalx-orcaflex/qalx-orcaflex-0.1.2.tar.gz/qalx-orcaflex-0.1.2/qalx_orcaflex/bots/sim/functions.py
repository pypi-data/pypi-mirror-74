...
def process_orcaflex(job: QalxJob) -> None:
    ...
    threading.Thread(target=update_job_progress, daemon=True).start()
    ...
    # update_queue.join() # wait until all the progress updates are pushed
