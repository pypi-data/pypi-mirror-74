...
use cpython::{
    NoArgs, ObjectProtocol, PyClone, PyObject, PyResult, PyTuple, Python, PythonObject, ToPyObject,
};
use errno::errno;
use log::{debug, error};
...
impl SendFileInfo {
    ...
    pub fn send_file(&mut self, out: &mut dyn AsRawFd) -> bool {
        ...
        if count == 0 {
            ...
            let res = unsafe { libc::sendfile(out.as_raw_fd(), self.fd, &mut self.offset, count) };
            match res {
                -1 => {
                ...
                num_written if num_written < count as isize => self.done = true,
                _ => {
...
py_class!(pub class FileWrapper |py| {
    ...
    def __new__(_cls, filelike: PyObject, blocksize: Option<i64>=None) -> PyResult<Self> {
...
pub trait SendFile {
    ...
    fn send_file(&mut self, out: &mut dyn AsRawFd, gilstate: PyGILState_STATE, py: Python) -> bool;
    fn update_content_length(&mut self, content_length: usize, py: Python);
    // XXX used only for testing
    fn new(py: Python, fd: RawFd, bs: i64) -> PyResult<FileWrapper>;
}
...
impl SendFile for FileWrapper {
    ...
    fn send_file(&mut self, out: &mut dyn AsRawFd, gilstate: PyGILState_STATE, py: Python) -> bool {
        with_released_gil(gilstate, || {
            self.sendfileinfo(py).borrow_mut().send_file(out)
        })
    }
    ...
    fn update_content_length(&mut self, content_length: usize, py: Python) {
        self.sendfileinfo(py)
            .borrow_mut()
            .update_content_length(content_length);
    }
