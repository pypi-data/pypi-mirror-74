...
use crate::response::{handle_request, WSGIResponse, Write};
use crate::server::{would_block, SetBlocking};
use crate::workerpool::MARKER;

pub trait NonBlockingWrite: Write + Send + Source {}
impl<T: Write + Send + Source> NonBlockingWrite for T {}

pub trait BlockingWrite: Write + Send + SetBlocking {}
impl<T: Write + Send + SetBlocking> BlockingWrite for T {}
...
pub fn write_non_blocking_worker<T: NonBlockingWrite>(
    idx: usize,
    server_info: SocketAddr,
    threadapp: PyObject,
    script_name: String,
    ...
    let thread_globals =
        with_gil(|py: Python, _gs| WSGIGlobals::new(server_info, script_name.as_str(), py));
    loop {
        match rcv.recv() {
                ...
                match out {
                    Some(mut connection) => {
                        let complete = with_gil(|py, gs| {
                            handle_request(&threadapp.clone_ref(py), &thread_globals, &mut req, py);
...
pub fn write_blocking_worker<T: BlockingWrite + Debug>(
    idx: usize,
    server_info: SocketAddr,
    threadapp: PyObject,
    script_name: String,
        ...
        match app {
            Ok(_) => {
                ...
                let t2 = thread::spawn(move || {
                    ...
                    input.send((MARKER, (WSGIRequest::new(16), None))).unwrap();
                });
                write_blocking_worker(23, si, app, sn.to_string(), rcv.clone());
...
app = simple_app"#,
        ...
        match app {
            Ok(_) => {
                ...
                let token = Token(42);
                let expected = b"HTTP/1.1 200 OK\r\nContent-type: text/plain\r\nExpires: Sat, 1 Jan 2000 00:00:00 GMT\r\nVia: pyruvate\r\n\r\nHello world!\n";
                let (mut input, rcv) = spmc::channel::<(Token, (WSGIRequest, Option<WriteMock>))>();
                let connection = WriteMock::new(20, false);
                let mut f = connection.file.reopen().unwrap();
                input.send((token, (req, Some(connection)))).unwrap();
                input.send((MARKER, (WSGIRequest::new(16), None))).unwrap();
                write_non_blocking_worker(23, si, app.clone_ref(py), sn.to_string(), rcv.clone());
                ...
                input.send((token, (req, Some(connection)))).unwrap();
                input.send((MARKER, (WSGIRequest::new(16), None))).unwrap();
                write_non_blocking_worker(23, si, app, sn.to_string(), rcv.clone());
