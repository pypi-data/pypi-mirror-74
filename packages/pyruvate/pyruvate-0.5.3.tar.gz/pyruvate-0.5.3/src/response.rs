...
use crate::pyutils::{close_pyobject, with_gil, PyGILState_STATE};
...
// blanket implementation needed for mio::net::TcpStream
impl<T: io::Write + AsRawFd> Write for T {}

fn wsgi_iterable<'p>(
    obj: PyObject,
    py: Python<'p>,
) -> Result<PyIterator<'p>, PythonObjectDowncastError> {
    unsafe {
...
pub struct WSGIResponse {
    ...
    pub written: usize,
    pub sendfile_complete: bool,
}

impl WSGIResponse {
    pub fn new() -> WSGIResponse {
        WSGIResponse {
            pyobject: None,
            iterable: None,
            start_response: None,
            ...
            Some(obj) => match PyIterator::from_object(py, obj.clone_ref(py)) {
                Ok(mut iter) => match iter.next() {
                    ...
                    Some(Ok(any)) => match any.cast_as::<PyBytes>(py) {
                        Ok(cont) => bytes = cont.data(py).to_vec(),
                        Err(e) => {
    ...
    pub fn write_chunk<T: Write>(
        ...
        if !self.last_chunk & (self.written == 0) {
        ...
        match out.write(&self.current_chunk[self.written..]) {
            Ok(n) => {
                self.written += n;
                debug!("{} bytes written", self.written);
        ...
        if self.sendfileinfo {
            if let Some(ob) = self.pyobject.as_mut() {
                self.sendfile_complete = match ob.extract::<FileWrapper>(py) {
                    Ok(mut fw) => {
                        debug!("self.content_length: {:?}", self.content_length);
                        if let Some(cl) = self.content_length {
                            fw.update_content_length(cl, py);
                            debug!(
                                "File Wrapper content length: {:?}",
                                fw.sendfileinfo(py).borrow().content_length
                            );
                        }
        ...
        out.flush()?;
        Ok(self.last_chunk && chunk_complete && self.sendfile_complete)
    }

    pub fn complete(&self) -> bool {
        // needed in case of EAGAIN error
        self.last_chunk && (self.written == 0)
    }
}

impl Drop for WSGIResponse {
    fn drop(&mut self) {
        debug!("Dropping WSGI Response");
        if let Some(ob) = self.pyobject.as_mut() {
            with_gil(|py, _gs| match close_pyobject(ob, py) {
                Err(e) => {
...
pub fn handle_request(
            ...
            match StartResponse::new(env, Vec::new(), py) {
                Ok(sr) => {
                    ...
                    let args = PyTuple::new(
                        py,
                        &[
                            envarg.as_object().clone_ref(py),
                            sr.as_object().clone_ref(py),
                        ],
                    );
                    let srobj = sr.as_object().clone_ref(py);
                    debug!(
                        "Refcounts application: {:?} start_response: {:?}, environ: {:?}",
    ...
    req.response = Arc::new(Some(resp));
}
#[cfg(test)]
mod tests {
    ...
    use crate::request::WSGIRequest;
    use crate::response::{handle_request, WSGIResponse, HTTP500};
    use crate::server::SetBlocking;
    use crate::startresponse::{StartResponse, WriteResponse};
    ...
    fn test_write_chunk() {
        ...
        with_gil(|py, gs| match r.write_chunk(&mut connection, py, gs) {
            Err(e) => {
                debug!("Error: {:?}", e);
                assert!(false);
            }
...
app = simple_app"#,
        ...
        match app {
            Ok(_) => {
                ...
                let mut req = WSGIRequest::new(16);
                req.append(raw);
                req.parse_headers();
                handle_request(&app, &g, &mut req, py);
                if let Some(resp) = Arc::get_mut(&mut req.response).unwrap() {
