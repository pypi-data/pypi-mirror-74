...
def editor(app,
    ...
    def display_info(info_serie):
        ...
        if additionnal_info is not None:
            info_metadata = additionnal_info(tsa.engine, id_serie)
    ...
    def editable_table(tsa, id_serie, fromdate=None, todate=None):
        if not has_permission('viewseries'):
            return
    ...
    def dynamic_table(n_clicks, info_serie, graphdata):
        ...
        fromdate, todate = normalize_tstamps(tsa, id_serie, fromdate, todate)
        ...
        if alternative_table is not None:
            return alternative_table(
                tsa, id_serie, fromdate, todate, author, additionnal_info,
                prefix_link
            ) or editable_table(tsa, id_serie, fromdate, todate)

        return editable_table(tsa, id_serie, fromdate, todate)
    ...
    def store_id_rev(n_clicks, info_serie):
        ...
        if kind != 'primary' or not id_serie:
            return ''
        # layering violation ... does the api wants a last_id ?
        rev_id = tsa.tsh.last_id(tsa.engine, id_serie)
        return rev_id
    ...
    def insert_diff(n_clicks, info_serie, info_diff, rev_id_stored):
        ...
        rev_id_now = tsa.tsh.last_id(tsa.engine, id_serie)
        ...
        patch = fromjson(info_diff['diff'], tsname=id_serie)
        if tsa.metadata(id_serie, all=True)['tzaware']:
            patch.index = patch.index.tz_localize('UTC')
        try:
            diffts = tsa.update(
                id_serie, patch, author, manual=True
            )
        except Exception as error:
            tb.print_exc()
            return str(error)
        ...
        if len(diffts):
            msg = (f'Series {id_serie} has been succesfully updated by {author} '
