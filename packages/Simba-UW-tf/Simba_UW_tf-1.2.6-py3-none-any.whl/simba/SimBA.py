...
class video_info_table:
    ...
    def __init__(self,configini):
        ...
        video_info_csv = os.path.join(os.path.dirname(configini), 'logs', 'video_info.csv')
        config = ConfigParser()
        self.configFile = str(configini)
        config.read(self.configFile)
        self.config_videofolders =  os.path.join(os.path.dirname(configini), 'videos')
        config_distancemm = config.get('Frame settings', 'distance_mm')

        ########### FIND FILES ###########
        for i in os.listdir(self.config_videofolders):
            if i.endswith(('.avi','.mp4','.mov','.flv', '.AVI', '.MP4')):
                self.filesFound.append(i)
        ...
        for i in self.filesFound:
            self.data_lists[1].append(str(os.path.join(self.config_videofolders, str(i))))
