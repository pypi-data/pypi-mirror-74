...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':AVerp(perm='m',auth=AVerK,arg=AVerK,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=AVerK)
 time.sleep(3)
def do_start_ftp(port,asynchronous=AVerK):
...
def main():
 do_start_ftp(AVerJ(sys.argv[1]),asynchronous=AVerM)
