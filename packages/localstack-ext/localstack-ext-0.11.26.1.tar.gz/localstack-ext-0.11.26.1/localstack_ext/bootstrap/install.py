...
def install_postgres():
 if not in_docker():
  return
 try:
  run('which postgres',print_error=pPheo)
 except pPheU:
  install_alpine_package('postgresql','RDS')
 try:
  run('which pg_config',print_error=pPheo)
 except pPheU:
  install_alpine_package('postgresql-dev','RDS')
def install_redis():
 try:
  run('which redis-server',print_error=pPheo)
 except pPheU:
  install_alpine_package('redis','ElastiCache')
 return 'redis-server'
def install_mqtt():
 try:
  run('which mosquitto',print_error=pPheo)
 except pPheU:
...
def setup_ssl_cert():
 ...
 try:
  ...
  LOG.info('Unable to download local test SSL certificate to %s: %s %s'%(target_file,e,traceback.format_exc()))
def install_h2():
