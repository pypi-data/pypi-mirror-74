...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':FBoxD(perm='m',auth=FBoxX,arg=FBoxX,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=FBoxX)
 time.sleep(3)
def do_start_ftp(port,asynchronous=FBoxX):
...
def main():
 do_start_ftp(FBoxY(sys.argv[1]),asynchronous=FBoxr)
