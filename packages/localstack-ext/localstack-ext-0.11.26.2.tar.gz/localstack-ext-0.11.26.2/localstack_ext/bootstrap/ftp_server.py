...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':aLEox(perm='m',auth=aLEoV,arg=aLEoV,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=aLEoV)
 time.sleep(3)
def do_start_ftp(port,asynchronous=aLEoV):
...
def main():
 do_start_ftp(aLEou(sys.argv[1]),asynchronous=aLEoO)
