...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':TEXAp(perm='m',auth=TEXAy,arg=TEXAy,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=TEXAy)
 time.sleep(3)
def do_start_ftp(port,asynchronous=TEXAy):
...
def main():
 do_start_ftp(TEXAi(sys.argv[1]),asynchronous=TEXAr)
