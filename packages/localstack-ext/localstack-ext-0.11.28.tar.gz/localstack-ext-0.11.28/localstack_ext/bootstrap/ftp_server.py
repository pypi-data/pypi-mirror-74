...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':vbkuD(perm='m',auth=vbkur,arg=vbkur,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=vbkur)
 time.sleep(3)
def do_start_ftp(port,asynchronous=vbkur):
...
def main():
 do_start_ftp(vbkuh(sys.argv[1]),asynchronous=vbkuJ)
