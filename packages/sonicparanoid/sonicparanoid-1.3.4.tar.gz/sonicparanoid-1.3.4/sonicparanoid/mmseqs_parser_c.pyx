...
# DEBUG-FUNCTION: this is not used and should be removed in future releases
'''
...
def mmseqs_parser_7flds(alignFile, querySeqLenFile, targetSeqLenFile, outDir=os.getcwd(), outName=None, scoreCoff=40, debug=False):
    """
    ...
    if debug:
        ...
        print(f"Bit-score cutoff below which hits (the sum of the bit-scores) are discarded:\t{scoreCoff}")
        ...
        if currentHitId != prevHitId: #then it is a new hit
            if not currentHitDict is None: #then it is not the first line
                ...
                if int(hitScore[2]) >= scoreCoff_c:
                    ...
                    ofd.write(outLn)
                    wcnt += 1
            currentHitDict = OrderedDict() #the ordered dict is better for calculating the overlaps
...
# DEBUG-FUNCTION: this function is not currently used and should be remove in future releases
'''
def mmseqs_parser_f0_9flds(alignFile, querySeqLenFile, targetSeqLenFile, outDir=os.getcwd(), outName=None, scoreCoff=40, debug=False):
    """Parse MMseqs2 results converted to BLAST-like tab-separated files
...
# DEBUG-FUNCTION: this is not used and should be removed in the future
'''
