...
def consume_alignment_jobs(jobs_queue, results_queue, runDir:str, dbDir:str, alnDir:str, keepAln:bool, sensitivity:float, cutoff:float, pmtx:str) -> None:
    """
    ...
    while True:
        ...
        if (jobType == 0) or (jobType == 2) or (jobType == 4): # The first complete alignment
            ...
            parsedOutput, search_time, convert_time, parse_time, tot_time = mmseqs_1pass(inSeq, dbSeq, dbDir=dbDir, runDir=inputSeqInfoDir, outDir=pairAlnDir, tmpDirName=tmpMMseqsDirName, keepAlign=keepAln, sensitivity=sensitivity, evalue=1000, cutoff=cutoff, pmtx=pmtx, threads=threads, debug=False)
            del tot_time
            # exit if the BLAST formatted file generation was not successful
            if not os.path.isfile(parsedOutput):
        ...
        if (jobType == 3) or (jobType == 4): # Complete alignments
            ...
            parsedOutput, search_time, convert_time, parse_time, tot_time = mmseqs_1pass(reducedBPath, reducedAPath, dbDir=essentialFaDir, runDir=essentialFaDir, outDir=pairAlnDir, tmpDirName=tmpMMseqsDirName, keepAlign=keepAln, sensitivity=sensitivity, evalue=1000, cutoff=cutoff, pmtx=pmtx, threads=threads, debug=False)
            # add execution times to the output list
...
def mmseqs_1pass(inSeq, dbSeq, dbDir=os.getcwd(), runDir=os.getcwd(), outDir=os.getcwd(), tmpDirName=None, keepAlign=False, sensitivity=4.0, evalue=1000, cutoff=40, pmtx="blosum62", threads=4, debug=False):
    ...
    if debug:
        ...
        # print(f"Python3 binaries for parsing:\t{pythonPath}")
    ...
    parse_blast_output(blastLikeOutput, inSeq, dbSeq, parsedOutput, runDir, cutoff, debug)
...
def mmseqs_search(inSeq, dbSeq, dbDir=os.getcwd(), outDir=os.getcwd(), tmpDirName=None, sensitivity=4.0, pmtx="blosum62", evalue=1000, threads=4, cleanUp=False, debug=False):
    ...
    searchCmd: str = f"{get_mmseqs_path()} search {queryDBpath} {targetDBpath} {rawOutPath} {tmpDir} -s {str(sensitivity)} --threads {threads:d} -v 0 {mtxSettings} --min-ungapped-score {minUngappedScore} --alignment-mode 2 --alt-ali 10"
...
def parse_blast_output(inBlastOut:str, query:str, target:str, outPath:str, runDir:str, cutoff:int=40, debug:bool=False):
    ...
    parser.mmseqs_parser_7flds(inBlastOut, qSeqLenPath, tSeqLenPath, outDir=outDir, outName=outName, scoreCoff=cutoff, debug=False)
...
def perform_parallel_essential_alignments_plus(requiredAln: Dict[str, Tuple[float, int, int]], protCntDict:Dict[str, int], runDir:str, dbDir:str, alnDir:str, create_idx:bool=True, sensitivity:float=4.0, cutoff:int=40, pmtx:str="blosum62", essentialMode:bool=True, threads:int=4, keepAln:bool=False, debug:bool=False) -> None:
    auxDir: str = os.path.join(runDir, "aux")
    """Create FASTA subsets in parallel."""
    if debug:
    ...
    runningJobs = [mp.Process(target=consume_alignment_jobs, args=(essential_queue, results_queue, runDir, dbDir, alnDir, keepAln, sensitivity, cutoff, pmtx)) for i_ in range(threads)]
