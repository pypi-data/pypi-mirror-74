...
def check_gcc() -> Tuple[str, str, bool]:
    ...
    if gccPath != None:
        ...
        rOut = subprocess.run("gcc --version", shell=True, capture_output=True)
...
def sanity_check(infoDict: Dict[str, str], debug:bool=False) -> bool:
    """
    Make sure there no uncompleted alignments, from previous runs.
    ...
    for sp in spList:
        ...
        for r, dirs, files in os.walk(tmpAlnSpDir, topdown=True):
            ...
            for tmpName in files:
                if tmpName[0] == "_":
                    os.remove(os.path.join(r, tmpName))
...
def main():
    ...
    if not update_run:
        ...
        spFile, pairsFile, requiredPairsDict = orthodetect.run_sonicparanoid2_multiproc_essentials(mappedInPaths, outDir=runDir, tblDir=pairwiseDbDir, threads=threads, alignDir=alignDir, mmseqsDbDir=dbDirectory, create_idx=idx_dbs, sensitivity=sensitivity, cutoff=coff, confCutoff=0.05, pmtx=pmtx, lenDiffThr=args.max_len_diff, overwrite_all=overwrite, overwrite_tbls=owOrthoTbls, update_run=update_run, keepAlign=args.keep_raw_alignments, essentialMode=not(complete_aln), debug=debug)
