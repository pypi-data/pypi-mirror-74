...
class Client:
    ...
    def list_datasets_with_similar_name(self, name: str) -> List[str]:
        ...
        datasets = []
        while True:
            chunk = self.list_datasets(chunk_position, chunk_size, include_inactive=True).datasets
            # inactive datasets are ignored by `list_datasets_with_similar_name` in servers > 1.8.0 
            # but are needed for proper chunk length comparison
            datasets.extend(d.name for d in chunk if d.import_status <= 0)
    ...
    def list_models_with_similar_name(self, name: str) -> List[str]:
        ...
        models = []
        while True:
            chunk = self.list_models(chunk_position, chunk_size).models
            models.extend(m.description for m in chunk)
            if len(chunk) < chunk_size:
                break
            chunk_position += chunk_size
        return [m for m in models if m.endswith(name)]
    ...
    # PATCHED
    def get_experiment_tuning_suggestion(self, dataset_key: str, target_col: str, is_classification: bool, is_time_series: bool, config_overrides: str, cols_to_drop: str) -> ModelParameters:
        """
    ...
    def get_experiment_preview(self, dataset_key: str, validset_key: str, classification: bool, dropped_cols: List[str], target_col: str, is_time_series: bool, enable_gpus: bool, accuracy: int, time: int, interpretability: int, config_overrides: str, reproducible: bool, resumed_experiment_id: str) -> str:
        """

        """
        req_ = dict(dataset_key=dataset_key, validset_key=validset_key, classification=classification, dropped_cols=dropped_cols, target_col=target_col, is_time_series=is_time_series, enable_gpus=enable_gpus, accuracy=accuracy, time=time, interpretability=interpretability, config_overrides=config_overrides, reproducible=reproducible, resumed_experiment_id=resumed_experiment_id)
    ...
    def get_global_importance(self, model_type: str, importance_type: str, mli_key: str) -> List[VarImpTable]:
        """

        """
        req_ = dict(model_type=model_type, importance_type=importance_type, mli_key=mli_key)
        res_ = self._request('get_global_importance', req_)
        return [VarImpTable.load(b_) for b_ in res_]

    def get_local_importance(self, model_type: str, importance_type: str, mli_key: str, row_idx: int, offset: int, count: int) -> List[VarImpTable]:
        """

        """
        req_ = dict(model_type=model_type, importance_type=importance_type, mli_key=mli_key, row_idx=row_idx, offset=offset, count=count)
        res_ = self._request('get_local_importance', req_)
        return [VarImpTable.load(b_) for b_ in res_]
