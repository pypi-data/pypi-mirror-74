...
class Datasets:
    ...
    def _dataset_create(
        ...
        if not force and isinstance(data, str):
            _name = name or Path(data).name
            _utils.error_if_dataset_exists(self._client, _name)
        if data_source in self._simple_connectors:
            ...
            if data.__class__.__name__ == "DataFrame":
                with tempfile.NamedTemporaryFile(
                    ...
                    data.to_pickle(  # type: ignore
                        pkl.name, compression=None, protocol=4
                    )
