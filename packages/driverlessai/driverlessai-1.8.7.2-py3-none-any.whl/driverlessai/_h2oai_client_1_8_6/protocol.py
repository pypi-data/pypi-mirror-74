...
class Client:
    def __init__(self, address: str, username: Optional[str] = None, password: Optional[str] = None, verify=True, cert=None, use_tls_authentication: bool = False) -> None:
        self._autoviz = None
        ...
        if not use_tls_authentication:
            res = self._session.post(address + "/login", data=dict(username=username, password=password))
        else:
            res = self._session.get(address + "/tls_auth")
    ...
    def _request(self, method: str, params: dict) -> Any:
        ...
        res = self._session.post(self.address + "/rpc", data=req)
        if res.status_code != requests.codes.ok:
    ...
    def perform_stream_upload(self, file_path, skip_parse=False):
        ...
        with open(file_path, 'rb') as f:
            self._session.post(url, data=f, headers=headers)
    ...
    def perform_upload(self, file_path, skip_parse=False):
        ...
        with open(file_path, 'rb') as f:
            res = self._session.post(url, files={'dataset': f})
    ...
    def create_dataset_from_bigquery_sync(self, datasetid: str, dst: str, query: str) -> Dataset:
        ...
        args = GbqCreateDatasetArgs(datasetid, bucket_name, dataset_name, query)
    ...
    def create_dataset_from_snowflake_sync(self, region: str, database: str, warehouse: str, schema: str, role: str, optional_file_formatting: str, dst: str, query: str) -> Dataset:
        ...
        """
        args = SnowCreateDatasetArgs(region, database, warehouse, schema, role, dst, query, optional_file_formatting)
    ...
    def _wait_for_dataset(self, key):
        return self._wait_for_entity_job(lambda : self.get_dataset_job(key))

    def _wait_for_model(self, key):
        return self._wait_for_entity_job(lambda : self.get_model_job(key))

    def _wait_for_prediction(self, key):
        return self._wait_for_entity_job(lambda : self.get_prediction_job(key))

    def _wait_for_autoreport(self, key):
        return self._wait_for_entity_job(lambda : self.get_autoreport_job(key))

    def _wait_for_transformation(self, key):
        return self._wait_for_entity_job(lambda : self.get_transformation_job(key))

    def _wait_for_model_diagnostic(self, key):
        return self._wait_for_entity_job(lambda : self.get_model_diagnostic_job(key))

    def _wait_for_interpretation(self, key):
        return self._wait_for_entity_job(lambda : self.get_interpretation_job(key))

    def _wait_for_interpret_timeseries(self, key):
        return self._wait_for_entity_job(lambda : self.get_interpret_timeseries_job(key))

    def _wait_for_scoring_pipeline(self, key):
        return self._wait_for_entity_job(lambda : self.get_scoring_pipeline_job(key))

    def _wait_for_mojo_pipeline(self, key):
        return self._wait_for_entity_job(lambda : self.get_mojo_pipeline_job(key))

    def _wait_for_create_deployment(self, key):
        return self._wait_for_entity_job(lambda : self.get_create_deployment_job(key))
    ...
    def _wait_for_preview(self, key):
        return self._wait_for_entity_job(lambda : self.get_experiment_preview_job(key))

    def _wait_for_recipe_load(self, key):
        return self._wait_for_entity_job(lambda : self.get_custom_recipe_job(key))
    ...
    def _wait_for_dataset_split(self, key):
        return self._wait_for_entity_job(lambda : self.get_dataset_split_job(key))
    ...
    def list_model_iteration_data(self, key: str, offset: int, limit: int) -> List[AutoDLProgress]:
        """
    ...
    def make_prediction(self, model_key: str, dataset_key: str, output_margin: bool, pred_contribs: bool, keep_non_missing_actuals: bool, include_columns: List[str]) -> str:
        """
    ...
    def upload_experiment_artifacts(self, model_key: str, user_note: str, artifact_path: str, name_override: str) -> str:
        """

        """
        req_ = dict(model_key=model_key, user_note=user_note, artifact_path=artifact_path, name_override=name_override)
    ...
    def get_experiment_tuning_suggestion(self, dataset_key: str, target_col: str, is_classification: bool, is_time_series: bool, config_overrides: str, cols_to_drop: List[str]) -> ModelParameters:
        """

        """
        req_ = dict(dataset_key=dataset_key, target_col=target_col, is_classification=is_classification, is_time_series=is_time_series, config_overrides=config_overrides, cols_to_drop=cols_to_drop)
        res_ = self._request('get_experiment_tuning_suggestion', req_)
        return ModelParameters.load(res_)

    def get_experiment_preview(self, dataset_key: str, validset_key: str, classification: bool, dropped_cols: List[str], target_col: str, is_time_series: bool, time_col: str, enable_gpus: bool, accuracy: int, time: int, interpretability: int, config_overrides: str, reproducible: bool, resumed_experiment_id: str) -> str:
        """

        """
        req_ = dict(dataset_key=dataset_key, validset_key=validset_key, classification=classification, dropped_cols=dropped_cols, target_col=target_col, is_time_series=is_time_series, time_col=time_col, enable_gpus=enable_gpus, accuracy=accuracy, time=time, interpretability=interpretability, config_overrides=config_overrides, reproducible=reproducible, resumed_experiment_id=resumed_experiment_id)
    ...
    def link_dataset_to_project(self, project_key: str, dataset_key: str, dataset_type: str) -> bool:
        """
