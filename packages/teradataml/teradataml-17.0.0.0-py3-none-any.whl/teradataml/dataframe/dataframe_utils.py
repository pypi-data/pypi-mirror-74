...
from teradatasqlalchemy.types import FLOAT, INTEGER, NUMBER, DECIMAL, PERIOD_TIMESTAMP
                ...
                try:
                    if node_query_types[index] == AEDConstants.AED_QUERY_NODE_TYPE_ML_QUERY_MULTI_OUTPUT.value or\
                       ("OUT TABLE " in queries[index] and SQLPattern.SQLMR.value.match(queries[index])):
                        ...
                        _execute_node_return_db_object_name(parent_nodeid)
    ...
    def _validate_sort_col_type(sort_col_type, sort_col_values):
        ...
        if isinstance(sort_col_values, list):
            if sort_col_type == PythonTypes.PY_STRING_TYPE.value:
                ...
                if not all(isinstance(i, decimal.Decimal) for i in sort_col_values):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_DATETIME_TYPE.value:
                if not all(isinstance(i, datetime.datetime) for i in sort_col_values):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_TIME_TYPE.value:
                if not all(isinstance(i, datetime.time) for i in sort_col_values):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_DATE_TYPE.value:
                if not all(isinstance(i, datetime.date) for i in sort_col_values):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            ...
            if sort_col_type == PythonTypes.PY_STRING_TYPE.value:
                ...
                if not isinstance(sort_col_values, decimal.Decimal):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_DATETIME_TYPE.value:
                if not isinstance(sort_col_values, datetime.datetime):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_TIME_TYPE.value:
                if not isinstance(sort_col_values, datetime.time):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_DATE_TYPE.value:
                if not isinstance(sort_col_values, datetime.date):
                    DataFrameUtils.__validate_sort_type_raise_exception(sort_col_type)
            elif sort_col_type == PythonTypes.PY_BYTES_TYPE.value:
