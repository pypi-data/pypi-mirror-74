...
class _Validators:
    ...
    def _validate_dataframe_has_argument_columns(columns, column_arg, data, data_arg, is_partition_arg=False):
        ...
        return _Validators._validate_column_exists_in_dataframe(columns, data._metaexpr, column_arg=column_arg,
                                                                data_arg=data_arg)
    ...
    @staticmethod
    def _validate_function_arguments(arg_list):
        """
        ...
        for args in arg_list:
            ...
            if ((args[2] == True and args[1] is not None) or (args[2] == False)):
                ...
                if not typeCheckFailed and len(args) >= 5:
                    if args[4]:
                        _Validators._validate_input_columns_not_empty(args[1], args[0])
    ...
    @staticmethod
    def _validate_input_columns_not_empty(arg, arg_name):
        """
        ...
        if vantage_version not in supported_vantage_versions.keys():
            raise TeradataMlException(Messages.get_message(
                MessageCodes.CONFIG_ALIAS_VANTAGE_VERSION_NOT_SUPPORTED).format(vantage_version,
                                                                                ", ".join(
                                                                                    supported_vantage_versions.keys())),
