...
if sys.version_info[1]<8:
    ...
    pkg_resources.require("numpy==1.15.0")
    import numpy as np
else: 
    import numpy as np
import pandas as pd
...
class ANA():
    ...
    def read(self, index=[]):
        ...
        spot_num = [int(f[:-3].split('_')[2]) + 1 for f in file_list]
        file_names = [f.name for f in sorted(Path(self.directory).glob('*.h5'))]

        self.data = pd.DataFrame(data={'Experiment': str(self.directory),
                                       'Files': file_names,
                                       'Spot': spot_num,
                                       'Data': data_list})
    ...
    def xml2hdf(self):
        f1 = glob(str(self.directory / 'scalar*'))
        f2 = glob(str(self.directory / 'scalar*.h5'))
    ...
    def _filelist(self):
        self.files = [str(file) for file in sorted(self.directory.glob('scalar*.h5'))]
