...
class Cascade(BaseState):
    name = 'cascade'
    match = re.compile(r'Cascade\s*(?P<Cascade>\d+):     Group\s*(?P<Group>\d+)\s+Number\s*(?P<Number>\d+)')
...
class CascadeRanger(CascadeBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.cascade_ranger = cascade_ranger.parse(self.textbuf.getvalue(), self.context.config_debug_parser)
...
class CascadeSequin(CascadeBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.cascade_sequin = cascade_sequin.parse(self.textbuf.getvalue(), self.context.config_debug_parser)
...
class CascadeOrgiv(CascadeBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.cascade_orgiv = cascade_orgiv.parse(self.textbuf.getvalue(), self.context.config_debug_parser)

class CascadeEnd(BaseState):
    name = 'cascade_end'

    def startswith(self, line):
        match = re.compile(r'Computation time\s+([0-9]*[.])?[0-9]+([eE][-+]?\d+)?\s+seconds      Cascade storage\s+\d+')
        return match.fullmatch(line)
...
class FinalRangex(RootBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.final_rangex = final_rangex.parse(self.textbuf.getvalue(), self.context.config_debug_parser)
...
class FinalSlavex(RootBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.final_slavex = final_slavex.parse(self.textbuf.getvalue(), self.context.config_debug_parser)
...
class FinalSlavexAtom(RootBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.final_slavex_atom = final_slavex_atom.parse(self.textbuf.getvalue(), self.context.config_debug_parser)
...
class FinalSequex(RootBaseState):
    ...
    def on_exit(self):
        if self.context.config_dump_text_blocks:
            super().on_exit()
        self.context.final_sequex = final_sequex.parse(self.textbuf.getvalue(), self.context.config_debug_parser)
...
class FinalEnd(RootBaseState):
    ...
    def startswith(self, line):
        match = re.compile(r'Computation time\s+([0-9]*[.])?[0-9]+([eE][-+]?\d+)?\s+seconds      Maximum cascade storage\s+\d+')
        return match.fullmatch(line)
...
class End(BaseState):
    ...
    def startswith(self, line):
        match = re.compile(r'End of program:  total time\s+([0-9]*[.])?[0-9]+([eE][-+]?\d+)?\s+seconds')
        return match.fullmatch(line)
...
class Context():
    ...
    def __init__(self, 
        ...
        if self.config_cascade_table_output == 'SEPARATE':
            ...
            self.csv_cascade_report_detail_all = csv.writer(
            ...
            self.csv_cascade_report_detail_primary_all = csv.writer(
            ...
            self.csv_cascade_report_lattice_sites_all = csv.writer(
    ...
    def output_csv_cascade_report_detail(self):
        ...
        for d1, d2, d3 in zip(self.cascade_report_detail_1,
            ...
            if not primary_recoil_output and d1[0] == 1:
            ...
            with (self.outputdir / 'statistical_analysis_of_data.csv').open('w', newline='') as f:
                ...
                for k in ['Collisions', 'Atoms available for pairing', 'Sites available for pairing',
                 ...
                 'Focuson sequences', 'Truncated trajectories']:
                    if k in d:
                        rowdata = [k] + [d[k][v] for v in csv_header.final_slavex_number[1:-1]] + count
                        fcsv.writerow(rowdata)
...
class BlockSeparator():
    ...
    def build_states(self):
        ...
        self.initial.next_states = [self.config]
        self.config.next_states = [self.cascade]
