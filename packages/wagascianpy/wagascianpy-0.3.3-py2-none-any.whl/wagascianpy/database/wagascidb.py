...
BAD_RUNS_WAGASCI_UPSTREAM = range(33, 54)
BAD_RUNS_WAGASCI_DOWNSTREAM = range(33, 54)
BAD_RUNS_WALLMRD_SOUTH = range(1, 80)
BAD_RUNS_WALLMRD_NORTH = range(1, 60)
    ...
    def _update_wagasci_db(self):
        ...
        with wagascianpy.utils.utils.Cd(self._tmp_dir):
            for run_name in sorted(run_list):
                ...
                run.good_run_flag = not (run.run_number in BAD_RUNS_WAGASCI_UPSTREAM and
                                         run.run_number in BAD_RUNS_WAGASCI_DOWNSTREAM and
                                         run.run_number in BAD_RUNS_WALLMRD_SOUTH and
                                         run.run_number in BAD_RUNS_WALLMRD_NORTH)
