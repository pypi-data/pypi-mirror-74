        ...
        if graph.id == "BSD":
            ...
            for enabled_detector in self._topology.get_enabled():
                if graph.id == str(enabled_detector.name):
                    ...
                    graph.xdata, graph.ydata = self._data_harvester.gather_data(enabled_detector.name, self._only_good)
...
class DataQualityPlotter(Plotter, ABC):
    ...
    def gather_data(self, graph):
        ...
        if isinstance(graph, wagascianpy.plotting.graph.Graph2D):
            ...
            self._data_harvester.harvester = self._data_quality_harvester_class(
                database=None,
            ...
            for enabled_detector in self._topology.get_enabled():
                if graph.id == str(enabled_detector.name):
                    if not self._data_harvester.is_harvester_ready() or \
                            not isinstance(self._data_harvester.harvester, self._temperature_harvester_class):
                        self._data_harvester.harvester = self._temperature_harvester_class(
                            database=None,
                            repository=self._data_quality_location,
                            t2krun=None,
                            topology=self._topology,
                            filename=self._data_quality_filename)
                    graph.xdata, graph.ydata = self._data_harvester.gather_data(detector_name=enabled_detector.name)
