...
class QalxAdapter(object):
    ...
    def find(
        ...
        **kwargs,
        ...
        :param aggregate:   Optional Mongo aggregation pipeline argument that
                            can be used to perform and return aggregation
                            results
        ...
        entity_class = AggregationResult if aggregate else QalxListEntity
        entities = self._process_api_response(
            resp, entity_class=entity_class, child=self.entity_class
        )
        # `many` argument is redundant if `aggregate` is supplied
        if aggregate:
            return entities
...
class QalxUnpackableAdapter(QalxAdapter):
    ...
    def _child_list_response(child_adapter, query, **kwargs):
        return child_adapter.find(query=query, **kwargs)
    ...
    def _child_list_request(
        ...
        for _page in entity._chunks(kids, chunk_size=100):
            ...
            _resp = self._child_list_response(
                child_adapter=child_adapter, query=query, fields=child_fields
            )
            ...
            while _resp["query"]["next"] is not None:
                ...
                _resp = self._process_api_request(
                    "get", _resp["query"]["next"], fields=child_fields
                )

                unpacked_entities += self._process_api_response(
                    _resp,
                    entity_class=QalxListEntity,
                    child=self.child_entity_class,
                )["data"]
    ...
    def get(self, guid, child_fields=None, *args, **kwargs):
        """
