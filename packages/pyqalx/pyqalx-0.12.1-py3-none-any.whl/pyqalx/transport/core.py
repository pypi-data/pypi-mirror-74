...
class BasePyQalxAPI(object):
    def __init__(self, session):
        ...
        self._max_retries = {
            HTTPStatus.TOO_MANY_REQUESTS.value: {"retries": 0, "max": 2},
    ...
    def _should_retry(self, status_code):
        ...
        if status_code == _too_many_requests:
            ...
            max_attempts = too_many_requests["max"]
            if too_many_requests["retries"] < max_attempts:
                should_retry = True
                # Sleep to allow the throttle to expire
                sleep_for = 5
                too_many_requests["retries"] += 1
                remaining = max_attempts - too_many_requests["retries"]

                self.session.log.warning(
                    f"Got `{status_code}` response from API. Sleeping for "
                    f"`{sleep_for}` seconds before trying again. `{remaining}` "
