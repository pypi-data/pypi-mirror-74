...
fh = logging.FileHandler(filename = f'{path.join(__file__.split(".")[:-1])}_log_{LOG_TS}.log')
fh.setLevel(LOG_LEVEL)
fh.setFormatter(formatter)
logger.addHandler(fh)
Test_case = f'*Dynapipe* autoFS - Features Selection :: {LOG_TS}'
Test_comment = '-' * len(Test_case) * 3
Start_log = '#' * len(Test_case) * 3
logger.info(Start_log)
logger.info(Test_case)
logger.info(Start_log)
delete_old_log_files(directory = path,delete_flag = DELETE_FLAG, logger = logger, extension_list = ['.log'],filename_list = ['autoFS_log_'],log_ts = LOG_TS)
    ...
    def fit_fs_clf(self,tr_features,tr_labels):
        ...
        for selector in selectors:
            ...
            try:
                ...
                logger.info(f"Selector - {selector} is generating {fs_feature}selected features...")
        ...
        logger.info(f'All selected features are extended as the list:{selected_features}')
