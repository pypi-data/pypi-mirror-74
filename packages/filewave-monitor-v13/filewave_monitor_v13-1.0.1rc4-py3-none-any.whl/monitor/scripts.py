...
def provision_postgres_wal_interval():
    ...
    #
    cmds = [
        "/bin/sed -i 's/log_min_duration_statement = 2000/log_min_duration_statement = 200/g' /usr/local/filewave/fwxserver/DB/pg_data/postgresql.conf"
    ]
...
def provision_mtail_binary():
    ...
    for mtail_file in pkg_resources.resource_listdir("monitor", "config"):
        if mtail_file.endswith(".mtail"):
            data = pkg_resources.resource_string("monitor.config", mtail_file)
