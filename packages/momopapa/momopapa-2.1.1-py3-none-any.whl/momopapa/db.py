...
class GenericDBConnector(SSHTunnelMixin, object):
    ...
    def connect(self):
        ...
        except (ProgrammingError, pymysqlOperationalError):
            self.engine = create_engine(
    ...
    def create(self, df,tbl):
        df_schema = pd.DataFrame(data=None, columns=df.columns, index=df.index.name)
        pandabase.to_sql(df_schema, table_name=tbl, con=self.engine, how='create_only')

    def replace(self, df, tbl):
        df.to_sql(tbl, self.engine, if_exists='replace')

    def insert(self, df, tbl):
        pandabase.to_sql(df, table_name=tbl, con=self.engine, how='append')

    def upsert(self, df, tbl):
        pks = self.get_pks(tbl)
        df.set_index(pks, inplace=True)
        pandabase.to_sql(df, table_name=tbl, con=self.engine, how='upsert')
