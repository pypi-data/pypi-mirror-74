    ...
    def login():
        try:
            ...
            login_request = requests.post(Shield34Properties.api_base_url + '/auth/project-login', data=payload,
                                          headers=headers, verify=Shield34Properties.enable_ssl_certificate_verification, proxies=get_external_proxies(), timeout=30)
            if login_request.status_code == 200:
                ...
                Shield34Logger.logger.console("Login to Shield34 returned code {}".format(login_request.status_code))
                raise Shield34LoginFailedException
        except Exception as e:
            Shield34Logger.logger.console("Login to Shield34 failed with error {}".format(str(e)))
            raise Shield34LoginFailedException
