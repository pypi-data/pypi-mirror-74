    ...
    * ``sq:content-mime-type``: Document mime-type specified during upload
    ...
    def __init__(self, metadata_mapping=None, batch_size=10, batch_size_mb=150,
        ...
        if 'processing_config' in kwargs:
            msg = "processing_config is deprecated and is not supported" \
        ...
        if 'bulk_index' in kwargs:
            msg = 'bulk_index is supported by the new ingester automatically ' \
        ...
        if (default_mime_type_keyword and
                'sq:content-mime-type' not in mapping.get('default', {})):
            mapping.setdefault('default', {})['sq:content-mime-type'] = \
                'MIME Type'
        ...
        if source_id:
            ...
            self.uploader = ItemUploader(
                ...
                steps_config=default_steps_config, **kwargs)

        else:
            # Create pipeline workflow with given steps config
            self.uploader = ItemUploader(
                pipeline_workflow_name=pipeline_workflow_name,
                steps_config=steps_config, **kwargs)
    ...
    def upload(self, filename, mime_type=None, title=None, doc_id=None,
               ...
               priority=0, pipeline_workflow_id=None):
        ...
        if not content_url or not item_id:
            ...
            content_size = old_div(len(content), 1024 / 1024)
        ...
        if self._items:
            if self.priority:
                self.uploader.upload(
                    self._items, priority=self.priority,
