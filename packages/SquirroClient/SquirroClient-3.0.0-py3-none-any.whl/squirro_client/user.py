...
class UserApiMixin(object):
    ...
    def get_user_data(self, user_id, api_version='v1'):
        ...
        url = '%(endpoint)s/%(version)s/%(tenant)s/users/%(user)s' % {
            'endpoint': self.user_api_url, 'version': api_version,
            'tenant': self.tenant, 'user': user_id}
    ...
    def get_authentication(self, user_id, service):
        ...
        url = ('%(ep)s/%(version)s/%(tenant)s'
               '/authentications/%(service)s/%(user_id)s') % {
                   'ep': self.user_api_url, 'version': self.version,
                   'tenant': self.tenant, 'service': service,
                   'user_id': user_id
               }
    ...
    def new_or_modify_authentication(self, user_id, service, service_user,
        ...
        url = ('%(ep)s/%(version)s/%(tenant)s'
               '/authentications/%(service)s/%(user_id)s') % {
                   'ep': self.user_api_url, 'version': self.version,
                   'tenant': self.tenant, 'service': service, 'user_id': user_id
               }

        # build data
        data = {
            'service_user': service_user, 'access_token': access_token
        }
        ...
        if access_token_expires is not None:
            data['access_token_expires'] = self._format_date(
                access_token_expires)
        if access_secret_expires is not None:
            data['access_secret_expires'] = self._format_date(
                access_secret_expires)
        ...
        res = self._perform_request(
            'post', url, params=params, data=json.dumps(data),
            headers=headers)
    ...
    def new_grant(self, user_id, type, for_client_id=None,
        ...
        params = {
            'type': type, 'client_id': self.client_id,
    ...
    def delete_session(self, user_id, session_id):
        ...
        url = ('%(ep)s/%(version)s/%(tenant)s'
               '/users/%(user)s/sessions/%(session_id)s') % {
                   'ep': self.user_api_url, 'version': self.version,
                   'tenant': self.tenant, 'user': user_id,
    ...
    def add_user(self, email=None, password=None, role=None):
        ...
        url = '%(ep)s/v0/%(tenant)s/users' % {
            'ep': self.user_api_url, 'tenant': self.tenant
        }
    ...
    def delete_user(self, user_id):
        ...
        url = '%(ep)s/%(version)s/%(tenant)s/users/%(user_id)s' % {
            'ep': self.user_api_url, 'version': self.version,
            'tenant': self.tenant, 'user_id': user_id,
        }
        res = self._perform_request('delete', url)
        return self._process_response(res, [204])
    ...
    def add_project_member(self, project_id, role, user_id=None,
                           group_id=None):
    ...
    def get_group(self, group_id):
        ...
        url = '%(ep)s/%(version)s/%(tenant)s/groups/%(group)s' % {
            'ep': self.user_api_url, 'version': self.version,
            'tenant': self.tenant, 'group': group_id}
        res = self._perform_request('get', url)
        return self._process_response(res, [200])
    ...
    def delete_group_member(self, group_id, user_id):
        ...
        """
        url = ('%(ep)s/%(version)s/%(tenant)s/'
               'groups/%(group)s/members/%(user)s') % {
                   'ep': self.user_api_url, 'version': self.version,
                   'tenant': self.tenant, 'group': group_id, 'user': user_id
               }
        res = self._perform_request('delete', url)
        return self._process_response(res, [204])
