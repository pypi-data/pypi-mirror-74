...
from .exceptions import (
    AuthenticationError, AuthorizationError, ClientError, ConflictError,
    ConnectionError, InputDataError, NotFoundError, RetryError, TransportError,
    UnknownError)
...
class SquirroClient(UserApiMixin, TopicApiMixin):
    ...
    def authenticate(self, tenant=None, access_token=None, refresh_token=None,
                     auth_service=None, auth_user=None,
        ...
        if access_token is not None:
            ...
            data = {
                'grant_type': 'access_token', 'access_token': access_token,
            }
            ...
            # by external id
            data = {
                'grant_type': 'ext_id', 'auth_service': auth_service,
                'auth_user': auth_user,
            }
            self._perform_authentication(dict(base, **data))
    ...
    def _perform_authentication(self, data):
        ...
        self._on_token_refresh(
            res.get('access_token'), res.get('refresh_token'))
    ...
    def _perform_request(self, method, url, **kwargs):
        ...
        for retry_count in range(2):
            ...
            try:
                res = func(
                    url, timeout=self.timeout_secs, **kwargs)
            except self._requests.exceptions.RetryError as ex:
                raise RetryError(None, ex[0])
    ...
    def _refresh_tokens(self, **kwargs):
        ...
        try:
            ...
            for try_count in range(3):
                ...
                log.info('Access token %r not refreshed on behalf of '
                         'refresh token %r. Retry number %d',
                         self.access_token, self.refresh_token, try_count)
    ...
    def _get_verify(self):
        ...
        if not hasattr(sys, "frozen"):
            ...
            return None
        exe_dir = os.path.dirname(str(sys.executable,
