...
class ItemUploader(object):
    ...
    def __init__(self, token=None, project_id=None, project_title=None,
                 ...
                 pipeline_workflow_name=None, pipeline_workflow_id=None,
                 timeout_secs=None, **kwargs):

        if source_secret:
            msg = '`source_secret` parameter is deperecated and is not ' \
                  'necessary anymore.'
            deprecation(msg)
            raise NotImplementedError(msg)

        if object_id:
            msg = '`object_id` parameter is deperecated and is not necessary ' \
                  'anymore.'
            deprecation(msg)
            raise NotImplementedError(msg)

        if processing_config:
            msg = '`processing_config` parameter is deperecated and is not ' \
        ...
        if source_id:
            ...
            if any([pipeline_workflow_id, pipeline_workflow_name,
                    steps_config]):
                log.warn('Ignoring parameters: `pipeline_workflow_id`, '
            ...
            if any([pipeline_workflow_name, steps_config]):
                log.warn('Ignoring parameters: `pipeline_workflow_name`, '
                         'and `steps_config`')
        ...
        try:
            ...
            log.info('Source %r in Project %r not present. Creating one',
                     source_id, self.project_id)
            source = self._create_source(source_name=source_name,
        ...
        log.info(
            ...
            self.source_id, workflow['id'], self.project_id)
    ...
    def _lookup_workflow(self, name=None, uuid=None):
        ...
        raise NotFoundError('Workflow "%s" with id "%s" was not found!' %
                            (name, uuid))
    ...
    def _load_config(self, kwargs):
        ...
        config.update(dict([(k, v) for k, v in kwargs.items()
                            if v is not None]))
    ...
    def _validate_config(self):
        ...
        if not any([self.config.get('project_id'),
            ...
            raise ValueError(
                'Parameters ("project_id"/"project_title") is required')

        if self.config.get('source_id') and self.config.get('steps_config'):
            raise ValueError('Parameters "source_id" and "steps_config" '
    ...
    def _lookup_source(self, source_id=None, source_name=None):
        ...
        if source_id:
            try:
                ...
                if source_name and source.get('name') != source_name:
                    raise ValueError(
                        ...
                        source_id, self.project_id, source_name)
            except NotFoundError:
                log.exception('Source %r in project %r not present.',
                              source_id, self.project_id)
                raise
            ...
            raise NotFoundError(
                'Must define either `source_id` or `source_name`.')
    ...
    def upload(self, items, priority=0, pipeline_workflow_id=None):
        """Sends ``items`` to Squirro.
        ...
        for idx in range(0, len(items), batch_size):
            ...
            res = self.client._perform_request('post', url, params=params,
    ...
    def _get_batch_size(self, items):
        ...
        if batch_size == -1:
            ...
            if items_bytes_size < MAX_BYTE_SIZE:
                ...
                assert nof_chunks >= 1
                batch_size = \
                    max(1, int(math.floor(float(len(items)) / nof_chunks)))
    ...
    def _fixup_keywords(self, item):
        ...
        for key, value in keywords.items():
            if isinstance(value, (Number, six.string_types)):
                ...
                raise TypeError('Cannot validate the keyword {0}. Type {1} '
    ...
    CONFIG_KEYS = [
        ...
        'source_name', 'source_ext_id', 'batch_size',
