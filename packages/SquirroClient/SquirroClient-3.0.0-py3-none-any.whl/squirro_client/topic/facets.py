...
class FacetsMixin(object):
    ...
    def new_facet(self, project_id, name, data_type=None, display_name=None,
        ...
        :param typeahead: Boolean Flag for toggling the typeahead.
            If 'True', and the `analyzed` propety of this facet has been set
            to `True` while creating the facet, the values as well as the name
            for this facet will be typeaheadable.  Otherwise, if the `analyzed`
    ...
    def modify_facet(self, project_id, id, display_name=None, group_name=None,
        ...
        data = dict([(k, v) for k, v in data.items() if (v is not None) or
