    ...
    def get_trenddetection_result(self, project_id, trenddetection_id,
        ...
        if include_historical_data:
            ...
            if len(ret['thresholds']) < 2:
                ...
                interval = (datetime.strptime(ret['thresholds'][1].
                                              get('timestamp'), time_format) -
                            datetime.strptime(
                                ret['thresholds'][0].get('timestamp'),
                                time_format)).total_seconds()

                historical_data_first_bucket = ret['thresholds'][0].get(
                    'timestamp')
                historical_data_last_bucket = (
                    datetime.strptime(ret['thresholds'][-1].get('timestamp'),
                                      time_format) +
                ...
                tds_aggs = aggs.get(
                    'trend_detection_values', {}).get(time_field)
