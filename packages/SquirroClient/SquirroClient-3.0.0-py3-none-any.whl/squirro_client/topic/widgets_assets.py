...
class WidgetsAndAssetsMixin(object):
    ...
    def get_asset(self, asset_type, name, global_asset=False,
                  include_files=False):
    ...
    def new_asset(self, asset_type, folder, global_asset=False):
        ...
        res = self._perform_request(
            'post', url, data={'config': json.dumps(config),
                               'data': tar_gz_base64})
