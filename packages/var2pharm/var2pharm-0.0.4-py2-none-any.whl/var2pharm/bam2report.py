...
def bam2report(conf: str) -> None:
    ...
    This is what a typical configuration file for ``bam2report`` looks like:
    ...
    This table summarizes the configuration parameters specific to ``bam2report``:
    ...
    for select_gene in select_genes:
        ...
        if control_gene != "NONE":
            s += (
                f"var2pharm bam2gdf {genome_build} $tg {control_gene} $bam \\\n"
    ...
    s = (
        ...
        "done\n"
        "\n"
        "var2pharm report $p/merged.sg-genotype.txt -o $p/report.html"
    )
    ...
    for select_gene in select_genes:
        s += f"{q} $p/gene/{select_gene}/shell/run.sh\n"

    s += (
        f"{q} $p/report.sh"
    )

    with open(f"{project_path}/bam2report-run.sh", "w") as f:
        f.write(s)
