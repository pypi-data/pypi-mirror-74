...
class OpenIDClient(object):
    ...
    async def _call_token_endpoint(self, body, headers, timeout):
        authserver_config = await self.authserver_config
        try:
    ...
    async def _verify_signature(self, id_token):
        authserver_config = await self.authserver_config
        return verify_signature(id_token, authserver_config.jwks)

    def get_jwk_set(self):
        return build_jwk_set(self.public_keys['keys'])
