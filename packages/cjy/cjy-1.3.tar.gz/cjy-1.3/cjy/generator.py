...
class TextGenerator:
    ...
    def download_resource(self):
        ...
        if os.path.exists('./font/mingliub.ttc') and \
            os.path.exists('./font/simsun.ttc') and \
                os.path.exists('./ids.txt') and \
                os.path.exists('./radicals.txt') :
        ...
        request.urlretrieve('https://fudan-ocr.oss-cn-shanghai.aliyuncs.com/resource/ids.txt', './ids.txt')
        request.urlretrieve('https://fudan-ocr.oss-cn-shanghai.aliyuncs.com/resource/radicals.txt', './radicals.txt')
        request.urlretrieve('https://fudan-ocr.oss-cn-shanghai.aliyuncs.com/resource/font/mingliub.ttc', './font/mingliub.ttc')
        request.urlretrieve('https://fudan-ocr.oss-cn-shanghai.aliyuncs.com/resource/font/simsun.ttc', './font/simsun.ttc')

    def generate(self,root='./image',num=-1,color=(255,0,0),max_length=8,compress=True):
...
class RadicalMaker:
    ...
    def get_word_image(self, word):
        ...
        def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=100, font="./font/simsun.ttc"):
        ...
        if np.sum(word_img) == 0:
            word_img = cv2ImgAddText(canvas, word, 100, 100, self.color, 50, "./font/mingliub.ttc")
    ...
    def image_add(self, img_base, img_new, left, top, right, bottom):
        ...
        # 随机旋转
        rotate_angle = random.randint(-2,2)
