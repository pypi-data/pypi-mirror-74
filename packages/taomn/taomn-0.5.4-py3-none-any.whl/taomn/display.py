...
help_url = 'https://docs.tao.com/masternode/taomn/'
...
def warning_ignoring_start_options(name: str) -> None:
    "Custom warning when taomn is ignoring the start options"
    warning(
...
def warning_remove_masternode(name: str) -> None:
    "Custom warning when taomn is removing masternode but no confirmation"
    warning(
...
def error_start_not_initialized() -> None:
    "Custom error when `taomn start` has never been used with `--name` option"
    error(
        'taomn doesn\'t manage any masternode yet\n'
        '         please use '
        '<hy>taomn start --name</hy> to get started'
    )
...
def error_breaking_change() -> None:
    ...
    error(
        ...
        '         '
        '<hy>taomn remove --confirm</hy>\n'
        '         '
        'and creating it back with the same options as the old one\n'
        '         '
        '<hy>taomn start --name ... --net ... --pkey ...</hy>'
    )
