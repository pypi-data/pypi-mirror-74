...
from taomn import __version__
from taomn.configuration import Configuration

logger = logging.getLogger('taomn')
docker_url = None


@click.group(help=('Tao MasterNode (taomn) is a cli tool to help you run a Tao'
                   'chain masternode'))
...
@click.command(help='Display Tao documentation link')
def docs() -> None:
    "Link to the documentation"
    display.link_docs()


@click.command(help='Start your Tao masternode')
...
@click.command(help='Stop your Tao masternode')
def stop() -> None:
...
@click.command(help='Show the status of your Tao masternode')
def status() -> None:
...
@click.command(help='Show details about your Tao masternode')
def inspect() -> None:
    "Show details about the tao masternode"
    configuration = Configuration(docker_url=docker_url)
    if configuration.force_recreate:
        display.error_breaking_change()
        sys.exit('\n')
    display.title_inspect_masternode(configuration.name)
    identity = configuration.services['tao'].execute(
        'echo $IDENTITY'
...
def update() -> None:
    "Update the tao masternode with the lastest images"
    configuration = Configuration(docker_url=docker_url)
    if configuration.force_recreate:
        display.error_breaking_change()
        sys.exit('\n')
...
@click.command(help='Remove your Tao masternode')
