...
import os
import textwrap
from mge_masker.mge_masker_functions import find_mges, create_mge_gff_file, mask_mges, print_default_regex_patterns
...
def parse_args():
    ...
    mask_mges.add_argument('-a', '--pseudogenome_alignment_path', help='path to a pseudogenome alignment file', type=lambda x: is_valid_file(parser, x), required=True)
...
def main():
    ...
    if (options.command == 'find_mges'):
        ...
        masked_alignment_file_path = mask_mges(options.pseudogenome_alignment_path, options.gff_file_path, options.masking_character)
