...
class UserCoupons(betterproto.Message):
    ...
    coupon_info: "UserCouponState" = betterproto.message_field(3)


@dataclass
class UserCouponState(betterproto.Message):
    # 新送阅读券数量
    ...
    # 剩余限免卡数量
    remain_item: int = betterproto.int32_field(7)
    # 剩余打折卡数量
    remain_discount: int = betterproto.int32_field(8)
    # 剩余优惠券数
    remain_coupon: int = betterproto.int32_field(9)
...
class InitInfo(betterproto.Message):
    ...
    # 从 2.12 版本开始漫读券相关信息从这里取 用户漫读券信息
    coupon: "UserCouponInfo" = betterproto.message_field(17)
    # 1: 专 > 福 > 全 2: 优先到期
...
@dataclass
class UserCouponInfo(betterproto.Message):
    # 新送阅读券数量
    new_coupon_num: int = betterproto.int32_field(1)
    # 是否存在过期漫读券，三天即将过期 0-不存在 1-存在
    coupon_will_expire: int = betterproto.int32_field(2)
...
class UserStub(betterproto.ServiceStub):
    ...
    @allure.step("""获取杂项配置信息""")
    def get_init_info(self, request: EmptyRequest) -> GetInitInfoResp:
        """获取杂项配置信息"""
        logging.getLogger(__name__).info(
            "start call " + "[get_init_info]" + " api" + "=>" + """获取杂项配置信息"""
        )
    ...
    def get_wallet(self, request: EmptyRequest) -> UserWalletResp:
        """用户剩余钱包"""
        logging.getLogger(__name__).info(
            "start call " + "[get_wallet]" + " api" + "=>" + """用户剩余钱包"""
        )
    ...
    def open_teenager_mode(self, request: TeenagerModeSignReq) -> DefaultResponse:
        """打开青少年模式"""
        logging.getLogger(__name__).info(
            "start call " + "[open_teenager_mode]" + " api" + "=>" + """打开青少年模式"""
        )
        response = DefaultResponse()
    ...
    @allure.step("""关闭青少年模式 code: 1 签名不对""")
    ...
    def get_pay_orders(self, request: UserPayOrdersReq) -> UserPayOrdersResp:
        """用户充值记录"""
        logging.getLogger(__name__).info(
            "start call " + "[get_pay_orders]" + " api" + "=>" + """用户充值记录"""
        )
    ...
    def get_buy_orders(self, request: UserBuyOrdersReq) -> UserBuyOrdersResp:
        """用户购买记录"""
        logging.getLogger(__name__).info(
            "start call " + "[get_buy_orders]" + " api" + "=>" + """用户购买记录"""
        )
    ...
    @allure.step("""查询订单购买所有章节信息""")
    def get_order_detail(self, request: OrderDetailReq) -> OrderDetailResp:
        """查询订单购买所有章节信息"""
        logging.getLogger(__name__).info(
            "start call " + "[get_order_detail]" + " api" + "=>" + """查询订单购买所有章节信息"""
        )
    ...
    @allure.step("""用户购买过的漫画(原用户自动购买记录)""")
    ...
    def take_gift_package(self, request: TakeGiftPackageReq) -> DefaultResponse:
        """拿相关礼包"""
        logging.getLogger(__name__).info(
            "start call " + "[take_gift_package]" + " api" + "=>" + """拿相关礼包"""
        )
        response = DefaultResponse()
    ...
    @allure.step("""用户登录接口 客户端在主站登录成功后立即调用本接口 如果本接口返回失败，则认为登录失败""")
    def login(self, request: EmptyRequest) -> LoginResp:
        """用户登录接口 客户端在主站登录成功后立即调用本接口 如果本接口返回失败，则认为登录失败"""
        logging.getLogger(__name__).info(
            "start call "
            + "[login]"
            + " api"
    ...
    @allure.step("""ios 获取评价弹窗信息""")
    def get_i_o_s_review(self, request: EmptyRequest) -> IOSReviewResp:
        """ios 获取评价弹窗信息"""
        logging.getLogger(__name__).info(
            "start call " + "[get_i_o_s_review]" + " api" + "=>" + """ios 获取评价弹窗信息"""
        )
    ...
    @allure.step("""关注官方账号发放漫读券""")
    def follow_offcial(self, request: EmptyRequest) -> FollowOffcialResp:
        """关注官方账号发放漫读券"""
        logging.getLogger(__name__).info(
            "start call " + "[follow_offcial]" + " api" + "=>" + """关注官方账号发放漫读券"""
        )
    ...
    @allure.step("""获取用户新人下载福利信息""")
    def get_newbie_info(self, request: EmptyRequest) -> NewbieInfoResp:
        """获取用户新人下载福利信息"""
        logging.getLogger(__name__).info(
            "start call " + "[get_newbie_info]" + " api" + "=>" + """获取用户新人下载福利信息"""
        )
    ...
    @allure.step("""获取 PC 新人大礼包""")
    def get_p_c_newbie_gift(self, request: EmptyRequest) -> PCNewbieGiftResp:
        """获取 PC 新人大礼包"""
        logging.getLogger(__name__).info(
            "start call " + "[get_p_c_newbie_gift]" + " api" + "=>" + """获取 PC 新人大礼包"""
        )
    ...
    def update_nick_name(self, request: UpdateNickNameReq) -> DefaultResponse:
        """编辑用户昵称"""
        logging.getLogger(__name__).info(
            "start call " + "[update_nick_name]" + " api" + "=>" + """编辑用户昵称"""
        )
        response = DefaultResponse()
    ...
    @allure.step("""领取 vip 福利等""")
    def get_vip_reward(self, request: VipRewardReq) -> VipRewardResp:
        """领取 vip 福利等"""
        logging.getLogger(__name__).info(
            "start call " + "[get_vip_reward]" + " api" + "=>" + """领取 vip 福利等"""
        )
