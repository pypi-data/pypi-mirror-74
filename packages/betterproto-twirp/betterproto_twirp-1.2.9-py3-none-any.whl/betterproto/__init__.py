...
class ServiceStub(ABC):
    ...
    def _do_twirp_rpc(self, api, request: Message = Message, response: Message = Message):
        ...
        logger.info(f'[请求headers]:{r.request.headers}')
        
        logger.info(f'[响应结果]:{r.json()}')
        logger.info(f'[响应http_code]:{r.status_code}')
        logger.info(f'[响应耗时]:{r.elapsed.total_seconds()}秒')
        logger.info(f'[X-Trace-Id]:{r.request.headers.get("X-Trace-Id")}')
