(function( ducts ) {
    ...
    ducts.query = '?uuid='+([1e7]+-1e3+-4e3+-8e3+-1e11).replace(/[018]/g, c => (c ^ crypto.getRandomValues(new Uint8Array(1))[0] & 15 >> c / 4).toString(16));
    //ducts.query = '?ts='+Date.now();
    ducts.query += ducts.token ? '&token='+ducts.token : '';
    ducts.query += ducts.user ? '&user='+ducts.user : '';
    ducts._local = {};
    ducts._local.libs = [
	'https://unpkg.com/what-the-pack/dist/MessagePack.min.js'
	,'https://www.WebRTC-Experiment.com/RecordRTC.js'
	, ...ducts.libs_plugin
	, ducts.context_url + '/main' + ducts.query
    ];
	...
	if (ducts._local.libs_index < ducts._local.libs.length) {
	    ...
	    lib_script.onload = function() { ducts._local.main(ducts.wsd); };
	}
    }
    ducts._local.append_next_lib();

    ducts._local.main = function(wsd) {
	ducts.msgpack = MessagePack.initialize(2**22);
	ducts.main(wsd);
    };

    ducts.main = ducts.main || function(wsd) {alert('ducts.main is not set.');};
...
window.ducts.Duct = class {
    ...
    constructor(wsd=null) {
	this.WSD = wsd || window.ducts.wsd;
	this.EVENT = this.WSD.EVENT;
	this.encode = ducts.msgpack.encode;
	this.decode = ducts.msgpack.decode;
	this.next_rid =
	...
	this.connection_listener = new window.ducts.ConnectionEventListener();
	...
	this.catchall_event_handler = (rid, eid, data) => {console.log('on_message eid='+eid)};
	this.uncaught_event_handler = (rid, eid, data) => {console.log('uncaught_message eid='+eid)};
	...
	return new Promise(function(resolve, reject) {
		...
		(event) => {
		    ws.onerror =
			(event) => {self.connection_listener.onerror(event);};
		    ws.onclose =
			(event) => {self.connection_listener.onclose(event);};
		    ws.onmessage = 
			(event) => {self._onmessage(self, event);};
		    self._ws = ws;
		    self._onopen(self, event);
		    self.connection_listener.onopen(event);
		    resolve(self);
		};
	    ws.onerror =
		(event) => {
		    self.connection_listener.onerror(event);
		    reject(event);
		};
	});
    }
	...
	try {
	    self.connection_listener.onmessage(event);
