...
class Instrument(object):
    ...
    def citics_industry(self, date=None):
        if self.has_citics_info():
            ...
            result = get_instrument_industry(self.order_book_id, date=date, level=1, source='citics')
            if result is None:
...
@export_as_api
def get_private_placement(order_book_ids, start_date=None, end_date=None, progress="complete", market="cn"):
    """获取定增数据
    ...
    data = get_client().execute(
        "get_private_placement", order_book_ids, start_date, end_date, progress, market=market
    )
    if not data:
        return
    progress_map = {
