...
def s3downloader(write_pipe, read_pipe, endpoint, per_obj_fnx):
    # info('s3downloader: started. endpoint=' + endpoint)
    if (per_obj_fnx != None):
        transformAst = infin_ast.extract_transform(TRANSFORM_ONE_OBJ, src_str=per_obj_fnx)
    ...
    while (True):
        ...
        if (op == 'download'):
            ...
            if (per_obj_fnx == None):
                ...
                glb['bucket'] = bucketname;
                glb['key'] = filename;
                glb['object_bytes'] = bts;
                try:
...
def download_objects(endpoint, bucketname, array_of_files, is_csv, per_obj_fnx):
    start_run()
    ...
    for i in range(num_threads()):
        ...
        p = Process(target=s3downloader, args=(w1, r2, endpoint, per_obj_fnx,))
        p.start()
        processes.append(p)
...
def actually_run_transformation(client, endpoint, cache_path, is_pandas_df, bucketname,\
    ...
    objects = download_objects(endpoint, bucketname, array_of_files, is_csv, None)
...
def read_raw_and_xform_one_object(time_spec, service_name, bucketname,\
    ...
    list_one_dir(client, bucketname, prefix_in, True, array_of_files)
    print('Total Number Of Objects: ' + str(len(array_of_files)))
    objects = download_objects(endpoint, bucketname, array_of_files, False, s)
    return objects
