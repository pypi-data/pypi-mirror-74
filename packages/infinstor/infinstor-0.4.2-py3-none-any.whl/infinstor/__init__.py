    ...
    while (True):
        ...
        if (op == 'download'):
            ...
            if (glb == None):
                ...
                # info("Executing infin_transform_one_object for " + filename)
                try:
...
def read_raw_and_xform_one_object(time_spec, service_name, bucketname,\
    ...
    # print('Globals=')
    # pprint.pprint(glb)
    info("Execution of global static complete. status_str=" + status_str)

    array_of_files = []
    list_one_dir(client, bucketname, prefix_in, True, array_of_files)
    info('Total Number Of Objects: ' + str(len(array_of_files)))
    try:
        start_run()
        objects = download_objects(endpoint, bucketname, array_of_files, False, glb)
    finally:
        end_run()
    return objects
