...
def _embed_cli(cfg, is_cli_call=True):
    ...
    Args:
        ...
        cfg[from_folder]: (str) If specified, the dataset is loaded \
            from the folder
    ...
    if not checkpoint:
        checkpoint = _model_zoo(cfg['model']['name'])
        if not checkpoint:
            msg = 'Cannot download checkpoint for {} '.format(cfg['model']['name'])
    ...
    df = embeddings_to_pandas(embeddings, labels, filenames)
