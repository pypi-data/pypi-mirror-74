...
class Extract:
    ...
    ##########
    ##########
    def save(self,savefolder=None,savename_prefix=None,overwrite_fits=False):
        if savefolder is None:
            savefolder = self.data['savefolder']
        if savename_prefix is None:
            savename_prefix = self.data['objname']
        # save self.data
        tmp = json.dumps(self.data)
        string = './{0}/{1}_info.json'.format(savefolder,savename_prefix)
        f = open(string,'w')
        f.write(tmp)
        f.close()
        ...
        string = './{0}/{1}_extract.fits'.format(savefolder,savename_prefix)
        hdul.writeto(string,overwrite=overwrite_fits)
        print('Save {0}'.format(string))
        # save trace as csv
        ...
        for i in {'DYDX','DLDP'}:
            ...
            for j,jj in enumerate(tmp[i]):
                tmpp[j] = tmp[i][j]
            tmp[i] = tmpp.copy()
        string = './{0}/{1}_trace.csv'.format(savefolder,savename_prefix)
        ...
        for i in {'BBX','BBY'}:
            ...
            for j,jj in enumerate(tmp[i]):
                tmpp[j] = tmp[i][j]
            tmp[i] = tmpp.copy()
        string = './{0}/{1}_bbox.csv'.format(savefolder,savename_prefix)
        tmp = pd.DataFrame(tmp)
        tmp.to_csv(string)
        print('Save {0}'.format(string))
        # save 1d as csv
        ...
        for i in {'instrument'}:
            ...
            tmpp[0] = self.extract[i]
            tmp[i] = tmpp.copy()
        string = './{0}/{1}_1d.csv'.format(savefolder,savename_prefix)
        tmp = pd.DataFrame(tmp)
        tmp.to_csv(string)
        print('Save {0}'.format(string))        
    ##########
    ##########
    ##########
    def examine(self,central_ww,width_ww,
                zshift=0.,
                params_savgol={'do_savgol':True,
                               'window_length':5,
                               'polyorder':0
                              },
                plot_title=None,fontsize=12,
                save=True,savename_prefix=None,saveformat='pdf'
               ):
        ...
        if save:
            savefolder = self.data['savefolder']
            string = './{0}/{1}_examine.{2}'.format(savefolder,savename_prefix,saveformat) if savename_prefix is not None else './{0}/plot_examine.{1}'.format(savefolder,saveformat)
    ...
    def show(self,save=False,savename_prefix=None,saveformat='pdf',
        ...
        fig.tight_layout()
        if save:
            string = './{2}/{0}_spc1d.{1}'.format(self.data['objname'],saveformat,savefolder) if savename_prefix is None else './{2}/{0}_spc1d.{1}'.format(savename_prefix,saveformat,savefolder)
