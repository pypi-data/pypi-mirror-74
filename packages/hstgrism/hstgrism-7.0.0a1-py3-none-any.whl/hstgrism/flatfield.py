...
class FlatField:
    ...
    '''
    def __init__(self,flatfile,tracefile):
    ...
    def _get(self):
        ...
        self.data['NAX1'] = nax1
        self.data['NAX2'] = nax2
        self.data['ROOTNAME'] = self.tracefile.split('/')[-1].split('_')[0]
        # flat
        n = len(tmp)
        tmpp = {}
        for i in range(n):
    ...
    ##########
    ##########
    def save(self,savename_prefix=None,overwrite=True):
        hdu = fits.PrimaryHDU()
        ...
        hdul[0].header['ROOTNAME'] = self.data['ROOTNAME']
        hdul[0].data = self.flatfield.copy()
        if savename_prefix is None:
            savename_prefix = self.data['ROOTNAME']
        string = '{0}_flat.fits'.format(savename_prefix)
        hdul.writeto(string,overwrite=overwrite)
        print('Save {0}'.format(string))
    ##########
    ##########
    ##########
    def show(self,
             save=False,savename_prefix=None,saveformat='pdf',
             params={'figsize':(5,5),
                     'minmax':(5.,99.),
                     'cmap':'viridis',
                     'fontsize':12
        ...
        if save:
            if savename_prefix is None:
                savename_prefix = self.data['ROOTNAME']
            string = '{0}_flat.{1}'.format(savename_prefix,saveformat)
