...
class Background:
    def __init__(self,container,objname='None',gfile=None,tfile='None',
                 automask=True,
    ...
    def save(self):
        ...
        string = './{1}/{0}_bbox.json'.format(saveprefix,savefolder)
        f = open(string,'w')
        f.write(tmppp)
        f.close()
        print('Save {0}'.format(string))
    ...
    def show_bkg3d(self,save=False,
        ...
        if save:
            ...
            plt.savefig(string,format=saveformat,bbox_inches='tight')
            print('Save {0}'.format(string))
    def show_bbox2d(self,save=False,
                    params={'figsize':(10,10),
                            'minmax':(10.,80.),
                            'xpertick':10,
                            'fontsize':12,
        ...
        plt.tight_layout()
        if save:
            string = './{2}/{0}_bbox2d.{1}'.format(saveprefix,saveformat,savefolder)
            plt.savefig(string,format=saveformat,bbox_inches='tight')
            print('Save {0}'.format(string))
