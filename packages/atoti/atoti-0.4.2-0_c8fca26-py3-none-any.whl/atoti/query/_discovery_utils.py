"""Utilities to build query objects from discovery."""
...
def _create_hierarchy(
    ...
    levels = ImmutableMapping(
        {
            level.name: QueryLevel(level.name, dimension_name, discovery_hierarchy.name)
            for level in discovery_hierarchy.levels
            if level.type != "ALL"
        }
    )
    return QueryHierarchy(
        discovery_hierarchy.name, dimension_name, levels, discovery_hierarchy.slicing
    )
...
def create_cubes_from_discovery(
    ...
    """Create a mapping of all the cubes in the discovery.
