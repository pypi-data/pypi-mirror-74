...
def _has_eula_changed(copied_eula: str) -> bool:
    ...
    previous = re.sub(r"(\d+\.\d+\.\d+(\.\d+)?)", "", copied_eula)
    new = re.sub(r"(\d+\.\d+\.\d+(\.\d+)?)", "", EULA or "")
    return previous != new


def _log_about_eula(edition: Edition) -> List[Output]:
    outputs = []
...
def _monitor_license_expiry(edition: Edition, end_date: datetime) -> List[Output]:
    outputs = []
    now = datetime.now()
    ...
    if end_date < now:
        outputs.append(
            Output(
                f"To keep using {product_name}, {required_action}.",
        ...
        if remaing_days <= 7:
            outputs.append(
                Output(
                    ...
                    To keep using it, {required_action} in the coming {remaing_days} days.
                    """,
                    OutputType.WARNING,
                )
            )
...
def get_license_outputs(edition: Edition, end_date: datetime) -> List[Output]:
    """Generate list of outputs about the EULA and monitor license expiry."""
    outputs = _log_about_eula(edition) + _monitor_license_expiry(edition, end_date)
    return [
        Output(dedent(output.content).strip(), output.output_type) for output in outputs
    ]


def check_license(edition: Edition, end_date: datetime):
    """Log about the EULA and monitor license expiry."""
    for output in get_license_outputs(edition, end_date):
