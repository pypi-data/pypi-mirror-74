...
class SPI:
  ...
  def __init__(self, board=None, device_num=0, sck=None, mosi=None, miso=None, cs=None, baudrate=100000, polarity=0, phase=0, bits=8):
    ...
    self.board = board
    if mosi:  #SoftSPI
      self.obj = SoftSPI(board, sck, mosi, miso, cs, baudrate, polarity, phase, bits)
    elif self.board.boardname == "":  #RPiSPI
      self.obj = RPiSPI(device_num)
    else:  #DuinoSPI
      self.obj = DuinoSPI(self, device_num)
...
class IIC:
  def __init__(self, board=None, bus_num=0):
    self.bus_num = bus_num
    self.board = board
    if self.board.boardname == "":
      self.obj = RPiIIC(board, bus_num)
    else:
      self.obj = DuinoIIC(board, bus_num)
