...
class CorrectGrammar():
    ...
    def correct_grammar_lex(self, errors, latex, latex_list, index=0, previous_errors=[]):
        ...
        Args:
            errors (list): Errors found in previous step: either check_grammar_lex.py or check_grammar_sintax.py
            latex (list): First latex structure.
            latex_list (list): [description]
            index (int, optional): Index of error. Default to 0.
            previous_errors (list, optional): This list stores all previous symbol (index) attemptions to fix the error. Defaults to [].
        ...
        helpers.debug("\n[base_grammar.py] correct_grammar_lex() | List of errors: {0}".format(errors))

        previous_attemptions = []

        # Adiciona tentativas anteriores no array
        ...
        if len(errors) > 0:
            ...
            if not isinstance(pred, list): # When there's predictions it is a numpy array, not a list.
                ...
                def get_new_index(pred):
                    ...
                    helpers.debug("[base_grammar.py] correct_grammar_lex() | Gets new index and prediction from next index with higher prediction")
                ...
                def recur_get_new_index(pred):
                    ...
                    if new_identification in errors[index]['attempts'] or new_identification in previous_attemptions:
                ...
                # Updated 'latex' and 'latex_list'. It is NOT a copy so, it is already updated.")
