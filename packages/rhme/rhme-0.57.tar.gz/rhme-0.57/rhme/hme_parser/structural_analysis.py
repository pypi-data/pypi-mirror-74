...
class StructuralAnalysis():
    ...
    def __preprocessing(self, symbols):
        ...
        for i in range(0,len(symbols)):
            ...
            # centroide estava muito abaixo ou muito acima nesses casos. 
            # deixei como: 1/3, 2/3
            '''
            [0-9] b
            ...
            #[0-9], b <<< até validação | (), {}, [], sqrt
            if re.search("^[0-9]$", str(s['label'])) or s['label'] == '19' or \
                s['type'] == 'Open'  or s['type'] == 'Close' or \
                s['label'] == '23':

                s['centroid_class'] = 'Ascending'
                s['centroid'][1] = s['ymin'] + (2/3) * (s['h']) # 3/5

            elif s['label'] == '25': # y sqrt | na validação -> y sqrt ( { [
    ...
    def __main_parsing(self, symbols):
        ...
        helpers.debug('\n[parser.py] __main_parsing() | STARTING symbol index: %d ' % sstart)
        ...
        while not Q.is_empty():
            ...
            while not Q.is_empty():
                ...
                print('\n[parser.py] __main_parsing() | find baseline of symbol: ', temp1, listin[temp1]['label'])

                helpers.debug('\n[parser.py] __main_parsing() | temp2 hor...')
                temp2 = self.__hor(listin, temp1) # b, -
                print('\n[parser.py] __main_parsing() | temp2: ', temp2, listin[temp2]['label'])

                while temp2 != -1:
                    print('[parser.py] __main_parsing() | ... while temp2')
                    listin[temp2]['checked'] = True
                    print('[parser.py] __main_parsing() |  ...wall attributes of temp1: ', listin[temp1]['wall'])
                    ...
                    '''
                    print('[parser.py] __main_parsing() | ...wall attributes of temp2: ', listin[temp2]['wall'])
                    ...
                    b.wall.right = c.xmin
                    '''
                    print('[parser.py] __main_parsing() | ...updated wall attributes of temp1: ', listin[temp1]['wall'])
                    temp1 = temp2 # b
                    temp2 = self.__hor(listin, temp1) # c - 1
                    print('[parser.py] __main_parsing() | new temp2: ', temp2)
            S.push("EOBL")
            ...
            while not S.is_empty():
                ...
                helpers.debug('[parser.py] __main_parsing() | symbol: %s ' % temp1)

                label = int(listin[temp1]['label'])
                helpers.debug('[parser.py] __main_parsing() | temp1 label: %s' % label)

                upperThreshold = listin[temp1]['ymin'] + ((1/6.5) * listin[temp1]['h']) # 1/6
                lowerThreshold = listin[temp1]['ymin'] + ((5.5/6.5) * listin[temp1]['h']) # 5/6
                ''' mudança no xmin e xmax por causa da imagem 412.png em que a "perna" do "a" ultrapassa a fração'''
                leftThreshold = ( listin[temp1]['xmin'] + ((1/6) * listin[temp1]['w']) ) if label != 10 else listin[temp1]['xmin']
                rightThreshold = ( listin[temp1]['xmax'] - ((1/6) * listin[temp1]['w']) ) if label != 10 else listin[temp1]['xmax']
                ...
                R = [
                    ...
                    {'super': [
                        [rightThreshold, listin[temp1]['wall']['top']], # left, top
                        [listin[temp1]['wall']['right'], upperThreshold] # right, bottom
                    ]},
                    {'subsc': [
                        [rightThreshold, lowerThreshold], # left, top
                ...
                for region in R:
                    ...
                    if (region_name == 'super' and not operators) or \
                        ...
                        (region_name == 'contains' and int(listin[temp1]['label']) == 23):
    ...
    def __overlap(self, symbolIndex, top, bottom, listin):
        ...
        while listIndex > 0 and stop == False:
            print('[parser.py] __overlap() | xmin, xmin', listin[listIndex-1]['xmin'], listin[symbolIndex]['xmin'])
        ...
        helpers.debug('[parser.py] __overlap() | listIndex: %d ' % listIndex)
        helpers.debug('[parser.py] __overlap() | n: %d ' % n)
        helpers.debug('[parser.py] __overlap() | top: %d ' % top)
        helpers.debug('[parser.py] __overlap() | bottom: %d ' % bottom)
        ...
        while listIndex < n and listin[listIndex]['xmin'] < listin[symbolIndex]['xmax']:
            ...
            print('\n[parser.py] __overlap() | ... listIndex: ', listIndex)
            print('[parser.py] __overlap() | ... label: ', listin[listIndex]['label'])
            print('[parser.py] __overlap() | ... centroid: ', listin[listIndex]['centroid'])
            print('[parser.py] __overlap() | ... xmin: ', listin[listIndex]['xmin'])
            print('[parser.py] __overlap() | ... xmax: ', listin[listIndex]['xmax'])
            print('[parser.py] __overlap() | ... max length: ', (listin[listIndex]['xmax'] - listin[listIndex]['xmin']))
            print('[parser.py] __overlap() | ... len(x_intersection): ', len(x_intersection))
            print('[parser.py] __overlap() | ... min_line/2: ', min_line/2)
    ...
    def __start(self, listin, R):
        ...
        helpers.debug('[parser.py] __start() | region [R]: left: %d, right: %d, top: %d, bottom: %d' %
        ...
        while leftmostIndex == -1 and listIndex < n:
            helpers.debug('[parser.py] __start() | ... symbol index: %d' % listIndex)
            helpers.debug('[parser.py] __start() | ... symbol label: %s' % listin[listIndex]['label'])
            print('[parser.py]__start() | ... symbol centroid: ', listin[listIndex]['centroid'])
    ...
    def __hor(self, listin, index):
        ...
        top = listin[index]['ymin'] + (listin[index]['h'] * (1/6.5)) # 1/6
        bottom = listin[index]['ymin'] + (listin[index]['h'] * (5.5/6.5)) # 5/6
        ...
        if label in range(10,17):
            ...
            for s in range(0, len(listin)):
                ...
                if not checked:
                    ...
                    helpers.debug('[parser.py] __hor() | ... symbol coordinates: xmin: %s xmax: %s ymin: %s ymax: %s' % (symbol['xmin'], symbol['xmax'], symbol['ymin'], symbol['ymax']))
    ...
    def __token_substitution(self, latex, grammar, subst):
        ...
        def __change_label(i, label, substitution_list):
            for substitution_index in range(0, len(substitution_list)):
                ...
                i, nomatch = __list_substitution(i, nomatch, aux, initial_index, label, substitution_list, substitution_index)
