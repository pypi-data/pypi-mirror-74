...
__version__ = get_distribution("pyplatform-datawarehouse").version
...
def show_me(module=None, docstring=False):
    ...
    Keyword Arguments:
        module {str,os.path,types.ModuleType} -- module, submodule or scriptpath (default=pyplatform)
    ...
    functions = re.compile('^def.*|^###.*')
    all_content = re.compile('^def.*|^###.*|^s*"""|.*"""')
