...
class Spliter:
    ...
    def _do_cut(self, char, index, paragraph, total_len):
        ...
        if SYMBOLS['quotation_right'].match(char):  # "xx"，情况不切句 其他""情况切句
            self.count_quote = 0
            if index + 1 >= total_len or SYMBOLS['end_symbols'].match(paragraph[index + 1]):  # 双引号后面不是逗号,就切分
                self.add_to_sntnc()
                return
        elif SYMBOLS['quotation_en'].match(char):
            self.count_quote += 1
            if self.count_quote % 2 == 0:
                self.count_quote = 0
                if index + 1 >= total_len or SYMBOLS['end_symbols'].match(paragraph[index + 1]):  # 双引号后面不是切分符号不切分
                    self.add_to_sntnc()
                    return
        elif SYMBOLS['quotation_left'].match(char):
            self.count_quote += 1
        # -- 假定所有引号和括号都完美合上了 -- #
...
“我费了好大气力……”'''
    print('\n'.join(spliter.cut_to_sentences(text)))