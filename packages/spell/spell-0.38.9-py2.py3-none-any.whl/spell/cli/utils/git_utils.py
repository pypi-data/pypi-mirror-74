...
def detect_repo(
    ...
    if git_repo is None:
        if not allow_missing:
            raise ExitException(
                "A git repository needs to be specified to run this command.", SPELL_BAD_REPO_STATE,
            )
        if force:
...
def clean_hardlinked_repo(ctx, repo):
    git_dst = os.path.join(ctx.obj["config_handler"].spell_dir, "repos", repo.git_dir[1:],)
    ...
    if has_untracked(git_repo):
        if not force:
            click.confirm(
                ...
                "\n{}\nContinue the run anyway?".format(
                    git_repo.working_dir, resource_type, get_untracked(git_repo)
                ),
                default=True,
                abort=True,
            )
...
def get_tracked_repo_path(repo, path):
    ...
    # Get the path relative to repo root and split into intermediate dirs so we
    # can navigate dir-by-dir through the repo file tree
    relpath = os.path.relpath(realpath, repo_root)
    path_pieces = []
    curr_path = relpath
    while curr_path != "":
    ...
    for piece in path_pieces:
        ...
        for candidate in tree.blobs:
            if candidate.name == piece:
                return relpath if os.path.isfile(realpath) else None
        if not was_found:
            return None
    return relpath
