...
class CmdFileParser:
    ...
    def _parse_option(self, key, value, param):
        ...
        elif (param.nargs != 1 or param.multiple) and isinstance(value, str):
            return shlex.split(value)  # Use the shlex library to mimic shell parsing
        else:
            return value
...
class CmdFileMixin:
    ...
    def make_parser(self, ctx):
        ...
        parser = CmdFileOptionParser(self.param_lookup, ctx=ctx,)
...
class GroupWithCmdFile(CmdFileMixin, click.Group):
    def __init__(self, *args, use_cmd_file_params=True, **kwargs):
