...
from spell.cli.utils import command
...
@command(
    name="add", short_help="Creates a new machine type for executing Spell Runs and Workspaces",
)
@click.pass_context
...
def scale_machine_type(ctx, name, min_machines, max_machines, idle_timeout, default_auto_resume):
    ...
    if machine_type and not machine_type["is_spot"]:
...
@click.command(name="get-token", short_help="Delete a machine type")
@click.argument("name")
