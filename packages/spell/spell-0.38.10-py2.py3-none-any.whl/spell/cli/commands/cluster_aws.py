...
def create_aws(ctx, name, profile, vpc_id):
    ...
    with api_client_exception_handler():
        ...
        url = "https://web.spell.ml/{}/clusters/{}".format(ctx.obj["owner"], cluster["name"])
        click.echo(
...
def eks_init(
    ...
    load_balancer_visiblity = click.prompt(
    ...
    try:
        ...
        if len(asgs) == 0 or len(asgs) > 1:
            raise ExitException(
                "Failed to find AutoScalingGroup for cluster. Contact support@spell.ml for assistance"
            )
    ...
    try:
        cloud = "eks_public" if load_balancer_visiblity == "public" else "eks"
...
def create_eks_cluster(
    ...
    try:
        ...
        # TODO(libeks): Tear down cloudformation cluster
        # First wait for the cluster to be in state `ROLLBACK_COMPLETE`, then delete using
        # `aws cloudformation delete-stack --profile aws2 --region=<region> --stack-name=eksctl-<cluster_name>`
