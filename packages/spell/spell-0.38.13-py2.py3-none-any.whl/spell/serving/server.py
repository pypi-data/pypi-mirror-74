    ...
    if module_path:
        if not python_path:
            raise InvalidServerConfiguration(
                "Python path must be provided if module path is provided"
            )
        ...
        raise InvalidServerConfiguration("Entrypoint must be provided if module_path is not")
    else:  # TODO(Justin) Remove after entrypoint deletion
