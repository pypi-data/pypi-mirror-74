...
class TestUbuntuPlatformFeature(unittest.TestCase):
    ...
    def test_xenial(self):
        xenial = features.UbuntuPlatformFeature('xenial')
        # Inject known values
        xenial._force_release('Ubuntu', '16.04', 'xenial')
        self.assertTrue(xenial.available())


class TestAtLeastUbuntu(unittest.TestCase):

    def assertReleaseIsOlder(self, old, new):
        old_index = features.ubuntu_releases.index(old)
        new_index = features.ubuntu_releases.index(new)
        self.assertTrue(old_index < new_index)

    def test_ubuntu_releases(self):
        self.assertReleaseIsOlder('xenial', 'artful')
        self.assertReleaseIsOlder('xenial', 'bionic')
        self.assertReleaseIsOlder('artful', 'bionic')

    def test_bionic_on_xenial(self):
        at_least_bionic = features.AtLeastUbuntu('bionic')
        at_least_bionic._force_release('Ubuntu', '16.04', 'xenial')
        self.assertFalse(at_least_bionic.available())

    def test_xenial_on_bionic(self):
        at_least_xenial = features.AtLeastUbuntu('xenial')
        at_least_xenial._force_release('Ubuntu', '18.04', 'bionic')
        self.assertTrue(at_least_xenial.available())

    def test_unknown_release(self):
        at_least_unknown = features.AtLeastUbuntu('unknown')
        at_least_unknown._force_release('Ubuntu', '18.04', 'bionic')
        self.assertFalse(at_least_unknown.available())
...
class TestTestRequires(unittest.TestCase):
    ...
    def test_skip_not_available(self):
        ...
        class LocalTest(unittest.TestCase):
            ...
            def test_it(inner):
                features.test_requires(inner, feature)
                raise self.failureException('This should have been skipped')
    ...
    def test_errors_when_used_without_a_feature(self):
        ...
        LocalTest('test_it').run(res)
        self.assertEqual(1, res.testsRun)
        assertions.assertLength(self, 1, res.errors)
        traceback_msg = res.errors[0][1]
        msg = "'object' object has no attribute 'available'\n"
        self.assertTrue(traceback_msg.endswith(msg))
