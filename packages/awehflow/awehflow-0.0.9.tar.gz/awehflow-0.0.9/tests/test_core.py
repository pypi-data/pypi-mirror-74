...
class TestDagLoader(TestCase):
    ...
    def test_load_dag_dependencies(self):
        ...
        task_ids = list(map(lambda task: task.task_id, dag.tasks))
        self.assertCountEqual(task_ids, ['pre_hook_1', 'start', 'sensor_3', 'stage_3', 'stage_4', 'success'])
        ...
        self.assertCountEqual(list(map(lambda task: task.task_id, start.upstream_list)), ['pre_hook_1'])
