    ...
    def __build_dag(self, config: Config) -> DAG:
        ...
        if pre_hook_configs:
            pre_hooks = DependencySensors(
                sensor_configs=pre_hook_configs,
                dag=dag,
        ...
        if dependency_configs:
            dependencies = DependencySensors(
                sensor_configs=dependency_configs,
                dag=dag,
                params=config.get('params')
            )

            for dependency_check in dependencies.roots:
                start >> dependency_check >> tasks.roots
        else:
            start >> tasks.roots
...
class SubDag:
    ...
    def __build_sub_dag(self, task_configs, params, event_handlers):
        ...
        tmp_params = merge_dicts({'job_name': self.job_name}, params)
...
class DependencySensors:
    """Builds and loads configured dependency sensors into DAG"""

    def __init__(self, sensor_configs, dag, params):
        self.tasks = {}
        self.dag = dag
        self.__build_sub_dag(sensor_configs, params)

    @property
    def leaves(self):
        return [self.tasks[task_id] for task_id in self.cached_task_ids]

    @property
    def roots(self):
        return self.leaves

    def __build_sub_dag(self, sensor_configs, params):
        """
        Instantiate all sensors in parallel
        """

        for sensor_config in sensor_configs:
            task = DagLoader.build_task(
                task_id=sensor_config.get('id'),
                dag=self.dag,
                operator=sensor_config.get('operator'),
                params=params,
                timeout=0,  # It should only try once, and fail if condition is not met
                **sensor_config.get('params')
            )
            self.tasks[task.task_id] = task

        self.cached_task_ids = [task.task_id for task in self.tasks.values()]
