...
class ImageHat():
    ...
    def __set_host(self, value: int) -> None:
        ...
        if value == 1 and (current_state & self.__usb_s == 0) and (current_state & self.__VUSB_EN == self.__VUSB_EN):
            return

        if value == 2 and (current_state & self.__usb_s == self.__usb_s) and (current_state & self.__VUSB_EN == self.__VUSB_EN):
            return
