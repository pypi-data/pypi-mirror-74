...
from rho_ml import RhoModel, StoredModel, LocalModelStorage,\
...
def store_rho_model(model: Type[RhoModel],
    ...
    headers = _get_sermos_api_headers(api_key=access_key)
...
def _determine_load_local_or_remote(
    ...
    if local_model_path is not None and not force_search:
        ...
        api_response = _get_model_api_response(
            model_name=model_name, version_pattern=version_pattern,
            access_key=access_key, get_model_endpoint=get_model_endpoint)
...
def load_rho_model(model_name: str,
    ...
    local_or_remote, retrieval_info = _determine_load_local_or_remote(
        model_name=model_name, version_pattern=version_pattern,
        delimiter_pattern=delimiter_pattern, access_key=access_key,
...
def _search_model_api_response(access_key: str,
    ...
    if version_pattern is not None:
        query_params['version_pattern'] = version_pattern

    logger.debug("Requesting available models info from Admin API...")

    try:
        r = requests.post(
            url=search_model_endpoint, headers=headers, json=query_params)
        response_data = r.json()
    except Exception as e:
...
def search_rho_model(model_name: str,
                     version_pattern: str,
                     access_key: Optional[str] = None,
                     search_model_endpoint: Optional[str] = None,
                     ) -> List[Dict[str, str]]:
    ...
    resp = _search_model_api_response(
        access_key, model_name, version_pattern, search_model_endpoint
    )
