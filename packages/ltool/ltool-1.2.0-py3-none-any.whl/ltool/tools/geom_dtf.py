...
import pandas as pd
import numpy as np
from tools.wavelet import wavelet
from scipy.signal import argrelmax, argrelmin
import xarray as xr
import warnings
warnings.filterwarnings("ignore")

def feature_id(alt, sig, alpha, peak_margin):
...
def geom_prop(rl_flag, bases, tops, alt, sig):
...
def merge_layers(geom, alt, sig, alpha):
    ...
    if len(geom) > 0:
        ...
        layers = geom.copy().values
        
        i = 0
        icount = layers.shape[0]
    
        while (i + 1 < icount):   
            x = alt[(alt >= layers[i, 1]) & (alt <= layers[i + 1, 3])]
            y = sig[(alt >= layers[i, 1]) & (alt <= layers[i + 1, 3])]
            p2p = sig[(alt >= layers[i, 4]) & (alt <= layers[i + 1, 4])]            # dist = np.std([layers[i, 8], layers[i, 7], layers[i + 1, 6], layers[i + 1, 8]], ddof = 1)

            diff = layers[i + 1, 1] - layers[i, 3]
            curv = np.polyfit(x, y, 2)[0]

            std = np.std(p2p, ddof = 1)
            crit_m = ((std < 1.) & (curv < 0.) & (diff < win))  
   
            if crit_m == True:
                layers[i, 8] = np.max([layers[i, 8], layers[i + 1, 8]])
                layers[i, 7] = layers[i + 1, 7]
                layers[i, 3] = layers[i + 1, 3]
                layers[i, 10] = (layers[i, 8] - np.max([layers[i, 6], layers[i, 7]]))
                layers[i, 11] = (layers[i, 7] - layers[i, 6])
                layers = np.delete(layers, i + 1, axis = 0)
                icount = layers.shape[0]
            else:
                i = i + 1
    
        rl_flag = layers[:, 0].astype(float)
        bases = layers[:, 1].astype(float)
        tops = layers[:, 3].astype(float)
