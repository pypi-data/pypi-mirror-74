...
def database(meas_id, cfg):
    ...
    files = np.array([os.path.join(cfg.inp, x[6]) for x in query])
    ...
    return(files, rpath, exts, alphas, det_lims, ids)

def local(cfg):
    
    files = np.array(glob.glob(os.path.join(cfg.inp,'*')))
    
    exts = np.array([re.split('[.]', x)[-2] for x in files])
    
    sel = np.array([f'b{x}' for x in cfg.wave])
