...
class ProfitExtractAsync:
    ...
    async def get_data_content(self, connector: str, fields=None, values=None, operators=None, orderbyfields=None) -> json:
        """
        ...
        while not self.got_all_results:
            async with aiohttp.ClientSession(headers=authorization_header) as session:
                requests = [self.simple_get_request(url=url,
                                                    params={**parameters, **{
                                                        "skip": self.take * (i + self.batch_number * self.batch_size),
                                                        "take": self.take}},
                                                    session=session) for i in range(self.batch_size)]
                total_response = await asyncio.gather(*requests)
                self.batch_number += 1

        return total_response

    async def simple_get_request(self, url: str, params: dict, session: aiohttp.ClientSession) -> json:
        """
        ...
        async with session.get(url=url, params=params) as resp:
            ...
            if len(response['rows']) < self.take:
