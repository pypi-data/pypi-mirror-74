...
class ProfitExtractAsync:
    ...
    async def get_data_content(self, connector: str, fields=None, values=None, operators=None, orderbyfields=None, batch_size: int = 25, take: int = 40000) -> json:
        """
        ...
        while not self.got_all_results:
            async with aiohttp.ClientSession(headers=authorization_header) as session:
                requests = [self.simple_get_request(url=url,
                                                    params={**parameters, **{
                                                        "skip": take * (i + self.batch_number * batch_size),
                                                        "take": take}},
                                                    session=session,
    ...
    async def simple_get_request(self, url: str, params: dict, session: aiohttp.ClientSession, take : int) -> json:
        ...
        :return: data in json format
        """
        async with session.get(url=url, params=params, take=take) as resp:
            if resp.status == 200:
