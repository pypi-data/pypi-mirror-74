...
class DetectDuplicatesTask(Task):
    ...
    def _run(self):
        ...
        if project_container.dst_context and "_id" in project_container.dst_context:
            project_id = project_container.dst_context["_id"]
            self._check_new_session_container_study_instance_uids(project_id)
            self._check_new_acquisition_container_study_instance_uids(project_id)
    ...
    def _check_new_session_container_study_instance_uids(self, project_id):
        ...
        result = self.fw.check_uids_exist(
            ContainerUidcheck(sessions=list(uids), project_id=project_id)
        )
        item_ids = set()
    ...
    def _check_new_acquisition_container_study_instance_uids(self, project_id):
        ...
        result = self.fw.check_uids_exist(
            ContainerUidcheck(acquisitions=list(uids), project_id=project_id)
        )

        acq_number_items = []
        non_acq_number_items = []
        for uid in result["acquisitions"]:
            try:
                series_uid, acquisition_number = uid.rsplit("_", maxsplit=1)
                acq_number_items.append(f"{series_uid}{acquisition_number}")
            except ValueError:
                non_acq_number_items.append(uid)

        for item in self.db.find_all_items_with_uid(
            M.UID.series_instance_uid.in_(non_acq_number_items)
        ):
            add_error(item)

        for item in self.db.find_all_items_with_uid(
            M.UID.series_instance_uid.concat(M.UID.acquisition_number).in_(
                acq_number_items
            )
        ):
            add_error(item)
