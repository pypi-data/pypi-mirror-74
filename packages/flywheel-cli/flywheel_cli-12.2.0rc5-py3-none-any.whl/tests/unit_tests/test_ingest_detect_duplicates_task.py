...
def mocked_detect_duplicates_task(db):
    ...
    db.find_one_container.return_value = T.ContainerOut(
        path="group/project",
...
def test_run_dup_in_fw(mocked_detect_duplicates_task, create_item):
    ...
    item_1 = create_item(
        filename="a.txt", existing=True, container_path="group/project",
    )
    item_2 = create_item(filename="b.txt", container_path="group/project/subject",)
    item_3 = create_item(
        filename="c.txt", existing=True, container_path="group/project/subject2",
    )
...
def test_run_dup_in_upload_set_first_items(mocked_detect_duplicates_task, create_item):
    ...
    db_mock.count_all_item.return_value = 3
    db_mock.duplicated_sop_instance_uid_item_ids.return_value = []
    item_1 = create_item(filename="a.txt", container_path="group/project/subject",)
    item_2 = create_item(filename="a.txt", container_path="group/project/subject",)
    item_3 = create_item(filename="b.txt", container_path="group/project/subject2",)
    db_mock.get_items_sorted_by_dst_path.return_value = [item_1, item_2, item_3]
...
def test_run_dup_in_upload_set_last_items(mocked_detect_duplicates_task, create_item):
    ...
    item_1 = create_item(filename="a.txt", container_path="group/project/subject",)
    item_2 = create_item(filename="b.txt", container_path="group/project/subject",)
    item_3 = create_item(filename="b.txt", container_path="group/project/subject",)
    db_mock.get_items_sorted_by_dst_path.return_value = [item_1, item_2, item_3]
    expected_error_code = errors.DuplicateFilepathInUploadSet.code
...
def test_run_dup_in_upload_set_middle_items(mocked_detect_duplicates_task, create_item):
    ...
    item_2 = create_item(filename="b.txt", container_path="group/project/subject",)
    item_3 = create_item(filename="b.txt", container_path="group/project/subject",)
    item_4 = create_item(filename="b.txt", container_path="group/project/subject",)
    item_5 = create_item(filename="c.txt", container_path="group/project/subject",)
...
def test_new_study_instance_uids(db, detect_duplicates_task, create_uid, sdk_mock):
    ...
    detect_duplicates_task._check_new_session_container_study_instance_uids(project_id)
...
def test_new_series_instance_uids(db, detect_duplicates_task, create_uid, sdk_mock):
    sdk_mock.check_uids_exist.return_value = {"acquisitions": ["1.2", "1.1_2"]}
    ...
    assert set(["1.1", "1.1_2", "1.2"]) == set(args[0].acquisitions)
    ...
    detect_duplicates_task.insert_errors.push.assert_has_calls(
        [
            ...
            mock.call(
                {"item_id": item1.id, "code": errors.SeriesInstanceUIDExists.code}
            ),
        ],
        any_order=True,
    )
...
def test_fw_uid_check_not_called(db, detect_duplicates_task, create_uid, mocker):
    fw_mock = mock.Mock(spec=Flywheel)
    fw_mock.check_uids_exist.return_value = {"acquisitions": [], "sessions": []}
    mocker.patch("flywheel_cli.util.get_sdk_client", return_value=fw_mock)

    project = db.create_container(
        path="project", level=T.ContainerLevel.project, dst_context={}
    )
    container1 = db.create_container(
        path="path", level=T.ContainerLevel.session, existing=False
    )
    ...
    assert fw_mock.check_uids_exist.call_count == 0


def test_fw_uid_check_called(db, detect_duplicates_task, create_uid, mocker):
    fw_mock = mock.Mock(spec=Flywheel)
    fw_mock.check_uids_exist.return_value = {"acquisitions": [], "sessions": []}
    mocker.patch("flywheel_cli.util.get_sdk_client", return_value=fw_mock)
    ...
    assert fw_mock.check_uids_exist.call_count == 2
