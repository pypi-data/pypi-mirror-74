...
def test_score_errors(score_args, expected_exception, expected_message):
    with contextlib2.ExitStack() as stack:
        ...
        if 'timeseries_settings' in score_args:
            stack.enter_context(pytest.warns(UserWarning))

        stack.enter_context(pytest.raises(expected_exception, match=expected_message))
...
def test_score_job_data(mock_client, score_args, expected_job_data):
    ...
    with contextlib2.ExitStack() as stack:
        ...
        stack.enter_context(pytest.raises(RuntimeError, match='short-circuit'))
