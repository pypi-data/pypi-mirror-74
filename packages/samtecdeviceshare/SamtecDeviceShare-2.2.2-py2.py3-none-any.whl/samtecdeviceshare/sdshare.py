...
from samtecdeviceshare.helper import is_online, get_ip_address, setup_wifi_hotspot, setup_wifi_client, awaitify, valide_wpa_passphrase
...
class SamtecDeviceShare:
    ...
    async def open(self) -> None:
        ...
        self.background_task = asyncio.create_task(self.update())
    ...
    async def launch_hot_spot(self, credentials: Optional[NetworkCredentials] = None):
        ...
        if credentials.ssid is None:
            credentials.ssid = self.settings.wifi.ssid
            credentials.passphrase = self.settings.wifi.passphrase
        ...
        if not valide_wpa_passphrase(credentials.passphrase):
    ...
    async def launch_wifi_client(self, credentials: NetworkCredentials):
        ...
        if credentials.ssid is None:
            credentials.ssid = self.settings.wifi.ssid
            credentials.passphrase = self.settings.wifi.passphrase
            credentials.identity = self.settings.wifi.identity
    ...
    async def update_app(self) -> bool:
        ...
        await self.set_app_lock(locked=False)
        self.update_task = asyncio.create_task(self.supervisor_update_check(force=True))
        return True
    ...
    async def get_device_state(self) -> DeviceState:
        return DeviceState(
            ...
            app_update_check=self.update_task is not None and not self.update_task.done(),
    ...
    async def perform_ota_update_check(self, credentials: Optional[NetworkCredentials]) -> None:
        try:
            ...
            self.update_task = asyncio.create_task(self.perform_ota_update_task())
            return
        except Exception as err:
            logger.exception('OTA Update Check: Failed due to error: %s', err)
            await self.launch_default_wifi()
