...
@app.get("/data.json", response_model=AppDataV1, status_code=status.HTTP_200_OK)
async def get_app_data():
    return AppDataV1(type=sds.app_info.name, port=sds.app_info.web_port)

@app.get("/img.png")
async def get_app_img() -> bytes:
...
@app.get("/api/v2/device", response_model=DeviceState, status_code=status.HTTP_200_OK)
async def get_device_state():
    return await sds.get_device_state()

@app.get("/api/v2/app", status_code=status.HTTP_200_OK)
async def get_app_state():
    try:
        return await sds.get_supervisor_app_state()
    except Exception as err:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to get app state w/ error: {0}'.format(err))

@app.get("/api/v2/app/logs")
async def get_app_logs() -> bytes:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to get app logs w/ error: {0}'.format(err))

@app.get('/api/v2/network/wifi/credentials', response_model=NetworkCredentialCleaned, status_code=status.HTTP_200_OK)
...
@app.post('/api/v2/device/blink', status_code=status.HTTP_200_OK)
...
@app.post('/api/v2/device/restart', status_code=status.HTTP_202_ACCEPTED)
async def restart_device() -> TextResponseEnum:
    success = await sds.restart_app()
    return TextResponseEnum.from_bool(success)

@app.post('/api/v2/app/update/check', status_code=status.HTTP_204_NO_CONTENT)
async def app_update_check(credentials: Optional[NetworkCredentials]) -> TextResponseEnum:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to perform update check w/ error: {0}'.format(err))
    except Exception as err:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to perform update check w/ error: {0}'.format(err))

@app.post('/api/v2/app/update/install', status_code=status.HTTP_204_NO_CONTENT)
async def app_update_install() -> TextResponseEnum:
    try:
        await sds.update_app()
        return TextResponseEnum.Success
    except Exception as err:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to perform update w/ error: {0}'.format(err))

@app.post('/api/v2/network/wifi/credentials', status_code=status.HTTP_200_OK)
async def set_wifi_network(credentials: NetworkCredentials) -> TextResponseEnum:
    try:
        await sds.launch_wifi_client(credentials)
        return TextResponseEnum.Success
    except Exception as err:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to update WiFi credentials w/ error: {0}'.format(err))

@app.post('/api/v2/network/wifi/hotspot', status_code=status.HTTP_200_OK)
async def set_wifi_hotspot(credentials: Optional[NetworkCredentials])-> TextResponseEnum:
    try:
