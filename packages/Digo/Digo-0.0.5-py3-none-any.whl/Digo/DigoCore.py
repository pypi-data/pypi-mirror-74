...
class Core():
    ...
    def __init__(self, api, workspace_name, project_name):
        ...
        self.__step: int = 0
        self.login()
        atexit.register(ExceptionHandler.exit_process, args=[self.stateUpdate])
    ...
    def stateUpdate(self, state):
        ...
        Network.sendTrainState(api_key=self.__user.api_key, account_id=self.__user.id, state=state)