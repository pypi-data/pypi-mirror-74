        ...
        if self.fconfig:
            if self.test_lbs is None:
                ...
                for attr, attr_vals in self.fconfig.items():
                    if attr not in self.test_feat:
                        self.api.logger.warning(
                            f"Key '{attr}' in fairness config does not exist in test features"
                        )
                    else:
                ...
                self._log_fai_data(fairness_metrics, self.fconfig)
    ...
    def _log_fai_data(self, fai_data: Dict[str, pd.DataFrame], fairness_config: Dict):
        if self.api.has_api_settings:
            ...
            for att_name, fairness_metrics in fai_data.items():
                ...
                metrics[att_name] = fmeasures.to_dict()
                metrics[att_name]["confusion_matrix"] = confusion_matrix
