...
class Exporting():
    ...
    def setup(self):
        ...
        """
        self.comp_name, self.cnxn, self.date, self.modules = setup(
            self.exportObj)
        self.exportObj.update({'created': self.date})
        self.result_module = import_module(
            '.'+self.modules['result_module'], package='hazus.legacy.exporting.results')

    def getData(self):
        """Queries and parses the data from SQL Server, preparing it for exporting
        """
        self.hazus_results_dict, self.subcounty_results, self.county_results, self.damaged_essential_facilities = self.result_module.read_sql(
            self.comp_name, self.cnxn, self.exportObj)

    def toCSV(self):
        """Exports the study region data to CSVs
        """
        self.result_module.to_csv(self.hazus_results_dict, self.subcounty_results,
                                  self.county_results, self.damaged_essential_facilities, self.exportObj)

    def toShapefile(self):
        """Exports the study region data to Shapefile(s)
        """
        self.gdf = self.result_module.to_shp(
            self.exportObj, self.hazus_results_dict, self.subcounty_results)
    ...
    def toReport(self):
        ...
        try:
            ...
            self.toShapefile()
        self.report_module = import_module(
            '.'+self.modules['report_module'], package='hazus.legacy.exporting.reports')
        self.report_module.generate_report(
            self.gdf, self.hazus_results_dict, self.subcounty_results, self.county_results, self.exportObj)
