...
class MessageProcessor(threading.Thread, metaclass=ServerMaker):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                logger.info('Установлено соединение с ПК {}', client_address)
                client.settimeout(5)
            ...
            try:
                ...
                logger.error('Ошибка работы с сокетами: {}', err.errno)
    ...
    def remove_client(self, client):
        """
        Метод-обработчик клиента, с которым потеряна связь.
        Ищет клиента в словаре клиентов и удаляет его со списков и базы
        """
        logger.info('Клиент {} отключился от сервера.', client.getpeername())
    ...
    def process_client_message(self, message, client):
        """
        Метод-обработчик сообщений от клиентов, принимает словарь-сообщение
        от клиента, проверяет корректность, отправляет словарь-ответ в случае необходимости.
        """
    ...
    def service_update_lists(self):
        """
        Метод для отправки сервисного сообщения с кодом 205
        для синхронизации списка контактов в клиентской части
        """
