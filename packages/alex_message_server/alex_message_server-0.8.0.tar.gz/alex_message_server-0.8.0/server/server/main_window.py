"""
Модуль рабочего окна серверной части приложения "Мессенджер"
"""
...
class MainWindow(QMainWindow):
    ...
    Содержит всю основную логику работы серверного модуля.
    Конфигурация окна создана вручную и загружается при запуске модуля.
    """
    ...
    def __init__(self, database, server, config):
        ...
        # Статусбар
        self.statusBar()
        self.statusBar().showMessage('Сервер в работе...')
        ...
        self.setWindowTitle('Админка сервера')
    ...
    def create_users_model(self):
        """
        Метод создания модели и заполняем таблицу подключений.
        """
    ...
    def check_btn_remove(self):
        """
        Метод, устанавливающий флаг состояния кнопки удаления пользователей.
        Если список пользователей равен нулю, то флаг = False, иначе - True
        """
        if self.database.users_list == 0:
