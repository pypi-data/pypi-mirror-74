# coding: utf-8

"""
    Pulp 3 API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)  # noqa: E501

    The version of the OpenAPI document: v3
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest

import pulpcore.client.pulpcore
from pulpcore.client.pulpcore.api.artifacts_api import ArtifactsApi  # noqa: E501
from pulpcore.client.pulpcore.rest import ApiException


class TestArtifactsApi(unittest.TestCase):
    """ArtifactsApi unit test stubs"""

    def setUp(self):
        self.api = pulpcore.client.pulpcore.api.artifacts_api.ArtifactsApi()  # noqa: E501

    def tearDown(self):
        pass

    def test_create(self):
        """Test case for create

        Create an artifact  # noqa: E501
        """
        pass

    def test_delete(self):
        """Test case for delete

        Delete an artifact  # noqa: E501
        """
        pass

    def test_list(self):
        """Test case for list

        List artifacts  # noqa: E501
        """
        pass

    def test_read(self):
        """Test case for read

        Inspect an artifact  # noqa: E501
        """
        pass


if __name__ == '__main__':
    unittest.main()
