...
from bearer_agent.py3.filters import FILTERS_BY_NAME
...
def setup_hooks(cfg):
    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_urllib3"), "urllib3"
    )

    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_http"), "http.client"
    )

    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_aiohttp"), "aiohttp"
    )

    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_httpcore"), "httpcore"
    )

    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_twisted_web"), "twisted.web._newclient"
    )
...
class DataCollectionRules(Option):
    ...
    default = []
    desc = """\
        list of data collection rules to filter a request or response
        """
