...
from azureml.automl.core.shared.exceptions import ArgumentException, CacheException, ErrorTypes
...
def _set_env_variables(subscription_id, resource_group, workspace_name, experiment_name,
    ...
    if any(var is None for var in df_value_list):
        raise ArgumentException.create_without_pii("{0}: Value can't be None".format(var))
        ...
        with _AMLLogUploader(current_run, self.worker_id):
            try:
                ...
                if run_id.endswith("setup"):
                    ...
                    if (not skip_featurization(self.automl_settings.featurization,
                        ...
                        # We have a featurized data set now, which means the
                        # data needed pre-processing. We save the hard work done to transform the data
                    ...
                    _automl_perform_best_run_explain_model(
                        ...
                        logger, ComputeTargets.ADB, current_run,
