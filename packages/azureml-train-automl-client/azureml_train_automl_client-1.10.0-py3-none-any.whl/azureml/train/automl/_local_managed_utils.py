...
def get_data_args(workspace: Workspace, fit_params: Dict[str, Any], local_path: str) -> \
    ...
    dataset_args = _handle_data(data_dict, local_path)
...
def _compose_args(dataset_args: List[Any], name: str, data_type: str, value: Optional[Any]) -> List[Any]:
    arg = "--{}"
    arg_type = "--{}-dtype"
...
def _handle_data(data_dict: Dict[str, AbstractDataset],
                 local_path: str) -> List[Any]:
    ...
    for data_name, data_value in data_dict.items():
        if isinstance(data_value, AbstractDataset):
            ...
            raise DataException("Received unrecognized type for {}. Data should be a DataSet, a Pandas DataFrame"
                                "or a Numpy array.".format(data_name))
    return dataset_args
