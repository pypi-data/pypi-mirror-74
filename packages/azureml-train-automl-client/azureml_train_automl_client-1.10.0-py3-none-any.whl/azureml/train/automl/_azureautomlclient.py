...
from azureml._common._error_definition.user_error import ArgumentBlankOrEmpty, NotFound
from azureml._common.exceptions import AzureMLException
from azureml._restclient.constants import RunStatus, SNAPSHOT_MAX_FILES, SNAPSHOT_BATCH_SIZE, ONE_MB,\
...
from azureml.exceptions import ServiceException as AzureMLServiceException, UserErrorException, SnapshotException
...
class AzureAutoMLClient:
        ...
        if run_configuration.framework.lower() != 'python':
            raise ConfigException._with_error(
                AzureMLError.create(
                    ...
                    arguments=run_configuration.framework, supported_values="python"
                )
            )
    ...
    def _fit_remote_core(self, run_configuration, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None,
        ...
        try:
            ...
            self._jasmine_client.post_remote_jasmine_snapshot_run(
                self.parent_run_id, json.dumps(definition), snapshotId)
            ...
            if ((str(SNAPSHOT_MAX_SIZE_BYTES / ONE_MB) in se.message) or (str(SNAPSHOT_MAX_FILES) in se.message)):
                raise UserErrorException(se.message)
            else:
                raise
        except (AzureMLException, AzureMLServiceException):
            raise
        except Exception as e:
            raise ClientException.create_without_pii("Error occurred when trying to submit a \
            remote run to AutoML Service.") from None
    ...
    def _validate_input(self, parent_run_dto: CreateParentRunDto) -> None:
