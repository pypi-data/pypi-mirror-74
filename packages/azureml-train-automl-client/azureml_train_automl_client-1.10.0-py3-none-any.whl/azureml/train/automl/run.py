...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import InvalidOperationOnRunState, \
    InvalidArgumentType, FeatureUnsupportedForIncompatibleArguments, ConflictingValueForArguments
...
class AutoMLRun(Run):
        ...
        with log_server.new_log_context():
            ...
            if run_status in RUNNING_STATES:
                ...
                raise ConfigException._with_error(
                    AzureMLError.create(
                        InvalidOperationOnRunState, target="cancel", operation_name="cancel",
                        run_id=self.run_id, run_state=run_status
                    )
                )
    ...
    def cancel_iteration(self, iteration):
        ...
        if child_run_status in RUNNING_STATES:
            ...
            raise ConfigException._with_error(
                AzureMLError.create(
                    InvalidOperationOnRunState, target="cancel", operation_name="cancel_iteration",
                    run_id=childrun_id, run_state=child_run_status
                )
            )
    ...
    def get_output(self,
        ...
        with log_server.new_log_context():
            ...
            with logging_utilities.log_activity(
                ...
                curr_run, child_runs_and_scores = self._get_run_internal(iteration, metric, "get_output")
    ...
    def _download_automl_model(self, curr_run: Run) -> Optional[Any]:
        with logging_utilities.log_activity(
            ...
            try:
                import xgboost.core.XGBoostError as XGBoostError
            ...
            try:
                ...
                missing_package_generic = "The model you attempted to retrieve requires '{pkg}' to be installed "\
                    "in the current environment. You should install '{pkg}{ver}' and then rerun the "\
                    "previous command."
                if e.name == 'xgboost':
                    raise OptionalDependencyMissingException.create_without_pii(
                        missing_package_generic.format(pkg='xgboost', ver='<=0.90'),
                        reference_code=ReferenceCodes._MISSING_OPTIONAL_DEPENDENCY_XGBOOST
                    ) from e
                elif e.name == 'fbprophet':
                    raise OptionalDependencyMissingException.create_without_pii(
                        missing_package_generic.format(pkg='fbprophet', ver='==0.5'),
                        reference_code=ReferenceCodes._MISSING_OPTIONAL_DEPENDENCY_FBPROPHET
                    ) from e
                elif e.name not in ['runtime', 'azureml.train.automl.runtime']:
                    raise
                msg = "Cannot retrieve model since azureml-train-automl-runtime is not installed locally. "\
                    "Please install this dependency to enable returning a model from this method."
                logging.warn(msg)
                logger.warn(msg)
            except Exception as e:
                ...
                if isinstance(e, XGBoostError):
                    import xgboost
                    raise OptionalDependencyMissingException.create_without_pii(
                        "The model you attempted to retrieve requirest 'xgboost' to be installed at '<=0.90'. "
                        "You have 'xgboost=={}', please reinstall 'xgboost<=0.90' and rerun the previous "
                        "command".format(xgboost.__version__),
                        reference_code=ReferenceCodes._MISSING_OPTIONAL_DEPENDENCY_XGBOOST_VERSION
                    ) from e
                raise
        return None
    ...
    def register_model(self, model_name=None, description=None, tags=None, iteration=None, metric=None):
        ...
        with log_server.new_log_context():
            ...
            with logging_utilities.log_activity(
                ...
                best_run, _ = self._get_run_internal(iteration, metric, "register_model")
    ...
    def _fail_with_error(self, exception: BaseException) -> None:
        logger.error("Marking Run {} as Failed.".format(self.id))
        ...
        # target etc. We extract that dictionary of fields (called RootError), and fail the run directly with it.
        if isinstance(exception, AzureMLServiceException):
            ...
            # Set the error code based on exception type and mark the run as failed
            # AzureMLExceptions are automatically unrolled into an ErrorResponse, and the run is populated with
            # the right fields (e.g. error code, message, target etc.)
