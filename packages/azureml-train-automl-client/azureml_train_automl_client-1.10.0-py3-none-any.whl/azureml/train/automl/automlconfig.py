...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import InvalidArgumentForTask, \
    ...
    InvalidCVSplits, InvalidInputDatatype, InputDataWithMixedType, AllowedModelsUnsupported, \
...
from azureml.core import Environment, ScriptRunConfig, Run
...
from azureml.train.automl._environment_utilities import is_prod
from azureml.train.automl import _azureautomlclient, _azureautomlsettings
from azureml.train.automl._local_managed_utils import get_data_args, is_docker_installed, modify_managed_run_config

from azureml.automl.core.shared.constants import SupportedModelNames, SupportedModels
...
def _automl_static_submit(automl_config_object: 'AutoMLConfig',
    ...
    settings = {k: v for (k, v) in automl_config_object.user_settings.items() if k not in fit_params}

    automl_run = _start_execution(
        experiment,
        settings,
        fit_params,
        automl_config_object,
        run_config,
        compute_target,
        parent_run_id,
        show_output)
...
def _default_execution(experiment: Experiment,
    ...
    settings_obj = _azureautomlsettings.AzureAutoMLSettings(experiment, **automl_settings)
...
# TODO remove this once 761413 is finished
def _check_conda_lock() -> None:
    lock_path = os.path.join(".azureml", "locks", "azureml_conda_lock")
    linux_path = os.path.join(os.environ.get('HOME', ''), lock_path)
    windows_path = os.path.join(os.environ.get('USERPROFILE', ''), lock_path)

    msg = "The conda environment is currently locked by another AzureML job. Further job submission will " \
          "wait until the other process finishes. If there are no other jobs running, please delete {}"
    try:
        if os.path.exists(linux_path):
            logging.warning(msg.format(linux_path))
        elif os.path.exists(windows_path):
            logging.warning(msg.format(windows_path))
    except Exception:
        pass
...
def _local_managed(experiment: Experiment,
    ...
    try:
        with tempfile.TemporaryDirectory() as local_path:
            ...
            settings_obj = _azureautomlsettings.AzureAutoMLSettings(**settings)
            automl_estimator = _azureautomlclient.AzureAutoMLClient(experiment, settings_obj)

            parent_run = automl_estimator._create_parent_run_for_local_managed(parent_run_id)
            if show_output:
                print("Parent Run ID: {}".format(parent_run.id))
        ...
        if script_run.get_status() == "Failed":
            msg = "Submitting local managed run failed with {}"
            raise ClientException(msg.format(script_run.get_details().get('error')))\
                .with_generic_msg(msg.format("[MASKED]"))
    except Exception as e:
        logging_utilities.log_traceback(e, logger)
        if parent_run is not None:
            parent_run._fail_with_error(e)
        raise
...
def _start_execution(
        ...
        settings: Dict[str, Any],
        fit_params: Dict[str, Any],
        automl_config_object: 'AutoMLConfig',
    ...
    :param settings: User settings not part of fit params.
    :param fit_params: User settings passed directly to azureautomlclient's fit method.
    :param automl_config_object: The AutoMLConfig.
    ...
    if run_config is None and compute_target is None:
        ...
        automl_config_object.user_settings["compute_target"] = constants.ComputeTargets.LOCAL
    ...
    if is_managed and not settings.get("enable_local_managed"):
    ...
    if automl_config_object.user_settings.get('spark_context') is not None:
        print("Running on remote or ADB.")
        automl_run = _default_execution(experiment, settings, fit_params, False, show_output)
    elif in_process is True:
        logger.info("Submitting local legacy run.")
        try:
            import azureml.train.automl.runtime
            automl_run = _default_execution(experiment, settings, fit_params, True, show_output, parent_run_id)
            ...
            if e.name not in ['runtime', 'azureml.train.automl.runtime']:
                raise
            logger.info("Tried to submit local legacy experiment but runtime package is missing. Falling"
                        "back to local managed.")
            print("Azureml-train-automl-runtime is not installed. Submitting run to a new conda environment.")
            run_config = RunConfiguration()
            automl_run = _local_managed(experiment, run_config, settings, fit_params, show_output, parent_run_id)
    elif is_managed:
        logger.info("Submitting local managed run.")
        print("Running on local conda or docker.")
        automl_run = _local_managed(experiment, run_config, settings, fit_params, show_output)
    else:
        logger.info("Submitting remote or ADB run.")
        print("Running on remote or ADB.")
        automl_run = _default_execution(experiment, settings, fit_params, False, show_output)
...
class AutoMLConfig(object):
    ...
    :raises azureml.automl.core.shared.exceptions.EmptyDataException: Raised for configuration parameters that
    ...
    def __init__(self,
        ...
        if task == constants.Tasks.CLASSIFICATION:
            ...
            if y_min is not None or y_max is not None:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        InvalidArgumentForTask, arguments="y_min/y_max", task_type=constants.Tasks.CLASSIFICATION
                    )
                )
            ...
            if num_classes is not None:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        InvalidArgumentForTask, arguments="num_classes", task_type=constants.Tasks.REGRESSION
                    )
                )
        ...
        if task in constants.Tasks.ALL_IMAGE and not self.user_settings["enable_dnn"]:
            raise ConfigException._with_error(
                AzureMLError.create(
        ...
        try:
            ...
            self.user_settings["scenario"] = scenario
        except KeyError:
            if is_prod() or force_curated_environment:
                self.user_settings["scenario"] = constants.Scenarios.SDK_COMPATIBLE
            pass
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        if self._run_configuration.framework.lower() != 'python':
            raise ConfigException._with_error(
                AzureMLError.create(
                    ...
                    arguments=self._run_configuration.framework, supported_values="python"
                )
            )
        ...
        for key in ['X', 'y', 'sample_weight', 'X_valid', 'y_valid', 'sample_weight_valid',
            ...
            if value is not None:
                if key == 'cv_splits_indices':
                            ...
                            if dataprep_utilities.is_dataflow(item):
                                raise ConfigException._with_error(
                                    AzureMLError.create(
                                        ...
                                        supported_types=", ".join(constants.SupportedInputDatatypes.ALL)
                                    )
                                )
                            else:
                    ...
                    elif has_pipeline_pkg and isinstance(value, PipelineOutputTabularDataset):
        ...
        if has_training_input_pandas_obj and self.user_settings['spark_context'] is not None:
            raise ConfigException._with_error(
                AzureMLError.create(
                    ...
                    supported_types=", ".join(constants.SupportedInputDatatypes.REMOTE_RUN_SCENARIO)
                )
            )
        ...
        self._validate_streaming_allowed_models(is_timeseries, compute_target)
    ...
    def _validate_streaming_allowed_models(self, is_timeseries: bool, compute_target: str) -> None:

        def _handler(scenario: str, should_raise: bool) -> None:
            error_msg = "'allowed_models' consists of unsupported algorithms for {} scenarios."

            if should_raise:
                logger.error(error_msg.format(scenario))
                raise ConfigException._with_error(
                    AzureMLError.create(
                        AllowedModelsUnsupported, target="allowed_models",
                        allowed_models=", ".join(allowed_models), scenario=scenario
                    )
                )

            logger.warning(error_msg.format(scenario))

        allowed_models = self.user_settings.get('allowed_models', [])
        if allowed_models:
            # Ensure allowed algorithms don't all contain streaming only algorithms
            ...
            wlm_all_streaming = set(allowed_models).issubset(streaming_supported_algorithms)
            wlm_some_streaming = set(allowed_models).intersection(streaming_supported_algorithms)

            if wlm_all_streaming or wlm_some_streaming:
                if compute_target == constants.ComputeTargets.LOCAL:
                    _handler("local", wlm_all_streaming)
                if is_timeseries:
                    _handler("forecasting", wlm_all_streaming)
