...
class BaseExperiment(CommonExperiment):
    ...
    def __init__(
        ...
        log_env_gpu=True,
        log_env_host=True,
        display_summary=None,  # deprecated
        log_env_cpu=True,
        ...
        Args:
            ...
            auto_weight_logging: Default(False) - allows you to enable/disable automatic histogram logging of biases and weights
        ...
        if display_summary is not None:
            LOGGER.info(
                "display_summary has been deprecated; use display_summary_level"
            )

        if get_config("comet.display_summary") is not None:
            LOGGER.warning(
        ...
        self.auto_weight_logging = auto_weight_logging
    ...
    def log_confusion_matrix(
        ...
        **kwargs
        ...
        For more information, see comet_ml.ConfusionMatrix
        """
    ...
    def log_table(self, filename, tabular_data=None, headers=False):
        # type: (str, Optional[Any], Union[Sequence[str], bool]) -> Optional[Dict[str, str]]
        """
        Log tabular data.

        Args:
            filename: str (required), a filename ending in ".csv", or ".tsv"
            ...
            headers: bool or list, if True, will add column headers automatically
                ...
                then it will be used as headers.
        ...
        if self.alive:
            ...
            if type(graph).__name__ == "Graph":  # Tensorflow Graph Definition
                try:
                    ...
                    LOGGER.warning(
                        "Failed to convert Tensorflow graph to JSON", exc_info=True
                    )
                    return None
        ...
        if framework in ["keras", "tf-keras"]:
            ...
            if self.alive:
                return KerasCallback(
                    ...
                    log_histograms=self.auto_weight_logging,
                )
            else:
