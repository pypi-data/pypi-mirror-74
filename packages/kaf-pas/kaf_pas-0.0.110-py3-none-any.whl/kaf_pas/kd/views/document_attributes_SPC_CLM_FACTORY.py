...
def Document_attributes_SPC_CLM_FACTORY_Fetch(request):
    return JsonResponse(
        DSResponse(
            request=request,
            data=Document_attributes.objects.
                select_related('attr_type').
                filter(attr_type__code='SPC_CLM_FACTORY').
                get_range_rows1(
                request=request,
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_FACTORY_Add(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_FACTORY_Update(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_FACTORY_Remove(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_FACTORY_Lookup(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_FACTORY_Info(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_FACTORY_Copy(request):
