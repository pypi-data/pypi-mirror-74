...
def Document_attributes_SPC_CLM_MARK_Fetch(request):
    return JsonResponse(
        DSResponse(
            request=request,
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_MARK_Add(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_MARK_Update(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_MARK_Remove(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_MARK_Lookup(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_MARK_Info(request):
...
@JsonResponseWithException()
def Document_attributes_SPC_CLM_MARK_Copy(request):
