...
class Item_line(AuditModel):
    ...
    edizm = ForeignKeyProtect(Ed_izm, verbose_name='Еденица измерения', related_name='Item_line_EdIzm', null=True, blank=True)
    section = CodeField()
    ...
    @property
    def qty(self):
        if self.SPC_CLM_COUNT == None:
            return None

        if self.SPC_CLM_COUNT.value_int == None:
