...
def Demand_Info(request):
    return JsonResponse(DSResponse(request=request, data=Demand.objects.get_queryset().get_info(request=request), status=RPCResponseConstant.statusSuccess).response)
