        ...
        with transaction.atomic():
            for id, mode in tuple_ids:
                if mode == 'hide':
                    ...
                    Launch_item_viewManager.check_launch(launch=Launch_item_line_view.objects.get(id=id).launch)
...
class Launch_item_line_view(AuditModel):
    ...
    edizm = ForeignKeyProtect(Ed_izm, verbose_name='Еденица измерения', related_name='EdIzm_l', null=True, blank=True)
    section = NameField()
