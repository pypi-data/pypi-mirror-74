...
from isc_common import delAttr, NotImplement
...
class Production_order_opersManager(CommonManager):
    ...
    def refreshRows(ids):
        if isinstance(ids, int):
            ids = [ids]
        records = [Production_order_opersManager.getRecord(record) for record in Production_order_opers.objects.filter(id__in=ids)]
    ...
    def deleteFromRequest(self, request):
        ...
        with transaction.atomic():
            production_ext.delete_operation(ids=ids)
