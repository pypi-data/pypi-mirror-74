...
from isc_common import setAttr, delAttr, get_list_nums_key_dict, delAttr1
...
class ItemQuerySet(CommonManagetWithLookUpFieldsQuerySet):
    ...
    def create(self, **kwargs):
        ...
        item = delAttr1(kwargs, 'id')
        setAttr(item, 'operation', res)
        setAttr(item, 'hcreator', res.creator)
        Item_history.objects.create(**item)
        return res
...
class ItemManager(CommonManagetWithLookUpFieldsManager):
    ...
    def find_item(item):
        ...
        if isinstance(item, int):
            ...
            raise Exception(f'item must be Item or int')
    ...
    def check_4_production(item):
        ...
        with connection.cursor() as cursor:
            cursor.execute(f'''select count(*) from ckk_item_not_typed_top_level_view
                                where parent_id = %s or child_id = %s''', [id, id])
            qty, = cursor.fetchone()
            if qty == 0:
                cursor.execute(f'''select count(*) from ckk_item_typed_top_level_view
                                    where parent_id = %s or child_id = %s''', [id, id])
                qty, = cursor.fetchone()
            return qty > 0

    @staticmethod
    def delete_recursive(item_id, delete_lines=False, soft_delete=None, user=None, props=TurnBitOn(0, 0), document=None, show_progress=True):
    ...
    def rec(data):
        ...
        res = Item.objects.filter(id=_data.get('id')).update(**data)
    ...
    def updateFromRequest(self, request, removed=None, function=None):
        ...
        with transaction.atomic():
            ...
            if data.get('mode') == 'move':
                ...
                res = []
                for _data in get_list_nums_key_dict(data):
                    res.append(ItemManager.rec(_data.copy()))
                return res
    ...
    def lookupFromRequest(self, request):
        ...
        if value_str == None and value_int != None:
    ...
    @staticmethod
    def get_verstion(STMP_1, STMP_2, props, version=None, inc=1):
        ...
        if isinstance(data, dict) and isinstance(data.get('records'), list):
            ...
            with managed_progress(
                ...
                try:
                    ...
                    raise ex
        raise Exception(f'Not data.')

    @staticmethod
    def _getQtyChilds(records):
        ...
        if isinstance(records, list):
            with connection.cursor() as cursor:
                for record in records:
                    cursor.execute(f'''WITH RECURSIVE r AS (
                                                                SELECT *, 1 AS level
                                                                FROM ckk_item_refs
