...
class Operations_template_resourceManager(CommonManagetWithLookUpFieldsManager):
            ...
            with connection.cursor() as cursor:
                ...
                for row in rows:
                    ...
                    for item in rows_item:
                        ...
                        if not first_step:
                            # print(f'id: {id}')
                            res = Operations_template_resource.objects.filter(id=id).delete()
                            # print(res)
                        else:
                            first_step = False
    ...
    def updateFromRequest(self, request, removed=None, function=None):
        ...
        if data.get('location_fin_id') == None:
            location = Locations.objects.get(id=data.get('location_fin__code'))
            setAttr(data, 'location_fin_id', location.id)
...
class Operations_template_resource(AuditModel):
    ...
    class Meta:
        ...
        constraints = [
            UniqueConstraint(fields=['template'], condition=Q(location=None) & Q(location_fin=None) & Q(resource=None), name='Operations_template_resource_unique_constraint_0'),
            UniqueConstraint(fields=['resource', 'template'], condition=Q(location=None) & Q(location_fin=None), name='Operations_template_resource_unique_constraint_1'),
            UniqueConstraint(fields=['location', 'template'], condition=Q(location_fin=None) & Q(resource=None), name='Operations_template_resource_unique_constraint_2'),
            UniqueConstraint(fields=['location', 'resource', 'template'], condition=Q(location_fin=None), name='Operations_template_resource_unique_constraint_3'),
            UniqueConstraint(fields=['location_fin', 'template'], condition=Q(location=None) & Q(resource=None), name='Operations_template_resource_unique_constraint_4'),
            UniqueConstraint(fields=['location_fin', 'resource', 'template'], condition=Q(location=None), name='Operations_template_resource_unique_constraint_5'),
            UniqueConstraint(fields=['location', 'location_fin', 'template'], condition=Q(resource=None), name='Operations_template_resource_unique_constraint_6'),
            UniqueConstraint(fields=['location', 'location_fin', 'resource', 'template'], name='Operations_template_resource_unique_constraint_7'),
        ]
