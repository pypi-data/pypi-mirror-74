    ...
    def createFromRequest(self, request, removed=None):
        ...
        with transaction.atomic():
            ...
            for item in Tmp_buffer.objects.filter(value_off__isnull=False, props=4, user=request.user).exclude(value_off=0):
                ...
                setAttr(child, 'opertype', settings.OPERS_TYPES_STACK.CALC_DETAIL_MINUS_TASK)
                setAttr(child, 'status', settings.OPERS_TYPES_STACK.CALC_DETAIL_MINUS_TASK_STATUSES.get('new'))
