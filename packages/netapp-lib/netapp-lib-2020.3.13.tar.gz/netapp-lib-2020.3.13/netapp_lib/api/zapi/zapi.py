...
import netapp_lib.exceptions as NAException

LOG = logging.getLogger(__name__)


class NaApiError(NAException.NetAppLibException):
...
class NaServer(object):
    ...
    def invoke_api(self, api_name=None, api_args=None, enable_tunneling=True):
        """Invoke a Data ONTAP API using dict semantics.

            :param api_name: Name of API (omit if included as api_args root)
            :param api_args: API arguments (optional)
            :param enable_tunneling: API tunneled to vfiler/vserver if True
        """

        # Ensure valid arguments
        if not api_name and not api_args:
            raise ValueError('api_name and/or api_args must be supplied.')
        if api_name and not isinstance(api_name, six.string_types):
            raise ValueError('api_name must be a string.')
        if api_args and not isinstance(api_args, dict):
            raise ValueError('api_args must be a dict.')

        # If not provided, strip out API name from the dictionary root
        if not api_name:
            keys = api_args.keys()
            if len(keys) != 1:
                raise ValueError('If api_name is not specified, api_args must '
                                 'have exactly one root key, which contains '
                                 'the API name.')
            api_name = keys[0]
            api_args = api_args[api_name]

        # Build NaElement request
        request = NaElement(api_name)
        if api_args:
            request.translate_struct(api_args)

        # Invoke API and return data as dict
        element_response = self.invoke_successfully(request, enable_tunneling)
        xml_response = element_response.to_string()
    ...
    def invoke_elem(self, na_element, enable_tunneling=False):
        ...
        try:
            ...
            raise NaApiError(e.code, e.reason)
            ...
            raise NaApiError('Unexpected error', repr(e))
    ...
    def _enable_tunnel_request(self, netapp_elem):
        ...
        if hasattr(self, '_vfiler') and self._vfiler:
            if hasattr(self, '_api_major_version') and \
                ...
                raise ValueError('ontapi version has to be at least 1.7'
        ...
        if hasattr(self, '_vserver') and self._vserver:
            if hasattr(self, '_api_major_version') and \
                ...
                raise ValueError('ontapi version has to be at least 1.15'
...
class NaElement(object):
    ...
    def __setitem__(self, key, value):
        """Dict setter method for NaElement.
           ...
           Accepts dict, list, tuple, str, int, float as valid value.
        """
        if key:
            if value:
...
def invoke_api(na_server, api_name, api_family='cm', query=None,
    ...
    if not (na_server or isinstance(na_server, NaServer)):
        msg = "Requires an NaServer instance."
        raise NAException.InvalidInput(reason=msg)
