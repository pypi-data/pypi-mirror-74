...
def create_kernel_body(
    python_pkgs: List[str], extra_python_pkgs: List[str], extra_apt_pkgs: List[str]
) -> str:
    return f"""
import subprocess
from pathlib import Path
...
def push_install_kernel(
    ...
    extra_apt_dependencies: List[str],
    quiet: bool = False,
) -> KernelPushResponse:
    ...
    kernel_body = create_kernel_body(
        dependencies, extra_dependencies, extra_apt_dependencies
    )
...
def install(
    ...
    extra_apt_dependencies = kkt.get("extra_apt_dependencies", [])
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        (target_dir / "apt").mkdir(exist_ok=True)
