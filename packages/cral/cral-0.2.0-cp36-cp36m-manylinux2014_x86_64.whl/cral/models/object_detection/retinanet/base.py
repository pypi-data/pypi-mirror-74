...
from .utils import __create_pyramid_features, __build_pyramid, __build_anchors, default_submodels, RetinanetConfig

from .postprocessing import  RegressBoxes, ClipBoxes, FilterDetections
...
def get_retinanet(
    ...
    assert feature_extractor in _ALLOWED_BACKBONES, '`{}` feature extractor is not yet supported, choose from below ::{}'.format(feature_extractor, '\n'.join(_ALLOWED_BACKBONES))

    if feature_extractor in ('efficientnetb0', 'efficientnetb1', 'efficientnetb2', 'efficientnetb3', 'efficientnetb4', 'efficientnetb5', 'efficientnetb6'):
    ...
    feature_extractor_model, preprocessing_function = classification_networks[feature_extractor](
        include_top=False, 
        weights=weights, 
        input_tensor=inputs, 
        pooling=None)
    ...
    if C3_name == None:
        C3_name = _DEFAULT_BACKBONE_FEATURE_EXTRACTOR_LAYER_NAMES[feature_extractor]['C3']

    if C4_name == None:
        C4_name = _DEFAULT_BACKBONE_FEATURE_EXTRACTOR_LAYER_NAMES[feature_extractor]['C4']

    if C5_name == None:
        C5_name = _DEFAULT_BACKBONE_FEATURE_EXTRACTOR_LAYER_NAMES[feature_extractor]['C5']
    ...
    return retinanet(inputs, C3, C4, C5, num_classes=num_classes, num_anchors=num_anchors_per_location, name='{}_retinanet'.format(feature_extractor)), preprocessing_function

def get_retinanet_fromconfig(feature_extractor, num_classes, config, weights='imagenet', base_trainable=True):
...
def get_prediction_model(model, config):
    ...
    if sorted(model.output_names) == ['classification', 'regression']:
        ...
        assert isinstance(config, RetinanetConfig),'Expected an instance of cral.models.object_detection.RetinanetConfig'
