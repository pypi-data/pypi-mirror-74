...
class RetinanetConfig:
    ...
    def __init__(self, 
        ...
        assert isinstance(sizes, list)  and len(sizes)==5, 'expected a list of 5 elements'
        sizes = list(map(int, sizes))

        assert isinstance(strides, list)  and len(strides)==5, 'expected a list of 5 elements'
    ...
    def check_equality(self,other):
        ...
        if isinstance(other,RetinanetConfig):
            ...
            for (key1,val1),(key2,val2) in zip(old_dict.items(),curr_dict.items()):
                if not  ( (key1==key2) and str(val1)==str(val2) ) :
                    if key1==key2:
                        assert False, f"Value of {key1} changes from {val1} --> {val2}"
                    else:
                        assert False, f"Value of ({key1} : {val1}) changes to ({key2} : {val2})"
                    return False
            return True
        return False
