...
and model versions. This is a lower level API than the :py:mod:`cral.tracking.tracking.fluent` module,
and is exposed in the :py:mod:`cral.tracking.tracking` module.
"""

from cral.tracking.entities import ViewType
from cral.tracking.exceptions import MlflowException
from cral.tracking.protos.databricks_pb2 import FEATURE_DISABLED
from cral.tracking.protos.service_lite_pb2 import LogParam, LogMetric 
from cral.tracking.store.tracking import SEARCH_MAX_RESULTS_DEFAULT
#from cral.tracking.tracking._model_registry.client import ModelRegistryClient
from cral.tracking.tracking.registry import UnsupportedModelRegistryStoreURIException
from cral.tracking.tracking._tracking_service import utils
from cral.tracking.tracking._tracking_service.client import TrackingServiceClient
from cral.tracking.utils import experimental

from cral.tracking.store.tracking.rest_store import RestStore
from cral.tracking.lite_extensions.client_utils import get_host_uri
...
class MlflowClient(object):
    ...
    def __init__(self, tracking_uri=None, registry_uri=None):
        ...
        :param tracking_uri: Address of local or remote tracking server. If not provided, defaults
                             to the service set by ``cral.tracking.tracking.set_tracking_uri``. See
                             `Where Runs Get Recorded <../tracking.html#where-runs-get-recorded>`_
                             for more info.
        :param registry_uri: Address of local or remote model registry server. If not provided,
                             defaults to the service set by ``cral.tracking.tracking.set_tracking_uri``.
        """
        final_tracking_uri = tracking_uri#tracking_uri or utils.get_tracking_uri()
    ...
    def _get_registry_client(self):
        ...
        :raises: py:class:`cral.tracking.exceptions.MlflowException` if the py:class:`ModelRegistryClient`
    ...
    def get_run(self, run_id):
        """
        Fetch the run from backend store. The resulting :py:class:`Run <cral.tracking.entities.Run>`
        contains a collection of run metadata -- :py:class:`RunInfo <cral.tracking.entities.RunInfo>`,
        as well as a collection of run parameters, tags, and metrics --
        :py:class:`RunData <cral.tracking.entities.RunData>`. In the case where multiple metrics with the
        same key are logged for the run, the :py:class:`RunData <cral.tracking.entities.RunData>` contains
        the most recently logged value at the largest step for each metric.

        :param run_id: Unique identifier for the run.

        :return: A single :py:class:`cral.tracking.entities.Run` object, if the run exists. Otherwise,
                 raises an exception.
        """
    ...
    def get_metric_history(self, run_id, key):
        ...
        :return: A list of :py:class:`cral.tracking.entities.Metric` entities if logged, else empty list
        """
        return self._tracking_client.get_metric_history(run_id, key)

    def create_run(self, experiment_id, start_time=None, tags=None):
        """
        Create a :py:class:`cral.tracking.entities.Run` object that can be associated with
        metrics, parameters, artifacts, etc.
        Unlike :py:func:`cral.tracking.projects.run`, creates objects but does not run code.
        Unlike :py:func:`cral.tracking.start_run`, does not change the "active run" used by
        :py:func:`cral.tracking.log_param`.

        :param experiment_id: The ID of then experiment to create a run in.
        :param start_time: If not provided, use the current timestamp.
        :param tags: A dictionary of key-value pairs that are converted into
                     :py:class:`cral.tracking.entities.RunTag` objects.
        :return: :py:class:`cral.tracking.entities.Run` that was created.
        """
        return self._tracking_client.create_run(experiment_id=experiment_id, start_time=start_time, tags=tags)

    def list_run_infos(self, experiment_id, run_view_type=ViewType.ACTIVE_ONLY):
        """:return: List of :py:class:`cral.tracking.entities.RunInfo`"""
        return self._tracking_client.list_run_infos(experiment_id, run_view_type)

    def list_experiments(self, view_type=None):
        """
        :return: List of :py:class:`cral.tracking.entities.Experiment`
        """
    ...
    def get_experiment(self, experiment_id):
        ...
        :return: :py:class:`cral.tracking.entities.Experiment`
        """
    ...
    def log_metric(self, run_id, key, value, timestamp=None, step=None):
        ...
        """
        metric = LogMetric(key=str(key), value=value, timestamp=timestamp, step=step)
    ...
    def list_artifacts(self, run_id, path=None):
        ...
        :return: List of :py:class:`cral.tracking.entities.FileInfo`
        """
    ...
    def set_terminated(self, run_id, status=None, end_time=None):
        ...
        :param status: A string value of :py:class:`cral.tracking.entities.RunStatus`.
    ...
    def search_runs(self, experiment_ids, filter_string="", run_view_type=ViewType.ACTIVE_ONLY,
        ...
        :param run_view_type: one of enum values ACTIVE_ONLY, DELETED_ONLY, or ALL runs
                              defined in :py:class:`cral.tracking.entities.ViewType`.
        ...
        :return: A list of :py:class:`cral.tracking.entities.Run` objects that satisfy the search
    ...
    def create_registered_model(self, name):
        ...
        :return: A single object of :py:class:`cral.tracking.entities.model_registry.RegisteredModel`
                 created by backend.
        """
    ...
    def update_registered_model(self, name, new_name=None, description=None):
        ...
        :return: A single updated :py:class:`cral.tracking.entities.model_registry.RegisteredModel` object.
        """
    ...
    def list_registered_models(self):
        ...
        :return: List of :py:class:`cral.tracking.entities.model_registry.RegisteredModel` objects.
        """
    ...
    def get_registered_model_details(self, name):
        """
        :param name: Name of the registered model to update.
        :return: A single :py:class:`cral.tracking.entities.model_registry.RegisteredModelDetailed` object.
        """
    ...
    def get_latest_versions(self, name, stages=None):
        ...
        :param stages: List of desired stages. If input list is None, return latest versions for
                       for ALL_STAGES.
        :return: List of `:py:class:`cral.tracking.entities.model_registry.ModelVersionDetailed` objects.
        """
    ...
    def create_model_version(self, name, source, run_id):
        ...
        :return: Single :py:class:`cral.tracking.entities.model_registry.ModelVersion` object created by
                 backend.
        """
    ...
    def get_model_version_details(self, name, version):
        ...
        :param name: Name of the containing registered model.
        :param version: Version number of the model version.
        :return: A single :py:class:`cral.tracking.entities.model_registry.ModelVersionDetailed` object.
        """
    ...
    def search_model_versions(self, filter_string):
        ...
        :param filter_string: A filter string expression. Currently supports a single filter
                              ...
                              ``run_id = '...'``.
        :return: PagedList of :py:class:`cral.tracking.entities.model_registry.ModelVersion` objects.
        """
