...
from cral.tracking.lite_extensions.client_utils import catch_mlflowlite_exception, _get_experiment_id, _get_run_id, _EXPERIMENT_ID_ENV_VAR, _RUN_ID_ENV_VAR, _runid_exists


from cral.tracking.entities import Run, RunStatus, Param, RunTag, Metric, ViewType
...
from cral.tracking.tracking.client import MlflowClient
from cral.tracking.tracking import artifact_utils
from cral.tracking.tracking.context import registry as context_registry
from cral.tracking.utils import env
from cral.tracking.utils.databricks_utils import is_in_databricks_notebook, get_notebook_id
from cral.tracking.utils.mlflow_tags import MLFLOW_PARENT_RUN_ID, MLFLOW_RUN_NAME
from cral.tracking.utils.validation import _validate_run_id
...
class ActiveRun(Run):  # pylint: disable=W0223
    """Wrapper around :py:class:`cral.tracking.entities.Run` to enable using Python ``with`` syntax."""
...
def start_run(run_name=None, nested=False):
    ...
    :param run_name: Name of new run (stored as a ``cral.tracking.runName`` tag).
                     Used only when ``run_id`` is unspecified.
    :param nested: Controls whether run is nested in parent run. ``True`` creates a nest run.
    :return: :py:class:`cral.tracking.ActiveRun` object that acts as a context manager wrapping
             the run's state.
    """
    ...
    experiment_id = str(_get_experiment_id()) #if isinstance(experiment_id, int) else experiment_id
    ...
    if len(_active_run_stack) > 0 and not nested:
        raise Exception(("Run with UUID {} is already active. To start a new run, first end the " +
                         "current run with cral.tracking.end_run(). To start a nested " +
...
def active_run():
    ...
    **Note**: You cannot access currently-active run attributes
    (parameters, metrics, etc.) through the run returned by ``cral.tracking.active_run``. In order
    to access such attributes, use the :py:class:`cral.tracking.tracking.MlflowClient` as follows:
...
def log_batch(metrics={}, params={}, tags={}, step=None):
    ...
    MlflowClient().log_batch(run_id=run_id, metrics=metrics_arr, params=params_arr, tags=tags_arr)
#TO DO: @pk00095
#remove this
...
def _get_or_start_run():
    ...
    return start_run() 
    #return start_run(run_id=run_id, experiment_id=experiment_id)
