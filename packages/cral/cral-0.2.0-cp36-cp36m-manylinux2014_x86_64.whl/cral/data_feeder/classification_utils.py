...
import glob, os, tqdm, json, math, sys,tempfile
import pandas as pd
import numpy as np
import cv2
...
from .utils import _bytes_feature, _float_feature, _int64_feature, _bytes_list_feature, _float_list_feature, _int64_list_feature
...
def create_tfrecords(meta_json, dataset_csv, out_path):
    ...
    train_set = dataset_df[dataset_df['train_only']==True]

    training_images_num = len(train_set)
    meta_info['num_training_images'] = training_images_num
    training_images_per_shard = int(math.ceil(training_images_num / _NUM_SHARDS))
    ...
    for index, row in train_set.iterrows():
      ...
      if label_str not in label_file_pointer:
        ...
        label_file_pointer[label_str] = tf.io.TFRecordWriter(os.path.join(out_path,'statistics',label_str)+'.tfrecord')
    ...
    test_set = dataset_df[dataset_df['train_only']==False]
...
def parse_tfrecords(filenames, height, width, num_classes, processing_func=None, augmentation=None, batch_size=32, num_repeat=-1):
        ...
        def decode_cast_resize(image_string):
          image = tf.cast(tf.image.decode_jpeg(image_string), tf.keras.backend.floatx())
        ...
        def _parse_function(serialized): #, height=height, width=width, num_classes=num_classes):
            ...
            # image = tf.map_fn(decode_cast_resize, parsed_example['image_raw'], dtype=tf.keras.backend.floatx())
            # one_hot_label = tf.map_fn(make_onehot, tf.cast(parsed_example['image_label'], tf.int32), dtype=tf.keras.backend.floatx())

            #image_height, image_width = parsed_example['image/height'], parsed_example['image/width']
            ...
            for batch_index in range(batch_size):
                ...
                image = tf.cast(tf.image.decode_jpeg(parsed_example['image_raw'][batch_index]), tf.keras.backend.floatx())
                image = tf.image.resize(image, size=(height, width))
                #apply augmentation here
                image = tf.numpy_function(func=augment_image, inp=[image], Tout=tf.keras.backend.floatx())

                if processing_func != None:
                  #image = tf.py_function(func=processing_func, inp=[image], Tout=tf.keras.backend.floatx())#(image/127.5) - 1
                  image = tf.numpy_function(func=processing_func, inp=[image], Tout=tf.keras.backend.floatx())#(image/127.5) - 1

                image.set_shape([None, None, 3])

                #return {'input_1':image}, lbl #use sparse categorical cross entropy

                one_hot_label = tf.one_hot(tf.cast(lbl, tf.int32), depth=num_classes)

                image_batch.append(image)
                label_batch.append(one_hot_label)

            return tf.convert_to_tensor(image_batch), tf.convert_to_tensor(label_batch)
