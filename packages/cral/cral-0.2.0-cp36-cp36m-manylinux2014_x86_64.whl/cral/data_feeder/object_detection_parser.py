...
class DetectionBase(ABC):
    ...
    @tf.function
    @abstractmethod
    def yield_image_regression_classification(self, image_batch, bboxes_batch, labels_batch):
        return image_batch, bboxes_batch, labels_batch

    @tf.function
    def normalize_image_batch(self, image):
      return tf.numpy_function(self.normalize_image, [image],Tout=tf.keras.backend.floatx())
    ...
    def _parse_function(self, serialized):
        ...
        scale = tf.cond(largest_side * tf.cast(scale, tf.int32) > self.max_side, lambda: self.max_side / largest_side, lambda: scale)
        ...
        regression_batch, classification_batch = self.yield_image_regression_classification(xmin_batch, ymin_batch, xmax_batch, ymax_batch, label_batch, image_batch)

        normalized_image_batch = tf.map_fn(lambda x: self.normalize_image_batch(x), image_batch, dtype=tf.keras.backend.floatx()) 

        return normalized_image_batch , {'regression':regression_batch, 'classification':classification_batch}
...
class TfrecordParser(DetectionBase):
    ...
    def yield_image_regression_classification(self, xmin_batch, ymin_batch, xmax_batch, ymax_batch, label_batch, image_batch):
        ...
        for index in range(self.batch_size):
            ...
            image = tf.numpy_function(print_annotations_dims, [image, bboxes, labels], Tout=tf.uint8)
