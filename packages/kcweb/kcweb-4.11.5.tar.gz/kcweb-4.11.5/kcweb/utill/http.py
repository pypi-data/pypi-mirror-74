...
class Http:
    ...
    def openurl(self,url,method="GET",data=None,files=None,allow_redirects=True):
        """模拟浏览器请求
        ...
        if self.set_cookies and isinstance(self.set_cookies,str):
            self.cookieserTdict()
        response=self.req.request(method, url,data=data,files=files,proxies=self.set_proxies,cookies=self.set_cookies,headers=self.set_header,timeout=self.set_timeout,verify=self.set_verify,allow_redirects=allow_redirects)
