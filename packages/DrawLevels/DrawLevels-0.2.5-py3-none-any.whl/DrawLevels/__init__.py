...
class Scheme:
  ...
  def Draw(self, Filename):
	...
	#=========== DRAW LEVELS ============
    for L in self.Levels:
        ctx = DrawLevel(ctx, HEIGHT - L.Energy, 0.1 * self.WIDTH, self.WIDTH - 0.1 * self.WIDTH, L.Colour, L.Width, L.Label, self.FontSize)

	#=========== DRAW TRANSITIONS ============
    for T in self.Transitions:
        ctx = DrawTransition(ctx, x, x, HEIGHT - T.Start, HEIGHT - T.End, T.Colour, T.Width, T.Label, self.FontSize)
        x = x + dx
