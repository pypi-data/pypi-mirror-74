...
#
# prod_cert = config_parameters['PROD_CERT']
# prod_key = config_parameters['PROD_KEY']
...
class IoTService:
	...
	def __init__(self, write_key=None, device_info={}):
		...
		self.secure_cert_path = 'axon' + self.config_parameters['SECURE_CERT_PATH']
		...
		self.unique_id = self.config_parameters['CLIENT_ID']
	...
	def core_connect(self):
		...
		# While this pre-emptively sets is_connected before callback, this should work for now
	...
	def core_publish(self, data):
		...
		# topic = self.unique_id
		self.primary_MQTTClient.publish(topic, json.dumps(data), 0)
	...
	async def get_official_certs(self, callback, isRotation=False):
		...
		return await self.orchestrate_provisioning_flow(callback)
	...
	async def orchestrate_provisioning_flow(self, callback):
		...
		# Subscribe to Register Thing response (Before bootstrap credentials get switched out)
	...
	def assemble_certificates(self, payload):
		...
		print("Registering Thing: " + self.unique_id)
	...
	def basic_callback(self, client, userdata, msg):
		"""Method responding to the device-data publish attempt. Demonstrating a successful pub/sub with new certificate.
		"""
	...
	def new_cert_pub_sub(self):
		"""Method testing a call to the 'device-data' topic (which was specified in the policy for the new certificate)
		"""
		topic = self.device_topic()
		# topic = self.unique_id
