...
class DataAccessLayer:
	def __init__(A,db_url,cacert_param=_A,cert_param=_A,key_param=_A,yl_obj=_A,app_ns=_A,opaque=_A):
		F=yl_obj;E=key_param;D=cert_param;C=cacert_param;B=db_url;A.app_ns=_A;A.engine=_A;A.metadata=_A;A.leafrefs=_A;A.referers=_A;A.ref_stat_collectors=_A;A.global_root_id=_A;A.table_keys=_A;A.schema_path_to_real_table_name=_A;A.config_false_prefixes=_A
	...
	async def handle_get_config_request(C,data_path):
		A=data_path;assert A!='';assert not(A!=_B and A[-1]==_B);B=re.sub(_F,'',A);F=await C.handle_get_data_request(A,B,ContentType.CONFIG_TRUE);D=re.sub(_N,'',B)
	...
	async def handle_post_config_request(B,data_path,request_body,create_callbacks,change_callbacks,opaque):
		C=request_body;A=data_path;assert A!='';assert not(A!=_B and A[-1]==_B);assert type(C)==dict
		with B.engine.begin()as D:
			E=B._get_dbpath_for_data_path(A,ContentType.CONFIG_TRUE,D)
			if E==_A:raise ParentNodeNotFound(_Q+A+_M)
	...
	async def _recursively_post_subtable_data(B,pid,data_path,req_body_iter,jsob,jsob_data_path,create_callbacks,opaque,conn):
		...
		if type(C)==dict:
			...
			for P in C.copy():K=E+_B+P;R=await B._recursively_post_subtable_data(pid,K,C[P],M,N,D,I,J)
		elif type(C)==list and A in B.table_keys:
			if A in B.table_keys:
				while C:
					F=C.pop(0);assert type(F)==dict;S=re.sub(_N,'',A);L={};L[S]=F;G=B.table_keys[A];H={};H[_H]=pid;H[G]=F[G];H[_K]=L;T=B.schema_path_to_real_table_name[A];U=B.metadata.tables[T]
					try:Q=J.execute(U.insert().values(**H))
					except sa.exc.IntegrityError as V:raise NodeAlreadyExists(_R+G+'" with value "'+H[G]+_S)
	...
	async def handle_delete_config_request(B,data_path,delete_callbacks,change_callbacks,opaque):
		A=data_path;assert A!='';assert A!=_B;assert A[-1]!=_B
