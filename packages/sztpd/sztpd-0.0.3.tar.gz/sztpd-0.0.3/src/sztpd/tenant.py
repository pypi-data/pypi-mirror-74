...
class TenantViewHandler(RouteHandler):
	...
	async def handle_post_config_request(G,request):
		...
		P={_U+O:F[H]};A=await G.native.handle_post_config_request_lower_half(J,D.query_string,P)
		if A.status!=201:
	...
	async def handle_put_config_request(G,request):
		...
		if B==_A:
			...
			I={_U+S:E[K]}
		A=await G.native.handle_put_config_request_lower_half(L,F.query_string,I)
		if A.status!=204:
