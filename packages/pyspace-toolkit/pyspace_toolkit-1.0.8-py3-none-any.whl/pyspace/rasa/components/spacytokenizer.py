...
from pyspace.nlp.toolkit.spacy import SpacyRegexTokenizer, SpacyMatcherMerger, SpacyStanzaPipeline
...
class SpacyTokenizer(Tokenizer, SparseFeaturizer, EntityExtractor):

    
    defaults = {
        "regex_tokenizer_bool":True,
        "emoji_bool": True,
        "stanza_bool": True,
        "matcher_bool": True,
    }
    ...
    def __init__(
        ...
        self.regex_tokenizer_bool = self.component_config["regex_tokenizer_bool"]
    ...
    def _apply_spacy(self, message: Message, attribute: Text) -> None:
                    ...
                    if attribute == INTENT:
                        ...
                        if attribute == TEXT:
                            ...
                            if entities:
                                ...
                                alignments = align_fuzzy_multi([e['value'] for e in entities], [t.text for t in doc], jaro_similarity)

                                aligned_entities = []
                                for alig in alignments:
                                    for token_idx in alig[4]:

                                        input_entities = [entities[entity_idx] for entity_idx in alig[3]]

                                        role = [ie['role'] for ie in input_entities if 'role' in ie ]
                                        role = role[-1] if role != [] else []
                                        entity = [ie['entity'] for ie in input_entities ][-1]

                                        token = doc[token_idx]
                                        start = len(''.join([t.text for t in doc[:token_idx]])) + token_idx
                                ...
                                explain_entities = False

                                if explain_entities:

                                    print()
                                    print()
                                    print('input',[(e['value'],e['entity'],e['start'],e['end'])  for e in entities])
                                    print([t.text for t in doc])
                                    print()
                                    [print(item) for item in alignments]
                                    print()
                                    print('output',[(e['value'],e['entity'],e['start'],e['end'])  for e in aligned_entities])
                                    print()

                                message.set(ENTITIES, aligned_entities)
