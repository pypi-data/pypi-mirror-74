...
class SpacyRegexTokenizer():
    ...
    def __init__(self, nlp, replace_patterns=[], token_patterns=[]):

        self.nlp = nlp
        self.replace_patterns = replace_patterns
        self.token_patterns = token_patterns
        self.base_tokenizer = self.custom_tokenizer(self.nlp, self.token_patterns)

    def __call__(self, text):
        for p, r in self.replace_patterns:
            text = re.sub(p, r, text, flags=re.I)
