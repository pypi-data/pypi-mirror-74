...
from ..compat import MappingProxyType, lru_cache
...
class Objective:
    ...
    def __str__(self):
        if self._function is None:
            return "find a variable assignment"
        else:
            dir_str = "minimize" if self._direction == self.MIN else "maximize"
            return "{} {}".format(dir_str, self._function.string)
...
class Problem(object):  # object inheritence necessary with Python 2.
    ...
    def __init__(self, copyOptions=None, useOptions=None, **extra_options):
        ...
        #: Maps variables to number of occurences in objective or constraints.
        self._var_count = {}
    ...
    def variables(self):
        ...
        :returns:
            ...
            the :class:`Problem` instance as well as on the variables' names.
        """
    ...
    def constraints(self):
        ...
        # Clear variable registry except for the objective function variables.
        self._variables.clear()
        self._var_count.clear()
        self._register_variables(self.no.function.variables)
    ...
    def objective(self, value):
        self._unregister_variables(self.no.function.variables)
    ...
    def no(self):
        ...
        The same as the :attr:`~picos.Objective.normalized` attribute of the
        :attr:`objective`.
        """
    ...
    def value(self):
        ...
        If all variables that appear in the objective function are valued, in
        particular after a successful solution search, this is the numeric value
        of the objective function. If the objective function is not fully valued
        or if the problem is a feasiblity problem without an objective function,
        this is :obj:`None`.

        .. note::
            The Python special functions :class:`int`, :class:`float`,
            :class:`complex` and :func:`round` as well as the special method
            ``__index__`` make use of this value when applied to a
            :class:`Problem`.
        """
    ...
    def strategy(self):
        ...
        When a strategy is reused, modifications to the variables, constraints,
        and objective of a problem are passed step by step through the
        strategy's reformulation pipeline while existing reformulation work is
        not repeated. If the solver also supports these kinds of updates, then
    ...
    def dual(self):
        ...
        This property is intended for educational purposes. If you want to solve
        the primal problem via its dual, use the :ref:`dualize <option_dualize>`
        option instead.
        """
        return self.prepared(solver="cvxopt", dualize=True)
    ...
    def _var_groups(self):
        """Support :meth:`_var_group_string`."""
        vars_by_type = {}
    ...
    @lru_cache()
    def _var_group_string(self, group):
    ...
    def _con_group_string(self, group):
        ...
        try:
            template, data = parameterized_string([str(c) for c in group])
        except ValueError:
    ...
    def __str__(self):
        ...
        if objDir == "max":
            ...
            if self._constraints:
                string += "  find an assignment\n"
        else:
            assert False, "Unexpected objective direction '{}'.".format(objDir)
        ...
        # Print variables.
        if self.variables:
            string += "  {}\n".format("for" if objDir == "find" else "over")
            for group in self._var_groups:
                string += wrapper.fill(self._var_group_string(tuple(group)))
                string += "\n"
        ...
        # Enclose with lines.
        width = max(len(line) for line in string.splitlines())
        lines = "-"*width

        return "{}\n{}{}".format(lines, string, lines)

    def __index__(self):
        return self._objective.__index__()

    def __int__(self):
        return self._objective.__int__()

    def __long__(self):
        # Python 2 only.
        return self._objective.__long__()

    def __float__(self):
        return self._objective.__float__()

    def __complex__(self):
        return self._objective.__complex__()

    def __round__(self, ndigits=None):
        return self._objective.__round__(ndigits)

    # --------------------------------------------------------------------------
    # Bookkeeping methods.
    # --------------------------------------------------------------------------

    def _register_variables(self, vars):
        """Register the variables of an objective function or constraint."""
        # Register every variable at most once per call.
        if not isinstance(vars, (set, frozenset)):
            raise TypeError("Variable registry can (un)register a variable "
                "only once per call, so the argument must be a set type.")

        # Retrieve old and new variables as mapping from name to object.
        oldVars = self._variables
        newVars = OrderedDict(
            (var.name, var) for var in sorted(vars, key=(lambda v: v.name)))

        # Check for variable name clashes within the new set.
        if len(newVars) != len(vars):
            raise ValueError(
                "The object you are trying to add to a problem contains "
                "multiple variables of the same name. This is not allowed.")

        # Check for variable name clashes with existing variables.
        for name in set(oldVars).intersection(set(newVars)):
            if oldVars[name] is not newVars[name]:
                raise ValueError("Cannot register the variable {} with the "
                    "problem because it already tracks another variable with "
                    "the same name.".format(name))

        # Keep track of new variables.
        self._variables.update(newVars)

        # Count up the variable references.
        for var in vars:
            self._var_count.setdefault(var, 0)
            self._var_count[var] += 1

    def _unregister_variables(self, vars):
        """Unregister the variables of an objective function or constraint."""
        # Unregister every variable at most once per call.
        if not isinstance(vars, (set, frozenset)):
            raise TypeError("Variable registry can (un)register a variable "
                "only once per call, so the argument must be a set type.")
        ...
        for var in vars:
            ...
            # Make sure the variable is properly registered.
            assert name in self._variables and var in self._var_count, \
                "Tried to unregister a variable that is not registered."
            assert self._var_count[var] >= 1, \
                "Found a nonpostive variable count."

            # Count down the variable references.
            self._var_count[var] -= 1

            # Remove a variable with a reference count of zero.
            if not self._var_count[var]:
                self._var_count.pop(var)
    ...
    def add_constraint(self, constraint, key=None):
        ...
        # Register the constraint's variables.
        self._register_variables(constraint.variables)
    ...
    def add_list_of_constraints(self, lst, it=None, indices=None, key=None):
        ...
        :param list(picos.constraints.Constraint) lst: Constraints to add.
        :param it: DEPRECATED
    ...
    # TODO: Add Problem.require replacing add_constraint and
    ...
    def remove_constraint(self, idOrIndOrCon):
        ...
        # Unregister the variables added by the removed constraints.
        for con in removedCons:
            self._unregister_variables(con.variables)
    ...
    # TODO: Document removal of Problem.remove_all_variable_bounds.
    # --------------------------------------------------------------------------
    ...
    def get_valued_variable(self, name):
        ...
        variable's :attr:`values <picos.expressions.Expression.value>` instead
    ...
    def copy(self):
        ...
        theCopy = Problem(copyOptions=self._options)

        # Duplicate the variables.
        newVars = {name: var.copy() for name, var in self.variables.items()}

        # Make copies of constraints on top of the new variables.
        for group in self._con_groups:
            theCopy.add_list_of_constraints(
                constraint.replace_variables(newVars) for constraint in group)

        # Make a copy of the objective on top of the new variables.
        direction, function = self._objective
        if function is not None:
            theCopy.objective = direction, function.replace_variables(newVars)

        return theCopy

    def continuous_relaxation(self, copy_continuous_variables=True):
        """Return a continuous relaxation of the problem.

        This is done by replacing integer variables with continuous ones.

        :param bool copy_continuous_variables:
            Whether variables that are already continuous should still be
            copied. If this is :obj:`False`, then the relxation shares these
            variables with the original problem.
        """
        theCopy = Problem(copyOptions=self._options)
        ...
        for name, var in self.variables.items():
            if isinstance(var, expressions.CONTINUOUS_VARTYPES):
                if copy_continuous_variables:
                    newVars[name] = var.copy()
                else:
                    newVars[name] = var
            elif isinstance(var, expressions.IntegerVariable):
                newVars[name] = expressions.RealVariable(
                    name, var.shape, var._lower, var._upper)
            elif isinstance(var, expressions.BinaryVariable):
                newVars[name] = expressions.RealVariable(name, var.shape, 0, 1)
    ...
    def clone(self, copyOptions=True):
        ...
        The copy is constrained by the same constraint objects and thereby
        references the existing variables that appear in these constraints.

        The clone can be modified to describe a new problem, but whenever a
        solution is applied to its variables, some of these variables can be the
        same that still appear in the original problem. If this is not a problem
        to you, then cloning can be much faster than copying.

        :param bool copyOptions: Whether to make an independent copy of the
            problem's options. Disabling this will apply any option changes to
            the original problem as well but yields a (very small) reduction
            in cloning time.
        """
        ...
        # Make the variable registry independent.
        theClone._variables = self._variables.copy()
        theClone._var_count = self._var_count.copy()
    ...
    def prepared(self, steps=None, **extra_options):
        ...
        :param int steps:
            ...
            reformulation :meth:`~picos.reforms.ExtraOptions` is executed, which
            may also output the problem itself, depending on ``extra_options``.
        ...
        if steps == 0:
            ...
            raise ValueError("The pipeline {} has only {} reformulation steps "
                "to choose from.".format(strategy, numReforms, steps))
    ...
    def solve(self, **extra_options):
        ...
        access the solution as follows:
            ...
            - The variables' :attr:`~picos.expressions.Expression.value` is set
              according to the primal solution. You can in fact query the value
              of any expression involving valued variables like this.
            - The constraints' :attr:`~picos.constraints.Constraint.dual` is set
        ...
        with picos_box(show=verbose):
            ...
            if not self._strategy:
                if verbose:
                    ...
                    " for {}".format(get_solver(options.solver).names()[1])
                    if options.solver else ""))
    ...
    def minimize(self, obj, **extra_options):
        ...
        :type obj: :class:`~picos.expressions.Expression`
        :param extra_options: A sequence of additional solver options.
    ...
    def check_current_value_feasibility(self, tol=1e-5, inttol=None):
        ...
        :param float tol: Largest tolerated absolute violation of a constraint
            or variable bound. If ``None``, then the ``abs_prim_fsb_tol`` solver
            option is used.
        :param inttol: DEPRECATED
        ...
        all_cons += [
            variable.bound_constraint for variable in self.variables.values()
        ...
        for constraint in all_cons:
            ...
            # TODO: Add a "violation" interface to constraints?
            if isinstance(constraint, constraints.LMIConstraint):
    ...
    def numberOfVars(self):
        ...
        return sum(variable.dim for variable in self.variables.values())
    ...
    def set_var_value(self, name, value):
        """Set the :attr:`~picos.expressions.Expression.value` of a variable.
