...
class DashBoard(object):
    ...
    def _formatAsStr(self, stage, abbr, value, global_mile=-1):
        ...
        if mode == 'raw':
            ...
            if global_mile >= 0:
                misc.imsave(os.path.join(self.param['log_path'], '%s/%s_%d.jpg'%(stage, abbr, global_mile)), value)
            return ''
        elif mode == 'tailor':
    ...
    def _log(self, gauge=True, tachograph=True, record=None):
        if gauge:
            boards = {}
            # clustering
