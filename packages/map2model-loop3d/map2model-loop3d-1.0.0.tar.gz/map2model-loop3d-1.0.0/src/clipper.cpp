/*******************************************************************************
 *                                                                              *
 * Author    :  Angus Johnson * Version   :  6.2.1 * Date      :  31 October
 *2014                                                 * Website   :
 *http://www.angusj.com                                           * Copyright :
 *Angus Johnson 2010-2014                                         *
 *                                                                              *
...
/******************************************************************************
 * Modified by Vitaliy Ogarko (2016) to get information on
 * what edges of a polygon the intersecting points belong to.
 *******************************************************************************/
...
bool SlopesEqual(const TEdge &e1, const TEdge &e2, bool UseFullInt64Range) {
  ...
  if (UseFullInt64Range)
    return Int128Mul(e1.Delta.Y, e2.Delta.X) ==
...
void Clipper::InsertScanbeam(const cInt Y) {
  // if (!m_Scanbeam.empty() && Y == m_Scanbeam.top()) return;// avoid
  // duplicates.
  m_Scanbeam.push(Y);
}
//------------------------------------------------------------------------------
...
OutPt *Clipper::AddLocalMinPoly(TEdge *e1, TEdge *e2, const IntPoint &Pt) {
  ...
  if (prevE && prevE->OutIdx >= 0 && (TopX(*prevE, Pt.Y) == TopX(*e, Pt.Y)) &&
      SlopesEqual(*e, *prevE, m_UseFullRange) && (e->WindDelta != 0) &&
      (prevE->WindDelta != 0)) {
...
void Clipper::SetZ(IntPoint &pt, TEdge &e1, TEdge &e2) {
  ...
  TEdge2 edge1 = TEdge2(e1.Top, e1.Bot);
  TEdge2 edge2 = TEdge2(e2.Top, e2.Bot);

  if (e1.PolyTyp == ptClip) {
    intersect.PolygonEdge = edge1;
    intersect.FaultEdge = edge2;
  } else {
    intersect.PolygonEdge = edge2;
    intersect.FaultEdge = edge1;
  }
  ...
  while (e) {
    if (IsIntermediate(e, topY)) {
      ...
      if (ePrev && ePrev->Curr.X == e->Bot.X && ePrev->Curr.Y == e->Bot.Y &&
          ...
          SlopesEqual(*e, *ePrev, m_UseFullRange) && (e->WindDelta != 0) &&
          (ePrev->WindDelta != 0)) {
