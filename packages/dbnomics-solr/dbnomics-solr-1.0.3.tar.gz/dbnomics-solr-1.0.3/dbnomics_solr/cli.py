...
def index_provider(
    ...
    logger.info("Using indexed_at %r for all documents", indexed_at)
    ...
    if not desired_datasets_codes_actions:
        ...
        response = requests.post(
            "{}/update/json/docs".format(solr_url), data=dataset_solr_iter
        )
...
def delete_obsolete_documents(
    ...
    if datasets_codes_to_delete:
        ...
        for dataset_code in datasets_codes_to_delete:
            ...
            solr.delete(
                q=provider_criteria & dataset_criteria, commit=False,
            )

    # Delete obsolete series of indexed datasets older than indexed_at.
    ...
    # Delete obsolete datasets (and their series) older than indexed_at.
    if full and not datasets and not excluded_datasets:
        type_criteria = Q(type="dataset") | Q(type="series")
        obsolete_docs_query = (
            type_criteria & Q(provider_code=provider_code) & ~Q(indexed_at=indexed_at)
        )
        nb_obsolete_docs = solr.search(q=obsolete_docs_query).hits
        if nb_obsolete_docs > 0:
            logger.info(
...
def iter_dataset_solr(
    solr, repo, storage, provider_json, indexed_at, desired_datasets_codes_actions,
):
    ...
    for dataset_index, (dataset_code, action) in enumerate(
        ...
        if action == INDEXATION:
            ...
            for series_json, metadata in dataset_dir.iter_series_json(
...
def get_dataset(solr, provider_code, dataset_code):
    ...
    results = solr.search(
        q=Q(type="dataset", provider_code=provider_code, code=dataset_code), rows=1,
    )
