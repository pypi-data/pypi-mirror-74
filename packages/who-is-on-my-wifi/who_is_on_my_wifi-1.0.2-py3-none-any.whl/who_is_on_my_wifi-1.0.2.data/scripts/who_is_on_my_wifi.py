...
def license():
    ...
    pth = pth+"/who_is_on_my_wifi-1.0.2.dist-info/"
...
def device():
    ...
    DeviceList = [nam, product_name, MAC, IP_host, IP_All, hostname, WifiName]
    return DeviceList
...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter, description=string, epilog=textwrap.dedent("""
                                        ...
                                        """))
    parser.add_argument('-v', '--version', action='version', version='%(prog)s 1.0.2', help='show current version')
