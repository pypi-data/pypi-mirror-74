...
class MetricsPusher:
    ...
    def __init__(self,
        ...
        if not metrics_service_name:
            metrics_service_name = environ['METRICS_SERVICE_NAME']
        if not metrics_service_port:
            metrics_service_port = environ['METRICS_SERVICE_PORT']
        if not metrics_pod_port:
            metrics_pod_port = environ['METRICS_POD_PORT']
    ...
    def push_metrics(self, registry):
        for gateway in self.metrics_pod_ips:
            try:
                push_to_gateway(gateway, job=self.job, registry=registry)
