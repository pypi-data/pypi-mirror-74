...
class Trainer:
    ...
    def initialize_config(self, connection_params, config_file_path="./config.ini", eslib=None, classifier=False):
        ...
        # MINIO
        try:
            if str(helpers.config['MINIO']["secure"]).lower() == "true":
                helpers.minioClient = Minio(minio_host + ':' + minio_port, minio_access, minio_secret, secure=True, region="eu-west-1")
            else:    
                helpers.minioClient = Minio(minio_host + ':' + minio_port, minio_access, minio_secret, secure=False, region="eu-west-1")
        ...
        if eslib is None:
            ...
            if elasticsearch_password != "" and elasticsearch_username != "":
                ...
                ok, _es = eslib.start_connection_secure(elasticsearch_host, elasticsearch_port, elasticsearch_username, elasticsearch_password)
            else:
    ...
    def initialize(self, connection_params, classifier=False):
        ...
        if elasticsearch_password != "" and elasticsearch_username != "":
            self.logger.debug("Secured elasticsearch connection")
            ok, _es = self.eslib.start_connection_secure(elasticsearch_host, elasticsearch_port, elasticsearch_username, elasticsearch_password)
        else:
