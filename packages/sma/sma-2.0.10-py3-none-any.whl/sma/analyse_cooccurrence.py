...
def cooccurrenceTableFull(G : nx.Graph, 
    ...
    a dictionary which maps vertices to subdictionaries mapping motif classes (taken
    from the classificator) to the integer representing the number of occasions this
    vertic occurs in a motif of this class. If ``to_array`` is flipped, the result 
    ...
    :returns: cooccurrences table featuring values for all classes of motifs
...
def identifyGaps(G : nx.Graph, 
    ...
    in the specified motif (the motif is *open*), the contribution is definied the
