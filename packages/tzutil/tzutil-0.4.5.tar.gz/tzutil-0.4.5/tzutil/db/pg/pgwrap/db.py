...
class SafeNamedTupleCursor(NamedTupleCursor):
  def _make_nt(self, namedtuple=namedtuple):
    return namedtuple(
      "Record", [d[0] for d in self.description or ()],
      rename=True
    )
...
class connection(object):
  def __init__(
    ...
    params = urllib.parse.urlparse(
      url or os.environ.get('DATABASE_URL')
  ...
  def prepare(
    self, statement, params=None, name=None, call_type=None
  ):
    """
            >>> db = connection()
    ...
    with self.cursor() as c:
      c.execute(
        'PREPARE %s %s AS %s' % (name, params, statement)
      )
  ...
  def cursor(self, cursor_factory=None):
    return cursor(
      self.pool, cursor_factory or self.default_cursor,
...
class cursor(object):
  ...
  def __enter__(self, name=None):
    ...
    self.cursor = self.connection.cursor(
      name=name, cursor_factory=self.cursor_factory
    )
    if self.hstore:
  ...
  def _build_select(
    self, table, where, order, columns, limit, offset, update
  ):
  ...
  def select(
    ...
    return self.query(
      self._build_select(
        table, where, order, columns, limit, offset, update
      ), where
    )

  def count_one(self, *args, **kwds):
    return self.count(*args, **kwds)[0].count

  def count(self, table, where=None):
    return self.query(
      f"SELECT COUNT(1) FROM {table} {sqlop.where(where)}", where
    )

  def get(self, table, id, column):
    return self.select_one(table, dict(id=id),
  ...
  def _build_join(
    self, tables, where, on, order, columns, limit, offset
  ):
    ...
    return 'SELECT %s FROM %s ' % (sqlop.columns(columns), tables[0]) + \
        ...
        sqlop.where(where) + sqlop.order(order) + \
        sqlop.limit(limit) + sqlop.offset(offset)
  ...
  def join(
    ...
    return self.query(
      self._build_join(
        tables, where, on, order, columns, limit, offset
      ), where
    )
  ...
  def insert(self, table, values, returning=None, on_conflict=0):
    """
            >>> db = connection()
            ...
            3
        """
    _values = ['%%(%s)s' % v for v in list(values.keys())]
    sql = 'INSERT INTO %s (%s) VALUES (%s)' % (
      table, ','.join(list(values.keys())), ','.join(_values)
    )
...
if __name__ == '__main__':
  ...
  tables = (
    (
      'doctest_t1', '''id SERIAL PRIMARY KEY,
                               name TEXT NOT NULL,
