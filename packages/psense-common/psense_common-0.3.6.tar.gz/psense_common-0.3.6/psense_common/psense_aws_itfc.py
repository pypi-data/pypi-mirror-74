...
class PSenseAWSInterface(object):
    ...
    def verif_experiment(self, experiment_name=""):
        ...
        try:
            ...
            assert len(self.experiment_info["sensor"]["electrodes"]) > 0
            assert isinstance(self.experiment_info["sensor"]["electrodes"][0], dict)
        except BaseException:
            log.info("[{}] invalid.".format(experiment_name))
            self.exp_is_valid = False
            return False
    ...
    def add_bulk_sensordata(self, data):
        ...
        for sample in data:
            ...
            if len(requests) >= self.batch_req_limit:
                log.info(
                    "batch of [{}] items. First item: {}".format(
                ...
                try:
                    response = self.db.batch_write_item(
                        RequestItems={"SensorData": requests}
                    )
        ...
        try:
            log.info(
                "batch of [{}] items. Last item: {}".format(len(requests), requests[-1])
            )

            if self.test_mode:
                pass

            response = self.db.batch_write_item(RequestItems={"SensorData": requests})
            log.debug("response: {}".format(response))
        except BaseException:
            return False
