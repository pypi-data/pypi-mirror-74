...
class AuditQuerySet(CommonQuerySet):
    def _check_ed_izm_qty(self, **kwargs):
        if kwargs.get('ed_izm') != None and kwargs.get('qty') == None or kwargs.get('ed_izm') == None and kwargs.get('qty') != None:
            raise Exception(f'ed_izm and qty not be along')
        ...
        if isinstance(_criteria, list):
            for criterion in _criteria:
                if criterion.get('fieldName') == 'parent_id':
                    ...
                    if (item_id, int):
                        criteria_lst = [criterion for criterion in _criteria if criterion.get('fieldName') == 'parent_id' and criterion.get('value') != None]
    ...
    def update(self, **kwargs):
        if kwargs.get('lastmodified') == None:
