...
class DSRequest(RPCRequest):
    ...
    def check_old(self, request):
        ...
        if self.user == None:
            ...
            try:
                self.ws_channel = request.GET.get('ws_channel')
                self.ws_port = request.GET.get('ws_port')
                self.host = request.GET.get('host')

                if self.user_id == None:
                    self.user_id = request.GET.get('user_id')

                if self.user_id == None and self.ws_channel != None:
                    self.user_id = User.objects.get(username=self.ws_channel.split('_')[1]).id
                if self.user_id != None:
                    self.user = User.objects.get(id=self.user_id)
                    self.is_admin = self.user.is_admin
                    self.is_develop = self.user.is_develop
                else:
                    self.user = None
    ...
    def __init__(self, request):
        ...
        if request != None:
            if request.body != b'':
                ...
                self.check_old(request=request)
            else:
                self.check_old(request=request)
