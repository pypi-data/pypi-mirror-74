...
from isc_common import setAttr, Wrapper
...
class Body:
    ...
    def __init__(self, body, session):
        ...
        if not isinstance(body, dict):
            raise Exception('body must by dict')

        self.body = BodyWrapper(**body)
        if self.body.transaction != None:
        ...
        if self.body.httpHeaders != None:
            self.body.httpHeaders = Wrapper(**self.body.httpHeaders)
        elif self.body.data != None and self.body.data.get('httpHeaders') != None:
            self.body.httpHeaders = Wrapper(**self.body.data.get('httpHeaders'))

    @property
    def login(self):
        from isc_common.auth.models.user import User
        if self.body.login != None:
            return self.body.login
        elif self.body.httpHeaders != None and self.body.httpHeaders.USER_ID:
            return User.objects.get(id=self.body.httpHeaders.USER_ID).username
        else:
            if self.session != None and self.session._session != None:
                username = self.session._session.get('ws_channel').split('_')[1]
                return username
