...
from azureml.automl.core.shared.forecasting_exception import (GrainAbsent,
                                                              InvalidTsdfArgument,
                                                              WrongShapeDataError)
from azureml.automl.core.shared.reference_codes import ReferenceCodes
from azureml.automl.core.shared.types import GrainType
from azureml.automl.runtime.shared import metrics_utilities as metrics_utils
...
class LightGBMClassifier(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=None, n_jobs=1, problem_info=None, **kwargs):
        ...
        if self._min_data_str not in kwargs and \
            ...
            raise InvalidArgumentException.create_without_pii(
                "Failed to initialize LightGBMClassifier. Neither min_data_in_leaf nor min_child_samples passed",
                target='LightGbm')
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs: Any) -> "LightGBMClassifier":
        ...
        try:
            ...
            if 'std::bad_alloc' in str(e):
                raise MemorylimitException.from_exception(
                    e, msg=constants.ClientErrors.EXCEEDED_MEMORY, target='LightGbm', has_pii=False
                )
...
class XGBoostClassifier(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=0, n_jobs=1, problem_info=None, **kwargs):
        ...
        if not xgboost_present:
            raise InvalidArgumentException.create_without_pii(
                "xgboost not installed, please install xgboost for including xgboost based models",
                target='XGBoostClassifier')
...
class CatBoostClassifier(ClassifierMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=0, thread_count=1, **kwargs):
        ...
        if not catboost_present:
            raise InvalidArgumentException.create_without_pii(
                "Failed to initialize CatBoostClassifier. CatBoost is not installed",
                target='CatBoostClassifier')
...
class NBWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    def fit(self, X, y, **kwargs):
        ...
        try:
            ...
            raise MemorylimitException.from_exception(
                me, msg=constants.ClientErrors.EXCEEDED_MEMORY, target='NBWrapper', has_pii=False
            )
        except Exception as e:
...
class TruncatedSVDWrapper(BaseEstimator, TransformerMixin, _AbstractModelWrapper):
    ...
    def __init__(
        ...
        if self.n_components_str not in self.args:
            raise InvalidArgumentException.create_without_pii(
                "Failed to initialize TruncatedSVDWrapper. n_components not passed", target='TruncatedSVDWrapper')
        self.model = None
...
class SGDClassifierWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    def partial_fit(self, X, y, **kwargs):
        ...
        if self._calibrated:
            raise InvalidArgumentException.create_without_pii(
                "Failed to partially fit SGDClassifier. Calibrated model used")
...
class LightGBMRegressor(BaseEstimator, RegressorMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=None, n_jobs=1, problem_info=None, **kwargs):
        ...
        if self._min_data_in_leaf not in kwargs and \
                self._min_child_samples not in kwargs:
            raise InvalidArgumentException.create_without_pii(
                "Failed to initialize LightGBMRegressor. Neither min_data_in_leaf nor min_child_samples passed",
                target='LightGBMRegressor')
...
class XGBoostRegressor(BaseEstimator, RegressorMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=0, n_jobs=1, problem_info=None, **kwargs):
        ...
        if not xgboost_present:
            raise InvalidArgumentException.create_without_pii(
                "Failed to initialize XGBoostRegressor. xgboost is not installed, please install xgboost for "
                "including xgboost based models", target='XGBoostRegressor')
...
class CatBoostRegressor(RegressorMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=0, thread_count=1, **kwargs):
        ...
        self.model = None
        if not catboost_present:
            raise InvalidArgumentException.create_without_pii("CatBoost not installed", target='CatBoostRegressor')
...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    # Recoverable errors.
    ERR_NOTHING_TO_PREDICT = (
        "Actual values are present for all times in the input data frame - there is nothing to forecast. "
        "Please set 'y' values to np.NaN for times where you need a forecast.")
    ...
    ERR_DF_CONTAINS_NAN = ("One of X_pred columns contains only NaNs. "
                           "If it is expected, please run forecast() with ignore_data_errors=True.")
    ERR_MISSING_Y = ("All of the values in y_pred are NA or missing. " +
                     "At least one value of y_pred should not be NA or missing.")
    ...
    def __init__(self,
        ...
        if "ts_transformer" not in vars() or ts_transformer is None:
            raise InvalidArgumentException.create_without_pii(
                'Failed to initialize ForecastingPipelineWrapper: {}'.format(
                    ForecastingPipelineWrapper.FATAL_NO_TS_TRANSFORM), target='ForecastingPipelineWrapper')
    ...
    def _check_data(self, X_pred: pd.DataFrame,
        ...
        if X_pred is not None and not isinstance(X_pred, pd.DataFrame):
            raise InvalidTsdfArgument(
                ForecastingPipelineWrapper.FATAL_WRONG_X_TYPE.format(type(X_pred))).with_generic_msg(
                    ForecastingPipelineWrapper.FATAL_WRONG_X_TYPE.format('[Masked]'))
        if y_pred is not None and not isinstance(y_pred, pd.DataFrame) and not isinstance(y_pred, np.ndarray):
            raise InvalidTsdfArgument(
                ForecastingPipelineWrapper.FATAL_WRONG_Y_TYPE.format(type(y_pred))).with_generic_msg(
                    ForecastingPipelineWrapper.FATAL_WRONG_Y_TYPE.format('[Masked]'))
        if forecast_destination is not None and not isinstance(forecast_destination, pd.Timestamp) and not isinstance(
            ...
            raise InvalidTsdfArgument(
                ForecastingPipelineWrapper.FATAL_WRONG_DESTINATION_TYPE.format(
                    type(forecast_destination))).with_generic_msg(
                ForecastingPipelineWrapper.FATAL_WRONG_DESTINATION_TYPE.format('[Masked]'))
        # Check wrong parameter combinations.
        if (forecast_destination is None) and (X_pred is None):
            raise InvalidTsdfArgument(ForecastingPipelineWrapper.FATAL_NO_DESTINATION_OR_X_PRED, has_pii=False)
        if (forecast_destination is not None) and (X_pred is not None):
            raise InvalidTsdfArgument(ForecastingPipelineWrapper.FATAL_DESTINATION_AND_X_PRED, has_pii=False)
        if (forecast_destination is not None) and (y_pred is not None):
            raise InvalidTsdfArgument(ForecastingPipelineWrapper.FATAL_DESTINATION_AND_Y_PRED, has_pii=False)
        if X_pred is None and y_pred is not None:
            # If user provided only y_pred raise the error.
            raise InvalidTsdfArgument(ForecastingPipelineWrapper.FATAL_Y_ONLY, has_pii=False)
    ...
    def _check_max_horizon_and_grain(self, grain: GrainType,
        ...
        if last_train is None:
            if self.short_grain_handling():
                ...
                raise GrainAbsent(ForecastingPipelineWrapper.FATAL_NO_GRAIN_IN_TRAIN, has_pii=False)
        if self._lag_or_rw_enabled():
            ...
            if horizon > self._ts_transformer.max_horizon:
                raise WrongShapeDataError(ForecastingPipelineWrapper.FATAL_MAX_HORIZON_EXCEEDED, has_pii=False)
        ...
        if X_pred is not None:
            ...
            if y_pred.shape[0] != X_pred.shape[0]:
                ...
                raise WrongShapeDataError(
                    ForecastingPipelineWrapper.FATAL_DATA_SIZE_MISMATCH, has_pii=False)
            if isinstance(y_pred, pd.DataFrame):
                if self._ts_transformer.target_column_name not in y_pred.columns:
                    raise InvalidTsdfArgument(
                        ForecastingPipelineWrapper.FATAL_NO_TARGET_IN_Y_DF, has_pii=False)
                X_copy = pd.merge(
                    left=X_copy,
                    right=y_pred,
                    how='left',
            ...
            if self._ts_transformer.dict_latest_date == {}:
                raise GrainAbsent(
                    ForecastingPipelineWrapper.FATAL_NO_LAST_DATE, has_pii=False)
            dfs = []
            for grain_tuple in self._ts_transformer.dict_latest_date.keys():
                if pd.Timestamp(forecast_destination) <= self._ts_transformer.dict_latest_date[grain_tuple]:
                    raise WrongShapeDataError(
                        ForecastingPipelineWrapper.FATAL_EARLY_DESTINATION, has_pii=False)
    ...
    def _infer_missing_data(
        ...
        for grain, df_one in X.groupby(self.grain_column_names):
            first_unknown_y_date = self._get_last_y_one_grain(df_one, grain, ignore_data_errors)
            ...
            if self._ts_transformer.dict_latest_date.get(grain) is None:
                if not self.short_grain_handling():
                    ...
                    raise GrainAbsent(
                        ForecastingPipelineWrapper.FATAL_NO_DATA_CONTEXT, has_pii=False)
            else:
    ...
    def _recursive_forecast_one_grain(self,
        ...
        while origin_time <= df_pred[self.time_column_name].max():
            ...
            try:
                ...
                if err._exception_message == ForecastingPipelineWrapper.ERR_NOTHING_TO_PREDICT:
                    X_trans = self._regular_forecast(df_pred_expand, True)
                else:
                    raise err
            y_fcst = X_trans[constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN].values
            X_fcst_last = X_trans.assign(**{ForecastingPipelineWrapper.TEMP_PRED_COLNAME: y_fcst})
    ...
    def _recursive_forecast(self,
        ...
        for grain_one, df_one in X_copy.groupby(self.grain_column_names):
            if self.is_grain_dropped(grain_one):
                continue
            X_tmp = self._recursive_forecast_one_grain(df_one, ignore_data_errors)
            X_rlt.append(X_tmp)
    ...
    def _check_data_rolling_evaluation(self,
        ...
        if y_pred_unknown:
            # TODO: update to comply with SDK error guidelines.
            raise DataException(ForecastingPipelineWrapper.ERR_MISSING_Y, has_pii=False)
    ...
    def align_output_to_input(self, X_input: pd.DataFrame, transformed: pd.DataFrame) -> pd.DataFrame:
        ...
        *Note:* transformed will be modified by reference, no copy is being created.
        :param X_input: The inpyt data frame.
    ...
    def _warn_or_raise(
        ...
        if ignore_data_errors:
            ...
            raise DataException(warning_text, has_pii=False)
    ...
    def forecast_quantiles(self,
        ...
        if self._horizon_idx is not None:
            ...
            if self._use_recursive_forecast(X_copy_tmp, ignore_data_errors):
                horizons = transformed_data.values[:self.max_horizon, self._horizon_idx].astype(int)
            else:
...
class PipelineWithYTransformations(sklearn.pipeline.Pipeline):
    ...
    def fit(self, X, y, y_min=None, **kwargs):
        ...
        :type y_min: Optional[numpy.ndarray]
        :param kwargs: Other parameters
        :type kwargs: dict
    ...
    def transform(self, y):
        ...
        if self.y_min is None:
            ...
            if self.safe:
                ...
                raise ArgumentException.create_without_pii(
                    'LogTransformer error. y_min greater than observed minimum in y', target='LogTransformer')
        else:
            try:
                return self.return_y(
    ...
    def transform(self, y):
        ...
        if self.y_min is None and not (self.power > 0 and self.power % 2 == 1):
            ...
            if self.safe:
                ...
                raise ArgumentException('PowerTransformer error. y_min greater than observed minimum in y',
                                        target='PowerTransformer', has_pii=False)
        else:
            try:
                return self.return_y(
                    self.scaler.transform(y.reshape(-1, 1)).reshape(-1, ),
    ...
    def inverse_transform(self, y):
        ...
        if self.y_min is None and \
            ...
            if self.safe:
                ...
                raise DataException.create_without_pii(
                    'Failed to inverse transform. y_min greater than observed minimum in y', target='PowerTransformer')
        else:
    ...
    def transform(self, y):
        ...
        if self.lambda_val is None:
            ...
            if self.safe:
                ...
                raise ArgumentException.create_without_pii(
                    'BoxCoxTransformer error. y_min greater than observed minimum in y', target='BoxCoxTransformer')
        else:
            try:
                return boxcox(
                    self.scaler.transform(y.reshape(-1, 1)).reshape(-1, ),
