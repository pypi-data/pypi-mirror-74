    ...
    def _create_cv_splits(self, X, frac_test, frac_valid, CV,
        ...
        if self._cv_split_type == _CVType.CustomCrossValidationSplit:
            ...
            if not max_horizon:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        ArgumentBlankOrEmpty, target=constants.TimeSeries.MAX_HORIZON,
                        argument_name=constants.TimeSeries.MAX_HORIZON,
            ...
            if max_horizon <= 0:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        ArgumentOutOfRange, target=constants.TimeSeries.TARGET_ROLLING_WINDOW_SIZE,
                        argument_name="{} ({})".format(constants.TimeSeries.TARGET_ROLLING_WINDOW_SIZE, max_horizon),
