    ...
    def split(self, X, y=None, groups=None):
        ...
        if not isinstance(X, TimeSeriesDataFrame):
            raise InvalidArgumentException.create_without_pii(
                "X not a TimeSeriesDataFrame.", target='RollingOriginValidator')
        ...
        if X.infer_single_freq() is None:
            raise DataException(
                "Could not infer a single frequency for all time-series.", has_pii=False)
        ...
        if (n_splits - 1) * n_step_temp + 1 + num_horizon > n_samples:
            msg = ("Series is too short for the requested number of CV folds or requested horizon." +
                   "Cannot have (n_splits - 1)*n_step + 1 + num_horizon = {0} greater " +
                   "than the number of samples: {1}.")
            raise DataException(
                msg.format(
                    (n_splits - 1) * n_step_temp + 1 + num_horizon, n_samples)).with_generic_msg(
