...
def _y_nan_check(output: Dict[str, Union[pd.DataFrame, np.array]]) -> \
    ...
    if y is not None and pd.isnull(y).any():
        ...
        if y_reduced.shape[0] == 0:
            raise DataException('All label data is NaN.', has_pii=False)
        output['X'] = X_reduced
        output['y'] = y_reduced
...
def _get_column_data_type_as_str(array: DataSingleColumnInputType) -> str:
    ...
    if array is None:
        raise DataException("The input array is None", has_pii=False)
    ...
    if not isinstance(array, np.ndarray) and not isinstance(array, pd.Series) and \
        ...
        raise DataException("Not an instance of [ndarray, Series, or Categorical array]. Found: {actual_type}"
...
def _check_dimensions(
    ...
    if x_shape is not None and y_shape is not None and x_shape[0] == 0:
        raise DataException("X and y data do not have any samples.", has_pii=False)
    if x_valid_shape is not None and y_valid_shape is not None and x_valid_shape[0] == 0:
        raise DataException("X_valid and y_valid data do not have any samples.", has_pii=False)
...
def sparse_std(x):
    ...
    if not scipy.sparse.issparse(x):
        raise DataErrorException(
            "x is not a sparse matrix", target="x", reference_code="utilities.sparse_std", has_pii=False)
...
def sparse_isnan(x):
    ...
    if not scipy.sparse.issparse(x):
        raise DataErrorException(
            "x is not sparse matrix", target="x", reference_code="utilities.sparse_isnan", has_pii=False)
...
def check_input(df: pd.DataFrame) -> None:
    ...
    if not isinstance(df, pd.DataFrame) and not isinstance(df, np.ndarray):
        raise DataErrorException(
            ...
            target="df", reference_code="utilities.check_input", has_pii=False)
