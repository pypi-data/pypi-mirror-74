...
def get_period_offsets_from_dates(reference_index, time_index, freq,
    ...
    if freq is None:
        ...
        raise DataException(error_message, has_pii=False)
    # Check if frequency is valid.
    try:
        to_offset(freq)
    except ValueError as e:
        raise DataException.from_exception(e, has_pii=True).with_generic_msg('Incorrect frequency string.')
    ...
    if any(ref not in date_grid for ref in reference_index):
        ...
        if misalignment_action == 'warn':
            ...
            raise DataException(msg.format(date_grid)).with_generic_msg(msg.format('[Masked]'))
...
def make_groupby_map(df, groupby_cols_or_indices):
    ...
    def get_col_or_index(colname):
        if colname in df.columns:
            ...
            raise DataFrameMissingColumnException(error_message, has_pii=False)
    # ------------------------------------------------------
...
def subtract_list_from_list(minuend, subtrahend):
    ...
    if not isinstance(minuend, list):
        raise DataException('Minuend must be of type {0}'.format(list), has_pii=False)
    if not isinstance(subtrahend, list):
        raise DataException('subtrahend must be of type {0}'.format(list), has_pii=False)
...
def invert_dict_of_lists(dictionary):
    ...
    if not isinstance(dictionary, dict):
        raise DataException('Input must be a dictionary!', has_pii=False)
