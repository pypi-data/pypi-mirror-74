...
def get_metric_ranges(task, for_assert_sane=False):
    """This function is deprecated. Please use azureml.automl.runtime.shared.score functions."""
    return metrics_utilities.get_metric_ranges(task, for_assert_sane=for_assert_sane)


def get_worst_values(task, for_assert_sane=False):
    """This function is deprecated. Please use azureml.automl.runtime.shared.score functions."""
    return metrics_utilities.get_worst_values(task, for_assert_sane=for_assert_sane)
...
def get_max_values(task, for_assert_sane=False):
    """This function is deprecated. Please use azureml.automl.runtime.shared.score functions."""
    return metrics_utilities.get_max_values(task, for_assert_sane=for_assert_sane)
...
def compute_metrics(y_pred: np.ndarray,
    ...
    if task == constants.Tasks.CLASSIFICATION:
        ...
        return compute_metrics_forecasting(y_pred, y_test, horizons, metrics,
                                           y_max, y_min, y_std,
                                           sample_weight=sample_weight,
                                           bin_info=bin_info)
    else:
...
def compute_metrics_classification(y_pred_probs: np.ndarray,
    ...
    logger.warning("azureml.automl.runtime.shared.metrics.compute_metrics_classification is deprecated. "
    ...
    return scoring.score_classification(y_test, y_pred_probs, metrics,
                                        class_labels, trained_class_labels,
                                        sample_weight=sample_weight, y_transformer=y_transformer,
                                        use_binary=False, logger=logger)


def _infer_classification_inputs(class_labels, trained_class_labels, y_test, y_pred_probs):
    """
    ...
    if class_labels is None:
        ...
        if test_labels.shape[0] >= n_train_labels:
            ...
            raise InvalidArgumentException("class_labels are required to compute classification metrics")
    ...
    if trained_class_labels is None:
        if class_labels.shape[0] > n_train_labels:
            ...
            raise InvalidArgumentException("trained_class_labels are required to compute classification metrics")
        else:
...
def compute_metrics_regression(y_pred: np.ndarray,
    ...
    if logger is None:
        logger = logging_utilities.NULL_LOGGER
    logger.warning("azureml.automl.runtime.shared.metrics.compute_metrics_regression is deprecated. "
...
def compute_metrics_forecasting(y_pred: np.ndarray,
    ...
    if logger is None:
        logger = logging_utilities.NULL_LOGGER
    logger.warning("azureml.automl.runtime.shared.metrics.compute_metrics_forecasting is deprecated. "
...
def compute_mean_cv_scores(
    ...
    if logger is None:
        logger = logging_utilities.NULL_LOGGER
    logger.warning("azureml.automl.runtime.shared.metrics.compute_mean_cv_scores is deprecated. "
                   "Please use azureml.automl.runtime.shared.score.scoring.aggregate_scores")

    return scoring.aggregate_scores(scores, metrics, logger=logger)
