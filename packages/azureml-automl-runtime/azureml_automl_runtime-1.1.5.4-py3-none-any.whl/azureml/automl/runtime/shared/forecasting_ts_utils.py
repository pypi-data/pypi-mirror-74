...
def get_stl_decomposition(ts_values: np.ndarray, seasonality: int = 1) \
    ...
    if np.any(pd.isnull(ts_values)):
        raise DataException('Cannot calculate STL decomposition on an array with NA values.', has_pii=False)

    endog = ts_values.squeeze()
    if seasonality > len(endog):
        raise DataException(
            'Series must be at least as long as input seasonality: {}'.format(seasonality)).with_generic_msg(
            'Series must be at least as long as input seasonality: [Masked]')
...
def last_n_periods_split(tsdf: 'TimeSeriesDataFrame',
    ...
    try:
        ...
        raise DataException('Test size should be numeric.', has_pii=False)

    if not isinstance(tsdf, TimeSeriesDataFrame):
        raise DataException('Input must be a data frame.', has_pii=False)

    if test_size <= 0:
        raise DataException("Expected 'test_size' > 0", has_pii=False)
