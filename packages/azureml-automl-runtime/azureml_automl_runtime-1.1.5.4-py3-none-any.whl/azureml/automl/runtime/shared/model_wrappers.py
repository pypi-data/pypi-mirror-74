...
from azureml.automl.core.shared.exceptions import ArgumentException, UntrainedModelException
...
from azureml.automl.core.shared.limit_function_call_exceptions import MemorylimitException
...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    FATAL_DESTINATION_AND_X_PRED = ("Input prediction data X_pred and forecast_destination are both set. " +
                                    "Please provide either X_pred or a forecast_destination date, but not both.")
    FATAL_DESTINATION_AND_X_PRED = ("Input prediction data y_pred and forecast_destination are both set. " +
                                    "Please provide either X_pred or a forecast_destination date, but not both.")
    ...
    def target_lags(self) -> List[int]:
        ...
        lag_operator = self._ts_transformer.pipeline.get_pipeline_step(TimeSeriesInternal.LAG_LEAD_OPERATOR)
    ...
    def target_rolling_window_size(self) -> int:
        ...
        rolling_window = self._ts_transformer.pipeline.get_pipeline_step(TimeSeriesInternal.ROLLING_WINDOW_OPERATOR)
