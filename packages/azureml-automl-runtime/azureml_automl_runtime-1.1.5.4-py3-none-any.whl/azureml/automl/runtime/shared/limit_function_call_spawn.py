...
from azureml.automl.core.shared.limit_function_call_exceptions import CpuTimeoutException, TimeoutException, \
...
class EnforceLimits:
    ...
    def subprocess_func(func: 'Callable[..., T]',
        ...
        try:
            ...
            res = (None, MemorylimitException.create_without_pii())
        except OSError as e:
            # errno and strerror do not expose PII
            msg = '[OSError {} ({})] {}'.format(e.errno,
                                                errno.errorcode.get(e.errno, 'Unknown'),
                                                os.strerror(e.errno))
            res = (None, SubprocessException.create_without_pii(msg))
        except Exception as e:
            res = (None, e)
