...
from azureml.automl.core.shared.limit_function_call_exceptions import CpuTimeoutException, MemorylimitException, \
    SubprocessException, TimeoutException
from .types import T
...
def run_in_proc(working_dir: Optional[str],
    ...
    try:
        ...
        with open(output_file_name, 'rb') as output_file:
            val, err = cast(Tuple[T, Optional[BaseException]], dill.load(output_file))
        if err is not None:
