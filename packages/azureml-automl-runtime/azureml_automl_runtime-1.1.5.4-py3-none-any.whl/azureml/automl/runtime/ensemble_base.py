...
class EnsembleBase(BaseEstimator, ABC):
    ...
    def _download_model(child_run, index, remote_path, logger):
        ...
        try:
            ...
            logger.info("Download fitted model of size {0} for child run {1}".format(
            ...
            eof_exception = AutoMLEnsembleException.from_exception(
                e, "EOFError for child_run {0} at remote path {1}".format(child_run.id, remote_path), has_pii=False)
            return child_run, None, eof_exception
