    ...
    def __init__(self, onnx_res: Dict[Any, Any],
        ...
        if _onnxrt_present:
            ...
            if not data_feed_mode:
                raise ConfigException("Invalid data feed mode passed in!", has_pii=False)
    ...
    def __init__(self, onnx_res: Dict[Any, Any],
        ...
        if _onnxrt_present:
            ...
            if not raw_to_onnx_column_mapping or not self.raw_col_schema:
                raise DataException("The onnx resource passed in is invalid. "
                                    "Please make sure the x_train data is "
                                    "pandas DataFrame type and it has column names during training.",
                                    has_pii=False)
    ...
    def _convert_input_to_onnx_compatible_types(self, X):
        ...
        for column in X:
            ...
            if col_type == NumericalDtype.Floating or col_type == NumericalDtype.MixedIntegerFloat:
                ...
                msg = "Unsupported data type [{}] in the raw data."
                raise DataException(msg.format(col_type)).with_generic_msg(msg.format('[Masked]'))
