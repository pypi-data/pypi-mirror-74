...
class AutoMLEmbeddingsProvider(AbstractWordEmbeddingsProvider):
    ...
    def __init__(self, logger: Optional[logging.Logger] = None,
        ...
        if embeddings_info is None:
            raise ConfigException(
                "Unsupported embeddings type {0}".format(embeddings_name),
                reference_code='automl_wordembeddings_provider.AutoMLEmbeddingsProvider.__init__'
            ).with_generic_msg("Unsupported embeddings type")
    ...
    def _load_from_disk(self) -> None:
        ...
        try:
            ...
            self._logger.warning("Model loading failed from the file:"
                                 "{path}.".format(path=self._embeddings_pickle_file))
