...
class NimbusMLTextTargetEncoder(AutoMLTransformer):
    ...
    def __init__(self, featurizer: Any,
        ...
        if not isinstance(featurizer, BasePipelineItem):
            raise NimbusMlTextTargetEncoderFeaturizerInvalidTypeException(
                ...
                reference_code='nimbus_ml_text_target_encoder.NimbusMLTextTargetEncoder.__init__',
                has_pii=False)
    ...
    def fit(self, X: DataInputType, y: DataSingleColumnInputType = None) -> "NimbusMLTextTargetEncoder":
        ...
        try:
            ...
            self._logger_wrapper("error", "Failed during fit call.")
            raise FitException.from_exception(
                e,
                target="NimbusMLTextTargetEncoder",
                reference_code='nimbus_ml_text_target_encoder.fit'
    ...
    def transform(self, X: DataInputType) -> DataInputType:
        ...
        try:
            ...
            self._logger_wrapper("error", "Failed during transform call.")
            raise TransformException.from_exception(
                e,
                target="NimbusMLTextTargetEncoder",
                reference_code='nimbus_ml_text_target_encoder.transform'
    ...
    def predict_proba(self, X: DataInputType) -> DataInputType:
        ...
        if self._model is None:
            self._logger_wrapper("error", "No model found for NimbusMLTextTargetEncoder.")
            raise TransformException(
                "No model found. Call 'fit' method first.",
                target="NimbusMLTextTargetEncoder",
                reference_code='nimbus_ml_text_target_encoder.predict_proba',
                has_pii=False
            )
        try:
            ...
            self._logger_wrapper("error", "Failed during predict_proba call.")
            raise
