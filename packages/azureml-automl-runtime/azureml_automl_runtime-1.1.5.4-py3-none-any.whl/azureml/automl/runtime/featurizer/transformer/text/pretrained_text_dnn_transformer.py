    ...
    def fit(self, X_train, y_train, X_valid=None, y_valid=None):
        if self.task_name not in ["classification", "regression"]:
            raise PretrainedTextDnnTransformerFitUnsupportedTaskException(
                ...
                reference_code="pretrained_text_dnn_transformer.PretrainedTextDNNTransformer.fit",
                has_pii=False)
        val_dataset = None
        ...
        try:
            ...
            raise FitException.from_exception(
                e, has_pii=True, reference_code="pretrained_text_dnn_transformer.PretrainedTextDNNTransformer.fit",
                target="PretrainedTextDNNTransformer--model download problem"
            ).with_generic_msg("Exception raised while initializing PretrainedTextDNNTransformer for fit")
    ...
    def _train(self, train_dataset, val_dataset=None, y_valid=None):
        ...
        for curr_epoch in train_iterator:
            for step, batch in enumerate(train_dataloader):
                ...
                # display progress bar only if training model is BERT
                if self.model_name == EmbeddingInfo.BERT_BASE_UNCASED:
                    self.curr_progress = float(global_step + 1) / self.num_total_steps * 100
                    ...
                    if has_observer and self.observer.run_instance is not None:
                        ...
                        carriage_return = False if (global_step + 1 == self.num_total_steps) else True
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # TODO: check to see type of label (int/float) for regression
            ...
            raise PretrainedTextDnnTransformerConvertUnsupportedTaskException(
                ...
                reference_code="pretrained_text_dnn_transformer.PretrainedTextDNNTransformer."
                               "convert_examples_to_features",
                has_pii=False)
