...
from azureml.automl.core.shared.forecasting_utils import get_period_offsets_from_dates, grain_level_to_dict
from azureml.automl.core.shared.forecasting_ts_utils import detect_seasonality_tsdf, get_stl_decomposition
...
class STLFeaturizer(AzureMLForecastTransformerBase):
    ...
    def __init__(self,
                 ...
                 seasonality: int = TimeSeriesInternal.SEASONALITY_VALUE_DETECT,
                 logger: Optional[logging.Logger] = None) -> None:
        """Constructor."""
        if not isinstance(seasonality, int):
            raise ConfigException(("The seasonality for the STL featurizer should be an integer, but got type {}. \
                Please set the seasonality to an integer value. ").format(type(seasonality)), has_pii=False,
        ...
        for grain, df_one in X.groupby_grain():
            if grain not in self._last_observation_dates.keys() or \
               grain not in self._first_observation_dates.keys():
                error_msg = 'The grain {} is not in the training set.'
                raise DataException(error_msg.format(grain),
                                    reference_code='stl_featurizer.STLFeaturizer.data_check'
            ...
            if df_one.time_index.min() < self._first_observation_dates[grain]:
                raise DataException('The forecast date cannot be less then training date.',
                                    has_pii=False, reference_code='stl_featurizer.STLFeaturizer.data_check')
    ...
    def fit(self,
        ...
        if self.seasonality == TimeSeriesInternal.SEASONALITY_VALUE_DETECT:
    ...
    def preview_column_names(self,
        ...
        if (tsdf is None and target is None) or\
           (tsdf is not None and target is not None):
            raise DataException('tsdf or target, but not both should be provided.', has_pii=False,
                                reference_code='stl_featurizer.STLFeaturizer.preview_column_names')

        target_name = tsdf.ts_value_colname if tsdf is not None else cast(str, target)
    ...
    def _fit_one_grain(self,
        ...
        if df_one.shape[0] < 2:
            error_msg = ('STL featurizer requires at least two data points in each grain. \
                    Grain {} contains only one point. Please remove this grain from the dataset.')
            raise DataException(error_msg.format(
                grain), reference_code='stl_featurizer.STLFeaturizer._fit_one_grain'
            ).with_generic_msg(error_msg.format('[MASKED]'))
        if self.seasonality >= len(df_one):
            error_msg = "Must have at least a full season of data for each grain. \
                                  Grain {} has {} observations with seasonality={}"
            raise DataException(error_msg.format(grain, len(df_one), self.seasonality),
                                reference_code='stl_featurizer.STLFeaturizer._fit_one_grain'
                                ).with_generic_msg(error_msg.format('[MASKED]', '[MASKED]', '[MASKED]'))
    ...
    def _transform_one_grain(self,
        ...
        if df_one_pred is not None:
            ...
            try:
                ...
                error_msg = 'Unable to determine horizon for grain {}'
                raise DataException(error_msg.format(grain),
                                    reference_code='stl_featurizer.STLFeaturizer._transform_one_grain'
                                    ).with_generic_msg(error_msg.format('[MASKED]'))
    ...
    def _assign_trend_season(self,
        ...
        if self._ts_value is None:
            ...
            # checked for None.
            raise DataException("Fit not called", has_pii=False,
    ...
    def _assert_damping_valid(self) -> None:
        ...
        if self.es_type[0] == 'N' and self.damped:
            error_msg = ('Inconsistent settings: `model_type`={0} and `damped`={1}. ' +
                         'Damping can only be applied when there is a trend term.')
            raise ConfigException(
                error_msg.format(self.es_type, self.damped),
    ...
    def _assert_mult_model_valid(self, series_values: pd.Series) -> None:
        ...
        if 'M' in self.es_type and (series_values <= 0.0).any():
            raise ConfigException(
                ...
                .format(self.es_type), has_pii=False,
