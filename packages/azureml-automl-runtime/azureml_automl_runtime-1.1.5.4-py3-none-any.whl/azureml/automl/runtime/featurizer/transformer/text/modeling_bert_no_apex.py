...
from azureml.automl.core.shared.transformer_runtime_exceptions import (
    ModelingBertNoApexInvalidHiddenSizeException, ModelingBertNoApexNotIntOrStrException)
...
if torch_present:
    ...
    BERT_PRETRAINED_CONFIG_ARCHIVE_MAP = {
        ...
        "bert/bert-large-uncased-whole-word-masking-finetuned-squad-config.json",
        'bert-large-cased-whole-word-masking-finetuned-squad':
        "https://s3.amazonaws.com/models.huggingface.co/bert/"
    ...
    def load_tf_weights_in_bert(model, config, tf_checkpoint_path):
        ...
        try:
            ...
            raise UserException.from_exception(
                e, target="load_tf_weights_in_bert",
                reference_code='modeling_bert_no_apex.load_tf_weights_in_bert'
            ).with_generic_msg("Exception raised while importing TensorFlow.")
        ...
        for name, array in zip(names, arrays):
            ...
            for m_name in name:
                if re.fullmatch(r'[A-Za-z]+_\d+', m_name):
                    ell = re.split(r'_(\d+)', m_name)
                else:
                    ell = [m_name]
                if ell[0] == 'kernel' or ell[0] == 'gamma':
                    pointer = getattr(pointer, 'weight')
                elif ell[0] == 'output_bias' or ell[0] == 'beta':
                    pointer = getattr(pointer, 'bias')
                elif ell[0] == 'output_weights':
                    pointer = getattr(pointer, 'weight')
                elif ell[0] == 'squad':
                    pointer = getattr(pointer, 'classifier')
                else:
                    try:
                        pointer = getattr(pointer, ell[0])
    ...
    class BertConfig(PretrainedConfig):
        ...
        def __init__(self,
            ...
            if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2 and
                                                                   isinstance(vocab_size_or_config_json_file, str)):
                ...
                raise ModelingBertNoApexNotIntOrStrException(
                    "First argument must be either a vocabulary size (int)"
                    "or the path to a pretrained model config file (str)",
                    has_pii=False,
                    reference_code='modeling_bert_no_apex.BertConfig.__init__'
                )

    # This class is normally only used if apex isn't installed.  For robustness against different compute
    # users' may have, the conditional apex import is removed here we always use BertLayerNorm
    ...
    class BertSelfAttention(nn.Module):
        def __init__(self, config):
            ...
            if config.hidden_size % config.num_attention_heads != 0:
                raise ModelingBertNoApexInvalidHiddenSizeException(
                    "The hidden size (%d) is not a multiple of the number of attention "
                    "heads (%d)" % (config.hidden_size, config.num_attention_heads),
                    has_pii=True,
                    reference_code='modeling_bert_no_apex.BertSelfAttention.__init__'
                ).with_generic_msg("The hidden size is not a multiple of the number of attention heads")
    ...
    class BertEncoder(nn.Module):
        ...
        def forward(self, hidden_states, attention_mask, head_mask=None):
            ...
            if self.output_hidden_states:
                outputs = outputs + (all_hidden_states,)  # type: ignore
            if self.output_attentions:
                outputs = outputs + (all_attentions,)  # type: ignore
    ...
    BERT_START_DOCSTRING = r"""    The BERT model was proposed in
        ...
        Parameters:
            config (:class:`~pytorch_transformers.BertConfig`):
    ...
    class BertModel(BertPreTrainedModel):
        ...
        Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
            ...
            **attentions**: (`optional`, returned when ``config.output_attentions=True``)
                list of ``torch.FloatTensor`` (one for each layer) of
    ...
    class BertForPreTraining(BertPreTrainedModel):
        ...
        Outpufdts: `Tuple` comprising various elements depending on the configuration (config) and inputs:
            **loss**: (`optional`, returned when both ``masked_lm_labels`` and ``next_sentence_label``
                are provided) ``torch.FloatTensor`` of shape ``(1,)``:
    ...
    class BertForMaskedLM(BertPreTrainedModel):
        ...
        Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
            **loss**: (`optional`, returned when ``masked_lm_labels`` is provided)
                ``torch.FloatTensor`` of shape ``(1,)``: Masked language modeling loss.
            **prediction_scores**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, config.vocab_size)``
                Prediction scores of the language modeling head (scores for each vocabulary token before SoftMax).
            **hidden_states**: (`optional`, returned when ``config.output_hidden_states=True``)
                list of ``torch.FloatTensor`` (one for the output of each layer + the output of the embeddings)
    ...
    class BertForNextSentencePrediction(BertPreTrainedModel):
        ...
        Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
            **loss**: (`optional`, returned when ``next_sentence_label`` is provided) ``torch.FloatTensor``
                of shape ``(1,)``:
                Next sequence prediction (classification) loss.
            **seq_relationship_scores**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, 2)``
    ...
    @add_start_docstrings("""Bert Model transformer with a sequence
                             classification/regression head on top (a linear layer on top of
    ...
    class BertForMultipleChoice(BertPreTrainedModel):
        ...
        Inputs:
            **input_ids**: ``torch.LongTensor`` of shape ``(batch_size, num_choices, sequence_length)``:
                ...
                See :func:`pytorch_transformers.PreTrainedTokenizer.encode` and
                :func:`pytorch_transformers.PreTrainedTokenizer.convert_tokens_to_ids` for details.
            **token_type_ids**: (`optional`) ``torch.LongTensor`` of shape
        ...
        Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
            ...
            **classification_scores**: ``torch.FloatTensor`` of shape ``(batch_size, num_choices)``
                where `num_choices` is the size of the second dimension
                of the input tensors. (see `input_ids` above).
        ...
        Examples::
            ...
            >>> input_ids = torch.tensor([tokenizer.encode(s) for s in choices]).unsqueeze(0)  #Batchsize 1, 2 choices
            >>> labels = torch.tensor(1).unsqueeze(0)  # Batch size 1
            >>> outputs = model(input_ids, labels=labels)
    ...
    @add_start_docstrings("""Bert Model with a span classification head on top for extractive
                             question-answering tasks like SQuAD (a linear layers on top of
