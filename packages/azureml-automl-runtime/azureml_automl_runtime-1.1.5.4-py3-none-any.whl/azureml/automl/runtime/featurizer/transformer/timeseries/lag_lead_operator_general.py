...
class LagLeadOperator(AzureMLForecastTransformerBase, OriginTimeMixin):
    ...
    IN_FIT_TRANSFORM = '_in_fit_transform'  # Used for backward compatibility check.
    ...
    def lags_to_construct(self, value: Dict[str, Union[int, List[int]]]) -> None:
        ...
        if not isinstance(value, dict):
            ...
            raise ConfigException(error_msg.format(
                dict), reference_code='lag_lead_operator_general.LagLeadOperator.lags_to_construct'
        ...
        try:
            ...
            raise ColumnTypeNotSupportedException(
                ...
                reference_code='lag_lead_operator_general.LagLeadOperator.lags_to_construct',
                has_pii=False)
    ...
    def dropna(self, value: bool) -> None:
        if not isinstance(value, bool):
            ...
            raise ClientException(error_message, has_pii=False,
                                  reference_code='lag_lead_operator_general.LagLeadOperator.dropna')
        self._dropna = value
    ...
    def origin_time_colname(self, value: str) -> None:
        if not isinstance(value, str):
            ...
            raise ColumnTypeNotSupportedException(
                ...
                reference_code='lag_lead_operator_general.LagLeadOperator.origin_time_colname',
    ...
    def overwrite_columns(self, value: bool) -> None:
        if not isinstance(value, bool):
            ...
            raise ClientException(
                error_message.format(value), reference_code='lag_lead_operator_general.\
                    LagLeadOperator.overwrite_columns').with_generic_msg(error_message.format('[MASKED]'))
        ...
        if len(columns_to_overwrite) > 0:
            ...
            if self.overwrite_columns:
                ...
                raise ConfigException(error_message.format(columns_to_overwrite),
                                      reference_code='lag_lead_operator_general.\
                                          LagLeadOperator._check_for_column_overwrites'
    ...
    def _construct_one_lag_by_time_with_origin(self,
        ...
        if not isinstance(lag_var, str):
            raise ColumnTypeNotSupportedException(
                ...
                reference_code='lag_lead_operator_general.LagLeadOperator._construct_one_lag_by_time_with_origin ',
                has_pii=False)
        ...
        if not isinstance(X, TimeSeriesDataFrame):
            raise NotTimeSeriesDataFrameException(
                ...
                reference_code='lag_lead_operator_general.\
                    LagLeadOperator._construct_one_lag_by_time_with_origin')
    ...
    def _construct_all_lags_by_time_with_origin(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        if not isinstance(X, TimeSeriesDataFrame):
            raise NotTimeSeriesDataFrameException(
                Messages.XFORM_INPUT_IS_NOT_TIMESERIESDATAFRAME, has_pii=False,
                reference_code='lag_lead_operator_general.\
                    LagLeadOperator._construct_all_lags_by_time_with_origin')
        if X.origin_time_colname is None:
            error_message = ('This method should only be called on a '
                             'TimeSeriesDataFrame in which `origin_time_colname` is set!')
            raise ClientException(error_message, has_pii=False, reference_code='lag_lead_operator_general.\
                    LagLeadOperator._construct_all_lags_by_time_with_origin')
    ...
    def _construct_one_lag_by_occurrence_with_horizon_origin(self,
        ...
        if not isinstance(lag_var, str):
            raise ColumnTypeNotSupportedException(
                'lag_var must be of type {0}'.format(str),
                reference_code='lag_lead_operator_general.LagLeadOperator.\
                                                      _construct_one_lag_by_occurrence_with_horizon_origin ',
                has_pii=False)
        if not isinstance(lag_order, int):
            raise ConfigException(
                'lag_order must be of type {0}'.format(int),
    ...
    def _construct_all_lags_by_occurrence_with_horizon_origin(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        if not isinstance(X, TimeSeriesDataFrame):
            raise NotTimeSeriesDataFrameException(
                Messages.XFORM_INPUT_IS_NOT_TIMESERIESDATAFRAME, has_pii=False,
                reference_code='lag_lead_operator_general.LagLeadOperator.\
                                                      _construct_all_lags_by_occurrence_with_horizon_origin')
        if (TimeSeriesInternal.HORIZON_NAME not in X.columns):
            error_message = ('This method should only be called on a '
                             'TimeSeriesDataFrame in which `horizon_origin` is set!')
            raise ClientException(error_message, has_pii=False,
    ...
    def _set_lag_option(self, X: TimeSeriesDataFrame) -> 'LagLeadOperator':
        """
    ...
    def transform(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        if not self._is_fit:
            ...
            raise ClientException(error_message, has_pii=False,
                                  reference_code='lag_lead_operator_general.LagLeadOperator.transform')
        # logic:
        ...
        if(self.lag_option == LagLeadOperator.LAG_BY_TIME):
            ...
            if hasattr(self, self.IN_FIT_TRANSFORM):
                self._in_fit_transfrom = False
    ...
    def fit_transform(self, X: TimeSeriesDataFrame, y: np.ndarray = None, **fit_params) -> TimeSeriesDataFrame:
        ...
        rv = super(LagLeadOperator, self).fit_transform(X, y, **fit_params)
        self._in_fit_transform = False
        return rv
