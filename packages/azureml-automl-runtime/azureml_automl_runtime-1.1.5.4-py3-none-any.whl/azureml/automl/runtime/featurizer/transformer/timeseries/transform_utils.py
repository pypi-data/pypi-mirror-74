...
class OriginTimeMixin(object):
        ...
        if isinstance(max_horizon, dict):
            ...
            if len(improper_types) > 0:
                raise InvalidTsdfArgument(
                    ...
                    reference_code='transform_utils.OriginTimeMixin.verify_max_horizon_input').with_generic_msg(
                    'All max_horizon values must be of integer type.')
            ...
            if len(not_positive) > 0:
                raise InvalidTsdfArgument(
                    ...
                    reference_code='transform_utils.OriginTimeMixin.verify_max_horizon_input',
                    has_pii=False)
    ...
    def create_origin_times(self, X: TimeSeriesDataFrame, max_horizon: Union[int, Dict[Any, int]],
        ...
        def add_origins_for_horizon(Xgr, h) -> pd.DataFrame:
        ...
        if X.grain_colnames is not None:
            ...
            if not isinstance(max_horizon, int):
                raise ForecastingTransformException(
                    ...
                    reference_code='transform_utils.OriginTimeMixin.add_origins_single_grain',
                    has_pii=False)
