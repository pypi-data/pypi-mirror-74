...
class CategoryBinarizer(AzureMLForecastTransformerBase):
        ...
        if verify.is_iterable_but_not_string(vals):
            if all(isinstance(c, str) for c in vals):
                ...
                raise ForecastingTransformException(
                    'column names must all be strings.', target="columns",
                    reference_code='category_binarizer.CategoryBinarizer.columns.not_all_str',
                    has_pii=False)
            ...
            raise ForecastingTransformException(
                'column names can be None, a string or an iterable containing strings', target="columns",
                reference_code='category_binarizer.CategoryBinarizer.columns.not_iterable_or_str',
                has_pii=False)
    ...
    def transform(self, X, y=None):
        ...
        if len(missing_cols) > 0:
            error = 'CategoryBinarizer.transform: Missing columns {}'.format(missing_cols)
            raise DataFrameMissingColumnException(error,
                                                  reference_code='category_binarizer.CategoryBinarizer.transform')
