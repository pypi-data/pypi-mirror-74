...
if pkg_dependencies_satisfied:
    ...
    class WordEmbed(BaseModel):
        def __init__(self, embeddings_name, static_embed=True, device='cpu'):
            ...
            self._word_to_idx = {}  # type: ignore
            self._lowercase_word_to_idx = {}  # type: ignore
            ...
            try:
                ...
                raise FitException.from_exception(
                    ...
                    reference_code='bilstm_attention_transformer.WordEmbed.__init__'
    ...
    def fit(self, x_train, y_train, x_valid=None, y_valid=None):
        ...
        try:
            if self._seed is not None:
                self.info_logger("Set seed for BiLSTM to {}".format(self._seed))
                self.set_seed()
            self.info_logger("---------------------------------------------------")
            self.info_logger("BiLSTMAttentionTransformer.fit() called")

            # print size of train data for debugging
            self.info_logger("Shape of training data = {}".format(x_train.shape))
            ...
            self.info_logger("Checking if data size exceeds {}".format(self._max_rows))
            if len(y_train) > self._max_rows:
                new_epochs = self._epochs * self._max_rows / len(y_train)
                self._epochs = max(int(new_epochs), 1)
                self.info_logger("Reduced number of epochs from {} to {} due to "
                ...
                if self._epochs <= 1.0:
                    ...
                    self.info_logger("Using {} rows out of {} rows for BiLSTM training"
            ...
            if self._model is None:
                self.info_logger("Continue training parameters from the best model of previous fit")
            ...
            for epoch in range(1, self._epochs + 1):
                ...
                try:
                    ...
                    raise FitException.from_exception(
                        ...
                        reference_code='bilstm_attention_transformer.BiLSTMAttentionTransformer.fit'
                ...
                self.info_logger('epoch:{}, batches:{}'.format(epoch, len(x_batches)))
                for i in range(len(x_batches)):
                    ...
                    if self._train_time_per_row_sec == 0.0:
                        ...
                        self.info_logger('train time per row sec: {:.6f}'.format(self._train_time_per_row_sec))
                    # Try to print the result every 10 batches.
                    if i % self._iter_cnt == 0:
                        self.info_logger('Batch[{}] - loss: {:.6f}'.format(i, loss.item()))
                        if self._device != 'cpu':
                            torch.cuda.empty_cache()
                epoch_end = datetime.datetime.now()
                self.info_logger("epoch:{} takes {} seconds".format(epoch, (epoch_end - current).total_seconds()))
                ...
                if x_valid is not None:
                    ...
                    self.info_logger('epoch:{}, validation accuracy: {}'.format(epoch, valid_accuracy))
                    ...
                    if self._early_stopping is not None:
                        if self._early_stopping.check_early_stop_train(valid_accuracy):
                            self.info_logger('early stopping the training at epoch:{}'.format(epoch))
                            break
            self.info_logger('average training time in seconds: {}'.format(ts / self._epochs))
            if x_valid is not None:
                self.info_logger('valid best accuracy: {:.4f}'.format(best_acc))
            if copied:
        ...
        try:
            ...
            try:
                ...
                raise TransformException.from_exception(
                    ...
                    reference_code='bilstm_attention_transformer.BiLSTMAttentionTransformer.transform.model_eval').\
            ...
            raise TransformException.from_exception(
                ...
                reference_code='bilstm_attention_transformer.BiLSTMAttentionTransformer.transform').\
                with_generic_msg("BiLSTMAttentionTransformer failed in transform.")
