...
class RawFeatureStats:
    ...
    def __init__(self, raw_column: pandas.Series):
        ...
        try:
            self.num_unique_vals = non_na_raw_column.unique().shape[0]
        except TypeError:
            # This will handle a 1d numpy array or flat list of hashable object only.
            # If it's a list of lists or 2d numpy array, it will still fail.
            self.num_unique_vals = non_na_raw_column.apply(
                lambda x: tuple(x) if isinstance(x, (list, np.ndarray)) else x).unique().shape[0]
        ...
        self.lengths = None     # type: Optional[pandas.Series]
        ...
        if self.total_number_vals == 0:
            ...
            self._fill_stats_if_text_or_categorical_feature(non_na_raw_column)
    ...
    def _fill_stats_if_text_or_categorical_feature(self, raw_column: pandas.Series) -> None:
        """
        ...
        self.lengths = raw_column_str.apply(len)
