...
from azureml.automl.core.shared.exceptions import AutoMLException, ClientException, DataException
...
class StreamingFeaturizer:
    ...
    def __init__(self,
        ...
        # Note: When instantiating the helper, we blacklist non-feature column names from surfacing as aliases
    ...
    def _validate_inputs(self):
        if not isinstance(self._training_data, Dataflow):
            raise DataException("Preprocessing for large data is only allowed on Dataflows, instead got training "
                                "data type as: {}".format(type(self._training_data).__name__), has_pii=False)
        if not self._label_column:
            msg = "Received an invalid name for 'label_column' : {}"
            raise DataException(msg.format(self._label_column)).with_generic_msg(msg.format('[Masked]'))
    ...
    def learn_transformations(self) -> StreamingFeaturizationTransformer:
        ...
        if len_subsampled_input_columns != len_training_data_columns:
            msg = ("The original dataset had {} columns, but Dataflow subsampling produced {} columns. "
                   "Please verify that any quotes or new-line characters are handled correctly.")
            raise DataException(msg.
                                format(len_training_data_columns, len_subsampled_input_columns)).with_generic_msg(
        ...
        try:
            ...
            msg = 'Streaming: Failed to learn the transformations.'
            self._logger.error(msg)
            raise DataException(msg, has_pii=False)
    ...
    def _get_transformations(self) -> List[NimbusMLPipelineItem]:
        ...
        if not flattened_transforms:
            ...
            raise DataException(error_msg, has_pii=False)
    ...
    def _get_transforms_for_column_purpose(self,
        ...
        if column_purpose == _FeatureType.Numeric:
            ...
            raise ClientException(
                ...
                reference_code='streaming_featurizer.StreamingFeaturizer._get_transforms_for_column_purpose'
