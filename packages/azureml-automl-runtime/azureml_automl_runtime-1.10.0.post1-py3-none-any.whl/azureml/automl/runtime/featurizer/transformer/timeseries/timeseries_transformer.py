...
from azureml.automl.core.shared.exceptions import ConfigException,\
    InvalidDataTypeException, ClientException, AutoMLException,\
    EmptyDataException, InvalidValueException, InsufficientDataException
...
from .forecasting_heuristic_utils import get_heuristic_max_horizon, analyze_pacf_per_grain, _log_warn_maybe
    ...
    def _partial_fit(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> None:
        ...
        for grain, df_one in tsdf.groupby_grain():
            if all(pd.isnull(v) for v in df_one.ts_value):
                msg = ("The grain {} does not contain data points. "
