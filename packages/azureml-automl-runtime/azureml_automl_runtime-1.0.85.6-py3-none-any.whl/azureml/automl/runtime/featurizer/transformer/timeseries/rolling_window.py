...
from automl.client.core.runtime.forecasting_verify import is_iterable_but_not_string, check_cols_exist, Messages
...
from automl.client.core.runtime.types import DataFrameApplyFunction
...
class RollingWindow(AzureMLForecastTransformerBase, OriginTimeMixin):
    ...
    def window_size(self, val: Union[int, pd.DateOffset]) -> None:
        if not np.issubdtype(type(val), np.signedinteger) and not isinstance(val, DateOffset):
            try:
                ...
                raise ConfigException('The window_size must be of type '
                                      'integer, a string that can be converted '
                                      'to DateOffset, or DateOffset. Instead '
                                      'got {0}'.format(type(val)))
    ...
    def transform_dict(self, val: Dict[DataFrameApplyFunction, Union[str, List[str]]]) -> None:
        if not isinstance(val, dict):
            raise ClientException('The transform_dict must be a dictionary. '
                                  'Instead got {0}'.format(type(val)))
    ...
    def window_opts(self, val: Dict[str, Any]) -> None:
        if not isinstance(val, dict):
            raise ClientException('The window_opts must be a dictionary. '
                                  'Instead got {0}'.format(type(val)))
    ...
    def transform_opts(self, val: Dict[DataFrameApplyFunction, Dict[str, Any]]) -> None:
        if not isinstance(val, dict):
            raise ClientException('The transform_opts must be a dictionary. '
                                  'Instead got {0}'.format(type(val)))
    ...
    def transform(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        :raises: :class:`ClientException`, :class:`NotTimeSeriesDataFrameException`
        """
        if not self._is_fit:
            raise ClientException(
                'RollingWindow.transform: fit must be called before transform')
