...
from automl.client.core.common.exceptions import ConfigException, DataException,\
    ClientException
from automl.client.core.common.forecasting_utils import flatten_list, invert_dict_of_lists
from automl.client.core.common.forecasting_verify import is_list_oftype, is_iterable_but_not_string, Messages
...
class LagLeadOperator(AzureMLForecastTransformerBase, OriginTimeMixin):
    ...
    def __init__(self,
        ...
        # "lag_by_time" is good for (almost) evenly spaced time series data, while
        # "lag_by_occurrence" is a better option for non-evenly spaced data
        self.lag_option = "lag_by_occurrence"
    ...
    def lags_to_construct(self, value: Dict[str, Union[int, List[int]]]) -> None:
        ...
        if not isinstance(value, dict):
            raise ConfigException('Value must be of type {0}'.format(dict))
            ...
            if np.issubdtype(type(v), np.signedinteger):
                ...
                error_message = \
                    ("In 'lags_to_construct' input argument, " +
                     ...
                     "ints. Instead got {}!".format(type(v)))
                raise ConfigException(error_message)
    ...
    def origin_time_colname(self, value: str) -> None:
        if not isinstance(value, str):
            error_message = \
                ('Input argument `origin_time_colname` must be a ' +
                 'single string, instead got {}'.format(type(value)))
            raise ColumnTypeNotSupportedException(error_message)
        ...
        if len(columns_to_overwrite) > 0:
            warning_message = \
                ('Some of the columns that are about to be '
                 ...
                 'TimeSeriesDataFrame: {}. '.format(columns_to_overwrite))
            if self.overwrite_columns:
                warning_message += 'They will be overwritten!'
                warn(warning_message, UserWarning)
            else:
                error_message = \
                    warning_message + ('Please set `overwrite_columns` '
    ...
    def _construct_one_lag_by_time_with_origin(self,
        ...
        if not isinstance(lag_var, str):
            raise ColumnTypeNotSupportedException('lag_var must be of type {0}'.format(str))
    ...
    def _construct_one_lag_by_occurrence_with_horizon_origin(self,
        ...
        if (TimeSeriesInternal.DUMMY_ORDER_COLUMN in X.columns):
            # remove imputed rows before generating lag_by_occurrence features
            X_temp = X[X[TimeSeriesInternal.DUMMY_ORDER_COLUMN].notnull()]

            if 1.0 * X_temp[X.ts_value_colname].isnull().sum() / X_temp.shape[0] > 0.9:
                # in this case, this is test data transformation, so we need to
                # also keep "cached real train data", even though their
                # _automl_original_order_col is null
                X_temp = X[(X[TimeSeriesInternal.DUMMY_ORDER_COLUMN].notnull()) | (X[X.ts_value_colname].notnull())]
    ...
    def _construct_all_lags_by_occurrence_with_horizon_origin(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        for lag_var, all_lag_orders in lags_to_construct_safe.items():
            ...
            if TimeSeriesInternal.DUMMY_ORDER_COLUMN in X.columns:
                ...
                # we will only generate lagging timeDiff features if data
                # is not evenly spaced (and thus has imputed rows)
                # since otherwise these timeDiff features will be constant
                # and not useful
                if (X[TimeSeriesInternal.DUMMY_ORDER_COLUMN].isnull().sum()) > 0:

                    # When we generate lag_by_occurrence features for the target column,
                    # we would also need to generate the associated lagging timeDiff
                    # features to work well
                    if lag_var == X.ts_value_colname:
                        # For generating lagging timeDiff features, we
                        # need to create a temp time column from the tsdf
                        # attributes, since lagging original time column is
                        # not allowed
                        temp_df_time_lag = pd.DataFrame(X).reset_index()

                        prefix = 'temp_'
                        postfix = "_timeDiffDays"
                        temp_time_colname = prefix + X.time_colname
                        X[temp_time_colname] = pd.to_datetime(temp_df_time_lag[X.time_colname].values)
                        del temp_df_time_lag
                        gc.collect()

                        for lag_order in all_lag_orders:
                            lag = self._construct_one_lag_by_occurrence_with_horizon_origin(
                                X, temp_time_colname, lag_order)
                            lag_name = lag.columns[0]

                            # timeDiff lag timestamp feature, need to remove "temp_" prefix from
                            # feature name
                            tsdf_with_lags[str(lag_name)[len(prefix):] + postfix] = (pd.to_datetime(
                                                                                     X[temp_time_colname].values) -
                                                                                     pd.to_datetime(
                                                                                         lag[lag_name].values)) \
                                / datetime.timedelta(days=1)
                            lag_name = str(lag_name)[len(prefix):] + postfix
                            columns_to_keep.append(lag_name)

                        # drop the tempory time column after generating time interval features
                        X.drop([temp_time_colname], inplace=True, axis=1)
    ...
    def _cache_trailing_training_data(self, X: TimeSeriesDataFrame) -> 'LagLeadOperator':
        ...
        if len(lags_to_construct_safe) > 0:
            ...
            if 1.0 * X[X.ts_value_colname].isnull().sum() / X.shape[0] < 0.02:
                # Data is (almost) evenly spaced, we will do lag_by_time
                self.lag_option = "lag_by_time"
            else:
                ...
                X = X[X[X.ts_value_colname].notnull()]
            ...
            if(self.lag_option == "lag_by_time"):
    ...
    def transform(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        if not self._is_fit:
            error_message = \
                ("For the LagOperator to work correctly, fit() "
        ...
        if(self.lag_option == "lag_by_occurrence"):
            if (TimeSeriesInternal.HORIZON_NAME not in X.columns):
