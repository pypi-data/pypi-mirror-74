...
from automl.client.core.runtime.forecasting_verify import (is_iterable_but_not_string)
    ...
    if is_iterable_but_not_string(reference_index):
        if len(reference_index) != len(time_index):
            raise ForecastingDataException(
                'Reference_index and time_index '
...
def make_groupby_map(df, groupby_cols_or_indices):
    ...
    def get_col_or_index(colname):
        if colname in df.columns:
            ...
            error_message = 'Column does not exist: {0}'
            raise DataFrameMissingColumnException(
                error_message.format('[Masked]'),
                error_message.format(colname))
    # ------------------------------------------------------
...
def subtract_list_from_list(minuend, subtrahend):
    ...
    if not isinstance(minuend, list):
        raise DataException('Minuend must be of type {0}'.format(list))
    if not isinstance(subtrahend, list):
        raise DataException('subtrahend must be of type {0}'.format(list))
...
def invert_dict_of_lists(dictionary):
    ...
    if not isinstance(dictionary, dict):
        raise DataException('Input must be a dictionary!')
