...
from typing import Any, cast, Dict, List, Optional, Tuple, Iterable, Iterator
...
from automl.client.core.runtime import problem_info, utilities
from automl.client.core.runtime._cv_splits import FeaturizedCVSplit, _CVSplits, _CVType
from automl.client.core.runtime.cache_store import MemoryCacheStore, CacheStore
...
class ClientDatasets(DatasetBase):
    ...
    def _init_dataset(self, name, task, X, y, sample_weight=None,
        ...
        if task == constants.Tasks.CLASSIFICATION:
            ...
            if self._dataset['num_classes'] != len(self._dataset['class_labels']):
                raise ValueError("num_classes and class_labels not consistent")
    ...
    def parse_simple_train_validate(self,
        ...
        """
        class_labels = np.union1d(y, y_valid)
        if num_classes is None:
            num_classes = class_labels.shape[0]

        self._init_dataset(
            name, task, X, y, sample_weight=sample_weight,
            num_classes=num_classes, class_labels=class_labels, y_min=y_min, y_max=y_max,
    ...
    def cache_dataset(self, keep_in_memory: bool = True) -> None:
        ...
        # 1. Have non none values,
        # 2. Could be memory heavy (represented by the keys in self._data_fields),
        # 3. Are not already cached in the cache store
        for key, value in self._dataset.items():
            if (value is not None and
                    key in self._data_fields and
    ...
    @staticmethod
    def decode_data(X: np.ndarray) -> np.ndarray:
        """
