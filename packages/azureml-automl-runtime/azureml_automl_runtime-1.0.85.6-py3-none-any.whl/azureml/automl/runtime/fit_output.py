...
class FitOutput:
    ...
    def failure_reason(self) -> Optional[str]:
        for error_name in self.errors:
            if self.errors[error_name]['is_critical']:
                exc = self.errors[error_name]['exception']
                return getattr(exc, '_error_code', ErrorTypes.Unclassified)
        return None
    ...
    def error_code(self) -> Optional[str]:
        for error_name in self.errors:
            if self.errors[error_name]['is_critical']:
                exc = self.errors[error_name]['exception']
                return getattr(exc, 'error_type', ErrorTypes.Unclassified)
        return None
