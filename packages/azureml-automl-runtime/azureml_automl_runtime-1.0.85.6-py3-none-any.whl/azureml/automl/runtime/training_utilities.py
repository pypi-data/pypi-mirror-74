...
from automl.client.core.common.exceptions import DataException, ConfigException, DataShapeError
        ...
        if isinstance(x, pd.DataFrame):
            for column in x.columns:
                if not utilities._check_if_column_data_type_is_numerical(
                    ...
                    raise DataException(without_featurization_error_str)
...
def _check_data_minimal_size(X: DataInputType, X_valid: DataInputType, automl_settings: AutoMLBaseSettings) -> None:
    ...
    if automl_settings.n_cross_validations is None:
        ...
        minimal_size = automl_settings.n_cross_validations * SmallDataSetLimit.TRAIN_VALID_MINIMAL_SIZE
    if X.shape[0] < minimal_size:
        raise DataShapeError(
            "The input data X has {} data points which is less than the minimum requirement size of {}. "
            "Please add more data to avoid future exceptions.".
            format(X.shape[0], minimal_size)
        )
    if X_valid is not None and X_valid.shape[0] < minimal_size:
        raise DataShapeError(
            "The input data X_valid has {} data points which is less than the minimum requirement size of {}. "
            "Please add more data to avoid future exceptions.".
            format(X_valid.shape[0], minimal_size)
        )
...
def _check_timeseries_input_and_get_tsdf(
                ...
                if is_validation_data:
                    if data_points < max_horizon:
                        print("WARNING: Validation set has fewer data points ({}) "
                              ...
                              format(data_points, max_horizon, grain_name_str))
    ...
    if not is_validation_data:
        ...
        _check_cv_gap_exist(tsdf, max_horizon, automl_settings.n_cross_validations)
    return tsdf
...
def _check_cv_gap_exist(tsdf: TimeSeriesDataFrame,
                        ...
                        grain_name: Optional[str] = None) -> None:
    """
...
def _extract_data_from_combined_dataframe(
    ...
    if sample_weight_column_name is not None:
        ...
        if sample_weight_column_name not in input.columns:
            raise DataException('The sample weight column {} is not found in input data, '
                                'please double check'.format(sample_weight_column_name))
    ...
    if label_column_name not in input.columns:
        raise DataException('The label column {} is not found in input data, '
