...
from automl.client.core.common.exceptions import ClientException, ErrorTypes, UserException, ServiceException
                        ...
                        if bypassing_model_explain is None:
                            if automl_settings.iteration_timeout_minutes is None or remaining_time is None or \
                                ...
                                print("Remaining time is not enough for model explanation. "
                                      "Please use the workflow described at "
                                      "https://github.com/Azure/MachineLearningNotebooks/blob/master/"
                                      "how-to-use-azureml/automated-machine-learning/model-explanation/"
                                      "auto-ml-model-explanation.ipynb to generate explanations for this iteration..")
                        else:
...
def _mark_run_as_complete(run: RunType, run_id: str, logger: logging.Logger) -> None:
    ...
    def _complete():
        ...
        if current_status != 'Completed':
            ...
            exception = ServiceException(error_msg)
            raise exception
        ...
        if iteration_timeout_min <= 0 and raise_exception:
            raise TimeoutException(
                "Iteration ran for {min} minutes. {err_msg}".format(
                    min=math.floor(running_min), err_msg=constants.ClientErrors.EXCEEDED_ITERATION_TIMEOUT_MINUTES))
...
def _check_fit_output_result(fit_output: FitOutput) -> None:
    ...
    if fit_output.fitted_pipeline == constants.Defaults.INVALID_PIPELINE_OBJECT:
        raise ClientException("Fitted model is empty.")
