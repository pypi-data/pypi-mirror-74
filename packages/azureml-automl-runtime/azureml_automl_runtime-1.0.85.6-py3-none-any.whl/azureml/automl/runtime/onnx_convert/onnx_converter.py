...
from automl.client.core.runtime.model_wrappers import _AbstractModelWrapper       # noqa: E402
from automl.client.core.runtime.model_wrappers import PipelineWithYTransformations       # noqa: E402
from automl.client.core.runtime.model_wrappers import PreFittedSoftVotingClassifier       # noqa: E402
from automl.client.core.runtime.model_wrappers import PreFittedSoftVotingRegressor       # noqa: E402
...
from automl.client.core.common.logging_utilities import function_debug_log_wrapped, _CustomStackSummary   # noqa: E402
...
class OnnxConverter:
    ...
    class _InitPhaseMetadata:
        ...
        def __init__(self, ini_types, ini_tp_to_infer_type_map, ini_tp_to_input_df_col_map,
                     df_col_to_df_col_idx_in_input, if_input_data_has_col_name, *args, **kwargs):
            self._metadata_dict = {
        ...
        def is_metadata_dict_valid(cls, metadata_dict: Dict[str, Any]) -> bool:
            return (metadata_dict is not None and
                    ...
                    metadata_dict[cls.IfInputDataHasColName] is not None
                    )
    ...
    def __init__(self, logger=None, version='', is_onnx_compatible=False,
        ...
        self._init_phase_metadata = None      # type: OnnxConverter._InitPhaseMetadata
        ...
        self.conversion_phase_initial_types = []     # type: List[Tuple[Any]]
    ...
    def initialize_input(self,
        ...
        try:
            ...
            for i_init_tp, column in enumerate(X.columns):
                ...
                self.init_phase_initial_types.append((col_name, onnx_type(shape=[1, 1])))
        ...
        if not self._is_initialized:
            ...
            try:
                if (metadata_dict is not None and metadata_dict and
                    ...
                    self.init_phase_initial_types = \
                        metadata_dict[self._InitPhaseMetadata.InitialTypes]
                    self.idx_indiv_ini_tp_to_infer_type_map = \
                    ...
                    self._if_input_data_has_column_name = \
                        metadata_dict[self._InitPhaseMetadata.IfInputDataHasColName]
                    self._is_initialized = True
                    self._logger_wrapper('info', evt_name=OnnxConvertConstants.EvtInitWithMetadata,
                                         status=OnnxConvertConstants.StatusEndSucceeded,
    ...
    def get_init_metadata_dict(self) -> Optional[Dict[str, Any]]:
        ...
        if self.is_initialized():
            return self._init_phase_metadata.get_metadata_dict()
        else:
            return None
    ...
    def convert(self, raw_model: Pipeline,
                model_name: str = '',
        ...
        try:
            ...
            self._logger_wrapper(err_log_level, evt_name=OnnxConvertConstants.EvtConvert,
                                 status=err_st,
                                 model_name=model_name, model_desc=model_desc, message="[Scrubbed]")
            pass
        finally:
            if err is None:
                msg_obj = {
    ...
    def _setup_initial_types_based_on_model(self, model):
        ...
        if trans is not None:
            ...
            if trans.mapper.built_default is None or trans.mapper.built_default is False:
                ...
                new_idx = 0
                for idx_indi_ini_types, input_var in enumerate(self.init_phase_initial_types):
                ...
                self.conversion_phase_initial_types = self.init_phase_initial_types
    ...
    def _get_stack_trace_msg(self):
        ...
        if traceback_obj is not None:
            stack_trace = ''.join(_CustomStackSummary.format(traceback.extract_tb(traceback_obj)))
        else:
