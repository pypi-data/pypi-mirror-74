...
from azureml.automl.core.shared.exceptions import ConfigException, DataException, InvalidArgumentException
...
class _CVSplits:
    ...
    def _find_cv_split_type(self, frac_test, frac_valid,
        ...
        if self._is_time_series:
            ...
            raise ConfigException("Not a valid cross validation scenario", target='_CVSplits', has_pii=False,
    ...
    def _validate_cv_parameters(self, X, frac_test, frac_valid, CV,
        ...
        if self._cv_split_type is _CVType.TimeSeriesSplit:
            if not isinstance(self._timeseries_param_dict, dict):
                raise InvalidArgumentException(
                    'Improper param dictionary for time-series data. Received value: {}'.format(
                        self._timeseries_param_dict), target='_CVSplits',
                    reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_TS_PARAM_DICT)
            if frac_test or frac_valid:
                raise ConfigException(
                    'Fractional CV splitting is not supported for time-series', target='_CVSplits', has_pii=False,
                    reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_TS_FRAC)
            if cv_splits_indices:
                raise ConfigException(
                    'Custom CV splits are not supported for time-series', target='_CVSplits', has_pii=False,
                    reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_TS_CV_SPLITS)
            if test_cutoff is not None:
                raise ConfigException(
                    'Test cutoff index input is not supported for time-series', target='_CVSplits', has_pii=False,
                    reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_TS_TEST_CUTOFF)
            if CV is None:
                raise ConfigException(
                    'The number of cross-validation folds must be set for time-series',
                    target='_CVSplits', has_pii=False, reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_TS_CV)

        if frac_test and frac_test >= 1.0:
            raise ConfigException("Test set fraction set must be less than 1.0", target='_CVSplits', has_pii=False,
                                  reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_FRAC_TEST)

        if frac_valid and frac_valid >= 1.0:
            raise ConfigException("Validation set fraction set must be less than 1.0",
                                  target='_CVSplits', has_pii=False,
        ...
        if frac_test and frac_valid and frac_test + frac_valid >= 1.0:
            raise ConfigException("Sum of test and validation set fractions must be less than 1.0",
                                  target='_CVSplits', has_pii=False,
                                  reference_code=ReferenceCodes._VALIDATE_CV_PARAMETERS_FRAC_VALID_TEST)

        # sanity check on CV
        if CV is not None and CV <= 0:
            raise ConfigException("The number of cross validations is less than or equal to zero",
                                  target='_CVSplits', has_pii=False,
        ...
        for train, valid in cv_splits_indices:
            for item in [train, valid]:
                if not isinstance(item, np.ndarray):
                    raise ConfigException(
                        "Items in the cv_splits_indices list should be of type List[numpy.array]",
                        target='_CVSplits', has_pii=False,
                        reference_code=ReferenceCodes._VALIDATE_CUSTOM_CV_SPLITS_NUMPY_ARRAY_TYPE)
                if np.max(item) >= max_size:
                    raise ConfigException(
                        "train index or valid index is out of bound", target='_CVSplits', has_pii=False,
                        reference_code=ReferenceCodes._VALIDATE_CUSTOM_CV_SPLITS_MAX_INDEX)
                if np.min(item) < 0:
                    raise ConfigException(
                        "train index or valid index is out of bound", target='_CVSplits', has_pii=False,
                        reference_code=ReferenceCodes._VALIDATE_CUSTOM_CV_SPLITS_MIN_INDEX)
                if len(np.unique(item)) != item.shape[0]:
                    raise ConfigException(
                        "train index or valid index has duplicates", target='_CVSplits', has_pii=False,
    ...
    def _create_cv_split_one_timeseries(self, X_tsdf, CV, max_horizon, rownum_colname):
        ...
        :type X_tsdf: TimeSeriesDataFrame
    ...
    def _create_cv_splits(self, X, frac_test, frac_valid, CV,
        ...
        if self._cv_split_type is None:
            raise ConfigException("Cross validation type not found", target='_CVSplits', has_pii=False,
        ...
        if self._cv_split_type == _CVType.CustomCrossValidationSplit:
            ...
            if (not max_horizon) or (max_horizon <= 0):
                raise ConfigException(
                    ('{} must be set and greater than zero for Rolling Origin Validation. ' +
                     'Value given: {}').format(constants.TimeSeries.MAX_HORIZON, max_horizon),
