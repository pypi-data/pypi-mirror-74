...
from azureml.automl.core.shared import logging_utilities as logging_utilities
...
from azureml.automl.core.shared.exceptions import PipelineRunException, ClientException, AutoMLException
...
from azureml.automl.runtime.shared.resource_limits import DEFAULT_RESOURCE_LIMITS, SafeEnforceLimits
...
class _PipelineOutputInternal:
    ...
    def map_from_results(
            ...
            training_type: str,
            pipeline_spec: Optional[pipeline_spec_module.PipelineSpec] = None) -> '_PipelineOutputInternal':
        if len(results) <= 2:
        ...
        error_msg = 'Failed while executing pipeline in stage {}.'.format(training_type)
        if pipeline_spec:
            error_msg = ' '.join([error_msg, str(pipeline_spec)])
        ...
        if training_type in [training_types.TrainAndValidation, training_types.TrainValidateTest]:
            ...
            if status:
                raise PipelineRunException(error_msg, target=PipelineRunException.PIPELINE_RUN, has_pii=False)
            ...
            train_time = [x[result_types.TRAIN_TIME] for x in results[result_types.CV_METRICS]]
            model = results[result_types.MODELS][training_type]
...
def run_pipeline(automl_settings: AutoMLBaseSettings,
    ...
    with logging_utilities.log_activity(logger, activity_name=constants.TelemetryConstants.RUN_PIPELINE_NAME,
        ...
        if automl_settings.is_timeseries:
            problem_info.timeseries_param_dict = utilities._get_ts_params_dict(automl_settings)
        ...
        with automl_run_context.get_run() as run:
            current_run = None if automl_settings.track_child_runs else run
        ...
        if results is None:
            raise PipelineRunException("Failed to train pipeline. Status: {}. Pipeline: {}".format(
                status, str(pipeline_spec)), target=PipelineRunException.PIPELINE_OUTPUT, has_pii=False)
        ...
        if automl_pipeline.is_ensemble_pipeline:
            ...
            pipeline_output_internal = _PipelineOutputInternal.map_from_results(
                results, training_type, pipeline_spec)
            ...
            if training_type in \
                ...
                if result_full is None:
                    raise PipelineRunException("Failed while training full result. Status: {}. Pipeline: {}".format(
                        status, str(pipeline_spec)), target=PipelineRunException.PIPELINE_OUTPUT, has_pii=False)
                ...
                pipeline_output_internal_train_full = _PipelineOutputInternal.map_from_results(
                    result_full, constants.TrainingType.TrainFull, pipeline_spec)
