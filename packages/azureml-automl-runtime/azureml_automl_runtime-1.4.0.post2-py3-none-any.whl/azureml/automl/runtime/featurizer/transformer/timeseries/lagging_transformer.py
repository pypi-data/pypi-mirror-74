...
from azureml.automl.core.shared.exceptions import InsufficientDataException, InvalidDataTypeException
    ...
    def transform(self, x):
        ...
        if not isinstance(x, pd.DataFrame) and not isinstance(x, np.ndarray) \
            ...
            raise InvalidDataTypeException(
                "x should be dataframe or numpy array or scipy sparse matrix", has_pii=False,
                reference_code='lagging_transformer.LaggingTransformer.transform')
        ...
        if x.shape[0] < self._lag_length:
            ...
            raise InsufficientDataException(
                "Input does not have enough rows.", has_pii=False,
                reference_code='lagging_transformer.LaggingTransformer.transform')
