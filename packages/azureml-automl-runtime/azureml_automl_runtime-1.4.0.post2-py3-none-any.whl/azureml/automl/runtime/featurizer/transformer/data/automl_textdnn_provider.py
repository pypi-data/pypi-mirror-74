...
class AutoMLPretrainedDNNProvider(AbstractWordEmbeddingsProvider):
    ...
    def __init__(self, logger: Optional[logging.Logger] = None,
        ...
        if embeddings_info is None:
            raise ConfigException("Unsupported pretrained dnn type {0}".format(model_name),
                                  reference_code=ReferenceCodes._TEXT_DNN_PROVIDER_INIT).with_generic_msg(
                "Unsupported pretrained dnn type")
    ...
    def _load_from_disk(self) -> None:
        ...
        if self._dnn_dir_name is None:
            self._logger.warning("Model loading failed from the folder: {path}".format(path=self._dnn_dir_name))
        else:
