...
from azureml.automl.core.shared.exceptions import InvalidArgumentException, PredictionException, AutoMLException
...
class ClientRunner:
    ...
    def activity_logger(self) -> ActivityLogger:
        """
        Lazily instantiate an activity logger.
        ...
        if self._log_config is None:
            ...
            try:
                # TODO: Remove this reference (will require more logger refactoring to move field setting logic
                # to the handler from the logger)
                ...
                logger = TelemetryActivityLogger(namespace=namespace,
                                                 ...
                                                 custom_dimensions=custom_dimensions)  # type: ActivityLogger
                if self.execution_context is not None:
                    child_run_id = self.execution_context.run_id
                    logger.update_default_properties(
                        {
                            "parent_run_id": self.execution_context.parent_run_id,
                            "child_run_id": child_run_id,
                            # TODO Child run is "RunId" in StandardFields
                            "run_id": child_run_id
                        }
                    )
                return logger
    ...
    def time_fit(self, m, X, y, sample_weight=None):
        ...
        """
        with log_utils.log_activity(self.activity_logger, activity_name=constants.TelemetryConstants.TIME_FIT_NAME):
    ...
    def time_fit_input_dataset(self,
        ...
        :return: The time elapsed for fit.
        """
        with log_utils.log_activity(self.activity_logger, activity_name=constants.TelemetryConstants.TIME_FIT_INPUT):
            t = datetime.datetime.utcnow()  # time.process_time()
    ...
    def _run_train_valid(self, dataset, pipeline_spec,
        ...
        """
        with log_utils.log_activity(self.activity_logger,
    ...
    def _run_cv(self, dataset, pipeline_spec, problem_info,
        ...
        """
        with log_utils.log_activity(self.activity_logger, activity_name=constants.TelemetryConstants.RUN_CV_NAME):
            scores = []
            fit_times = []
            models = []
    ...
    def _run(self, dataset, pipeline_spec, problem_info, sets_to_run,
        ...
        """
        with log_utils.log_activity(self.activity_logger, activity_name=constants.TelemetryConstants.RUN_NAME):
            with dataset.open_dataset():
                ...
                if constants.TrainingType.TrainAndValidation in sets_to_run:
                    ...
                    try:
                        ...
                        log_utils.log_traceback(e, self.activity_logger)
                        results[TrainingResultsType.TRAIN_VALIDATE_STATUS] = "Failed"
                ...
                if constants.TrainingType.TrainFull in sets_to_run:
                    ...
                    try:
                        ...
                        log_utils.log_traceback(e, self.activity_logger)
                        results[TrainingResultsType.TRAIN_FULL_STATUS] = "Failed"
                ...
                if (constants.TrainingType.CrossValidation in sets_to_run or
                    ...
                    try:
                        ...
                        log_utils.log_traceback(e, self.activity_logger)
                        results[TrainingResultsType.CV_STATUS] = "Failed"
        ...
        with log_utils.log_activity(self.activity_logger,
            ...
            if self.task == Tasks.CLASSIFICATION:
                ...
                scores = scoring.score_classification(
                    ...
                    logger=self.activity_logger, use_binary=self._use_binary_metrics)
    ...
    def _predict(self, task, model, X_valid):
        ...
        """
        with log_utils.log_activity(self.activity_logger, activity_name=constants.TelemetryConstants.PREDICT_NAME):
    ...
    def run(self,
            ...
            current_run: Optional[Any] = None) -> Tuple[Any, Optional[BaseException]]:
        """
        ...
        if is_ensemble_iteration:
            kwargs['current_run'] = current_run
        ...
        limiter = SafeEnforceLimits(enable_limiting=enforce_limits, logger=self.activity_logger, **constraints)
        result, exit_status, _ = limiter.execute(self.working_dir, func, *(dataset, pipeline_spec, problem_info),
                                                 **kwargs)
        return result, exit_status

    def _run_ensembling_internal(self, dataset, pipeline_spec, problem_info, sets_to_run, current_run, **kwargs):
        with log_utils.log_activity(self.activity_logger,
                                    activity_name=constants.TelemetryConstants.RUN_ENSEMBLING_NAME):
            with dataset.open_dataset():
                pipeline = pipeline_spec.instantiate_pipeline_spec(
                    problem_info, is_sparse=dataset.get_is_sparse(), current_run=current_run)
