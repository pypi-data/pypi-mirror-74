...
class ObjectSpec:
    ...
    def prepare_kwargs(self, cls, problem_info,
                       ...
                       dataset_metadata=None, current_run=None):
        """Set self.prepared_kwargs, a modified copy of param_kwargs."""
        raise NotImplementedError()

    def instantiate(self, problem_info, random_state=None, num_threads=1, dataset_metadata=None, current_run=None):
        ...
        self.prepare_kwargs(cls, problem_info, random_state=random_state,
                            num_threads=num_threads, dataset_metadata=dataset_metadata, current_run=current_run)
...
class PreprocessorObject(ObjectSpec):
    ...
    def prepare_kwargs(self, cls, problem_info,
                       random_state=None, num_threads=1, dataset_metadata=None, current_run=None):
...
class SklearnObject(ObjectSpec):
    ...
    def prepare_kwargs(self, cls, problem_info,
        ...
        if argspec.args:
            ...
            if 'current_run' in argspec.args:
                self.prepared_kwargs['current_run'] = current_run
...
class EnsembleObject(ObjectSpec):
    ...
    def instantiate(self, problem_info, random_state=None, num_threads=1, dataset_metadata=None, current_run=None):
        ...
        self.prepare_kwargs(cls, problem_info, random_state=random_state,
                            num_threads=num_threads, dataset_metadata=None, current_run=current_run)
...
class PipelineSpec:
    ...
    def instantiate_pipeline_spec(self,
        ...
        pipe = PipelineSpec._instantiate_steps(objs, problem_info, random_state=random_state,
                                               ...
                                               dataset_metadata=dataset_metadata, current_run=current_run)
    ...
    def _instantiate_steps(spec_steps: List[ObjectSpec],
                           ...
                           dataset_metadata: Optional[Any] = None,
                           current_run: Optional[Any] = None) -> Union[pipeline.Pipeline, Any]:
        """
        ...
        for spec_step in spec_steps:
            ...
            if has_categoricals and accepts_categoricals and class_name not in categorical_params:
                preprocessor = PipelineSpec._instantiate_step(
                    spec_step, problem_info, random_state, current_run=current_run)
                step = PipelineSpec._wrap_categorical_preprocessor(preprocessor, problem_info)
            else:
                step = PipelineSpec._instantiate_step(
                    spec_step, problem_info, random_state, dataset_metadata, current_run)
    ...
    def _instantiate_step(spec_step: ObjectSpec,
                          ...
                          dataset_metadata: Optional[Any] = None,
                          current_run: Optional[Any] = None) -> Any:
    ...
    def __str__(self):
        pipeline_name = "Pipeline Class Names: {}".format(self.summary())
        pipeline_id = "Pipeline ID: {}".format(self.pipeline_id)
        return ' '.join([pipeline_name, pipeline_id])
