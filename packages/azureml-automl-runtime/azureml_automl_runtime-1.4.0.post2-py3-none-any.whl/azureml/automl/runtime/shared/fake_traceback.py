...
class FakeCode:
    ...
    @classmethod
    def deserialize(cls: 'Type[FakeCode]', d: Dict[str, Any]) -> Optional['FakeCode']:
        ...
        obj = cls.__new__(cls)  # type: Optional['FakeCode']
        obj.__dict__ = d
        return obj
...
class FakeFrame:
    ...
    @classmethod
    def deserialize(cls: 'Type[FakeFrame]', d: Dict[str, Any]) -> Optional['FakeFrame']:
        """Deserialize this object from a dict."""
        if d is None:
            return None
        # We do NOT call the constructor here because we need to set attributes on it directly.
        # __init__() calls __new__() under the hood, then runs the rest of the code inside itself.
        obj = cls.__new__(cls)  # type: Optional['FakeTraceback']
...
class FakeTraceback:
    ...
    def serialize(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'tb_frame': self.tb_frame.serialize(),
            'tb_lineno': self.tb_lineno,
            'tb_lasti': self.tb_lasti
        }

    @classmethod
    def deserialize(cls: 'Type[FakeTraceback]', d: Dict[str, Any]) -> Optional['FakeTraceback']:
        """Deserialize this object from a dict."""
        if d is None:
            return None
        # We do NOT call the constructor here because we need to set attributes on it directly.
