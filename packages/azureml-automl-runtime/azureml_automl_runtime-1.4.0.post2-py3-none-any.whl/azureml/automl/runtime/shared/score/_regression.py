...
class MAPE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        return _mape(self._y_test, self._y_pred, logger=self._logger)
...
class NormMeanAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        return ret / np.abs(self._y_max - self._y_min)  # type: ignore
...
class NormRMSE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        ret = np.sqrt(sklearn.metrics.mean_squared_error(
            self._y_test, self._y_pred, sample_weight=self._sample_weight,
            multioutput='uniform_average'))
        ret = ret / np.abs(self._y_max - self._y_min)  # type: ignore
...
def _mape(y_true: np.ndarray,
          y_pred: np.ndarray,
          logger: Optional[logging.Logger] = None) -> np.float64:
    """
...
def _smape(y_true: np.ndarray,
    ...
    if len(y_true_safe) < 1:
        if logger is not None:
            logger.warning('Skipping SMAPE calculation since there not enough non-zero, non-nan values.')
        return cast(np.float64, np.nan)
