...
from azureml.automl.core.shared.exceptions import MemorylimitException, PipelineRunException
...
def run_in_proc(working_dir: Optional[str],
    ...
    :param logger: logger to use in case of failure
    ...
    try:
        ...
        with open(output_file_name, 'rb') as output_file:
            val, err, tb = cast(Tuple[T, Optional[BaseException], Optional[Dict[str, Any]]], dill.load(output_file))
        if err is not None:
            # We MUST log the traceback here, because it is not possible to reraise with this fake traceback without
            # additional interpreter hacks involving manually constructing similar stack frames.
        ...
        # Clean up the temporary files. We don't want to remove them if the job fails, so that we can debug the
        # process execution.
...
def check_process_success(process: subprocess.Popen, stderr_file_name: str) -> None:
    """
