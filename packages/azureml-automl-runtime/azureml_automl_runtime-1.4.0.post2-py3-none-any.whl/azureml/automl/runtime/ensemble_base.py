...
from azureml.automl.runtime._offline_automl_run import _OfflineAutoMLRunUtil, _OfflineAutoMLRun
...
class EnsembleBase(BaseEstimator, ABC):
    ...
    LOCAL_MODEL_FILE_TEMPLATE = "model_{}.pkl"
    ...
    def fit_ensemble(self,
        ...
        fitted_pipelines = self._fetch_fitted_pipelines(logger, child_runs, model_artifact_name, goal)
        ...
        if total_pipelines_for_ensembling == 0:
            raise AutoMLEnsembleException('Could not find any models for running ensembling.',
    ...
    def _fetch_fitted_pipelines(self, logger, child_runs, model_artifact_name, goal):
    ...
    def _download_fitted_models_for_child_runs(self, logger, child_runs, model_remote_path):
        ...
        for index, run in enumerate(child_runs):
            result.append(self._download_model(run, index, model_remote_path, logger))
        return result
    ...
    @staticmethod
    def _download_model(child_run, index, remote_path, logger):
        ...
        try:
            local_model_file = EnsembleBase.LOCAL_MODEL_FILE_TEMPLATE.format(index)
            child_run.download_file(
                name=remote_path, output_file_path=local_model_file, _validate_checksum=True)
            logger.info("Downloaded fitted model of size {0} for child run {1}".format(
    ...
    def _get_child_runs(parent_run, ensemble_run):
        ...
        if isinstance(ensemble_run, _OfflineAutoMLRun):
            return _OfflineAutoMLRunUtil.get_all_sibling_child_runs(ensemble_run)
        else:
