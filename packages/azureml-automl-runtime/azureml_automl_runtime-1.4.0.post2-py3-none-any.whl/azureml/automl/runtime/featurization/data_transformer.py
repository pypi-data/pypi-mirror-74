...
class DataTransformer(AutoMLTransformer, FeaturizationInfoProvider):
    ...
    def fit_transform_with_logger(self,
                                  ...
                                  logger: Optional[logging.Logger] = None,
                                  feature_config: Optional[List[TransformerAndMapper]] = None,
                                  columns_types_mapping: Optional[Dict[str, np.dtype]] = None,
                                  column_purposes: Optional[List[StatsAndColumnPurposeType]] = None,
                                  engineered_feature_names: Optional[_GenerateEngineeredFeatureNames] = None,
    ...
    def configure_featurizers(self, df: DataInputType, y: DataSingleColumnInputType = None) -> \
            Tuple[Optional[List[TransformerAndMapper]],
                  Dict[str, np.dtype],
        ...
        """
        mappers = None  # type: Optional[List[TransformerAndMapper]]
        if isinstance(df, np.ndarray):
            df = pd.DataFrame(df)
        ...
        return mappers, self._columns_types_mapping, \
            self.stats_and_column_purposes, self._engineered_feature_names_class
    ...
    def fit(self, df: DataInputType, y: DataSingleColumnInputType = None) -> "DataTransformer":
        ...
        if self._is_onnx_compatible and self.mapper is not None:
            try:
                ...
                ).with_generic_msg("MemoryError caused by insuffcient memory while mapper fit.")
        ...
        if feature_config is None:
            raise ConfigException(exception_message="Feature config cannot be None.",
        ...
        for transformer_mapper in feature_config:
            ...
            try:
                ...
                raise MemorylimitException.from_exception(
                    e, target='TransformerMapperFit',
                    reference_code=ReferenceCodes._DATA_TRANSFORMER_FIT_DATAFRAME_MAPPER_MEMORYERROR
                ).with_generic_msg("MemoryError caused by insuffcient memory while transformer_mapper fit.")
            except Exception as ex:
    ...
    def transform(self, df: DataInputType) -> Union[pd.DataFrame, np.ndarray, sparse.spmatrix]:
        ...
        try:
            ...
            raise MemorylimitException.from_exception(
                e, target='ApplyTransforms',
                reference_code=ReferenceCodes._DATA_TRANSFORMER_TRANSFORM_MEMORYERROR
            ).with_generic_msg("MemoryError caused by insuffcient memory while applying transformation.")
        except Exception as ex:
        ...
        combiner = FeaturizedDataCombiners.get(is_there_a_sparse_column,
                                               ...
                                               memory_utilities._is_low_memory(self.logger))

        ret = combiner(features, **{'working_directory': self.working_directory,
                                    'logger': self.logger,
                                    'pickler': self.pickler})  # type: Union[pd.DataFrame, np.ndarray, sparse.spmatrix]
        return ret
        ...
        try:
            if self._enable_feature_sweeping:
                ...
                if sweeped_transforms is not None and len(sweeped_transforms) > 0:
                    self._logger_wrapper('info', 'Sweeping added the following transforms: {sweeped_transforms}'
                                         .format(sweeped_transforms=','
                                                 .join(map(self._pipeline_name_, sweeped_transforms))))
        ...
        if len(type_converted_columns) > 0:
            for col, col_type in type_converted_columns.items():
                try:
                    ...
                    raise DataErrorException(
                        "Error converting the input column [{0}] into fitted column type {1}.".format(col, col_type),
                        reference_code=ReferenceCodes._DATA_TRANSFORMER_TRANSFROM_WRONG_COLUMN_TYPE
                    ).with_generic_msg(
                        "Error converting the input column number [{0}] into fitted column type.".format(
