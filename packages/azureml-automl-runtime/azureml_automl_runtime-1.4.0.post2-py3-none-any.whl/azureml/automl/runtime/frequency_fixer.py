...
def get_tsdf_frequency_and_start(
    ...
    if grain_column_names is not None \
            ...
            and not isinstance(grain_column_names, tuple):
    ...
    if len(tsdf) == 0:
        ...
        # column. This theoretically may lead to the empty data frame. This sutuation should
...
def check_types(X: pd.DataFrame,
    ...
    if not isinstance(X, pd.DataFrame):
        raise InvalidDataTypeException('X must be of a DataFrame type.', has_pii=False)
    if y is not None and not isinstance(y, np.ndarray):
        raise InvalidDataTypeException('y must be of a numpy.ndarray type.', has_pii=False)
    ...
    if not np.issubdtype(X[time_column_name].dtype, np.datetime64):
        try:
            ...
            raise DataFormatException(
                msg.format(time_column_name, e),
                reference_code=ReferenceCodes._FREQUENCY_FIXER_CONVERT_INVALID_VALUE).with_generic_msg(
