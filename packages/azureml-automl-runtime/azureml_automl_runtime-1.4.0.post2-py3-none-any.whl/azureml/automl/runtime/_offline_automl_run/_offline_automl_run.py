...
import pytz
from azureml.automl.runtime._offline_automl_run._offline_automl_run_base import _OfflineAutoMLRunBase


class _OfflineAutoMLRun(_OfflineAutoMLRunBase):
    """
    ...
    def __init__(self, run_id: str, run_folder: str) -> None:
        ...
        self.created_time = datetime.now().replace(tzinfo=pytz.UTC)
    ...
    def start(self) -> None:
        """Start the run."""
        self.start_time = datetime.now().replace(tzinfo=pytz.UTC)

    def complete(self, _set_status: bool = True) -> None:
        """Complete the run."""
        self.end_time = datetime.now().replace(tzinfo=pytz.UTC)
    ...
    def get_uploaded_file_names(self) -> List[Any]:
        ...
        return self._read_reserved_metadata_list(_OfflineAutoMLRun.UPLOADS_FILE_NAME)
    ...
    def _store_uploaded_file_name(self, file_name: str) -> None:
        self._append_reserved_metadata_list(_OfflineAutoMLRun.UPLOADS_FILE_NAME, file_name)
