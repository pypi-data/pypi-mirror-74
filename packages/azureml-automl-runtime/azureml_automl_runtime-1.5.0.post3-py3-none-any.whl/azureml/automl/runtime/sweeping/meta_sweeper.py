...
class MetaSweeper:
    ...
    def _sweep_feature(self,
        ...
        log_config = logging_utilities.LogConfig.from_logger(logger)
        try:
    ...
    def _sweep_internal(sweepers: List[AbstractSweeper],
                        ...
                        log_config: logging_utilities.LogConfig) -> List[Tuple[str, Pipeline]]:
        return_transforms = []  # type: List[Tuple[Any, Any]]

        logger = MetaSweeper._get_logger(log_config)
    ...
    @staticmethod
    def _get_logger(log_config: logging_utilities.LogConfig) -> activity_logger.ActivityLogger:
        try:
            ...
            filename, verbosity, namespace = log_config.get_params()
            component_name = log_config.get_component_name()
            custom_dimensions = log_config.get_custom_dimensions()
            logger = TelemetryActivityLogger(namespace=namespace,
                                             filename=filename,
                                             verbosity=verbosity,
                                             component_name=component_name,
                                             custom_dimensions=custom_dimensions)
            return cast(activity_logger.ActivityLogger, logger)
        except Exception:
            return activity_logger.DummyActivityLogger()
