...
def check_sample_weight(x: DataInputType,
    ...
    if not isinstance(sample_weight, np.ndarray):
        raise InvalidDataTypeException(sample_weight_name + " should be numpy array").with_generic_msg(
            "sample_weight_name should be numpy array")

    if x.shape[0] != len(sample_weight):
        raise DataShapeException(
            sample_weight_name + " length should match length of " + x_name).with_generic_msg(
                "sample_weight_name length should match length of x_name")

    if len(sample_weight.shape) > 1:
        raise DataShapeException(
            sample_weight_name + " should be a unidimensional vector").with_generic_msg(
    ...
    if automl_settings.primary_metric in \
        ...
        raise ScenarioNotSupportedException(
            "Sample weights is not supported for these primary metrics: {0}"
...
def check_memory_limit(X: DataInputType, y: DataInputType) -> None:
    ...
    if avail_memory is not None:
        ...
        if avail_memory < needed_memory:
            ...
            raise DataShapeException(msg.format(memory_per_df, avail_memory, needed_memory)).with_generic_msg(
    ...
    if sample_weight_column_name is not None:
        ...
        if sample_weight_column_name not in input.columns:
            msg = ('The sample weight column {} is not found in input data, '
                   'please double check')
            raise EmptyDataException(
                msg.format(sample_weight_column_name)).with_generic_msg(msg.format(MASKED))
...
def _check_if_automl_model_is_explainable(automl_algo_name: str) -> bool:
    if constants.SupportedModels.Forecasting.Prophet in automl_algo_name:
        return False
    else:
        return True
