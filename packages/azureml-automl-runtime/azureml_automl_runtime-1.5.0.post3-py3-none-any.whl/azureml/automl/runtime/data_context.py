...
from azureml.automl.runtime.shared.cache_store import MemoryCacheStore
...
class BaseDataContext:
    ...
    def __init__(self,
        ...
        :param weight_column_name: target column name.
...
class RawDataContext(BaseDataContext):
    ...
    def __init__(self,
        ...
        if automl_settings_obj.enable_streaming:
            X = training_data.drop_columns([automl_settings_obj.label_column_name,
            ...
            X_valid = validation_data.drop_columns([automl_settings_obj.label_column_name,
                                                    automl_settings_obj.weight_column_name]) \
...
class TransformedDataContext(BaseDataContext):
    ...
    def __getstate__(self):
        ...
        return {'X': PickleableDataflow(self.X) if isinstance(self.X, Dataflow) else self.X,
                'y': self.y,
                'X_valid': PickleableDataflow(self.X_valid) if isinstance(self.X_valid, Dataflow) else self.X_valid,
    ...
    def __setstate__(self, state):
        ...
        # pickleable Dataflow object back to Dataflow
        if isinstance(state.get('X'), PickleableDataflow):
            self.X = state.get('X').get_dataflow()
        else:
            self.X = state['X']
        if isinstance(state.get('X_valid'), PickleableDataflow):
            self.X_valid = state.get('X_valid').get_dataflow()
        else:
    ...
    def _load_from_cache(self):
        ...
        self.transformers = retrieve_data_list.get('transformers', {})  # type: ignore
        self.cv_splits = retrieve_data_list.get('cv_splits')
        self._on_demand_pickle_keys = retrieve_data_list.get('_on_demand_pickle_keys')  # type: ignore
