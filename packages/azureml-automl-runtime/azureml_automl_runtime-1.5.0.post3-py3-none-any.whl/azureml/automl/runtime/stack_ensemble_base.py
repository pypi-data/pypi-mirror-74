...
from azureml.automl.runtime import _ensemble_selector, ensemble_base
from azureml.automl.runtime.featurizer.transformer.timeseries import TimeSeriesTransformer, TimeSeriesPipelineType
...
from azureml.automl.runtime.shared import runner
...
class StackEnsembleBase(ensemble_base.EnsembleBase, ABC):
    ...
    def _create_ensembles(self, logger, fitted_pipelines, selector):
        ...
        if selector.training_type == constants.TrainingType.TrainAndValidation:
            ...
            raise ConfigException("Unsupported training type ({})".format(selector.training_type))\
                .with_generic_msg("Unsupported training type.")
    ...
    def _split_train_set_for_scoring_train_valid(
        ...
        if self._automl_settings.is_timeseries:
            ...
            feat_config = ts_params_dict.pop("featurization_config", None)
            tst = TimeSeriesTransformer(pipeline_type=TimeSeriesPipelineType.FULL,
                                        logger=None,
                                        featurization_config=feat_config,
                                        **ts_params_dict)
        ...
        if meta_learner_type is None:
            ...
            if self._automl_settings.task_type == constants.Tasks.REGRESSION:
                ...
                raise ConfigException(
                    ...
                    .with_generic_msg("Unsupported task_type.")
