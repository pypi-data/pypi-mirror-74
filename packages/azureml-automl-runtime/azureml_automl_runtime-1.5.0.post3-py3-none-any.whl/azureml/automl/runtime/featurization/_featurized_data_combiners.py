    ...
    def disk_based_sparse_combiner(
        ...
        try:
            ...
            raise MemorylimitException.from_exception(
                ex, msg="MemoryError caused by insuffcient memory while trying to combining transformed data.",
                has_pii=False,
                reference_code=ReferenceCodes._DISK_BASED_SPARSE_COMBINER_MEMORY
            )
        except Exception as ex:
            raise TransformException.from_exception(ex,
                                                    msg="Exception while trying to combining transformed data",
            ...
            for f in pickle_files:
                try:
                    ...
                    logger.error('Error while removing pickle files in data_transformer.transform.')
            if len(os.listdir(tmp_dir)) == 0:
                try:
                    os.rmdir(tmp_dir)
                except Exception:
                    logger.error('Error while removing temp directory in data_transformer.transform.')
