...
class DataTransformer(AutoMLTransformer, FeaturizationInfoProvider):
    ...
    @logging_utilities.function_debug_log_wrapped()
    def fit(self, df: DataInputType, y: Optional[DataSingleColumnInputType] = None) -> "DataTransformer":
        """
        ...
        if not self.transformer_and_mapper_list:
            raise ConfigException(exception_message="TransformerAndMapper list cannot be empty.",
        ...
        for transformer_mapper in self.transformer_and_mapper_list:
            ...
            try:
                ...
                ).with_generic_msg("MemoryError caused by insufficient memory while transformer_mapper fit.")
            except Exception as ex:
