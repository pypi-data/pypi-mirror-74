...
from azureml.automl.core.shared.exceptions import ClientException, InvalidDataTypeException
...
class TimeSeriesDataFrame(pd.DataFrame):
        ...
        if val is None:
            ...
            if verify.is_iterable_but_not_string(val):
                for col in val:
                    if not isinstance(col, str):
                        raise ColumnTypeNotSupportedException(
                            'grain_colnames must all be str.', has_pii=False)
                    if col not in self.columns and col not in self.index.names:
                        raise DataFrameMissingColumnException(pii_message=self.GRAIN_ABSENT_TEMPL % col,
                ...
                if val not in self.columns and val not in self.index.names:
                    raise DataFrameMissingColumnException(pii_message=self.GRAIN_ABSENT_TEMPL % val,
                                                          target=DataFrameMissingColumnException.GRAIN_COLUMN)
                with catch_warnings():
                    simplefilter('ignore')
                    self.__grain_colnames = list([val])
                ...
                raise ColumnTypeNotSupportedException(
                    'grain column must be specified by a str value', has_pii=False)
            ...
            if isinstance(val, str):
                if val not in self.columns and val not in self.index.names:
                    raise DataFrameMissingColumnException(
                        pii_message=self.TIME_ABSENT_TEMPL % val,
                ...
                raise ColumnTypeNotSupportedException(
                    'time column must be specified by a single str value', has_pii=False)
    ...
    def origin_time_colname(self, val):
        if val is None:
            ...
            if isinstance(val, str):
                if val not in self.columns and val not in self.index.names:
                    raise DataFrameMissingColumnException(
                        pii_message=self.ORIGIN_MISSING_TEMPL % val,
                        target=DataFrameMissingColumnException.ORIGIN_COLUMN)
                with catch_warnings():
                    simplefilter('ignore')
                    self.__origin_time_colname = val
                ...
                raise ColumnTypeNotSupportedException(
                    'origin time column must be specified by a single str value', has_pii=False)
        ...
        if val is None:
            ...
            if isinstance(val, str):
                if val not in self.columns:
                    raise DataFrameMissingColumnException(
                        pii_message=self.TARGET_MISSING_TEMPL % val,
                        target=DataFrameMissingColumnException.VALUE_COLUMN)
                self.__ts_value_colname = val
            else:
                raise ColumnTypeNotSupportedException(
                    'The ts_value_colname property must be a string.', has_pii=False)
        ...
        if val is None:
            ...
            if isinstance(val, list) and isinstance(val[0], str):
                for col in val:
                    if col not in self.columns and col not in self.index.names:
                        raise DataFrameMissingColumnException(
                            pii_message=self.GROUP_MISSING_TEMPL % col,
                            target=DataFrameMissingColumnException.GROUP_COLUMN)
                my_group_colnames = val
            elif isinstance(val, str):
                if val not in self.columns and val not in self.index.names:
                    raise DataFrameMissingColumnException(
                        pii_message=self.GROUP_MISSING_TEMPL % val,
                        target=DataFrameMissingColumnException.GROUP_COLUMN)
                my_group_colnames = list([val])
            else:
                raise ColumnTypeNotSupportedException(
                    'group column must be specified by a str value', has_pii=False)
    ...
    def _extract_time_series(self, colnames=None):
        ...
        if verify.is_iterable_but_not_string(colnames):
            ...
            raise ColumnTypeNotSupportedException(
                ...
                has_pii=True).with_generic_msg('Unrecognized column type.')
        ...
        if len(not_in_frame) > 0:
            error_message = ("Columns {} not found in the "
                             "TimeSeriesDataFrame!")
            dfv = DataFrameValueException(error_message.format(not_in_frame), has_pii=True)
    ...
    def _check_time_column(self, col):
        if col in self.index.names:
            ...
            raise DataFrameMissingColumnException(
                pii_message=self.TIME_ABSENT_TEMPL % col,
                target=DataFrameMissingColumnException.TIME_COLUMN)
    ...
    def _to_datetime_index_func(self, time_index, format=None, *args, **kwargs):
        ...
        try:
            ...
            raise DataFrameValueException('Cannot turn the input into type'
                                          ...
                                          'datetime-like data.', has_pii=False)
    ...
    def _check_column_equal_across_origin(self, colname=None):
        ...
        if colname is not None:
            ...
            if not isinstance(colname, str):
                raise InvalidDataTypeException('colname must be of type {0}'.format(str), has_pii=False)
            # check if column is present in TSDF
            if colname not in self.columns:
                error = ("The column {} not found in the "
                         "TimeSeriesDataFrame!")
                raise DataFrameMissingColumnException(
                    pii_message=error.format(colname)
                )
            ...
            if self.time_colname is not None \
                ...
                if not np.all(all_equal.values):
                    # Uncomment when the PII message is correctly set in the exception
                    ...
                    raise DataFrameIncorrectFormatException(
                        exception_message=error_message_no_client_info,
                        pii_message=error_message)
        ...
        if self.index.names != index_keys:
            ...
            if any(self.duplicated(index_keys)):
                raise DuplicatedIndexException(
                    TimeSeriesDataFrame.DUPLICATED_INDEX_ERROR, has_pii=False)
    ...
    def reset_index(self, level=None, inplace=False, **kwargs):
        ...
        if isinstance(level, str):
            ...
            if all(isinstance(l, str) for l in level):
                ...
                raise DataFrameTypeException(error_message, has_pii=False)
        elif level is None:
            levels_to_reset = [lv for lv in range(self.index.nlevels)
                               if lv != final_time_col_level]
            warn(RESET_TIME_INDEX_MSG, UserWarning)
        else:
            raise DataFrameTypeException('Unsupported type given for level.', has_pii=False)
    ...
    def infer_freq_by_grain(self, return_freq_string=False):
        ...
        if self.time_colname is None:
            raise DataFrameFrequencyException(
                exception_message='Cannot infer frequency without a time index.', fas_pii=False)
    ...
    def __finalize__(self, other, method=None, **kwargs):
        ...
        # concat operation: using metadata of the first object
            ...
            if all(isinstance(x, TimeSeriesDataFrame) for x in other.objs):
                ...
                if len(indexes) > 1:
                    raise DataFrameIncorrectFormatException(
                        "index values for data_frames do not match.", has_pii=False)
                for name in self._metadata:
                    name_values_all = []
                    for o in other.objs:
                        if hasattr(o, name):
        ...
        if any(self.index.duplicated()):
            raise DuplicatedIndexException(TimeSeriesDataFrame.DUPLICATED_INDEX_ERROR, has_pii=False)
            ...
            if not isinstance(json_dict, dict):
                raise InvalidDataTypeException(
                    'Input of type {} can not be converted from json dictionary'.format(
                        type(json_str))).with_generic_msg("Input type cannot be converted to json.")
