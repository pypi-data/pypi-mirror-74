...
class StreamingFeaturizerTests(unittest.TestCase):
    ...
    def test_transformer_param_update_Imputer(self):
        ...
        try:
            ...
            streaming_featurizer = StreamingFeaturizer(X_train, 'label', featurization_config=featurization_config)
            streaming_transformer = streaming_featurizer.learn_transformations()
            streaming_transformer.fit(df)
            feat_data = streaming_transformer.transform(df)
            assert feat_data is not None
            self.assertIsNotNone(streaming_transformer)
    ...
    def test_featurize_with_nans(self):
        ...
        try:
            ...
            self.assertEqual(streaming_transformer._get_json_str_for_engineered_feature_name(
                'col1_MeanImputer'), '{"FinalTransformerName": "Transformer1", '
                '"Transformations": {"Transformer1": {"Input": ["col1"], "TransformationFunction": '
                '"Imputer", "Operator": "Mean", "FeatureType": "Numeric", "ShouldOutput": true, '
                '"TransformationParams": null}}, "Value": null}')
            self.assertEqual(streaming_transformer._get_json_str_for_engineered_feature_name('InvalidFeature'), None)
    ...
    def test_drop_transformer(self):
        ...
        try:
            ...
            self._check_featurization_summary(
                col1, FeatureType.AllNan, 0, [''], dropped='Yes')
        finally:
            os.remove(path)
