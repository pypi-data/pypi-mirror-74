...
class TestFTKSTLFeaturizer(unittest.TestCase):
    ...
    def test_fit_and_transform_logged(self):
        ...
        X_train, X_test = self._get_tsdfs(train_len=60,
                                          ...
                                          seasonality=5,
                                          freq='AS-JAN')
        transform = STLFeaturizer()
        utilities.assert_timeseries_fun_logged(transform, True, X_train, X_test)
        utilities.assert_timeseries_fun_logged(transform, False, X_train, X_test)
