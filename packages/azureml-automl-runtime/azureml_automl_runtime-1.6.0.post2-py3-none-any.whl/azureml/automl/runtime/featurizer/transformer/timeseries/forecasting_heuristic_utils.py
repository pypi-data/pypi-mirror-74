...
from azureml.automl.core.shared.exceptions import EmptyDataException
from azureml.automl.core.shared.forecasting_exception import (
    ForecastingDataException, DataFrameFrequencyException)
...
def _log_warn_maybe(logger: Optional[logging.Logger],
                    msg: str,
                    exception: Optional[BaseException] = None) -> None:
    """
    Function to log warning if logger is passed.
...
def analyze_pacf_one_grain(series: pd.Series, logger: Optional[logging.Logger] = None) ->\
    ...
    if period == 0:
        raise DataFrameFrequencyException(
            "The frequency is not supported.", has_pii=False,
    ...
    if any([pd.isna(x) for x in series[series.columns[0]]]):
        ...
        if len(series) == 0:
            raise EmptyDataException(
                "One of the series is empty.", has_pii=False,
                reference_code='forecasting_heuristic_utils.analyze_pacf_one_grain')
    ...
    try:
        ...
        _log_warn_maybe(logger, msg, e)
        pacf_input_series = series
    except BaseException as e:
        msg = "Lag and rolling window calculation. STL decomposition. Unknown error."
        _log_warn_maybe(logger, msg, e)
        pacf_input_series = series

    try:
        pac = pacf(pacf_input_series, nlags=lags)
        ...
        msg = "Lag and rolling window calculation. Unknown error."
        _log_warn_maybe(logger, msg, e)
        return (None, None)
...
def analyze_pacf_per_grain(dataframe: pd.DataFrame,
                           ...
                           max_grains: int = 100,
                           logger: Optional[logging.Logger] = None) -> Tuple[int, int]:
    """
    ...
    if grain_colnames is not None:
        ...
        pk = subset.groupby(grain_colnames)[target_colname].apply(lambda x: analyze_pacf_one_grain(x, logger))
        ...
        if len(modes) == 0:
            _log_warn_maybe(logger, INVALID_CORRELATION)
