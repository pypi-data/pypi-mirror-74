...
class TimeSeriesTransformer(AbstractTimeSeriesTransformer, FeaturizationInfoProvider):
    ...
    def __init__(self, pipeline_type: TimeSeriesPipelineType = TimeSeriesPipelineType.FULL,
        ...
        super(TimeSeriesTransformer, self).__init__(logger, featurization_config, **kwargs)
    ...
    def _construct_pre_processing_pipeline(self,
        ...
        if TimeSeriesInternal.LAG_LEAD_OPERATOR in self._transforms and \
           self.parameters.get(TimeSeries.FEATURE_LAGS) == TimeSeries.AUTO:
            ...
            lag_list = lag_op.lags_to_construct.get(TimeSeriesInternal.DUMMY_TARGET_COLUMN)
            # do not lag original boolean columns
            ...
            if lag_list is not None:
                for feature in real_numerical_columns:
                    lag_op.lags_to_construct[feature] = lag_list
        ...
        default_pipeline.add_pipeline_step(
            ...
            NumericalizeTransformer(
                include_columns=self._get_included_columns(tsdf, _FeatureType.Categorical),
                exclude_columns=self._get_excluded_columns(tsdf, _FeatureType.Categorical),
                logger=self.logger))
    ...
    def _partial_fit(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> None:
        ...
        super(TimeSeriesTransformer, self)._partial_fit(X, y)
    ...
    def _remove_lag_lead_and_rw_maybe(self, df: pd.DataFrame, y: Optional[np.ndarray]) -> None:
        ...
        if self._transforms.get(TimeSeriesInternal.LAG_LEAD_OPERATOR) is not None:
            ...
            # Added assert to avoid mypy gate failure
            assert lag_op is not None
            num_features_to_lag = len(lag_op.lags_to_construct)
            lags = list(lag_op.lags_to_construct.values())[0]
        ...
        try:
            ...
            total_num_columns = len(df.columns)
            if total_num_columns > 0:
                feature_lag_adjustment = num_features_to_lag * number_of_lags / total_num_columns
            else:
                feature_lag_adjustment = 0
    ...
    def get_target_lags(self) -> List[int]:
        ...
        self._raise_no_fit_exception_maybe('timeseries_transformer.TimeSeriesTransformer.get_target_lags')
        lag_operator = None
        if self.pipeline is not None:
            lag_operator = self.pipeline.get_pipeline_step(TimeSeriesInternal.LAG_LEAD_OPERATOR)
    ...
    def get_target_rolling_window_size(self) -> int:
        ...
        self._raise_no_fit_exception_maybe(
            'timeseries_transformer.TimeSeriesTransformer.get_target_rolling_window_size')
        rolling_window = None
        if self.pipeline is not None:
