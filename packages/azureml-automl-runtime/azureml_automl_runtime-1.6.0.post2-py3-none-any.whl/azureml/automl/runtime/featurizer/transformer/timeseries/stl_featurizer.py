...
from azureml.automl.core.shared.exceptions import (
    InsufficientDataException, ConfigException, ClientException, EmptyDataException,
    MissingValueException, TransformException)
...
def _extend_series_for_sm9_bug(series_values: np.ndarray,
    ...
    return cast(np.ndarray, series_ext)
...
class STLFeaturizer(AzureMLForecastTransformerBase):
            ...
            if grain not in self._last_observation_dates.keys() or \
               grain not in self._first_observation_dates.keys():
                ...
                raise EmptyDataException(
                    ...
                    reference_code='stl_featurizer.STLFeaturizer.data_check').with_generic_msg(
                        error_msg.format('[MASKED]'))
            if df_one.time_index.min() < self._first_observation_dates[grain]:
                raise InsufficientDataException(
                    'The forecast date cannot be less then training date.',
        ...
        if (tsdf is None and target is None) or\
           (tsdf is not None and target is not None):
            raise MissingValueException(
                'tsdf or target, but not both should be provided.', has_pii=False,
    ...
    def _fit_one_grain(self,
        ...
        if df_one.shape[0] < 2:
            ...
            raise InsufficientDataException(error_msg.format(
        ...
        if self.seasonality >= len(df_one):
            ...
            raise InsufficientDataException(
                error_msg.format(grain, len(df_one), self.seasonality),
                reference_code='stl_featurizer.STLFeaturizer._fit_one_grain').with_generic_msg(
                    error_msg.format('[MASKED]', '[MASKED]', '[MASKED]'))
