...
from azureml.automl.runtime.shared import limit_function_call_limits
from azureml.automl.core.shared import logging_utilities
from azureml.automl.runtime.shared import spawn_client
from azureml.automl.core.shared.exceptions import MemorylimitException
from azureml.automl.core.shared.limit_function_call_exceptions import (CpuTimeoutException,
                                                                       TimeoutException,
                                                                       SubprocessException)
...
class EnforceLimits:
    ...
    def execute(self,
        ...
        logger.info('Calling function {} with argument types: {}, {}'.format(
            func,
        ...
        try:
            ...
            exit_status = TimeoutException.create_without_pii()
    ...
    def subprocess_func(func: 'Callable[..., T]',
        ...
        :return:
        """
        limit_function_call_limits.set_limits(mem_in_mb, num_procs, wall_time_limit_in_s,
                                              cpu_time_limit_in_s, grace_period_in_s)

        try:
            # TODO: Make sure verbosity transfers over correctly when we create the child process.
            # Currently everything is set to WARNING by default.
            ...
            res = (None, MemorylimitException.from_exception(e))
        except OSError as e:
