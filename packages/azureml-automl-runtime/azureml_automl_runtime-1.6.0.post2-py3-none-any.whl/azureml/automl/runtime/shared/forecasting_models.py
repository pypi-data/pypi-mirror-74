...
class _MultiGrainForecastBase:
    ...
    def _get_num_parallel_process(
            ...
            max_cores_per_iteration: Optional[int]) -> int:
    ...
    def fit(self,
        ...
        num_par_process = self._get_num_parallel_process(
            len(self._grain_levels), self._max_cores_per_iteration)
        ...
        if num_par_process == 1:
            for lvl, series_frame in tsdf_bygrain:
                lvl, first, last, model = _MultiGrainForecastBase._fit_single_grain(self, lvl, series_frame)
                self._first_observation_dates[lvl] = first
                self._last_observation_dates[lvl] = last
        ...
        if num_par_process > 1:
            results = Parallel(n_jobs=num_par_process)(delayed(_MultiGrainForecastBase._fit_single_grain)(
                self,
                lvl,
                series_frame.copy()) for lvl, series_frame in tsdf_bygrain)
            # Parse results received from Parallell
            for result in results:
                lvl, first, last, model = result
                self._first_observation_dates[lvl] = first
                self._last_observation_dates[lvl] = last
                self._models[lvl] = model
    ...
    def _fit_single_grain(untrained_model: '_MultiGrainForecastBase',
                          ...
                          ) -> Tuple[GrainType, pd.Timestamp, pd.Timestamp, Any]:
        """
...
class AutoArima(_MultiGrainForecastBase):
    ...
    def _fit_single_grain_impl(self, X_pred_grain: pd.DataFrame, grain_level: GrainType) -> Any:
        ...
        if len(series_values) < constants.TimeSeriesInternal.ARIMA_TRIGGER_CSS_TRAINING_LENGTH:
            ...
            model = self.pmdarima.auto_arima(series_values, error_action="ignore", seasonal=False, method="CSS")
        return model
...
class ProphetModel(_MultiGrainForecastBase):
    ...
    def __init__(self, **kwargs: Any):
        ...
        # for tuning, let's try playing with some prophet parameters
