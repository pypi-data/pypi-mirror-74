...
class WorkerClient():
        ...
        while input_cmd.lower() != 'q':
            ...
            try:
                ...
                if cmd == 'h':
                    ...
                    self._show_memory_usage(uuid=self.uuid)
                elif cmd == 'w':
    ...
    def _show_memory_usage(self, uuid=None):
        ...
        print('=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
        print('                          Rocon Client Memory Usage                              ')
        print('uuid   : {}'.format(uuid))
        print('pid    : {}'.format(pid))
        print('rss    : {0:9.2f} (MB)  {1}'.format(mem_usage.rss/PER_UNIT,
                        '"Resident Set Size," this is the non-swapped physical memory a process has used.'))
        print('vms    : {0:9.2f} (MB)  {1}'.format(mem_usage.vms/PER_UNIT,
                        '"Virtual Memory Size", this is the total amount of virtual memory used by the process.'))
        print('shared : {0:9.2f} (MB)  {1}'.format(mem_usage.shared/PER_UNIT,
                        'memory that could be potentially shared with other processes'))
        print('text   : {0:9.2f} (MB)  {1}'.format(mem_usage.text/PER_UNIT,
                        '"Text Resident Set", the amount of memory devoted to executable code.'))
        print('lib    : {0:9.2f} (MB)  {1}'.format(mem_usage.lib/PER_UNIT,
                        'the memory used by shared libraries'))
        print('data   : {0:9.2f} (MB)  {1}'.format(mem_usage.data/PER_UNIT,
                        '"Data Resident Set", the amount of physical memory devoted to other than executable code.'))
        print('dirty  : {0:9.2f} (MB)  {1}'.format(mem_usage.dirty/PER_UNIT,
