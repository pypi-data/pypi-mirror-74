...
class RoconLogger(metaclass=SingletonMetaClass):
    def __init__(self, max_filesize = MAX_LOG_FILESIZE, max_backup_count=MAX_LOG_BACKUP_COUNT):
        ...
        self.enabled_remove_old_logs = False
        ...
        if not os.path.exists(self._path):
            ...
            if self.enabled_remove_old_logs is True:
                log_files = glob.glob('{}/*_utc_*.log'.format(self._path), recursive=False)
                for f in log_files:
                    try:
                        os.remove(f)
                    except OSError as e:
    ...
    def enable_file_handler(self, enable=True, pre_fix=None):
        ...
        if pre_fix:
            self._log_filename = '{}/{}_{}_utc_{}.log'.format(self._path, pre_fix, current_time, current_utc_time)
        else:
            self._log_filename = '{}/{}_utc_{}.log'.format(self._path, current_time, current_utc_time)
