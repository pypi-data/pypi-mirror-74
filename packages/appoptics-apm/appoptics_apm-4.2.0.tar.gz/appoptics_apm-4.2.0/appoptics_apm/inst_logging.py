...
Copyright (C) 2020 by SolarWinds, LLC.
All rights reserved.
"""
import inspect
import logging
    ...
    def filter(self, record):
        ...
        if record.msg != '':
            if self.log_trace_id == 'always':
                record.ao = {'traceId': convert_xtr_to_trace_id(util.last_id())}
        ...
        try:
            ...
            for logger in loggers:
                ...
                if callable(fn):
                    ...
                    util.logger.debug('update_filters: logger {}, {}(LogInjectionFilter).'.format(logger, f_name))
                else:
                    util.logger.warning('update_filters: could not call {} on logger {}.'.format(f_name, logger))
        except Exception as e:
            util.logger.error("AppOptics APM error: %s" % str(e))
        finally:
            logging._releaseLock()

    def get_log_trace_id(self):
        return convert_xtr_to_trace_id(util.last_id())
...
def wrap_logger_init(func, self, *f_args, **f_kwargs):
    ...
    try:
        if log_injector.is_injecting():
            ...
            util.logger.debug('wrap_logger_init adding LogInjectionFilter to logger {}.'.format(self))
    except Exception as e:
        util.logger.error(
...
def wrap_logging_module():
    if util.ready() and not log_injector.wrapped:
        try:
            ...
            util.logger.debug('wrap_logging_module wraps {} of {} object'.format(fn.__name__, logger.__name__))
            log_injector.set_injection_mode(util.config['log_trace_id'])
