...
class VaultBaseStep(Step):
  ...
  def __init__(self, host):
    ...
    self.vault = hvac.Client(url=host)
...
class VaultInitStep(VaultBaseStep):
  ...
  def run(self, payload:Payload, start_fn:Callable, end_fn:Callable, prompt_fn:Callable) -> Payload:
    ...
    self.vault.sys.enable_secrets_engine(backend_type="kv", path="daas_public", options={"version": 2})
    self.vault.sys.enable_secrets_engine(backend_type="kv", path="daas_private", options={"version": 2})
    end_fn("OK")
