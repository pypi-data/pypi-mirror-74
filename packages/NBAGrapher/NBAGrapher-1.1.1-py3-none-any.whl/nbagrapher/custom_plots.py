...
def prepare_shot_chart(figsize):
    fig, ax = plt.subplots(figsize=figsize)
    if figsize is None:
        fig.set_size_inches(4, 3.76)
    ax.plot([-220, -220], [0, 140], linewidth=2, color='black')
    ax.plot([220, 220], [0, 140], linewidth=2, color='black')
    ax.add_artist(matplotlib.patches.Arc((0, 140), 440, 315, theta1=0, theta2=180, facecolor='none', edgecolor="black", lw=2))
...
class ComplexRadar():
    def __init__(self, fig, variables, ranges,
        ...
        axes = [fig.add_axes([0,0.1,0.9,0.8],polar=True,
                label = "axes{}".format(i))
                for i in range(len(variables))]
        l, text = axes[0].set_thetagrids(angles,
                                         labels=variables,fontweight='bold')
    ...
    def plot(self, data, *args, **kw):
        ...
        self.ax.legend(loc="lower center",bbox_to_anchor=(0,0))
