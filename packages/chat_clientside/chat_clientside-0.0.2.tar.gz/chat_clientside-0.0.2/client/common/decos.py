...
def login_required(func):
    ...
    def checker(*args, **kwargs):
        ...
        from common.variables import ACTION, EXISTENCE
                ...
                if isinstance(arg, socket.socket):
                    ...
                    for client in args[0].names:
                        if args[0].names[client] == arg:
                            found = True
            # Теперь надо проверить, что передаваемые аргументы не existence
            # сообщение. Если existence, то разрешаем
            for arg in args:
                if isinstance(arg, dict):
                    if ACTION in arg and arg[ACTION] == EXISTENCE:
                        found = True
