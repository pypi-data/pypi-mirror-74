...
class BSSDataset(SKDataset):
    ...
    def binary_max(sp, debug):
        ...
        spm = rec2polar(spm_with_phase)[..., 0]  # unsqueeze to fit required channel dim in conv2d
        if debug:
            for i in sp:
        ...
        sp = torch.stack([rec2polar(s) for s in sp])[..., 0]
