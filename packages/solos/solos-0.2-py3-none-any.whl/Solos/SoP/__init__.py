...
class Trainer(Trainer):
    def hook(self, vrs):
        self.writer.add_image(self.state + 'GT', make_grid(vrs['gt']), global_step=self.absolute_iter)
        self.writer.add_image(self.state + 'predictions', make_grid(torch.sigmoid(vrs['pred'])),
        ...
        audio = self.processor('lin_freq_adap', 'istft')(pred_sp)
        self.writer.add_audio(self.state + '_audio', audio.view(-1) / audio.abs().max(),
                              global_step=self.absolute_iter,
                              sample_rate=16130,
                              walltime=None)
