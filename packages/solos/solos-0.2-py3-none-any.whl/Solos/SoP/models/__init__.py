"""
Sound of Pixels code taken from:
https://github.com/hangzhaomit/Sound-of-Pixels/tree/master/models
"""
...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args):
        ...
        for n in range(N):
            ...
            feat_frames[n] = activate(feat_frames[n], 'sigmoid')

        # 3. sound synthesizer
        pred_masks = [None for n in range(N)]
        for n in range(N):
            pred_masks[n] = self.net_synthesizer(feat_frames[n], feat_sound)
            pred_masks[n] = activate(pred_masks[n], 'sigmoid')
