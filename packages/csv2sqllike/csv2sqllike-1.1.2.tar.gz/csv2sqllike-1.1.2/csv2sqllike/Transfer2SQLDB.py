...
class Transfer2SQLDB(object):
    ...
    def create_table(self, table_name: str, input_pseudosql_or_df: pd.DataFrame, if_exists="replace", index=False, dtype=None) -> None:
    ...
    def insert_data(self, table_name: str, input_pseudosql_or_df: pd.DataFrame, field_type_dict=None, if_exists="append", index=False, dtype=None):
        if type(input_pseudosql_or_df) == type(pd.DataFrame()):
            input_pseudosql_or_df.to_sql(
                con=self.__connect_for_pd, name=table_name, if_exists=if_exists, index=index, dtype=dtype)
        else:
