...
job_id = os.environ["EXTRACTOR_JOB_ID"]
...
content_path = os.environ["EXTRACTOR_CONTENT_PATH"]
...
if environ.get("EXTRACTOR_METADATA") is not None:
    metadata_json = os.environ["EXTRACTOR_METADATA"]
...
def get_result_keys(extractor_name):
...
example:
```python
keys = contentai.get_result_keys("azure_videoindexer")
for key in keys:
    data = contentai.get_results("azure_videoindexer", key)
```
    """
    if running_in_contentai:
...
def get_results(extractor_name, key):
    """ get the contents of a particular key

example:
```python
# get another extractor's output
data = contentai.get_results("some_extractor", "output.csv")
```
    """
    if running_in_contentai:
...
def get_results_json(extractor_name, key):
    """ get the json contents of a particular key

example:
```python
# get another extractor's output
data = contentai.get_results_json("some_extractor", "data.json")
```
    """
    if running_in_contentai:
        path = f"/results/{extractor_name}/{key}"
...
def get_results_bytes(extractor_name, key):
    """ get the contents of a particular key in raw bytes

example:
```python
# get another extractor's output
data = contentai.get_results_bytes("some_extractor", "output.bin")
```
    """
    if running_in_contentai:
        path = f"/results/{extractor_name}/{key}"
