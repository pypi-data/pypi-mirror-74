...
-----------------------------------
default hg-branch naming can be overriden by option --branch=<hg-branch name>
...
it can be edited manual, or new pairs can append by option 
    --branch-add=<maps pair>
appending and updating this file controls by option -mapbranch=<options>
...
by default, to enable this operation use option:
    --merge-branch=(push\pull | try<push\pull> | skip ...
...
hgpushsvn: start of new svn-branch mirrors current hg`s one can be do now follows:
	...
	hgpullsvn -l --branch-add=<new svn-branch>=<hg-branch>
...
+   svnhead bookmark - provides bookmark topmost synced rev with bookmark "svnhead",
                and tophost synced rev of every branch as "svnhead/<hg_branch_name>"
...
hgsvn 0.3.13
+   ``hgpullsvn --noversioned_change=<skip|abort(default)>`` - allows ignore when try delete files that 
           not versioned.
...
+   add support pull\push merge revisions, and automation of push\pull dependent branches
    ...
    merge branches automation bases on branch-maping <svn base path> to <hg-branch name>. this map
    ...
    --branch-add='<svnpath>=<hgname>' - add/overwrite maping
...
__author__ = 'Antoine Pitrou, Andi Albrecht, AlexRayne'
