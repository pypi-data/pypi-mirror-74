...
class NominodeClient:
    ...
    def execution_uuid(self):
        return environ["execution_uuid"]

    @property
    def task_uuid(self):
        return environ["task_uuid"]

    @property
    def project_uuid(self):
        return environ["project_uuid"]
    ...
    def update_task_parameters(self, parameters: dict, task_uuid: str = None) -> None:
        ...
        :param parameters:
            Dictionary representing what the new parameters will be. This overwrites all existing parameters.
        :param task_uuid:
            UUID of the task to update, will default to the current task
        :type task_uuid: optional

        """
        task_uuid = task_uuid if task_uuid else self.task_uuid
        result = self.request(
