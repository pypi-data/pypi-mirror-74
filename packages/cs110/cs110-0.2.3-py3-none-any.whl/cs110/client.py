...
def get_user_preference():
    ...
    if connected:
        ...
        print("Autograder disabled because you are not connected to the internet.")
        user_input = 'n'
    print()
...
if '__ignore__' not in sys.argv:
    ...
    try:
        if debug or run_autograder == True:
            print("Running Autograder (Version " + str(client_version) + ")")
            print("Contacting Test Server . . . ", end='')
        
        # Generates the Post Data        
        post_data = { "user":os.getlogin(), "host":socket.gethostname(), "filename":filename, "filepath":file_path, "code":file_contents, "language":"python", "autograde":run_autograder, "version":client_version }
        response = requests.post("https://autograder.net/get_testcase.php", data=post_data, headers=post_headers)
    ...
    if run_autograder == True:
        ...
        try:
            ...
            if response_code == 200:
                exec(compile(source=response_message, filename='cs110.py', mode='exec'))
