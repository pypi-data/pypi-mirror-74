...
#
#
def workWithGeometryData(method=False, df=False, polys=False, ptsCoordCol=False, polygonsCoordCol=False, polyColorCol=False, polygonsLabel='polyOnPoint', pntsClr='red', polysClr='white'):
  ...
  if method=='ponp': return getPolygonOnPoints(df, polys, ptsCoordCol, polygonsCoordCol, polygonsLabel);
...
def readInGeometryData(url=False, porg=False, geom=False, lat=False, lng=False, revgeocode=False,  save=False, in_crs=4326, out_crs=False):
  ...
  def getGeoParams(url, porg, geom, lat, lng, revgeocode, save, in_crs, out_crs):
    ...
    if (not isinstance(url, pd.DataFrame) and not url): url = input("Please enter the location of your dataset: " )
    # if url[-3:] == 'csv' :
