...
def list_mfa_challenge_methods(ctx, user_id, **kwargs):
    ...
    table = PrettyTable(["ID", "challenge_type", "priority", "endpoint", "enabled"])
    for method in methods:
        md = method.metadata
        spec = method.spec
        table.add_row(
            [md.id, spec.challenge_type, spec.priority, spec.endpoint, spec.enabled]
        )
    table.align = "l"
    print(table)
...
@click.argument("user-id", default=None)
@click.option("--challenge_type", type=click.Choice(["web_push", "totp"]), default=None)
...
@click.option("--challenge-type", type=click.Choice(["web_push", "totp"]), multiple=True)
@click.option("--timeout-seconds", type=int, default=None)
@click.option("--send-now", is_flag=True)
@click.pass_context
def create_challenge(ctx, user_id, challenge_type, **kwargs):
    challenge = challenges.create_challenge(
        ctx, user_id, challenge_types=challenge_type, **kwargs
    )
...
@click.argument("challenge_type", type=click.Choice(["web_push", "totp"]))
@click.pass_context
...
def create_challenge_enrollment(ctx, user_id, **kwargs):
    ...
    output_entry(challenge_enrollment.to_dict())
...
@cli.command(name="list-challenge-enrollments")
@click.option("--user_id", default=None)
@click.option("--limit", type=int, default=500)
@click.pass_context
def list_challenge_enrollments(ctx, **kwargs):
    results = challenges.list_challenge_enrollments(ctx, **kwargs)
    print(challenges.format_challenge_enrollments(results))
