...
def exp_neighexp(
    ...
    **kwargs,
    ...
    if CONFIG.exp_neighcell_key not in adata.uns.keys():
        raise KeyError(f"{CONFIG.exp_neighcell_key} not found, please run spatialtis.exp_neighcells first.")
    ...
    if mp & (CONFIG.OS in ["Linux", "Darwin"]):
        ...
        for _ in tqdm(
            exec_iterator(results), **CONFIG.tqdm(total=len(results), desc="fit model", unit="regressor")
        ):
            pass
        ...
        results = []
        for comb, arr in tqdm(X.items(), **CONFIG.tqdm(desc="fit model", unit="regressor"),):
