...
class _DataFrameReader:
    ...
    def complete_incoming_dataframe(self, dataframe_id: str) -> 'pandas.DataFrame':
        ...
        partitions_dfs = [partitions_dfs[key] for key in sorted(partitions_dfs.keys()) if partitions_dfs[key].num_rows > 0]
        ...
        def verify_column_names():
            ...
            for partition in partitions_dfs:
                ...
                for (a, b) in zip(expected_names, found_names):
                    if a != b:
                        raise make_schema_error('partition column had different name than expected.', expected_names, found_names)
                ...
                for j in range(0, len(fields)):
                    ...
                    if column_type != fields[j].type:
                        if column_type == pyarrow.bool_():
                            ...
                            raise _InconsistentSchemaError('A partition has a column with a different type than expected.\n' +
                                'The type of column \'{0}\' in the first partition is {1}. Found a partition where its type is {2}.'.format(partition.schema.names[j], str(fields[j].type), str(column_type)))
    ...
    def _read_incoming_partition(self, dataframe_id: str, partition: int, partition_bytes: bytes, data_format: int):
        if not have_pyarrow():
        ...
        if data_format == 2: # No Columns partition.
        ...
        if dataframe_id in self._incoming_dataframes:
            ...
            _LoggerFactory.trace(logger, "dataframe_id_not_found", { 'dataframe_id': dataframe_id, 'current_dataframe_ids': str([key in self._incoming_dataframes.keys()]) })
...
def process_send_partition(request, writer, socket):
    ...
    try:
        ...
        with socket.makefile('rb') as input:
            ...
            get_dataframe_reader()._read_incoming_partition(dataframe_id, partition, partition_bytes, data_format)
            writer.write(json.dumps({'result': 'success'}) + '\n')
    except ImportError:
