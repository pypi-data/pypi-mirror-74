...
class EngineAPI:
    def __init__(self):
        ...
        def connect_to_requests_channel():
            self.set_host_secret(self.requests_channel.host_secret)
            self.set_host_channel_port(self.requests_channel.port)
    ...
    def set_host_channel_port(self, message_args: int, cancellation_token: CancellationToken = None) -> None:
        response = self._message_channel.send_message('Engine.SetHostChannelPort', message_args, cancellation_token)
        return response

    @update_aml_env_vars(get_engine_api)
    def set_host_secret(self, message_args: str, cancellation_token: CancellationToken = None) -> None:
        response = self._message_channel.send_message('Engine.SetHostSecret', message_args, cancellation_token)
        return response
    ...
    @update_aml_env_vars(get_engine_api)
    def validate_activity_source(self, message_args: typedefinitions.ValidateActivitySourceMessageArguments, cancellation_token: CancellationToken = None) -> typedefinitions.ActivitySourceValidationResult:
        response = self._message_channel.send_message('Engine.ValidateActivitySource', message_args, cancellation_token)
        return typedefinitions.ActivitySourceValidationResult(response) if response is not None else None
