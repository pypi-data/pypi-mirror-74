...
from azureml.dataprep.api.tracing._json_line_exporter import JsonLineExporter
from azureml.dataprep.api.tracing._span_processor import AmlBatchSpanProcessor, AmlSimpleSpanProcessor
from opentelemetry.ext import jaeger

from ._constants import ACTIVITY_INFO_KEY, ERROR_CODE_KEY, COMPLIANT_MESSAGE_KEY, OUTER_ERROR_CODE_KEY
from ._application_insights_trace_exporter import ApplicationInsightsTraceExporter
...
try:
    ...
    # TODO: #815896
    # telemetry_client = Loggers.default_logger(instrumentation_key=instrumentation_key).telemetry_client
    # exporters.append(AmlBatchSpanProcessor(ApplicationInsightsTraceExporter(telemetry_client)))
except Exception:
    HBI_MODE = False
...
if not os.environ.get('DPREP_DISABLE_SPAN_JSONL'):
    exporters.append(AmlSimpleSpanProcessor(JsonLineExporter(session_id=session_id, base_directory=log_directory)))

if os.environ.get('DPREP_ENABLE_JAEGER'):
    exporters.append(AmlBatchSpanProcessor(jaeger.JaegerSpanExporter(
        service_name="AzureML",
