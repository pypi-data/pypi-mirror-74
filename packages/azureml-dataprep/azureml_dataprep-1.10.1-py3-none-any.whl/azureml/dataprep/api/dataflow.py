...
from .engineapi.typedefinitions import (SkipMode, PromoteHeadersMode, FileEncoding, InvalidLineHandling,
                                        ...
                                        ActivitySourceValidationResult)
...
from ._pandas_helper import have_pandas, have_pyarrow, pyarrow_and_pandas_compatible, PandasImportError
...
class Dataflow:
    ...
    def to_pandas_dataframe(self,
        ...
        use_preppy = not have_pyarrow() or not pyarrow_and_pandas_compatible() or extended_types
        fallback = False
        if not use_preppy:
            random_id = str(uuid4())
            ...
            dataflow_to_execute = self.add_step('Microsoft.DPrep.WriteFeatherToSocketBlock', {
                ...
                'errorStrategy': self._on_error_to_enum_value(on_error),
                'dateTimeSettings': self._out_of_range_datetime_to_block_value(out_of_range_datetime)
            })
        ...
        if use_preppy:
            if not extended_types and not fallback:
                if not pyarrow_and_pandas_compatible():
                    warnings.warn('Your pandas and pyarrow versions are incompatible. '
                                 'Please install pyarrow>=0.12.0 for improved performance of to_pandas_dataframe. '
                                  'You can ensure the correct version is installed by running: pip install '
                                  'pyarrow>=0.12.0 --upgrade')
                else:
                    warnings.warn('Please install pyarrow>=0.11.0 (or pyarrow>=0.12.0 if pandas>=1.0.0) for improved performance of to_pandas_dataframe. '
                                  'You can ensure the correct version is installed by running: pip install '
                                  'pyarrow>=0.12.0 --upgrade')
            ...
            try:
                ...
                dataset = preppy_to_ndarrays(intermediate_files, False if fallback else extended_types, nulls_as_nan)
    ...
    @track(get_logger)
    def has_invalid_source(self):
        """
        Verifies if the Dataflow has invalid source.

        :return: True when Dataflow has source that is known to be invalid. False when source is valid or cannot be validated.
        """
        result = self._engine_api.validate_activity_source(
            ValidateActivitySourceMessageArguments(anonymous_activity=Dataflow._dataflow_to_anonymous_activity_data(self)))
        return result == ActivitySourceValidationResult.INVALID
    ...
    def write_to_sql(self,
        ...
        try:
            from ._datastore_helper import AzureSqlDatabaseDatastore
            ...
            raise ValueError('Please provide MSSQLDataSource or install "azureml-sdk" and provide AzureSqlDatabaseDatastore')
        ...
        .. remarks::
            ...
            * Tuple of :class:`FieldType.DATE <azureml.dataprep.FieldType>` and List of format strings (single format string is also supported)
            ...
            .. note::
                ...
                If you choose to convert a column to :class:`FieldType.DATE <azureml.dataprep.FieldType>` and do not provide \
