...
class WritableFuse(FuseBase):
    def __init__(self,
        ...
        super().__init__(log, invocation_id, mount_options)
    ...
    def flush(self, path, fh):
        ...
        with file_lock:
            try:
                self._engine_api.upload_file(
                    UploadFileMessageArguments(
                        self._data_dir.get_local_root(),
                        self._get_remote_stream_info(),
                        self._data_dir.get_target_path(path), True))
    ...
    def mkdir(self, path, mode):
        try:
            ...
            if self._datastore is not None:
                self._engine_api.create_folder(CreateFolderMessageArguments(self._get_remote_stream_info(path)))
    ...
    def rename(self, old, new):
        ...
        if not self._has_reference(old):
            if self._datastore is not None:
                try:
                    self._engine_api.move_file(MoveFileMessageArguments(self._get_remote_stream_info(), old, new, False))
                except Exception as e:
    ...
    def rmdir(self, path):
        ...
        if self._datastore is not None:
            try:
                self._engine_api.delete(DeleteMessageArguments(self._get_remote_stream_info(path)))
            except Exception as e:
