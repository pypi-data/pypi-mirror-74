...
"""
  ...
  def GetNameValue(self):
    if self.params[0] == '?':
      return '?', ''
    args = self.params.split(None, 1)
    if len(args) == 1:
      return args[0], ''
    return args[0], args[1]
  def GetFunc(self):
    name, value = self.GetNameValue()
    if name == '?':	# send back supported boolean functions
      self.Reply('? Status', '', 0)
    else:
      self.ErrParam()
  def SetFunc(self):
  ...
  def GetLevel(self):
    ...
    if name == '?':	# send back supported parameters
      self.Reply('? Value', 'AGC AF', 0)
    elif name == 'AGC':
      if self.app.BtnAGC.GetValue():
        self.Reply(name + ' Value', 4, 0)
      else:
        self.Reply(name + ' Value', 0, 0)
    elif name == 'AF':
      x = self.app.audio_volume	# audio_volume is 0 to 1.000
      self.Reply(name + ' Value', "%.5f" % x, 0)
    else:
      self.ErrParam()
  def SetLevel(self):
    name, value = self.GetNameValue()
...
class ConfigConfig(wx.ScrolledWindow):
  def __init__(self, parent, width):
    ...
    self.file_button_play_speaker = self.MakeFileButton(text, path, 10, 'play_audio')
    # file for playing samples
    text = "Receive saved I/Q samples from a file"
    path = application.file_name_play_samples
    self.file_button_play_iq = self.MakeFileButton(text, path, 11, 'play_samples')
    ...
    self.file_button_play_mic = self.MakeFileButton(text, path, 12, 'play_cq')
    ...
    if self.y > self.scroll_height:
      self.scroll_height = self.y
    self.SetScrollbars(1, 1, 100, self.scroll_height)
  def MakeFileButton(self, text, path, index, name):
    ...
    QS.set_file_name(b.index, name=path, enable=enable)
...
class App(wx.App):
  ...
  def __init__(self):
    ...
    try:        # isatty() fails for pythonw.exe
...
The new code supports multiple corrections per band.""")
    ...
    if sys.platform == 'win32':
      ...
      h = 0
    wisdom_path = os.path.join(os.path.dirname(self.local_conf.StatePath), 'quisk_wisdom.cache')
    ...
    if self.bottom_widgets:		# Extend the sliders to the bottom of the screen
      ...
      for i in self.slider_columns:
        ...
        item.SetSpan((rows, 1))
    self.OpenHardware()
    msg = QS.open_key(conf.quisk_serial_port, conf.quisk_serial_cts, conf.quisk_serial_dsr)
    if msg:
      print(msg)
    self.OpenSound()
  ...
  def MakeSoundDeviceList(self):
    ...
    # Play is 1 for play, 0 for capture. Prefix is "alsa:", "pulse:", "soundio:"
    self.sound_devices = []
    if sys.platform == 'win32':
      dev_capt, dev_play = QS.sio_sound_devices("wasapi")	# description, id, is_raw
      for description, device, raw in dev_capt:
        if raw:
          self.sound_devices.append((0, 'soundio:', description, device, '', ''))
      for description, device, raw in dev_play:
        if raw:
          self.sound_devices.append((1, 'soundio:', description, device, '', ''))
    else:
      ...
      dev_capt, dev_play = QS.pa_sound_devices()	# device, description, alsa description (alsa device) or ""
    ...
    if conf.pulse_audio_verbose_output:
      print ("Sound devices:")
  ...
  def SetSoundDevices(self):
    ...
    for name, is_play, index in sound_cards:
      ...
      if not value:
        ...
        continue
      if sys.platform == 'win32':
        found_desc, found_dev, a_dev, a_desc = self.GetSoundDevice(is_play, value, "soundio:")
        if not found_desc:
          found_desc = value
          found_dev = value
        if conf.use_fast_sound and name == "name_of_sound_play":
          windows_driver = DEV_DRIVER_SOUNDIO
        else:
  ...
  def OpenSound(self):
    ...
    QS.open_sound(0, conf.data_poll_usec, conf.latency_millisecs,
				...
				conf.mic_out_volume, conf.mic_playback_rate)
