...
int quisk_active_sidetone;		// quisk_active_sidetone == 0 No sidetone; == 1 sidetone from soundio; == 2 sidetone from old logic
...
)
{
   ...
   switch( dev->driver )
   {
      ...
      case DEV_DRIVER_DIRECTX:
         ...
         break;
      case DEV_DRIVER_SOUNDIO:
         nSamples = quisk_read_soundio(dev, cSamples);
         break;
#else
...
)
{
   ...
   switch( dev->driver )
   {
      ...
      case DEV_DRIVER_DIRECTX:
         ...
         break;
      case DEV_DRIVER_SOUNDIO:
         quisk_write_soundio(dev, nSamples, cSamples);
         break;
#else
      case DEV_DRIVER_PORTAUDIO:
...
{  // make sidetone samples
	...
	if (quisk_serial_key_down || quisk_hardware_cwkey) {
...
static int quisk_play_sidetone(struct sound_dev * dev)
{  // return 1 if we played the sidetone
#ifdef MS_WINDOWS
	return 0;
#else
...
{  // called in an infinite loop by the main program
...
#if DEBUG_MIC != 2
	if ( ! quisk_play_sidetone(&Playback))	// play sidetone
		play_sound_interface(&Playback, nSamples, cSamples, 1, quisk_audioVolume);	// else play radio sound
#endif
	...
	if (MicPlayback.handle) {		// Mic playback: send mic I/Q samples to a sound card
		...
		if (is_cw) {	// Transmit CW; use capture device for timing, not microphone
			...
			mic_count = 0;
			if (quisk_serial_key_down || quisk_hardware_cwkey || quiskSpotLevel >= 0) {
				while (cwCount >= 1.0) {
					if (cwEnvelope < 1.0) {
...
void quisk_close_sound(void)	// Called from sound thread
{
	...
	quisk_close_sound_soundio(CaptureDevices, PlaybackDevices);
#else
#ifndef QUISK_NO_PORTAUDIO
...
void quisk_open_sound(void)	// Called from GUI thread
{
	...
	Playback.dev_errmsg[0] = 0;
	MicCapture.dev_errmsg[0] = 0;
	MicPlayback.dev_errmsg[0] = 0;
	...
	Playback.dev_index = t_Playback;
	MicCapture.dev_index = t_MicCapture;
	MicPlayback.dev_index = t_MicPlayback;
...
void quisk_start_sound(void)	// Called from sound thread
{
	...
	quisk_start_sound_soundio(CaptureDevices, PlaybackDevices);
#else
#ifndef QUISK_NO_PORTAUDIO
...
void quisk_set_sidetone(void)
{
	if (quisk_isFDX || ! (rxMode == CWL || rxMode == CWU))
		quisk_active_sidetone = 0;		// No sidetone
	else if (Playback.driver == DEV_DRIVER_ALSA)
		quisk_active_sidetone = 3;		// Sidetone from alsa
	else if (Playback.driver == DEV_DRIVER_SOUNDIO)
		quisk_active_sidetone = 1;		// Sidetone from soundio
	else
