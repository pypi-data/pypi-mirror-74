...
int waterfall_scroll_mode = 1;		// draw the first lines multiple times
...
//if (bank == 0) printf("MeasureSquelch %.5f\n", MeasureSquelch[0].squelch);
	...
	case FDV_L:	// Extra modes added by Dave Roberts, G8KBB, June 2020.
		quisk_check_freedv_mode();
		// added conditional for 2400A/B mmodes
		// current coding assumes modem sample rate is 8000 16000 or 48000
		if( n_modem_sample_rate <= 16000 )
		...
		// current coding assumes speech sample rate is only 8000 or 16000 - needs tweak for others
		nSamples = quisk_dInterpolate(dsamples, nSamples, &Storage[bank].filtAudio24p3, 3);
		if( n_speech_sample_rate == 8000 )
			nSamples = quisk_dInterp2HB45(dsamples, nSamples, &Storage[bank].HalfBand7);
		break;
	}
...
// Samples range from about 2^16 to a max of 2^31.
	...
	if (quisk_is_key_down() && !quisk_isFDX) {	// The key is down; replace this data block
		...
		keyupEnvelope = 0;
		if (quisk_active_sidetone == 2 && (quisk_hardware_cwkey || quisk_serial_key_down)) {	// Play sidetone instead of radio for CW
...
{	// Return the filter sample rate as used by quisk_process_samples.
	...
	case FDV_U:	 // digital voice
	case FDV_L:
		filter_srate = n_speech_sample_rate;
		break;
	}
...
static PyObject * get_params(PyObject * self, PyObject * args)
{
	...
	if (!PyArg_ParseTuple (args, "s", &name))
		return NULL;
	//if (strcmp(name, "have_soundio") == 0)
	//	return PyInt_FromLong(have_soundio);
	Py_INCREF (Py_None);
	return Py_None;
}
...
#define IS_SW_CWKEY	quisk_serial_key_down
#define IS_HW_PTT	hardware_ptt
#define IS_SW_PTT	(key_is_down || is_PTT_down)

void quisk_set_play_state(int init)
{
	static double Time0;
	static int old_play_state = -1;

	if (init) {
		Time0 = QuiskTimeSec();
	}
	else switch (quisk_play_state) {
	case STARTING:
		if (QuiskTimeSec() - Time0 > 0.200) {
	...
	if (quisk_sound_state.verbose_sound && quisk_play_state != old_play_state) {
		old_play_state = quisk_play_state;
		printf("quisk_play_state %d\n", quisk_play_state);
	}
}
...
#ifdef MS_WINDOWS
	{"sio_sound_devices", quisk_sio_sound_devices, METH_VARARGS, "Return a list of available SoundIO sound device names."},
#endif
