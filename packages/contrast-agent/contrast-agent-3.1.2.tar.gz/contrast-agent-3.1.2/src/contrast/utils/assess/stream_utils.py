...
class BaseStreamProxy(ObjectProxy):
    """
    Base class for implementing stream proxies
    ...
    TODO: In the future we may want to instrument write and writelines. This would
        require us to treat the stream more like a propagator than a source because
        trusted data could be written to it.

    TODO: We may eventually need to consider the less common __iter__ method, which
        is currently not instrumented. Werkzeug (at least) does not use __iter__ when
        parsing wsgi.input; we may have to investigate WebOb's parser as well. It's
    ...
    TODO: In the future, we might be able to align this proxy hierarchy more closely
        with https://docs.python.org/3/library/io.html#class-hierarchy. Note that
        File objects in python 2 do not line up with this inheritance pattern:
...
class BaseRawIOProxy(BaseStreamProxy):
    ...
    TODO: We may eventually want to instrument `readinto(b)`
    """
...
def instrument_source_stream(stream):
    """
    Mark the provided stream as a source, and instrument methods that produce tracked content.
    This must be a stream that we have proxied. i.e., it has the `CS__METHOD_NAMES` attribute.

    @param stream: stream object proxied by BaseStreamProxy (must have the CS__METHOD_NAMES attribute)
    """
...
def _instrument_source_stream_method(stream, method_name):
    ...
    Only this particular stream instance is instrumented, because the patch is not applied
    at the class level.
    ...
    if orig_method is None:
        raise ValueError(
            "source stream missing expected attribute: {}".format(method_name)
        )
    new_method = _build_patch(orig_method)
    setattr(stream, method_name, new_method)


def _build_patch(orig_method):
    """
    Create a patch for the provided stream method. This is an instance method, so it is
    bound to a specific stream instance. This means that the `self` parameter needs not
    be passed to the original method.

    We access the underlying `self` of the bound instance method using the __self__
    back-reference.
    ...
    def patched_method(*args, **kwargs):
        ...
        apply_stream_source(
            orig_method.__name__, result, orig_method.__self__, result, args, kwargs
        )
        return result
