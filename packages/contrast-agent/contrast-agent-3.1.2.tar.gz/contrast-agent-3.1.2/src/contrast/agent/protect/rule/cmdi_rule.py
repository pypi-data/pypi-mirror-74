...
class CmdInjection(BaseRule, CommandInjectionREPMixin):
    ...
    def find_attack(self, context, command_string, **kwargs):
        if self.protect_excluded_by_code():
            return

        command_string = str(command_string) if command_string else None
        attack = super(CmdInjection, self).find_attack(
            context, command_string, **kwargs
        )

        if not attack and command_string:
            evaluations_for_rule = self.evaluations_for_rule(context)
            return self.find_probable_attack(
                context, command_string, evaluations_for_rule, **kwargs
            )
    ...
    def in_new_process(self):
        ...
        original_pid = self.settings.client.pid

        return current_pid != original_pid

    def find_probable_attack(
        self, context, command_string, evaluations_for_rule, **kwargs
    ):
        """
        ...
        attack = self.build_attack_with_match(
            context, most_likely, attack, command_string, **kwargs
        )
    ...
    def build_sample(self, context, evaluation, command, **kwargs):
    ...
    def infilter(self, context, match_string, **kwargs):
        ...
        if self.is_detect_chained_commands_enabled:
            ...
            self.detect_command_chaining(context, original_command, evaluation)

        super(CmdInjection, self).infilter(context, match_string, **kwargs)
    ...
    def detect_command_backdoor(self, context, command):
        ...
        if parameter_key and parameter_value:
            self.report_command_backdoor(
                context, command, parameter_key, parameter_value
            )

            raise SecurityException(self, "Command from input detected")

    def detect_command_chaining(self, context, command, evaluation):
        """
        ...
        if index != -1:
            self.report_chained_injection(context, evaluation, command, index)
    ...
    def find_matching_parameter(self, context, command):
        ...
        if request and isinstance(command, string_types):
            ...
            for parameter_key, parameter_values in iteritems(parameters):
                if parameter_values:
                    param = self._get_param_in_command(
                        parameter_values, normalized_command
                    )
    ...
    def report_command_backdoor(self, context, command, parameter_key, parameter_value):
        evaluations_for_rule = self.evaluations_for_rule(context)
        ...
        sample = self.create_backdoor_command_sample(
            context, evaluation, command, parameter_key, parameter_value
        )
    ...
    def report_chained_injection(self, context, evaluation, command, offset):
        start_idx = offset
        end_idx = len(command) - 1

        attack = self.build_or_append_attack(context, evaluation, None, command)

        result = self.build_attack_with_match(
            context, evaluation, attack, command, start_idx=start_idx, end_idx=end_idx
        )

        self._append_to_activity(context, result)

    def create_backdoor_command_sample(
        self, context, evaluation, command, parameter_key, parameter_value
    ):
        sample = self.build_base_sample(context, evaluation)
