...
class FlaskMiddleware(WSGIMiddleware):
    ...
    def get_routes_on_first_request(self, request):
        """
        If an app is None, we won't do coverage.

        Flask requires there to be an app in the request context. There is only a context on init, which we use, and
        during the `self.wsgi_app` call. That is why flask.current_app.app_context doesn't work. There is nothing on the
        LocalProxy request context thread yet.
        """
        if self.app is None:
            return
        super(FlaskMiddleware, self).get_routes_on_first_request(request)
    ...
    def get_view_func(self, request):
        ...
        try:
            match = adapter.match(
                request.get_path(), method=request.get_request_method()
            )
        except NotFound:
            match = None
