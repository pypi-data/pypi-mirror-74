...
class TriggerRule(BaseRule):
    ...
    def validate_tags(self, tags):
        ...
        for item in tags:
            if (
                item not in policy_constants.VALID_TAGS
                and item not in policy_constants.VALID_SOURCE_TAGS
            ):
    ...
    def _is_violated(self, node, source_properties):
        return node.trigger_action.is_violated(
            source_properties, self.required_tags, self.disallowed_tags
        )
    ...
    def is_violated(self, node, source):
        """
        Determine whether rule was violated based on source object
        """
        # The rule is violated if the object is marked as source
        if getattr(source, "cs__source", False):
            return True

        if hasattr(source, "cs__tracked"):
            # Using getattr for safety, even though we know it's there
            source_properties = getattr(source, "cs__properties", None)
        else:
            source_properties = get_properties(source)

        return self._is_violated(node, source_properties)
