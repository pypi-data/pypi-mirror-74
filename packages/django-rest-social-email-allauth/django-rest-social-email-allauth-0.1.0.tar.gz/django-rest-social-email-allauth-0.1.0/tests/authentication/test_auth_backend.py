...
def test_authenticate(email_factory, user_factory, verified_email_auth_backend):
    """
    ...
    assert (
        verified_email_auth_backend.authenticate(
            username=email.email, password="password", request=None
        )
        == user
    )
...
def test_get_user(verified_email_auth_backend, user_factory):
    """
	Passing in a user's ID should return the user with that ID.
	"""
    user = user_factory()

    assert verified_email_auth_backend.get_user(user.id) == user


def test_get_user_invalid_id(verified_email_auth_backend, db):
    """
	Passing in an ID with no corresponding user should return None.
	"""
    assert verified_email_auth_backend.get_user(1) is None
