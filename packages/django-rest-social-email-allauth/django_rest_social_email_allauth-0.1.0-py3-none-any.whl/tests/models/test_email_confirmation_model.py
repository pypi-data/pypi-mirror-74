...
def test_confirm(
    ...
    assert email_verification_listener.call_args[1]["sender"] == confirmation.__class__
...
def test_is_expired(email_confirmation_factory):
    """
	If it has has been more than a day since the confirmation was created it
    ...
    with mock.patch(
        "django.utils.timezone.now", autospec=True, return_value=future_time,
    ):
...
@mock.patch("email_utils.send_email")
def test_send(mock_send_email, email_confirmation_factory, settings):
    """
	Sending the confirmation should send to the associated 
	email address.	
	"""
