...
def test_delete(api_client, email_factory):
    ...
    url = reverse("rest-social-email-auth:email-detail", kwargs={"pk": email.pk})
    response = api_client.delete(url)

    assert response.status_code == status.HTTP_204_NO_CONTENT
    assert email.user.email_addresses.count() == 0
...
def test_get_anonymous(api_client):
    """
	Sending a GET request to the view as an anonymous user should return a
	permissions error.
	"""
    url = reverse("rest-social-email-auth:email-detail", kwargs={"pk": 1})
    response = api_client.get(url)
...
def test_update(api_client, email_factory):
	...
	email adddress associated with the endpoint.
	"""
