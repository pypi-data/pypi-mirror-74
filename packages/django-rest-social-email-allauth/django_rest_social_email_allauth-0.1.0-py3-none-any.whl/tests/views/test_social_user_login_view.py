...
class AccountSocialAccountTest(test.APITestCase):
        ...
        return json.dumps(
            {
                ...
                "redirect_uri": (
                    ("http://{}{}").format(
                        ...
                        reverse.reverse(
                            "rest-social-email-auth:user-social-login", args=(provider,)
                        ),
                    )
                ).replace("example.com", "localhost:8000"),
                "provider": provider,
            }
        )
    ...
    def test_social_account_for_discord(self):
        ...
        resp = self.client.get(
            reverse.reverse(
                "rest-social-email-auth:user-social-login", args=(self.provider,)
            )
        )
        ...
        self.assertEqual(
            custom_resp.json()["token"], custom_resp.json()["user"]["token"]
        )
