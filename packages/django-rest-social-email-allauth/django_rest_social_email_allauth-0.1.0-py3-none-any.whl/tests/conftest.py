"""
Configurations used to test the client_auth app in the email section platforms
"""
...
class MutableAppSettings(object):
    ...
    def __init__(self, settings_obj):
        """
		Create new mutable settings instance


		This is done by making a copy of the EMAIL_AUTH(Email Instance)
		configuration dictionary. Since the settings fixture is rolled 
		back at the end of each test, it sees that we changed the dictionary reference and points
		it back to the original


		Args:
			settings_obj:
				The object through which Django's settings
				are accessible. We rely in the fact that this
				object is rolled back after each test
		"""
        settings_obj.EMAIL_AUTH = copy.deepcopy(settings_obj.EMAIL_AUTH)

        # Since we pass __setattr__ through to the settings dictionary
        # We have to set attribures by modifyinh the instance's __dict__
        # directly
        self.__dict__["settings_obj"] = settings_obj
        self.__dict__["original"] = copy.deepcopy(settings_obj.EMAIL_AUTH)

    def __getattr__(self, name):
        """
		Attempt to loookup attributes using the application's settings.
    ...
    def __setattr__(self, name, value):
		...
		"""
        self.settings_obj.EMAIL_AUTH[name] = value
...
def api_rf():
    """
	Fixture to get a factory fot creating API requests.

	Returns:
		An instance of the request factory ptovided by DRF
	"""
...
def password_reset_token_factory(db):
    """
	Fixture to get the factory used to create password rest tokens.

	Returns:
		The factroy used to create ``PasswordResetToken`` intances.
	"""
