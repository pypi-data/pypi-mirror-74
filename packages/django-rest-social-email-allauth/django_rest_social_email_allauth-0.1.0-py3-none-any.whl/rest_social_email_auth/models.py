...
# Local Modules
from rest_social_email_auth import managers, app_settings, signals
...
class EmailAddress(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=__("email"))
    email = models.EmailField(max_length=255, unique=True, verbose_name=__("email"))
    is_primary = models.BooleanField(
        default=False,
        help_text=__("Boolean indicating if the email is the user's primary address"),
    ...
    def set_primary(self):
        ...
        logger.info(
            "Set %s as the primary email address for %s.", self.email, self.user,
        )
...
class EmailConfirmation(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=__("created_at"))
...
class PasswordResetToken(models.Model):
    ...
    key = models.UUIDField(
        default=uuid.uuid4,
        editable=False,
        help_text=__("The token that authorizes the user to reset their " "password."),
        verbose_name=__("key"),
    )
    ...
    def send(self):
		...
		"""
        context = {"reset_url": app_settings.PASSWORD_RESET_URL.format(key=self.key)}
