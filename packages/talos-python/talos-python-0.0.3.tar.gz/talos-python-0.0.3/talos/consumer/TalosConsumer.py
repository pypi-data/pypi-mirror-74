...
class TalosConsumer(object):
    ...
    def check_partition_task(self):
        ...
        self.partitionScheduledExecutor = Timer(interval=self.talosConsumerConfig
                                                .get_check_partition_interval(),
                                                function=self.check_partition_task)
        self.partitionScheduledExecutor.setName("checkPartitionTask-" + self.topicName)
        self.partitionScheduledExecutor.start()
    ...
    def check_worker_info_task(self):
        ...
        self.workerScheduleExecutor.setName("checkWorkerInfoTask-" + self.topicName)
    ...
    def renew_task(self):
        ...
        while maxRetry > 0:
            try:
                renewResponse = self.consumerClient.renew(renewRequest)
            except Exception as e:
                logger.error("Worker: " + self.workerId + " renew error: " + str(traceback.format_exc()))
                continue
            maxRetry -= 1
        ...
        if renewResponse and len(renewResponse.failedPartitionList) > 0:
            ...
            logger.error("The worker: " + self.workerId + " failed to renew partitions: " + str(failedRenewList))
        ...
        self.renewScheduleExecutor.setName("renewTask-" + self.topicName)
    ...
    def check_and_get_topic_info(self, topicTalosResourceName=None):
        ...
        if not topicTalosResourceName.topicTalosResourceName.topicTalosResourceName ==\
               response.topicTalosResourceName.topicTalosResourceName:
            logger.info("The consumer initialize failed by topic not found")
            raise InvalidArgumentError("The topic: " + str(topicTalosResourceName.
    ...
    def register_self(self):
        ...
        while tryCount >= 0:
            try:
                lockWorkerResponse = self.consumerClient.lock_worker(lockWorkerRequest)
            except Exception as e:
    ...
    def get_worker_info(self):
        ...
        queryWorkerResponse = self.consumerClient.query_worker(queryWorkerRequest)
