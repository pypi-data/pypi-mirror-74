...
class MyMessageCallback(UserMessageCallback):
	...
	def on_success(self, userMessageResult=None):
		...
		for message in userMessageResult.get_message_list():
			logger.info("success to put message: " + str(message.message))
...
class TalosProducerDemo:
	...
	def start(self):
		# init producer
		talosProducer = TalosProducer(producerConfig=self.producerConfig, credential=self.credential, topicName=self.topicName, topicAbnormalCallback=SimpleAbnormalCallback(), userMessageCallback=MyMessageCallback())
