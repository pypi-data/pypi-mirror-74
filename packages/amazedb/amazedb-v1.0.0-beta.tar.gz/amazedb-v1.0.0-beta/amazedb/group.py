...
class group:
    ...
    def insert(self, data: dict = {}, **moreData):
        ...
        with open(self.path, 'r+') as grp:

            # load the older data
            d = json.loads(Fernet(self.key).decrypt(grp.read().encode()).decode('utf-8'))
            ...
            # Write the new data
            grp.write(Fernet(self.key.encode()).encrypt(json.dumps(d).encode()).decode('utf-8'))
    ...
    def get(self, filters: dict, sortby: str = None):
        ...
        # Get them from the drive otherwise
            ...
            with open(self.path, 'r') as d:
                docs = json.loads(
                    Fernet(self.key.encode()).decrypt(
                        d.read().encode()
                    )
                    .decode('utf-8')
                )
    ...
    def update_one(self, filters: dict, nValues: dict):
        ...
        with open(self.path, 'r+') as f:
            docs = json.loads(Fernet(self.key.encode()).decrypt(f.read().encode()).decode('utf-8'))
    ...
    def update(self, filters: dict, nValues: dict):
        ...
        with open(self.path, 'r+') as f:
            ...
            f.write(
                Fernet(self.key.encode())
                .encrypt(json.dumps(docs).encode())
                .decode('utf-8')
            )
    ...
    def drop(self):
        ...
        with open(f'{self.parent.dbPath}/db/{self.parent.name}/metadata.json', 'r+') as f:
