...
class SenSuber(object):

    def __init__(self, req_url=f"http://model.ai-api.hrtps.com/v1/models/subsen_lstmcrf_epoch1_batch32_seq512:predict"):
        self.req_url = req_url
        model_mess_path = os.path.dirname(__file__)
        self.sub_process = load_processor(os.path.join(model_mess_path, "..", "data", "subsen", "model_info.json"))
    ...
    def predict(self, text: str):
        ...
        :return:
        """
        text = "".join(self.format_inp(text))
        texts = [tokenize(text)]
        ...
        return preds[0][1:], texts[0]

    def seq_sen_sub(self, text: str):
        """
        使用序列模型切分句子
        :param text:
        :return:
        ...
        preds, x_data = self.predict(text)
...
seq_sub = SenSuber().seq_sen_sub
limit_seq_sub = SenSuber().seq_sen_sub_len_limit
