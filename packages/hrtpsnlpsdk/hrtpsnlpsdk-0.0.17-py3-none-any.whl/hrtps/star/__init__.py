...
class STARCLFDetecter(object):
    ...
    def __init__(self, host, port):
        ...
        # self.processor = load_processor(os.path.join(model_mess_path, "..", "data", "starclf", "model_info.json"))
        self.processor = load_clf_processor(os.path.join(model_mess_path, "..", "data", "starclf", "model_info.json"))
        self.req_url = f"http://{self.host}/v1/models/corpus_pre_sen_linear_5e-5_5_128_epoch1_batch32:predict"

    def predict_sen(self, texts):
        texts = [tokenize(text) for text in texts]
        tensor = self.processor.process_x_dataset(texts)
        tensor = [{
            "Input-Token:0": i.tolist(),
            "Input-Segment:0": np.zeros(i.shape).tolist()
        } for i in tensor]

        r = requests.post(self.req_url, json={"instances": tensor})
        if r.status_code == 404:
    ...
    def predict_doc(self, sub_sens: list):
        """
        :param sub_sens: 切分后的文本
        :return:
        """

        all_probs = self.predict_sen(sub_sens)
        for i, all_prob in enumerate(all_probs):
            sub_sen_text = sub_sens[i]
            max_ind = np.argmax(all_prob)
            sub_pre_label = self.processor.idx2label[max_ind]
            sub_prob = all_prob[max_ind]
            yield sub_pre_label, sub_sen_text, sub_prob
