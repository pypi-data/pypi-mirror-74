...
def test_movies_moviepreview_movieid(movieId):
    data = {}
    ...
    url = Template("/v1/movies/moviePreview/{{movieId}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_movies_moviePreview_movieId(req,resp)
