...
def test_feeditems_screens_vodplaybackcontextbars(contentId):
    data = {}
    ...
    url = Template("/v1/feedItems/screens/vodPlaybackContextBars?contentId={{contentId}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_feedItems_screens_vodPlaybackContextBars(req,resp)
