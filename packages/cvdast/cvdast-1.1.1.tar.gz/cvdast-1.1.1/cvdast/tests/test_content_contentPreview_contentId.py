...
def test_content_contentpreview_contentid(contentId):
    data = {}
    ...
    url = Template("/v1/content/contentPreview/{{contentId}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_content_contentPreview_contentId(req,resp)
