...
def test_feeditems_screens_episodes(width, offset, height):
    data = {}
    data["width"] = width
    data["offset"] = offset
    data["height"] = height
    ...
    url = Template("/v1/feedItems/screens/episodes?height={{height}}&width={{width}}&offset={{offset}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_feedItems_screens_episodes(req,resp)
