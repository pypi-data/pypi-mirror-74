...
def test_feeditems_screens_searchcontextbars(width, height):
    data = {}
    data["width"] = width
    data["height"] = height
    ...
    url = Template("/v1/feedItems/screens/searchContextBars?height={{height}}&width={{width}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_feedItems_screens_searchContextBars(req,resp)
