...
def test_feeditems_search(offset, count):
    data = {}
    data["offset"] = offset
    data["count"] = count
    ...
    url = Template("/v1/feedItems/search?count={{count}}&offset={{offset}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_feedItems_search(req,resp)
