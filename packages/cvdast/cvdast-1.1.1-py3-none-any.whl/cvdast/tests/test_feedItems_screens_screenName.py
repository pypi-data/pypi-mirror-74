...
def test_feeditems_screens_screenname(width, pageCursor, screenName, height):
    data = {}
    data["width"] = width
    ...
    url = Template("/v1/feedItems/screens/{{screenName}}?pageCursor={{pageCursor}}&height={{height}}&width={{width}}").render(**data)
    resp = _trigger_requests("get", url,
                      header=req["headers"],
                      data=json.dumps(data))
    print(resp.status_code)
    print(resp.text)
    assertions.assert_for_feedItems_screens_screenName(req,resp)
