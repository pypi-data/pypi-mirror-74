...
class CloudvectorDAST(object):
    def __init__(self, APISpecOne, APISpecTwo, ce_host, ce_username, ce_password, config_file, cover_only_diff="n",
        ...
        if config_file:
            if os.path.exists(config_file):
                ...
                #   self._update_params_captured_for_auth(self.cv_config.get("authentication"))
        if APISpecOne:
    ...
    def create_pyfixtures(self):
        ...
        if self.cvdast_to_auth_info:
            payload = {}
            for k,v in self.cv_config["authentication"].get("params").items():
                if k in self.cvdast_to_auth_info["header"]:
                    self.cvdast_to_auth_info["header"][k]=str(v)
                if k in self.cvdast_to_auth_info["params"]:
                    payload[k] = v
            auth_api = self.cvdast_to_auth_info["url"]
            auth_api_headers = self.cvdast_to_auth_info["header"]
            auth_api_payload = payload
            auth_api_response_key = self.cv_config["authentication"]["response_key"]
        ...
        with open("tests/conftest.py", 'w+') as fh:
            fh.write(template.render(api_info=self.params_captured_in_traffic,AUTH_API=auth_api,
                                     AUTH_API_HEADERS=auth_api_headers,AUTH_API_PAYLOAD=json.dumps(auth_api_payload),
                                     AUTH_RESP_KEY=auth_api_response_key))
    ...
    def create_pytest_methods(self, cv_events, custom_validations):
        ...
        for _ in cv_events:
            ...
            if event["url"] not in apis_to_be_tested:
                ...
                apis_to_be_tested[api][event["method"]] = {}
                for k, v in event["header"].items():
                    if k in self.static_headers_from_cfg:
                        event["header"][k] = str(self.static_headers_from_cfg[k])
                apis_to_be_tested[api][event["method"]]["header"] = event["header"]
                apis_to_be_tested[api][event["method"]]["url"] = event["url"]  # str(event["host"]).lower() + event["url"]
                apis_to_be_tested[api][event["method"]]["params"] = new_spec_info.get(str(event["http-req-url"]).replace("//", "/"),{}).get(event["method"],{})
                if apis_to_be_tested[api][event["method"]]["params"] is None:
                    apis_to_be_tested[api][event["method"]]["params"] = []
                apis_to_be_tested[api][event["method"]]["params"] = list(set(apis_to_be_tested[api][event["method"]]["params"]))
                apis_to_be_tested[api][event["method"]]["params"].append("host")
                apis_to_be_tested[api][event["method"]]["params"].append("access_token")
                host_url = str(event["host"]).lower()
                hosts.append(host_url)
                self._create_assertions_map(event["http-req-url"], apis_to_be_tested[api][event["method"]]["params"], event["body"],
                                            event["rsp_body"])
    ...
    def _process_event_data(self, apis_to_check=[]):
        ...
        for event in events:
            ...
            request = {"url": str(event["attributes"]["http_path"]), "method": str(event["attributes"]["http_method"])}
            header = {}
            body = {}
            req_params_found = {}
