...
class FireServiceRota(object):
    ...
    def get_schedules(self):
        ...
        endpoint = FSR_ENDPOINT_MEMBERSHIPS.format(self._user['memberships'][0]['id'])

        params = { 'start_time': today.strftime("%Y-%m-%dT00:00:00%z"), 'end_time': tomorrow.strftime("%Y-%m-%dT00:00:00%z") }

        response = self._request('GET', endpoint=endpoint, params=params, log_msg_action='get schedule memberships',
                 auth_request=False)
    ...
    def get_standby_function(self, id):
        ...
        response = self._request('GET', endpoint=endpoint, log_msg_action='get standby function',
                 auth_request=False)
    ...
    def set_incident_response(self, id, status):
        ...
        self._request('POST', endpoint=endpoint, params=params, log_msg_action='set incident response', auth_request=False)
    ...
    def get_incident_response(self, id):
        ...
        response = self._request('GET', endpoint=endpoint, log_msg_action='get incident response', auth_request=False)
...
class FireServiceRotaIncidents(Thread, websocket.WebSocketApp):
    ...
    def __init__(self, url,
        ...
        :param on_close: function that get's called on socket close
        """
        self._url = url
