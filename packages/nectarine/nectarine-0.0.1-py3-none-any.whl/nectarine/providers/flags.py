...
from nectarine.typing import is_dataclass, is_primitive, is_tuple, is_tuple_of_unknown_length, is_linear_collection, \
    get_generic_args
from nectarine._utils import insert_at_path, try_convert


def make_tuple_action(types: Tuple[Type]):
...
def is_supported_type(type_: Type) -> bool:
    ...
    if is_linear_collection(type_):
        ...
        return is_primitive(value_type)
    if is_tuple(type_):
...
def _add_argument_for(parser: argparse.ArgumentParser, name, field):
    ...
    if field.type is bool:
        ...
        parser.add_argument(f"--{name}", nargs=len(values_types), action=make_tuple_action(values_types), **kwargs)


class Flags(ConfigurationProvider):
    ...
    def load_configuration(self, target_type: Type, strict=False) -> Dict[str, Any]:
        ...
        paths = ((path, field) for path, field in get_paths(target_type) if is_supported_type(field.type))
        arg_to_path = {}
...
def flags(
    ...
    """
    Configure a provider that uses the program arguments
