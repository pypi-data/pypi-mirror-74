...
from nectarine.providers.flags import flags
...
def load(
        target: Type,
        providers: List[Callable[[], ConfigurationProvider]],
        strict: bool = False,
):
    """
    ...
    """
    results = []
    for parser in reversed(providers):
        r = parser().load_configuration(target, strict=strict)
        results.append(r)
