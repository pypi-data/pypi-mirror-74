...
from modules.setup_questions import questions, flat_config, model_config
...
def parse_command_line_arguments(logger):
    ...
    if args.setup:
        logo = '''
              ...
              Thanks for using the iAuditor Exporter!'''
        print(Panel(logo, box=box.SIMPLE, expand=False))
...
def modify_choice(logger, settings, config_path):
    ...
    to_modify = questionary.select('Select the config option you want to change:', choices=options).ask()
...
def initial_setup(logger):
    ...
    if parsed_date:
        ...
        update_ls_q = ask_question(logger, f'''We are currently searching for inspections completed after {pretty_file_name}, would you like to change this? Please note: if you're exporting with SQL, we'll grab this date by querying the database so changing this won't change anything for SQL exports.''', 'bool')
        if update_ls_q:
            print(box_print('''
