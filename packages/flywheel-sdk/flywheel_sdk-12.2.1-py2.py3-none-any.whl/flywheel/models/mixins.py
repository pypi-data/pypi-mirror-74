...
class GroupMixin(ContainerBase, TagMethods, PermissionMethods):
    ...
    def get_permission_template(self, user_id, **kwargs):
        ...
        return self._invoke_container_api('get_group_user_permission_template', self.id, user_id, permission, kwargs)

    def update_permission_template(self, user_id, permission, **kwargs):
        """Update a user's permission template for a group"""
        return self._invoke_container_api('modify_group_user_permission_template', self.id, user_id, permission, kwargs)
...
class ProjectMixin(ContainerBase, TagMethods, NoteMethods, PermissionMethods, FileMethods, InfoMethods,
                   DownloadMethods, AnalysisMethods):
