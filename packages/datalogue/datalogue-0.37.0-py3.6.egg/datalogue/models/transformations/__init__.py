...
class StandardizeDates(Transformation):
    ...
    def __init__(self, model_url: Optional[str] = None, paths: Optional[List[List[str]]] = None,
    ...
    def __repr__(self):
        ...
        return "StandardizeDates(" + \
               f"model_url: {'' if self.model_url is None else self.model_url}, " + \
...
def _standardize_dates_from_payload(json: dict) -> Union[DtlError, StandardizeDates]:
    ...
    if isinstance(options, List) and len(options) > 0 and (
            options[0].get(Transformation.type_field) == StandardizeDates.include_new_nodes_type):
        include_new_nodes = True

    return StandardizeDates(model_url, paths, include_new_nodes)


class ParseAddresses(Transformation):
    """
    ParseAddresses: Runs data nodes through the address parsing service and augments resulting ADGs with nodes
     containing parts of the address: street_address, zip_code, city etc. Nodes in ADG are referenced
     by their materialized paths, encoded as array of labels.
    """
    ...
    def __repr__(self):
        return f"ParseAddresses(model_url: {'' if self.model_url is None else self.model_url}, " \
               f"paths: {'' if self.paths is None else ','.join(map(lambda path: '.'.join(path), self.paths))})"

    def _as_payload(self) -> dict:
        base = self._base_payload()
        if self.model_url != None:
            base['modelUrl'] = self.model_url
        if self.paths != None:
            base["paths"] = self.paths
...
def _parse_addresses_from_payload(json: dict) -> Union[DtlError, ParseAddresses]:
    ...
    return ParseAddresses(model_url, paths=paths)
