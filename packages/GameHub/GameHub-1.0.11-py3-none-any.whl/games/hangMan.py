...
def randomise(file_path=None) -> str:
    ...
    if file_path is None:
        ...
        if file_path is None:
            file_path = find_file(file_name, '../')
    scrabble = open(file_path, 'r')
    scrabble_read = scrabble.read()
    list_of_words = scrabble_read.split()
    random_word_picker = random.randint(0, len(list_of_words) - 1)
    scrabble.close()
    return list_of_words[random_word_picker]
