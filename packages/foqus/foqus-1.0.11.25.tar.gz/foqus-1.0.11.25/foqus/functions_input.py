        ...
        for kcat, cat in difference[customer_name + '_' + customer_type].items():
            ...
            if category_will_have_changes:
                ...
                try:
                    ...
                    for kphotos, photos in cat[0].items():
                        logger.info("-----delta var = photos:")
                        logger.info(photos)
                        for kchanges, changes in photos.items():
                            if str(kchanges) == '$insert':
                                status = 1
                    ...
                    if str(difference['okaidi_textile']).split(':')[0] == "{insert":
                        status = 1
                    elif str(difference['okaidi_textile']).split(':')[0] == "{delete":
                        status = 0
                    else:
                        status = 2
                    for tuplee in cat:
                        #print(tuplee[1])
                        new_category_objects.append(tuplee[1])
                    add_category_and_write_to_file(status, last_json_path, last_json, new_category_objects, last_json)
                except Exception as e:
...
def verif_folder_less_twenty(path, nb_classes):
    ...
    folder_less_20 = []
    #we will check if the number returned by max_nb_images is < batch size in this case it will cause error in training so we take max between it and batch/nb_classes
...
def process_customer_stream_from_json(json_path, customer_name, customer_type, project_name):
    ...
    logger.info("---- we are out of delta")
