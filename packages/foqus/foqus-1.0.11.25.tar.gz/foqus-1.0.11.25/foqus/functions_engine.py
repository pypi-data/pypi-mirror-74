...
from foqus.commons import count_product, get_count_product_validation, get_payment_details_cms_shopify, get_valid_post,\
    replace_special_caractere, shopify_data_and_train
...
def create_project(customer_name, customer_type, api_project, project_name, project_id):
    ...
    if api_project == 'classification':
        ...
        if project_name in similars_projects:
            ...
            return 0
    elif api_project == "detection_error":
        trainings_path = TRAININGS_PATH + customer_type + '/' + customer_name + '/detection_erreur/images'
        labels_path = TRAININGS_PATH + customer_type + '/' + customer_name + '/detection_erreur/' + 'labels/' + str(
            project_id)
        labels_paths3 = TRAININGS_S3 + customer_type + '/' + customer_name + '/detection_erreur/' + 'labels/' + str(
            project_id) + '/'
        labels_path_azure = AZURE_TRAINING_PATH + customer_type + '/' + customer_name + '/detection_erreur/' + \
                            'labels/' + str(project_id) + '/'
        if project_id not in all_user_projects(customer_name, customer_type, api_project):
            try:
                os.makedirs(trainings_path + '/' + str(project_id))
                os.makedirs(labels_path)

                if os.path.isfile(labels_path + '/' + customer_type + '_' + customer_name + '_labels_accuracy.json'):
                    with open(labels_path + '/' + customer_type + '_' + customer_name + '_labels_accuracy.json') \
                            as jsonfile:
                        labels = json.load(jsonfile)
                else:
                    labels = {}
                labels[project_id] = project_name
                with open(str(labels_path) + '/' + str(customer_type) + '_' + str(
                        customer_name) + '_labels_accuracy.json', 'w') as outfile:
                    json.dump(labels, outfile)
                try:
                    if USE_AWS:
                        upload_file_into_s3(
                            labels_path + '/' + customer_type + '_' + customer_name + '_labels_accuracy.json',
                            labels_paths3)
                    else:
                        upload_file_into_azure(labels_path_azure + customer_type + '_' + customer_name +
                                               '_labels_accuracy.json', labels_path + '/' + customer_type + '_' +
                                               customer_name + '_labels_accuracy.json')
                except Exception as e:
                    logger.error('Error in detection project %s' % e)
                    return 0
                create_update_project(customer_name, customer_type, project_id, api_project, 1, project_name, "{}", 0)
                logger.info("Detection project was created with success ")
                return 1

            except Exception as e:
                logger.error("Cannot create training directory for customer %s directory id %s the error is %s" % (
                    customer_name, project_id, e))
                return 0
        else:
            logger.error('projet exist')
            return 0
                    ...
                    if key == permission and value == 1:
                        if type_del_user in ["customer","SN1", "SN2", "SN3", "admin"]:
                            test = db.delete_user(table_name=DATABASE_USERS_MANAGEMENT_TABLE_NAME,
                                                  email=email)
                            if test is True:
                                msg = "Deleting user: " + email + " successfully..."
                            else:
...
def delete_user_data(customer_name, customer_type):
    ...
    if db.get_customer_info_from_customer_name(customer_name):
        ...
        move_directory(INPUT_S3 + customer_type + '/' + customer_name + '/images/', 'deleted_user')
        for file in list_repository_files(AZURE_INPUT_PATH + customer_type + '/' + customer_name + '/images/'):
            copy_azure_file(file, AZURE_CONTAINER_NAME, "deletedUser-" + AZURE_CONTAINER_NAME)
        logger.info('Archiving input directory for client %s %s' % (customer_name, customer_type))
        move_directory(OUTPUT_S3 + customer_type + '/' + customer_name + '/images/', 'deleted_user')
        for file in list_repository_files(AZURE_OUTPUT_PATH + customer_type + '/' + customer_name + '/images/'):
            copy_azure_file(file, AZURE_CONTAINER_NAME, "deletedUser-" + AZURE_CONTAINER_NAME)
        logger.info('Archiving output directory for client %s %s' %(customer_name, customer_type))
        move_directory(VECTORS_S3 + customer_type + '/' + customer_name + '/', 'deleted_user')
        for file in list_repository_files(AZURE_VECTORS_PATH + customer_type + '/' + customer_name + '/'):
            copy_azure_file(file, AZURE_CONTAINER_NAME, "deletedUser-" + AZURE_CONTAINER_NAME)
        logger.info('Archiving vectors directory for client %s %s' % (customer_name, customer_type))
        move_directory(TRAININGS_S3 + customer_type + '/' + customer_name + '/', 'deleted_user')
        for file in list_repository_files(AZURE_TRAINING_PATH + customer_type + '/' + customer_name + '/'):
            copy_azure_file(file, AZURE_CONTAINER_NAME, "deletedUser-" + AZURE_CONTAINER_NAME)
...
def save_payment_info(customer, customer_type, name, address, zipcode, country, city):
    db.insert_or_update_payment_info(PAYMENT_INFO, customer, customer_type, name, address, zipcode, country, city)
    return True


def shopify_payment_billing(customer_name, customer_type, customer_email, url_cms, token_cms, plan):
    from foqus.commons import get_api_version_shopify
    ...
    if plan == 'STANDARD':
        ...
        price = 499.0
    price_dollar = round(c.convert(price, 'EUR', 'USD'))
...
def payment_client(customer, customer_type, email,  plan_name, tva, methode_payment, payment_id=None, payer_id=None,
                   stripe_token=None):
    payment = db.get_client_payment(customer, customer_type)
    if methode_payment == 'stripe':
        date = datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")
        plan = db.get_plan_form_name(plan_name)
        month_pay = (int(plan[3]) / 12)
        to_pay = month_pay + (month_pay * float(tva))
        total = to_pay * 11
...
def delete_project(customer, customer_type, api_project, project_name, project_id):
    ...
    if project_name != "" and api_project == "similars":
        ...
        try:
            if os.path.isdir(input_path_project_to_delete):
                if USE_AWS:
                    move_directory(input_path_s3, 'deleted_project')
                else:
                    azure_move_directory(input_path_azure, input_path_azure_recovery)
                shutil.rmtree(input_path_project_to_delete)
                logger.info("input path -------- DELETED")
            if os.path.isdir(output_path_project_to_delete):
                if USE_AWS:
                    move_directory(output_path_s3, 'deleted_project')
                else:
                    azure_move_directory(output_path_azure, output_path_azure_recovery)
            ...
            if os.path.isdir(vectors_path_project_to_delete):
                if USE_AWS:
                    move_directory(vectors_path_s3, 'deleted_project')
                else:
                    azure_move_directory(vectors_path_azure, vectors_path_azure_recovery)
            ...
            if os.path.isdir(streams_path_project_similars_to_delete):
                if USE_AWS:
                    move_directory(streams_path_3, 'deleted_project')
                else:
                    azure_move_directory(streams_path_azure, streams_path_azure_recovery)
            ...
            if os.path.isdir(training_path_project_to_delete):
                if USE_AWS:
                    move_directory(training_path_s3_to_delete, 'deleted_project')
                else:
                    azure_move_directory(training_path_azure, training_path_azure_recovery)
        ...
        try:
            if USE_AWS:
                ...
                if os.path.isdir(streams_path_project_classification_to_delete):
                    move_directory(streams_path_s3_classification_to_delete, 'deleted_project')
    ...
    elif project_name != "" and project_id and api_project == "detection_error":
        list_directories = ['labels', 'images', 'bottlenecks', 'model', 'model_text', 'output', 'predictCustomerPATH',
                            'tmp']
        date_supp = strftime("%d-%m-%Y", gmtime())
        for directory in list_directories:
            directory_to_rename = TRAININGS_PATH + str(customer_type) + '/' + str(customer) + '/detection_erreur/' + \
                                  directory + "/" + project_id
            directory_s3_rename = TRAININGS_S3 + str(customer_type) + '/' + str(customer) + '/detection_erreur/' + \
                                  directory + "/" + project_id

            if os.path.isdir(directory_to_rename):
                os.rename(directory_to_rename, directory_to_rename + '_deleted_' + str(date_supp))
                logger.info("directory %s -------- RENAMED" % directory)
                move_directory(directory_s3_rename, 'deleted_project')
        return 1