...
from zuper_commons.ui.colors import color_ops_light
...
def debug_print0(
    ...
    if isinstance(x, Decimal):
        ...
        return color_typename2(str(x))

    if isinstance(x, Fraction):
        if x in known_fraction:
            return color_float(known_fraction[x])
        # return color_ops("Dec") + " " + color_float(str(x))
        return color_float(str(x))
...
def debug_print_typelike(x: TypeLike, dp_compact, dpa, opt: DPOptions, prefix: str, args) -> str:
    ...
    if is_NewType(x):
        ...
        w = get_NewType_arg(x)
        return color_synthetic_types(n)  # + '<' + debug_print_typelike(w, dpa, opt, '', args)
    if (
        (x is type(None))
        # or is_List(x)
        # or is_Dict(x)
...
def debug_print_dict(
    ...
    if len(x) == 0:
        if opt.omit_type_if_empty:
            return bracket_colors(bprefix + empty_dict) + ps
        else:
            return dps(type(x)) + " " + bracket_colors(bprefix + empty_dict) + ps
...
def debug_print_set(
    ...
    if len(x) == 0:
        if opt.omit_type_if_empty:
            return bracket_colors(bprefix + empty_set) + ps
        else:
            return dps(type(x)) + " " + bracket_colors(bprefix + empty_set) + ps
    ...
    if nlines == 0 and tlen < get_max_col(stack):
        ...
        if opt.spaces_after_separator:
            sep += " "
        res = bracket_colors(bprefix + popen) + sep.join(ss) + bracket_colors(pclose) + ps
        if opt.omit_type_if_short:
            return res
        else:
            return dps(type(x)) + " " + res
...
def debug_print_tuple(
    ...
    if nlines == 0 and tlen < get_max_col(stack):
        ...
        res = color_ops("(") + sep.join(ss) + x + color_ops(")") + ps
        if opt.omit_type_if_short:
            return res
        else:
            return dps(type(x)) + " " + res
...
def debug_print_str(x: str, *, prefix: str):
    ...
    if x == "":
        ...
        if x.strip() == x:
            return termcolor.colored(x, "magenta") + ps
        else:
            return termcolor.colored("'" + x + "'", "magenta") + ps
