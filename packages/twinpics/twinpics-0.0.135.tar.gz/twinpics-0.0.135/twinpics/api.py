...
def node_list_conversion(df_graph):
    ...
    for index, row in df_graph.iterrows():
        ...
        df_graph.loc[index, 'screen_name_num'] = my_dictionary[row["screen_name_mention"]]
...
def girvanNewman_alg(node_list,df_graph):
    DiG = building_DiGraph(node_list,df_graph,alg="str")
...
def leiden_alg (df_graph):
    ...
    DiG = building_DiGraph(list(node_list["id"]),df_graph,alg="num")
...
def graph_algoritm_selected(df_graph,selection,node_list):
    ...
    print(selection)
    if(selection =="1"):
        df_algComu = girvanNewman_alg(node_list,df_graph,alg="str")
    elif(selection =="2"):
        df_algComu = leiden_alg(df_graph,alg="num")
    elif(selection =="3"):
        df_algComu = infMap_alg(df_graph, alg="num")
    else:#default
...
def building_DiGraph(node_list, df_graph, alg_type):
    ...
    for i in range(len(df_graph)):
        if alg_type="str":            
            ...
            if(df["screen_name_mention"].iloc[i] != len(node_list)):
                DiG.add_edge(df['screen_name_num'].iloc[i],df['screen_name_mention_num'].iloc[i])
    return DiG
...
def graph_json_values(df_graph, df_algComu):
    ...
    for i in range(len(df_graph)):
        data['edges'].append({
          'screen_name': df_graph[col].iloc[i],
