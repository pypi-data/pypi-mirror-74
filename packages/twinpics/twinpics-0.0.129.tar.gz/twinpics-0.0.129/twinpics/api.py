...
def node_list_conversion(df_graph):
    ...
    df_graph["screen_name_mention_str"] = df_graph["screen_name_mention"]
    df_graph["screen_name_str"] = df_graph["screen_name"]
    for index, row in df_graph.iterrows():
        df_graph.loc[index, 'screen_name'] = my_dictionary[row["screen_name"]]
        df_graph.loc[index, 'screen_name_mention'] = my_dictionary[row["screen_name_mention"]]
...
def girvanNewman_alg(node_list,df_graph):
    DiG = building_DiGraph(node_list,df_graph)
...
def leiden_alg (df_graph):
    ...
    DiG = building_DiGraph(list(node_list["id"]),df_graph)
...
def infMap_alg(df_graph):
    ...
    for i in range(len(df_graph)):
        im.add_link(df_graph['screen_name'].iloc[i],df_graph['screen_name_mention'].iloc[i])
...
def graph_algoritm_selected(df_graph,selection, node_list):
    ...
    if(selection ==1):
        df_algComu = girvanNewman_alg(df_graph)
    elif(selection ==2):
...
def building_DiGraph(node_list, df):
    ...
    for i in range(len(df)):
        ...
        if((df['screen_name_mention'].iloc[i] !="Empty") | (df["screen_name_mention"].iloc[i] != len(node_list))):
...
def graph_json_values(df_graph, df_algComu):
    
    data = {}
    data['nodes'] = []
    for i in range(len(df_algComu)):
      
      data['nodes'].append({
          'screen_name': df_algComu["screen_name"].iloc[i],
          'community': str(df_algComu["community"].iloc[i])
      })
...
def user_timeline_extraction(node_list, consumerKey, consumerSecret,accessToken,accessTokenSecret,filepath, filename,n_groups=1,save_by=300):
    ...
    while ((k<= (len(node_list)+profile_iter)) & (ultima !=2) ):
      ...
      if(len(aux) != 0):  
        ...
        if path.exists(os.path.join(filepath,filename)):
          df2.to_csv(path_or_buf =os.path.join(filepath,filename),mode="a", index=False, header=False)
        else:
          df2.to_csv(path_or_buf = os.path.join(filepath,filename),index=False)         
        if len(list_users_noInfo) !=0:
          nodes = pd.DataFrame()
          nodes["nodos"] = list_users_noInfo
          if path.exists(os.path.join(filepath,"node_without_",filename)):
            nodes.to_csv(path_or_buf = os.path.join(filepath,"node_without_"+filename), mode ="a",index=False, header=False)          
          else:
             nodes.to_csv(path_or_buf = os.path.join(filepath,"node_without_"+filename), index=False)
        k = k+profile_iter        
        if ultima ==1:          
          ultima =2
      else:
    ...
    for i in range(len(data)):
        if i < len(data):
            ...
            if (len(possible_duplicated)==0):
                possible_duplicated.append({"screen_name": "Empty", "distance": "Empty", "tw_similarity": 0,"tw_match_tw": "Empty"})   
                ...
                for t in range(len(data["possible_duplicates"].iloc[i])):
                    ...
                    text_1 = list(itertools.chain(*text_1["text"]))   
                    text_2 = list(itertools.chain(*data.iloc[i]["text_tweets_days_list"]))               
                    if "Empty" in text_1:
                      text_1 = list(filter(("Empty").__ne__, text_1))
                    if "Empty" in text_2:  
                      text_2 = list(filter(("Empty").__ne__, text_2))
                    if "" in text_1:
                      text_1 = list(filter(("").__ne__, text_1))
                    if "" in text_2:  
                      text_2 = list(filter(("").__ne__, text_2))  
                    comparacion = [item for item in text_2 if item in text_1]
                    if len(comparacion) > 0:
                        data["possible_duplicates"].iloc[i][t]["tw_similarity"] = len(comparacion)/(data["num_userTw"].iloc[i])
                        data["possible_duplicates"].iloc[i][t]["text_match_tw"] = comparacion
                    else:
                        data["possible_duplicates"].iloc[i][t]["tw_similarity"] = 0
                        data["possible_duplicates"].iloc[i][t]["text_match_tw"] = "Empty"
    return data
...
def close_duplicates(data):
    close_duplicates_list = []
    for i in range(len(data)):
        close_duplicates = []
        for j in range(len(data.iloc[i]["possible_duplicates"])):
            if data.iloc[i]["possible_duplicates"][j]["tw_similarity"] >0.3:
                close_duplicates.append(data.iloc[i]["possible_duplicates"][j]["screen_name"])
...
def more_tweets_required(df,df_graph_features,consumerKey,consumerSecret,accessToken,accessTokenSecret,filepath,filename,n_groups,save_by):
    ...
    if len(node_list_moreData) >0:
        df_metadata_moreData, list_more_Data_noData = user_timeline_extraction(node_list_moreData,consumerKey,consumerSecret,accessToken,accessTokenSecret,filepath, filename,n_groups,save_by)
        df_metadata_moreData = pd.read_csv(os.path.join(filepath,filename))
        #df_metadata_moreData = text_to_list_features_conversor(df_metadata_moreData)
        df_metadata_moreData = advanced_features(df_metadata_moreData)
        df_metadata_moreData = merge_dataframes(df_metadata_moreData,df_graph_features)
        df_metadata_moreData = abnormal_categories_profiles(df_metadata_moreData)
        print(len(df_metadata_moreData))
        delete_rows = []
...
def contextual_analysis_profile(data, keyword_corpus, hashtag_corpus):  
    ...
    tweets_df["terrorist_suggestion"][(tweets_df["hashtag_score"]>0) | (tweets_df["keyword_score"]>0.3)] = 1
...
def terrorist_keywords_belonging(data):
	temp = data[data['keyword_score']>0.3].reset_index(drop=True)
	# Draw plot
