...
class FundScoreManager:
    ...
    def __init__(self, score_filter=ScoreFilter(), score_penalty_param=ScorePenaltyParams()):
        ...
        '''
        self.funcs = {
            'hs300': ScoreFunc(alpha=0.6/ 0.0444, beta=-0.3, track_err=-0.0/0.03939 ,fee_rate=-0.0/ 0.00266),
            'csi500': ScoreFunc(alpha=0.9/ 0.08291, beta=-0.1),
            'gem':ScoreFunc(alpha=0.45/0.04332, beta=-0.40, track_err=-0.15/0.04749),
    ...
    def get_fund_score(self, dt, index_id, is_filter_c, fund_score_funcs=None) -> dict:
        ...
        score, score_raw = self._get_score(index_id=index_id, dt=dt, cur_d=self.dts.index_fund_indicator_pack.loc[index_id, dt], is_filter_c=is_filter_c, fund_score_funcs=fund_score_funcs)
    ...
    def _get_score(self, index_id, dt, cur_d, is_filter_c,use_weekly_monthly_indicators=False,fund_score_funcs=None) :
        ...
        if cur_d.shape[0] > 1:
            # 不做归一化
            func = self.funcs.get(index_id) if fund_score_funcs is None or fund_score_funcs.get(index_id) is None else fund_score_funcs.get(index_id)
