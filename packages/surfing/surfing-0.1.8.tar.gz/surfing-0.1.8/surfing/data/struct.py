    ...
    def update(self, trade: FundTrade):
        ...
        if trade.is_buy:
            ...
            if trade.is_to_cleanup:
                trade.volume = self.volume
                trade.amount = trade.volume * p
                self.volume = 0
            else:
                self.volume = volume
...
class FundPosition:
    ...
    def calc_mv_n_w(self, 
        ...
        for pos_item in self.funds.values():
            if index_id and pos_item.index_id != index_id or pos_item.volume == 0:
                continue
    ...
    def add(self, fund_id, index_id, volume, avg_cost, update_time, is_permitted_fund):
        pos_item = FundPosItem(fund_id=fund_id, index_id=index_id, volume=volume, avg_cost=avg_cost, update_time=update_time, is_permitted_fund=is_permitted_fund)
    ...
    def update(self, trade: FundTrade):
        ...
        p = trade.trade_date if trade.trade_price else trade.mark_price
...
class ScoreFilter:
    ...
    def __post_init__(self):
        self.FilterIndexId = self.FilterIndexId or ['national_debt']
...
class ScorePenaltyParams:
    ...
    def __post_init__(self):
        self.FilterYearIndex = self.FilterYearIndex or ['national_debt', 'csi500']
