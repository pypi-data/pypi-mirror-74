    ...
    def check_fund_list(self, yesterday=None, wechat_bot=None):
        ...
        if new_funds is not None and new_delisted_funds is not None:
            ...
            if agg_funds:
                ...
                # 拆回到原来的两个列表里
                new_funds = {fund_id: fund_name_got.get(fund_id, None) for fund_id in sorted(new_funds)}
                new_delisted_funds = {fund_id: fund_name_got.get(fund_id, None) for fund_id in sorted(new_delisted_funds)}
        wechat_bot.send_new_fund_list(yesterday, new_funds, new_delisted_funds)
