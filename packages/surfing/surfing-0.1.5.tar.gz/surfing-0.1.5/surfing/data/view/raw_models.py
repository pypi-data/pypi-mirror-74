...
from sqlalchemy.dialects.mysql import DOUBLE, DATE, TINYINT, DATETIME, MEDIUMTEXT, YEAR
...
class EmIndexInfo(Base):
    ...
    SHORTNAME = Column('short_name', CHAR(32), nullable=False)  # 指数简称
    MAKERNAME = Column('maker_name', CHAR(32), nullable=False)  # 编制机构
    INDEXPROFILE = Column('index_profile', TEXT, nullable=False)  # 指数概况
...
class EmFundBenchmark(Base):
    ...
    CODES = Column('em_id', CHAR(16), primary_key=True) # 基金代码
    DATES = Column('datetime', DATE, primary_key=True) # 日期
    BENCHMARK = Column('benchmark', CHAR(255), nullable=False) # 业绩比较基准
...
class WindFundNav(Base):
    ...
    F_NAV_ACCUMULATED = Column(DOUBLE(asdecimal=False)) # 累积净值
    F_NAV_DIVACCUMULATED = Column(DOUBLE(asdecimal=False)) # 累积分红
    ...
    F_NAV_DISTRIBUTION = Column(DOUBLE(asdecimal=False)) # 累积单位分配
...
class WindFundManager(Base):
    ...
    F_INFO_WINDCODE = Column(CHAR(20)) # 万德基金ID
