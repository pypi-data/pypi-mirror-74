...
class FundPainter(object):
    ...
    def plot_mv_on_each_asset(fund_nav:pd.DataFrame, 
        ...
        for index_id in res:
            ...
            asset_weighted_index = index_price[[index_id]].join(asset_weighted_index).fillna(method='ffill').dropna()
    ...
    def plot_rolling_analysis( annual_ret:pd.DataFrame, 
                               ...
                               year:int):
        df1 = annual_ret.dropna()[['战略配置','战术配置']]
        index = df1.index
        saa1 = df1.战略配置
        taa1 = df1.战术配置
        df2 = annual_ret.dropna()[['战术配置','优选基金']]
        index = df2.index
        saa2 = df2.战术配置
        taa2 = df2.优选基金
        df3 = annual_ret.dropna()[['战略配置','优选基金']]
        index = df3.index
        saa3 = df3.战略配置
        taa3 = df3.优选基金
        ...
        pl.title(f'滚动年化收益对比', fontsize=20)
        plt.grid()
        plt.show()
        ...
        st = f'组合年化收益 {round(fund_annual_ret * 100, 3)}%  获取年化收益胜率 {rate}%'
        plt.suptitle(st,y=0.87,fontsize=17)
        plt.grid()
        plt.show()
