...
class HybridPainter(object):
    ...
    def plot_fund_alpha(fund_nav:pd.DataFrame, 
        ...
        res_index = []
        for index_id in res:
            index_df = pd.DataFrame(res[index_id])
            _res = []
            for idx, d in enumerate(rebalance_date[:-1]):
                start_d = d
                end_d = rebalance_date[idx + 1]
                index_df_i = index_df[index_df['start_date'] == start_d]
                fund_nav_i = fund_nav.loc[start_d:end_d,index_df_i.fund_id]
                ...
                volume_i = index_df_i[['volume','fund_id']].set_index('fund_id').T
                _df = pd.DataFrame(fund_nav_i * volume_i.values).sum(axis=1)
                if idx == 0:
                    begin_value = _df.values[0]
                _res.append(_df.diff(1).fillna(0))
            index_ret_df = pd.concat(_res).drop_duplicates(keep='first')
            index_ret_df.iloc[0] = begin_value
            index_ret_df = pd.DataFrame(index_ret_df).rename(columns={0:f'{index_id} funds'})
            df_sum = index_ret_df.cumsum().join(index_price[[index_id]]).copy()#.plot()
            df_sum = df_sum / df_sum.iloc[0]
            df_sum[f'{index_id}_ratio'] = df_sum[f'{index_id} funds'] / df_sum[index_id]
            df_sum = df_sum[[f'{index_id}_ratio']].rename(columns={f'{index_id}_ratio':index_id})
            res_index.append(df_sum.fillna(method='ffill'))
            
        df_plot = pd.concat(res_index, sort=True,axis=1).fillna(method='ffill')
        if 'mmf' in df_plot:
            df_plot=df_plot.drop('mmf',axis=1)
        
        res = []
        for index_id in df_plot:
            _index_stats = Calculator.get_stat_result_from_df(df=df_plot.reset_index(), date_column='index', value_column=index_id).__dict__
