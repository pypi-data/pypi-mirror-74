...
from ca_apm_agent.connections.constants import TRACE_ID, CORR_ID
...
logger = logging.getLogger(LOGGER_NAME[0] + '.probes.requests.http')
# Constants
HTTP = 'http'
URL = 'url'
HOST_NAME = 'hostName'
...
class HTTPReqProbe(Singleton):
    ...
    def start(self, context):
        ...
        headers.update(correlationId)
        context.cor_id = corId
        http_corid = corId
        
        context.set_params({URL: url, HOST_NAME: host_name, HOST_PORT: host_port, TRACE_ID: traceId, CORR_ID: http_corid})
