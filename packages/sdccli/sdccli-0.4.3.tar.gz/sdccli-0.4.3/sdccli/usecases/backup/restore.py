...
class RemoveOwnedAndRestoreOwnedDashboardsUseCase(RestoreMonitorUseCase):
    ...
    def execute(self):
        ...
        drop.dashboards(self._monitor, remove_unowned=False)
        return restore.dashboards(file_data, self._monitor, existing_dashboards={}, restore_unowned=False)
...
class RemoveAllAndRestoreAllDashboardsUseCase(RestoreMonitorUseCase):
    ...
    def execute(self):
        ...
        drop.dashboards(self._monitor, remove_unowned=True)
        return restore.dashboards(file_data, self._monitor, existing_dashboards={}, restore_unowned=True)
...
class RestoreAlertsUseCase(RestoreMonitorUseCase):
    ...
    def execute(self):
        ...
        # We update the notification channel ids of the file_data to restore in case
        # the real IDs of the notification channels have changed
        for alert in file_data:
            alert['notificationChannelIds'] = sorted(
                [nc['id'] for nc in res['notificationChannels'] if nc['name'] in alert['notificationChannelNames']])
