...
from gm.pb.account_pb2 import Order, OrderBusiness_CREDIT_BOM, OrderBusiness_CREDIT_SS, \
    ...
    OrderBusiness_CREDIT_CI, OrderBusiness_CREDIT_CO, OrderBusiness
...
def credit_get_borrowable_instruments(account_id='', df=False):
    # type: (Text, bool) -> List[Dict[Text, Any]]
    """
    查询标的证券，可做融券标的股票列表
    :param account_id:  账号id
    ...
    :return:
    """
    req = GetBorrowableInstrumentsReq(account_id=account_id)
    req = req.SerializeToString()
...
def credit_get_borrowable_instruments_positions(account_id='', df=False):
    # type: (Text, bool) -> List[Dict[Text, Any]]
    """
    查询券商融券账户头寸，可用融券的数量
    :param account_id:  账号id
    ...
    :return:
    """
    req = GetBorrowableInstrumentsPositionsReq(account_id=account_id)
    req = req.SerializeToString()
...
def credit_get_contracts(account_id='', df=False):
    # type: (Text, bool) -> List[Dict[Text, Any]]
    """
    查询融资融券合约
    :param account_id:  账号id
    ...
    :return:
    """
    req = GetCreditContractsReq(account_id=account_id)
    req = req.SerializeToString()
...
def credit_buying_on_margin(symbol, volume, price, order_type=OrderType_Limit, order_duration=OrderDuration_Unknown,
    ...
    return _credit_place_order(OrderBusiness_CREDIT_BOM, symbol, volume, price,
                               order_type, order_duration, order_qualifier, account_id)


def credit_short_selling(symbol, volume, price, order_type=OrderType_Limit, order_duration=OrderDuration_Unknown,
                         order_qualifier=OrderQualifier_Unknown, account_id=''):
    # type: (Text, int, float, OrderType, OrderDuration, OrderQualifier, Text) -> Dict[Text, Any]
    """
    融券卖出
...
def credit_collateral_in(symbol, volume, account_id=''):
    ...
    :return:
    """
    return _credit_place_order(OrderBusiness_CREDIT_CI, symbol, volume, 0, account_id=account_id)
...
def credit_collateral_out(symbol, volume, account_id=''):
    ...
    :return:
    """
    return _credit_place_order(OrderBusiness_CREDIT_CO, symbol, volume, 0, account_id=account_id)


def _credit_place_order(order_business, symbol, volume, price, order_type=OrderType_Limit,
                        order_duration=OrderDuration_Unknown,
                        order_qualifier=OrderQualifier_Unknown, account_id=''):
    # type: (OrderBusiness ,Text, int, float, OrderType, OrderDuration, OrderQualifier, Text) -> List[Dict[str, Any]]
