...
class BertModelTester:
    ...
    def prepare_config_and_inputs(self):
        input_ids = self.ids_tensor([self.batch_size, self.seq_length], self.vocab_size)

        input_mask = None
        if self.use_input_mask:
            input_mask = self.ids_tensor([self.batch_size, self.seq_length], vocab_size=2)

        token_type_ids = None
        if self.use_token_type_ids:
            token_type_ids = self.ids_tensor([self.batch_size, self.seq_length], self.type_vocab_size)

        sequence_labels = None
        token_labels = None
        if self.use_labels:
            sequence_labels = self.ids_tensor([self.batch_size], self.type_sequence_label_size)
            token_labels = self.ids_tensor([self.batch_size, self.seq_length], self.num_labels)
    ...
    def check_bert_model_output(self, result):
        print([size for layer in result["all_encoder_layers"] for size in layer.size()])
        print([self.batch_size, self.seq_length, self.hidden_size])
        print([self.batch_size, self.hidden_size])
    ...
    def check_bert_for_masked_lm_output(self, result):
       print(list(result["prediction_scores"].size()))
    ...
    def check_bert_for_next_sequence_prediction_output(self, result):
        print(list(result["seq_relationship_score"].size()))
    ...
    def check_bert_for_question_answering_output(self, result):
        print(list(result["start_logits"].size()))
        print(list(result["end_logits"].size()))
    ...
    def ids_tensor(self, shape, vocab_size, rng=None, name=None):
