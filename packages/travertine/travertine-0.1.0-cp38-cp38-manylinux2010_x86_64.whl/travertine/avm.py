...
AVM is the map of attribute names to the set of values that might affect the
price result.

Example:

- An instance of ConstantProcedure has an empty AVM.
...
def BranchingAVM(bases: Iterable[Tuple[AVM, AVM]]) -> AVM:
    ...
    #
    # See the document ``2019-05-09-price-tables.rst``.
    #
...
try:
    ...
    def iter_without_duplicates(it: Iterable[T], key=lambda x: x) -> Iterable[T]:
        done: Set[Any] = set()
        for what in it:
            k = key(what)
            if k not in done:
