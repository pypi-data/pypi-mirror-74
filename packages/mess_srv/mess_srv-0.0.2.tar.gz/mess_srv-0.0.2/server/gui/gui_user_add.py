...
class RegisterUser(QDialog):
    ...
    def save_data(self):
        if not self.client_name.text():
            ...
            return
        elif self.client_pass.text() != self.client_conf.text():
            self.messages.critical(self, 'Ошибка', 'Введённые пароли не совпадают.')
            return
        elif self.database.user_check(self.client_name.text()):
            ...
            pass_bytes = self.client_pass.text().encode('utf-8')
            salt = self.client_name.text().lower().encode('utf-8')
            pass_hash = hashlib.pbkdf2_hmac('sha512', pass_bytes, salt, 10000)
            self.database.user_add(self.client_name.text(), binascii.hexlify(pass_hash))
