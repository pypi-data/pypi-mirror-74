...
import six
import pprint
from swimbundle_utils import exceptions
from swimbundle_utils import flattener
...
class RestAPIMixin(object):
    ...
    def configure_status_actions(self, use_default=True, status_mapping_update=None, remove_status_from_mapping=None,
                                 verbose_errors=False, status_range_mapping_update=None):
        """
        :param use_default: Use the default custom swimlane errors.
        :param status_mapping_update: A dictionary to update the mapping of error codes to functions.  This must
                                      be the same format as the default_actions dictionary.  This will overwrite
                                      mappings defined in status_range_mapping_update.
        :param remove_status_from_mapping: A list of status codes to remove from the custom_status_actions mapping
        :param verbose_errors: If true, errors will return more information from the failed request.
        :param status_range_mapping_update: a dictionary with status ranges as keys and functions as values
    ...
    def request(self, method, endpoint, **kwargs):
        ...
        if response.status_code in self.custom_status_actions:
            self.custom_status_actions[response.status_code](response)
...
class BasicRestPaginationEndpoint(BasicRestEndpoint):
    ...
    def combine_responses(self, results):
        ...
        """
        return flattener.extend_nested_lists(results)
    ...
    def execute(self):
        ...
        self.result_count += self.count_result(response)
