...
from .tools import execute_cmd, db_connect
...
def make_credentials(
    ...
    password = (
        ...
        + choices("!@#$%^&*()_+-=[]{}|'")
        + choices(string.ascii_letters + string.digits + "!@#$%^&*()_+-=[]{}|'", k=4)
    )
    shuffle(password)
...
def df_compiler(d: list, m: str = "all", dist: str = "") \
        -> \
        list:
    """\
...
def data_transfer(
    ...
    # outs = [get_glob(i) for i in df.itertuples()]
    ...
    for i in df.itertuples():
        try:
            ...
            assert i.Run_Name not in ["H2", "H3", "H4", "VDJ"]
