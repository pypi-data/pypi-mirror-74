...
INTEGER_EXPRESSION = re.compile(r"^\d+$")
...
def paginated(data, page=None, limit=None, extra=None, transform=None):
    ...
    if isinstance(data, list):
        ...
        #
        # count() must be called after skip and limit are set:
        #
        # This doesn't change the result of count() however,
        # in some cases (i.e. not pymongo cursor but a cursor-like
        # elasticsearch result object), calling count() leads to
        # an actual query while setting skip and limit is supposed
        # to take place before the actual search call
        count = data.count()
    else:
    ...
    result = {"page": page, "total_pages": total_pages, "count": count, "data": data}
...
def json_response(data, code=200):
    ...
    return make_response(
        json.dumps(data, **json_kwargs), code, {"Content-Type": "application/json"}
    )
