...
class Logger:
    ...
    def _log(self, level: int, msg: str, **kwargs) -> None:
        ...
        entry = f"timestamp='{self.utcnow().isoformat('T')}Z' logger='{self.name}' level='{self._level_lookup[level]}' event='{msg}' {extras}\n"  # noqa

        # Log to stderr if at level error or greater, otherwise log to stdout.
        (level >= 4 and self.writeerr(entry)) or self.writeout(entry)
