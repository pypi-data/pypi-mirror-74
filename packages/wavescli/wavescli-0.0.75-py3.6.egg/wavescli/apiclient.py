...
class ApiClient(object):
    ...
    def list_executions(self, term=None, status=None, deleted=False, sort_by='-created_at', page=1, size=200):
        """
        Lists all executions that have not been deleted
        """
        url = self._build_url("executions")
        params = {'term': term, "status": status, "deleted": deleted, "sort_by": sort_by, "page": page, "size": size}
        response = requests.get(
            url,
            headers=self._get_headers(content_type='application/json'),
            params=params
