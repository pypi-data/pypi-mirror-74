    ...
    def check_whitelisting(ip_list):
        ...
        if ip:
            ...
            if ip not in ip_list:
                abort(make_response(jsonify({'message': 'Forbidden Access: {} Not an authorized IP'.format(ip),
                                             'status_code': 403})))
        else:
            return abort(make_response(jsonify({'Exception': "AuthFailure",
                                                'message': 'No IP found in RequestHeader,Please check Nginx config '
                                                           'or header of the request',
                                                'status_code': '424'})))
    ...
    def check_okta_token(**kwargs):
        ...
        if request.headers.__contains__('Authentication'):
            ...
            if not client.get(memcache_key):
                abort(make_response(jsonify({'message': 'Forbidden Access: Request not from an authorized user',
                                             'status_code': 403})))
        else:
            return abort(make_response(jsonify({'Exception': "AuthFailure",
                                                'message': 'No Authentication provided in header',
                                                'status_code': '424'})))
