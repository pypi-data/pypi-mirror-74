...
from azureml.automl.core.shared.exceptions import (
    OutOfBoundDataException, ConfigException, InvalidValueException, EmptyDataException)
...
class _DataGrainItem:
    ...
    def __init__(self,
        ...
        if lookup_end_ix and lookup_start_ix >= lookup_end_ix:
            raise InvalidValueException("end lookup index should be greater than start lookup index", has_pii=False)
    ...
    def _get_index_grain_from_lookup(self, idx) -> tuple:
        ...
        if located_grain is None:
            raise InvalidValueException(
                "passed in {0} is not found in datasets".format(idx)
            ).with_generic_msg("passed in idx is not found in datasets")
...
class TimeSeriesInferenceDataset(TimeSeriesDataset):
    ...
    def _check_required_prediction_horizon(X_df, horizon, time_column, target_column):
        if np.any(np.isnan(X_df[target_column])):
            ...
            raise EmptyDataException(ForecastingErrors.ERR_NOTHING_TO_PREDICT, has_pii=False)
        if X_df[X_df[time_column] >= forecast_origin].shape[0] > horizon:
            raise OutOfBoundDataException(ForecastingErrors.FATAL_MAX_HORIZON_EXCEEDED, has_pii=False)
