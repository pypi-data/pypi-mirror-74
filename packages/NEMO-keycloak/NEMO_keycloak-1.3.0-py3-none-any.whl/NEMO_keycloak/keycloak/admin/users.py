...
class Users(KeycloakAdminBase):
    ...
    def create(self, username, **kwargs):
        ...
        for key in USER_KWARGS:
            from NEMO_keycloak.keycloak.admin.clientroles import to_camel_case

            if key in kwargs:
                payload[to_camel_case(key)] = kwargs[key]

        return self._client.post(
            url=self._client.get_full_url(self.get_path("collection", realm=self._realm_name)),
...
class User(KeycloakAdminBase):
    ...
    def role_mappings(self):
        from NEMO_keycloak.keycloak.admin.user.userroles import UserRoleMappings

        return UserRoleMappings(realm_name=self._realm_name, user_id=self._user_id, client=self._client)

    @property
    def groups(self):
        from NEMO_keycloak.keycloak.admin.user.usergroup import UserGroups

        return UserGroups(realm_name=self._realm_name, user_id=self._user_id, client=self._client)
    ...
    def get(self):
        ...
        self._user = self._client.get(
            url=self._client.get_full_url(self.get_path("single", realm=self._realm_name, user_id=self._user_id))
        )
    ...
    def reset_password(self, password, temporary=False):
        ...
        result = self._client.put(
            url=self._client.get_full_url(
                self.get_path("reset_password", realm=self._realm_name, user_id=self._user_id)
            ),
            data=json.dumps(payload, sort_keys=True),
        )
        return result
