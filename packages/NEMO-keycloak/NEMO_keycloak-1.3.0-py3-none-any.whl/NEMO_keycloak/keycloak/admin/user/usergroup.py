...
class UserGroups(KeycloakAdminBase):
    ...
    def all(self):
        return self._client.get(
            url=self._client.get_full_url(self.get_path("collection", realm=self._realm_name, user_id=self._user_id))
        )

    def add(self, group_id):
        return self._client.put(
            url=self._client.get_full_url(
                self.get_path("single", realm=self._realm_name, user_id=self._user_id, group_id=group_id)
            ),
            data=json.dumps({"realm": self._realm_name, "userId": self._user_id, "groupId": group_id}),
        )
