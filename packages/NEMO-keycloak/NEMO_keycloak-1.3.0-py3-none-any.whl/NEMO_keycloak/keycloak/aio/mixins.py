...
from NEMO_keycloak.keycloak.aio.well_known import KeycloakWellKnown
from NEMO_keycloak.keycloak.mixins import WellKnownMixin as SyncWellKnownMixin
    ...
    async def __async_init__(self) -> "WellKnownMixin":
        async with self._realm._lock:
            if self._well_known is None:
                ...
                self._well_known = await KeycloakWellKnown(realm=self._realm, path=self._realm.client.get_full_url(p))
        return self
