...
from NEMO_keycloak.auth import get_remote_user
from NEMO_keycloak.response import HttpResponseNotAuthorized
...
class BaseKeycloakMiddleware(MiddlewareMixin):
    ...
    def set_session_state_cookie_(self, request, response):
        ...
        if not request.user.is_authenticated or not hasattr(request.user, "oidc_profile"):
            return response
        ...
        # Set a browser readable cookie which expires when the refresh token expires.
        response.set_cookie(
            cookie_name,
...
class KeycloakStatelessBearerAuthenticationMiddleware(BaseKeycloakMiddleware):
    ...
    def process_request(self, request):
        ...
        the exempt setting.
        """
        super(KeycloakStatelessBearerAuthenticationMiddleware, self).process_request(request=request)
        ...
        user = authenticate(request=request, access_token=request.META[self.header_key].split(" ")[1])
...
class RemoteUserAuthenticationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        request.user = SimpleLazyObject(lambda: get_user(request, origin_user=origin_user))
