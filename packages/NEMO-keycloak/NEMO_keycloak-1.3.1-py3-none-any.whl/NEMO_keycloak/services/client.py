...
from NEMO_keycloak.services.exceptions import TokensExpired
...
def get_keycloak_id(client: Client):
    """
    Get internal Keycloak id for client configured in Realm
    """
    keycloak_clients = client.admin_api_client.realms.by_name(name=client.realm.name).clients.all()
...
def get_authz_api_client(client: Client) -> KeycloakAuthz:
    return client.realm.realm_api_client.authz(client_id=client.client_id)


def get_openid_client(client: Client) -> KeycloakOpenidConnect:
    openid = client.realm.realm_api_client.open_id_connect(client_id=client.client_id, client_secret=client.secret)
...
def get_uma1_client(client: Client) -> KeycloakUMA1:
    return client.realm.realm_api_client.uma1


def get_admin_client(client: Client) -> KeycloakAdmin:
    """
...
def get_service_account_profile(client: Client) -> OpenIdConnectProfile:
    """
    ...
    oidc_profile = NEMO_keycloak.services.oidc_profile._update_or_create(
...
def get_new_access_token(client: Client) -> (Dict, datetime):
    """
...
def get_access_token(client):
    ...
    :param NEMO_keycloak.models.Client client:
    :rtype: str
    """
    ...
    try:
        return NEMO_keycloak.services.oidc_profile.get_active_access_token(oidc_profile=oidc_profile)
    except TokensExpired:
        token_reponse, initiate_time = get_new_access_token(client=client)
        oidc_profile = NEMO_keycloak.services.oidc_profile.update_tokens(
            oidc_profile=oidc_profile, token_response=token_reponse, initiate_time=initiate_time
        )
