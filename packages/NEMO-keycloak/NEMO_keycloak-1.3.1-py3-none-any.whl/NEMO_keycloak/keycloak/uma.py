...
class KeycloakUMA(WellKnownMixin, object):
    ...
    def resource_set_read(self, token: str, id: str) -> Dict:
        ...
        return self._realm.client.get(
            "{}/{}".format(self.well_known["resource_registration_endpoint"], id), headers=self.get_headers(token)
        )
    ...
    def resource_set_list(self, token: str, **kwargs) -> List:
        ...
        """
        return self._realm.client.get(
            self.well_known["resource_registration_endpoint"], headers=self.get_headers(token), **kwargs
        )

    def resource_create_ticket(self, token: str, id: str, scopes: List, **kwargs) -> Dict:
        """
        ...
        return self._realm.client.post(
            self.well_known["permission_endpoint"], data=self._dumps([data]), headers=self.get_headers(token)
        )

    def resource_associate_permission(self, token: str, id: str, name: str, scopes: List, **kwargs) -> Dict:
        """
    ...
    def permission_update(self, token: str, id: str, **kwargs) -> Dict:
        """
    ...
    def permission_delete(self, token: str, id: str) -> Dict:
        """
        ...
        """
        return self._realm.client.delete(
            "{}/{}".format(self.well_known["policy_endpoint"], id), headers=self.get_headers(token)
        )

    def permission_list(self, token: str, **kwargs) -> Dict:
        """
        Querying permission
        ...
        :rtype: dict
        """
        return self._realm.client.get(self.well_known["policy_endpoint"], headers=self.get_headers(token), **kwargs)
