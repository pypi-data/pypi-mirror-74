from NEMO_keycloak.keycloak.aio.mixins import WellKnownMixin
from NEMO_keycloak.keycloak.authz import KeycloakAuthz as SyncKeycloakAuthz, PATH_WELL_KNOWN, urlencode
...
class KeycloakAuthz(WellKnownMixin, SyncKeycloakAuthz):
    ...
    async def get_permissions(self, token, resource_scopes_tuples=None, submit_request=False, ticket=None):
        """
    ...
    async def eval_permissions(self, token, resource_scopes_tuples=None, submit_request=False):
        """
        ...
        permissions = await self.get_permissions(
            token=token, resource_scopes_tuples=resource_scopes_tuples, submit_request=submit_request
        )
