...
class Clients(KeycloakAdminBase):
    ...
    def all(self):
        return self._client.get(self._client.get_full_url(self.get_path("collection", realm=self._realm_name)))
...
class Client(KeycloakAdminBase):
    ...
    @property
    def roles(self):
        from NEMO_keycloak.keycloak.admin.clientroles import ClientRoles

        return ClientRoles(client=self._client, client_id=self._id, realm_name=self._realm_name)
