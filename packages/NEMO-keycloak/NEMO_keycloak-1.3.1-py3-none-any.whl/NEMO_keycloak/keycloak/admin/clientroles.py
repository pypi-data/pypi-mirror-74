...
ROLE_KWARGS = ["description", "id", "client_role", "composite", "composites", "container_id", "scope_param_required"]
...
class ClientRoles(KeycloakAdminBase):
    ...
    def by_name(self, role_name):
        return ClientRole(
            realm_name=self._realm_name, client_id=self._client_id, role_name=role_name, client=self._client
        )
    ...
    def create(self, name, **kwargs):
        ...
        return self._client.post(
            url=self._client.get_full_url(self.get_path("collection", realm=self._realm_name, id=self._client_id)),
    ...
    def update(self, name, **kwargs):
        ...
        return self._client.put(
            url=self._client.get_full_url(
                self.get_path("single", realm=self._realm_name, id=self._client_id, role_name=self._role_name)
            ),
            data=json.dumps(payload, sort_keys=True),
        )
