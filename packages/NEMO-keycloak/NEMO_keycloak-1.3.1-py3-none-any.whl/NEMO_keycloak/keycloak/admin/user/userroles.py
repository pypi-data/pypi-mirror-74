...
class UserRoleMappings(KeycloakAdminBase):
    ...
    @property
    def realm(self):
        return UserRoleMappingsRealm(realm_name=self._realm_name, user_id=self._user_id, client=self._client)


class UserRoleMappingsRealm(KeycloakAdminBase):
    _paths = {
        "available": "auth/admin/realms/{realm}/users/{id}" + "/role-mappings/realm/available",
    ...
    def available(self):
        return self._client.get(
            url=self._client.get_full_url(self.get_path("available", realm=self._realm_name, id=self._user_id))
        )
    ...
    def add(self, roles):
        ...
        return self._client.post(
            url=self._client.get_full_url(self.get_path("single", realm=self._realm_name, id=self._user_id)),
