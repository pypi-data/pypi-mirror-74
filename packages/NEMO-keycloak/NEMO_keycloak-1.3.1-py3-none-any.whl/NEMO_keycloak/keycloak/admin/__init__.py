...
class KeycloakAdmin(object):
    ...
    @property
    def realms(self):
        from NEMO_keycloak.keycloak.admin.realm import Realms

        return Realms(client=self)

    def post(self, url, data, headers=None, **kwargs):
        return self._realm.client.post(url=url, data=data, headers=self._add_auth_header(headers=headers))

    def put(self, url, data, headers=None, **kwargs):
        return self._realm.client.put(url=url, data=data, headers=self._add_auth_header(headers=headers))

    def get(self, url, headers=None, **kwargs):
        return self._realm.client.get(url=url, headers=self._add_auth_header(headers=headers))

    def delete(self, url, headers=None, **kwargs):
        return self._realm.client.delete(url=url, headers=self._add_auth_header(headers=headers), **kwargs)
