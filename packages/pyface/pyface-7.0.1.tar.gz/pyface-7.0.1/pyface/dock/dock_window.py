...
lock_action = Action(
    name="Lock Layout", action="on_lock_layout", style="toggle"
)
...
enable_features_action = Action(
    name="Show All", action="on_enable_all_features"
)

disable_features_action = Action(
    name="Hide All", action="on_disable_all_features"
)
...
class DockWindow(HasPrivateTraits):
    ...
    def _left_dclick(self, event):
        ...
        if sizer is not None:
            ...
            if isinstance(object, DockControl):
                ...
                if (
                    (dockable is None)
                    or (dockable.dockable_dclick(object, event) is False)
                ) and (object.style != "fixed"):
                    self.min_max(object)
    ...
    def right_up(self, event):
        ...
        if sizer is not None:
            ...
            if object is not None:
                ...
                if (
                    is_dock_control
                    and (object.dockable is not None)
                    and (
                        event.ShiftDown()
                ...
                if popup_menu is None:
                    ...
                    lock_action.checked = (len(controls) > 0) and controls[
                        0
                    ].locked
    ...
    def _mouse_move(self, event):
        ...
        if self._owner is not None:
            ...
            sizer = self.sizer
            if sizer is not None:
                object = self._object or sizer.ObjectAt(
                ...
                if isinstance(object, DockControl) and object.feature_activate(
                    event
                ):
    ...
    def on_edit(self, object=None):
        ...
        control_info = ControlInfo(
            **object.get("name", "user_name", "style", "user_style")
        )
        if control_info.edit_traits(parent=self.control).result:
            ...
            if name != "":
                object.name = name
            object.trait_set(
                **control_info.get("user_name", "style", "user_style")
            )
            self.update_layout()
    ...
    def _get_dw_db(self, mode="r"):
        try:
            return shelve.open(
                os.path.join(traits_home(), "dock_window"),
                flag=mode,
                protocol=-1,
            )
        except:
    ...
    def _get_layout_menus(self):
        ...
        if len(names) == 0:
            ...
            restore_actions = [
                Action(
                    name=name, action="self.on_restore_layout(%s)" % repr(name)
                )
                for name in names
            ]
            delete_actions = [
                Action(
                    name=name, action="self.on_delete_layout(%s)" % repr(name)
                )
                for name in names
            ]
        return [
    ...
    def wx_drag_any(self, x, y, data, drag_result):
        ...
        if ui_provider or isinstance(data, DockControl):
            ...
            window = self.control
            self._dock_info = dock_info = self.sizer.DockInfoAt(
            ...
            if (
                ...
                or (cur_dock_info.region is not dock_info.region)
                or (cur_dock_info.bounds != dock_info.bounds)
                or (cur_dock_info.tab_bounds != dock_info.tab_bounds)
            ):
...
class ControlInfo(HasPrivateTraits):
    ...
    traits_view = View(
        VGroup(
            HGroup(
                HGroup("name<100>{Label}", "3"),
                HGroup("user_name{Remember label}", show_left=False),
            ),
            HGroup(
                HGroup("style<101>", "3"),
                HGroup("user_style{Remember style}", show_left=False),
            ),
        ),
