...
def showtheheader():
    pprint.header('{:30s}{:>30s}'.format('Tests opérés pour i-Parapheur',
                                         'Resultat'))
    pprint.header('=' * 60)
...
def check_config_alfrescoglobal(varconf, ihmvarconf, version_presente):
    ...
    if parse_version("4.6.0") <= parse_version(version_presente):
        pprint.log("     todo 4.6.x added params: MANUAL CHECK !")
    if parse_version("4.7.0") <= parse_version(version_presente):
        pprint.log("     todo 4.7.x added params: MANUAL CHECK !")
...
def check_pdfrender_service():
    pprint.log("     todo 'poppler ou libriciel-pdf': MANUAL CHECK !")
    # Need?  -> poppler-utils libpoppler-cpp0v5 libnss3
    # Need ? ->  common/lib/libriciel-pdf
    pass
...
def check_side_service(srvname):
    pprint.header("#", False, ' ')
...
def check_crypto_service_config():
    check_side_service("crypto")


# Chapitre  pdf-stamp
def check_pdfstamp_service_config():
    check_side_service("pdf-stamp")


# Chapitre  Pes-viewer
def check_pesviewer_service_config():
    check_side_service("pes-viewer")


# chapitre  Pastell Connector
def check_pastellconnector_service_config():
    check_side_service("pastell-connector")
...
def barelyGuess_iParapheur_version_from_AMP(basedir):
    hashVersionDict = dict([
        ...
        ('c100e76ffa93809e721fcf29174c8ebbe77950535f1e1bb921ed0291ff8ff144', '4.7.0'),
        ('3b47d588bc97a568ee3b0b1b4654318e5653caf9a0bd01d23b97a17ec4d55892', '4.7.1'),
        ('e6a19c0f81203f07cf1436177890cab59c1eb0fd8c7d58db196c34c9922e7735', '4.7.2'),
        ('d464f1319488479e74f429e442add6b8c7c1bf5a2a92c50b3c6c8618660614b1', '4.7.3'),
        ('e690bfee9104cc6443ed89463002b3bda35443b37ae2b3b12129fcbe937e6396', '4.7.4')
        ])
...
def barelyGuess_iParapheur_version_from_WAR(ipWarHash):
    hashVersionDict = dict([
        ...
        ('6269cf4ded6bf4e4e235101c142857eb902530edbe464420dea2c9718b4b1b57', '4.7.0'),
        ('d57c9a980e26aa3ef22e272c2717c9fd8a3c0b381c65d58a453eb9af64b16a6c', '4.7.1'),
        ('5b70f42d376176f081d350948e56891dbf3e27578fe110e3dfa99018851b8838', '4.7.2'),
        ('2501882f4e9af74ec5827ce3dd0337286204d0caddbb10c99546fd0c0f502b80', '4.7.3'),
        ('0d2b2d31b8e4cd9eb4f77bc22f8251ee0605407b96c35c923aa239f2a522093a', '4.7.4')
        ])
...
def check_ws_mca_status(baseinstalldir):
    pprint.header("#", False, ' ')
    pprint.log("---- Check MCA status for web-services ----  ", True)
    pprint.log("     todo 'check_ws_mca_status' !")
...
def check_crontab_jobs(version_ip_int):
    ...
    expected_crontab = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                    '..', "files/crontab/" + str(version_ip_int) + ".txt")
    ...
    os.system('diff /tmp/expected_crontab.txt /tmp/clean_crontab.txt > /tmp/crontab_diff.txt')
...
nginxservice = check_https_service_config(config)
...
if parse_version("4.7.0") <= parse_version(version_ip):
    ...
    check_pdfrender_service()
    # Les connecteurs crypto , pdf-stamp , pes-viewer
    check_crypto_service_config()
    check_pdfstamp_service_config()
    check_pesviewer_service_config()
else:
