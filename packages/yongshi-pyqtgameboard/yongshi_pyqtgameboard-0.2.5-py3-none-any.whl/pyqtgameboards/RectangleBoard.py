...
class QRectangleboard(QtWidgets.QGraphicsView):
    ...
    def build_board_scene(self):
        """       
        Creates a gameboard of rows and columns of rectangles of a sepecific
        size. 

        the default board consists of a number of rectangles that touch at the horizontal tip. 
        The offset rectangles in between them, offset above and below, are in
    ...
    def build_tiles(self):
        ...
        # Create rectangles for all the rows and columns
        row = 1
        while row <= self.rows:
...
def test_empty_board():
    ...
    app()
    frame = QRectangleboard(rows = 21, columns = 11, horizontal=True)
    main(frame)
