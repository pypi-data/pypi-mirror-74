...
# 	Copyright (c) 2007, 2008, 2009, 2014, 2015, 2016, 2017, 2018, 2019, 2020 R.Dreas Nielsen
		...
		for ix, configfile in enumerate(config_files):
			if not configfile in self.files_read and os.path.isfile(configfile):
				...
				if cp.has_option(self._OUTPUT_SECTION, "export_row_buffer"):
					try:
						self.quote_all_text = cp.getint(self._OUTPUT_SECTION, "export_row_buffer")
					except:
...
class Database(object):
	...
	def drop_table(self, tablename):
		...
		self.commit()
	def populate_table_unbuffered(self, schema_name, table_name, rowsource, column_list, tablespec_src):
...
class PostgresDatabase(Database):
	...
	def import_tabular_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		if (encodings_match(input_enc, self.encoding) or input_enc in pg_encodings) \
...
def x_export(**kwargs):
	...
	if filefmt == 'txt' or filefmt == 'text':
		...
		write_query_to_xml(select_stmt, table, dbs.current(), outfile, append, desc=description)
		return None
	elif filefmt == 'json':
		...
		write_query_to_json_ts(select_stmt, dbs.current(), outfile, append, not notype, desc=description)
		return None
	elif filefmt == 'values':
...
metacommands.append(MetaCommand(ins_table_rxs(r'^\s*EXPORT\s+', ins_fn_rxs(r'\s+(?P<tee>TEE\s+)?(?P<append>APPEND\s+)?TO\s+', r'\s+AS\s+(?P<format>CSV|TAB|TSV|TABQ|TSVQ|UNITSEP|US|TXT|TXT-ND|PLAIN|ODS|JSON|XML|VALUES|HTML|LATEX|RAW|B64|FEATHER|HDF5)(?:\s+DESCRIP(?:TION)?\s+"(?P<description>[^"]*)")?\s*$')),
...
metacommands.append(MetaCommand(ins_table_rxs(r'^\s*EXPORT\s+', ins_fn_rxs(r'\s+(?P<tee>TEE\s+)?(?P<append>APPEND\s+)?TO\s+', r'\s+AS\s+(?P<format>JSON_TS|JSON_TABLESCHEMA)(?:\s+(?P<notype>NOTYPE))?(?:\s+DESCRIP(?:TION)?\s+"(?P<description>[^"]*)")?\s*$')),
									x_export, "EXPORT", "Write data from a table or view to a file."))
...
def x_debug_log_config(**kwargs):
	...
	exec_log.log_status_info("Config; Empty rows = %s\n" % conf.empty_rows)
...
def write_query_to_json_ts(select_stmt, db, outfile, append=False, write_types=True, desc=None):
	try:
		hdrs, rows = db.select_rowsource(select_stmt)
	except ErrInfo:
		raise
...
def write_query_to_xml(select_stmt, tablename, db, outfile, append=False, desc=None):
	global conf
	try:
		hdrs, rows = db.select_rowsource(select_stmt)
	except ErrInfo:
...
See the documentation for more complete descriptions of the metacommands.
   APPEND SCRIPT <script_1> TO <script_2>
   ...
   LOG "<text>"
   LOOP WHILE/UNTIL (<conditional_expression>)
