    ...
    for c in command_list:
        if str(message.content).startswith(c):
            ...
            if command_dict[c]["connect_to_voicechannel"] != False:
                if user != message.channel.author:
                    await message.channel.send(f"Only {user.name} can use this command. Please wait until the current song is finished then try again.")
                    return
                try:
                    if len(message.content.split(" ")) >= 2:
                        if "https://" in message.content.split(" ")[1]:
                            ...
                            if platform.system() == "Windows":
                                ...
                                if "Usage" not in subprocess.getoutput("youtube-dl"):
                                    if "Usage" not in subprocess.getoutput(r"C:\Users\knowl\Downloads\Youtube-Dl\youtube-dl.exe"):
                                        print("Please install YouTube-dl to play music.")
                                        return
                                subprocess.call(f"youtube-dl --extract-audio --audio-format mp3 -o {dir_path}\{song_name}.mp3 {message.content.split(' ')[1]}", shell=True)
                            if platform.system() != "Windows":
                                command_dict[c]["source_to_play"] = f"{dir_path}/{song_name}.mp3"
                                if "Usage" not in subprocess.getoutput("youtube-dl"):
                                    print("Please install Youtube-dl to continue")
                                    return
                                subprocess.call(f"/usr/bin/python3 /usr/local/bin/youtube-dl --extract-audio --audio-format mp3 -o {dir_path}/{song_name}.mp3 {message.content.split(' ')[1]}", shell=True)
                    ...
                    if command_dict[c]["source_to_play"] == None:
                        if link_given != True:
                            print(
                                "Bot will not play audio because no source file was given.")
                            return
            ...
            if command_dict[c]["disconnect_from_voice_channel"] != False:
                if user != message.author:
                    await message.channel.send(f"Only {user.name} can use this command. Please wait until this song is finished, then try again.")
                return
                try:
