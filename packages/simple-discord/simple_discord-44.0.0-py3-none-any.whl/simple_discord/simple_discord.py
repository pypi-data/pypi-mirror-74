                        ...
                        if platform.system() == "Windows":
                            ...
                            if "Usage" not in subprocess.getoutput("youtube-dl"):
                                if "Usage" not in subprocess.getoutput(r"C:\Users\knowl\Downloads\Youtube-Dl\youtube-dl.exe"):
                                    ...
                                    return
                            subprocess.call(f"youtube-dl --extract-audio --audio-format mp3 -o {dir_path}\{song_name}.mp3 'ytsearch:{message.content.split(' ')[1:]}'", shell=True)
                        if platform.system() != "Windows":
                            command_dict[c]["source_to_play"] = f"{dir_path}/{song_name}.mp3"
                            if "Usage" in subprocess.getoutput("youtube-dl"):
                                print("Please install Youtube-dl to continue")
                                return
                            subprocess.call(f"/usr/bin/python3 /usr/local/bin/youtube-dl --extract-audio --audio-format mp3 -o {dir_path}/{song_name}.mp3 \"ytsearch:{message.content.split(' ')[1:]}\"", shell=True)
