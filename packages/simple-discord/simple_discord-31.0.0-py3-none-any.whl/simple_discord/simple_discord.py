                ...
                try:
                    if len(message.content.split(" ")) >= 2:
                        if "https://" in message.content.split(" ")[1]:
                            ...
                            if platform.system() != "Windows":
                                command_dict[c]["source_to_play"] = f"{dir_path}/{song_name}.mp3"
                                subprocess.call(f"/usr/bin/python3 /usr/local/bin/youtube-dl youtube-dl --extract-audio --audio-format mp3 -o {dir_path}/{song_name}.mp3 {message.content.split(' ')[1]}", shell=True)
