...
def add_tasks_and_process_time_middleware(new_app):
    ...
    async def add_process_time_and_tasks(request: Request, call_next):
        ...
        print('contect_dict1', context.__dict__)
        print('sls sdk1', context.serverless_sdk)
        context.serverless_sdk.set_endpoint(request.url.path)
        response = await call_next(request)
        if response.status_code == 404 and g.settings.save_calls:
...
def create_handler(app):
    def handler(event, context):
        ...
        print('contect_dict', context.__dict__)
        print('sls sdk', context.serverless_sdk)

        print("EVENTD", event.__dict__)
