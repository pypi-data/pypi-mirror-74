...
def set_automated_followed_pool(
    ...
    """ Generare a user list based on the Digly followed usernames """
...
def unfollow(
    ...
    if (
        DiglyFollowed is not None
        and isinstance(DiglyFollowed, (tuple, list))
        and len(DiglyFollowed) == 2
        and DiglyFollowed[0] is True
        and DiglyFollowed[1] in ["all", "nonfollowers"]
    ):
        unfollow_track = DiglyFollowed[1]
        DiglyFollowed = True
    else:
        DiglyFollowed = False
    ...
    if (
        ...
        elif DiglyFollowed is True:
            logger.info("Unfollowing the users followed by Digly\n")
        ...
        # for `customList` and
        #  `DiglyFollowed` unfollow methods
        if customList is True or DiglyFollowed is True:
