...
from .exceptions import DiglyError
...
class Digly:
    ...
    def __init__(
        ...
        print("Digly Version: {}".format(__version__))
        ...
        Settings.Digly_is_running = True
        # workspace must be ready before anything
        if not get_workspace():
            raise DiglyError("Oh no! I don't have a workspace to work at :'(")
        ...
        if self.nogui:
            if not platform.startswith("win32"):
                ...
                raise DiglyError("The 'nogui' parameter isn't supported on Windows.")
        ...
        if self.split_db:
            Settings.database_location = localize_path(
                "db", "digly_{}.db".format(self.username)
            )
        ...
        self.logger = self.get_digly_logger(self.show_logs, log_handler)
        ...
        if selenium_local_session:
            ...
            if len(err_msg) > 0:
                raise DiglyError(err_msg)

    def get_digly_logger(self, show_logs: bool, log_handler=None):
        """
        ...
        if existing_logger is not None:
            ...
            # initialize and setup logging system for the Digly object
    ...
    def login(self):
        ...
        # Digly uses page_delay speed to implicit wait for elements,
    ...
    def set_use_clarifai(
        ...
        Raises:
            DiglyError if os is windows
        """
        if self.aborting:
            return self

        # if os.name == 'nt':
        #    raise DiglyError('Clarifai is not supported on Windows')
    ...
    def unfollow_users(
        ...
        digly_followed_enabled: bool = False,
        digly_followed_param: str = "all",
        ...
        try:
            unfollowed = unfollow(
                ...
                (digly_followed_enabled, digly_followed_param),
                nonFollowers,
                allFollowing,
                style,
    ...
    def end(self, threaded_session: bool = False):
        ...
        Settings.Digly_is_running = False
