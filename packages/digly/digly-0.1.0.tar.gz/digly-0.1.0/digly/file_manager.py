...
def set_workspace(path=None):
    ...
    if not Settings.Digly_is_running:
        if path:
        ...
        message = (
            ...
            " Digly has started :>\t~using existing.."
        )
        highlight_print(
            Settings.profile["name"], message, "workspace", "info", Settings.logger
        )
...
def update_locations():
    ...
    if not Settings.database_location:
        Settings.database_location = localize_path("db", "digly.db")
...
def verify_workspace_name(path):
    """ Make sure chosen workspace name is Digly friendly """
