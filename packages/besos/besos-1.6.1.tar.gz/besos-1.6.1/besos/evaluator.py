...
Currently there are four specific evaluators: EvaluatorGeneric, AdaptiveSR(adaptive sampling),
EvaluatorEP(EnergyPlus), and EvaluatorEH(PyEHub).
...
def _list_cache(
    ...
    size: Optional[int] = config.cache["size"],
    typed: bool = config.cache["typed"],
):
    """Decorator that allows the lru_cache (last recently used cache)
    to work on functions that receive lists"""

    # TODO add complete type hint
    def _list_cache_decorator(f: Callable) -> Callable:
        if enabled:
...
class AbstractEvaluator(ABC):
    ...
    def _uncached_call(self, values: Sequence, **kwargs) -> tuple:
        ...
        try:
            ...
            if self.error_mode == "Failfast":
                ...
                # TODO: Move error_value to flat format as well
    ...
    def __call__(self, values: Sequence, **kwargs) -> tuple:
        ...
        if self.multi:
            return self._uncached_call(values, **kwargs)
        else:
            return self._call(values, **kwargs)
    ...
    def estimate_time(self, df: DF, processes: int = 4) -> None:
    ...
    def df_apply(self, df: DF, keep_input=False, processes: int = 4, **kwargs) -> DF:
    ...
    def _convert_error_mode_from_old_format(self, error_value):
        ...
        if isinstance(outputs, Sequence) or isinstance(constraints, Sequence):
            warnings.warn(warning, FutureWarning)
            if outputs is None:
                outputs = [None,] * self.problem.num_outputs
            outputs = list(outputs)
            if constraints is None:
                constraints = [None,] * self.problem.num_constraints
...
class EvaluatorGeneric(AbstractEvaluator):
    ...
    def eval_single(self, values: Sequence, **kwargs) -> Sequence:
...
class EvaluatorEP(AbstractEvaluator, ABC):
    ...
    def __init__(
        ...
        *,
        ...
        :param distributed: Using this flag will save eplus configuration files into memory.
            It lowers performance, but it enables parallel computing with remote workers
    ...
    def df_apply(
        ...
        *,
        ...
        for j, parameter in enumerate(self.problem.inputs):
            if not isinstance(parameter, int):
                # FIXME: This code block looks broken
                if parameter.mode != "Set":
                    names = self.problem.names("inputs")
                    if parameter.mode != "Multiply":
                        df.rename(
                            columns={names[j]: names[j] + " Multiplier"},
                            inplace=True,
                            errors="raise",
                        )
                    if parameter.mode != "Add":
                        df.rename(
                            columns={names[j]: names[j] + " Addend"},
                            inplace=True,
                            errors="raise",
                        )
        if keep_input:
    ...
    def eval_single(self, values: list, out_dir=None, keep_dirs=False):
        ...
        if self.multi and self.distributed:
            # the remote worker can't access the original epw file
...
class EvaluatorEH(AbstractEvaluator, ABC):
    ...
    def __init__(
        ...
        self.hub = hub
        self.problem = problem
        with open(Path(config.baseDir, "config.yaml"), "r") as file:
            self.config_settings = yaml.safe_load(file)

    # Need to overwrite __call__ due to default __call__ caching failing on time series values.
    def __call__(self, values: Sequence, **kwargs) -> tuple:
