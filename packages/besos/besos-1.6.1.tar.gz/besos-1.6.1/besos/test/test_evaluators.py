...
import sampling
from evaluator import EvaluatorEH, EvaluatorEP, EvaluatorGeneric
...
def oldstyle_hub_problem():
    parameters = []
    message = "Use a PathSelector and a Parameter instead. These have been swapped in automatically."
...
@pytest.mark.slow
def test_distributed_evaluatorEP(building, problem, regtest):
...
def test_epw_file_rename(building):
    ...
    with pytest.warns(FutureWarning, match="epw_file has been deprecated"):
        evaluator = EvaluatorEP(
            problem=Problem(), building=building, epw_file="test/path"
        )
    assert evaluator.epw == "test/path"
