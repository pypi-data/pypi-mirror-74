...
def get_eso_version(eso) -> Version:
    ...
    first_line_regex = re.compile(
        ...
        (?P<version_number>[\d.]+)-(?P<hash>[0-9a-z]{10}),\ 
        YMD=(?P<date>\d\d\d\d\.\d\d\.\d\d+)\ (?P<time>\d?\d:\d\d)$""",
        re.VERBOSE,
    )
...
def read_eso(
    out_dir: str = config.out_dir, file_name="eplusout.eso", version=None,
) -> _results_format:
    ...
    if version:
        warn(
            ...
            "Teh version argument is deprecated "
...
def time_series_values(results: EPResults):
    """Returns the entire value column from some results formatted as a pandas DF

    :param values: the values to display
    :return: the formatted DF of the values in the collection.
    """
