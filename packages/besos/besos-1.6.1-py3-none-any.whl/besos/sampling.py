...
def dist_sampler(
    ...
    data = {}
    for i, (name, param) in enumerate(zip(problem.names("inputs"), problem.inputs)):
        # TODO: is there a better way to do this?
        if type(param.value_descriptor).__name__ == "DependentParameter":
            values = data[problem.names("inputs")[param.value_descriptor.index]]
            data[name] = _sample_param(param, values)
        else:
            data[name] = _sample_param(param, samples[:, i])
...
def MC_Voronoi_estimate(P, problem):
    ...
    # initialize arrays
    V_P = np.zeros([len(P),])  # Vector of all Voronoi sample estimates
