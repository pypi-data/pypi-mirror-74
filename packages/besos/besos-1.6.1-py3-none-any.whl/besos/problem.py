...
class Problem(IO_Objects.ReprMixin):
    ...
    def to_df(
        ...
        types = [
            p.pd_type if hasattr(p, "pd_type") else None
            for p in self.expand_parts(parts)
        ]
    ...
    def pre_optimisation(self):
        """Prepare optimisation for dependent parameter and non objectives.
        """
        for i in range(self.num_inputs):
            if type(self.inputs[i].value_descriptor).__name__ == "DependentParameter":
                self.depend_params[i] = []
        if self.add_outputs:
            self.add_outputs_list = []

    def overwrite_values(self, values):
        """Replace dataframe value with dependent parameter's value.
        """
        if not self.depend_params:
            return values
        l_values = list(values)
        for dp in self.depend_params:
            depend_value = self.get_depend_value(
                self.inputs[dp].value_descriptor, l_values
            )
            l_values[dp] = depend_value
            self.depend_params[dp].append(depend_value)
        return tuple(l_values)

    def get_depend_value(self, depend_param, values):
        """Return value of dependent parameter
        """
        return depend_param.get_value(values[depend_param.index])
    ...
    def overwrite_df(self, df):
        """Insert dependent parameter's value and/or add_outputs' value to dataframe
        """
        labels = df.columns.values
        for index in self.depend_params:
            df.drop(columns=labels[index], inplace=True)
            df.insert(index, labels[index], self.depend_params[index])
        if self.add_outputs:
            l = self.get_non_objective(df)
            for i, data in enumerate(l):
                df.insert(
                    len(df.columns), self.add_outputs[i].name, data,
                )
        return df
