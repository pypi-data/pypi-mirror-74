...
class HydroEconCoupling(object):
    ...
    def _build_water_user_matrix(self):
        """ Resturns a pandas DataFrame in which the index are nodes in the network and columns are farm id's. Non-zero
        entries are Farm objects that divert water from the node indicated by the index.

        The function loops through nodes in the network, find farms diverting from it and construct a matrix of
    ...
    def simulate_all_users(self, lst_scenarios, path_kf_info=None):
            ...
            for farm in self.farms_table.values[self.farm_idx]:
                if obs.get("farm_id") == farm.id:
                    if path_kf_info is None:
                        print("Simulating farm id %i with name %s using standard PMP" % (farm.id, farm.name))
                        farm.simulate(**obs)
                    if isinstance(path_kf_info, str):
                        print("Simulating farm id %i with name %s using stochastic PMP" % (farm.id, farm.name))
                        kf = kalmanfilter.KalmanFilter(farm,
                                                       fn_info_file=os.path.join(
                                                           path_kf_info, str(farm.id) + '_kf_info.json'))
                        kf.simulate(obs, fn_write_ensemble_states=os.path.join(
                                                           path_kf_info, str(farm.id) + '_simul_ensemble.h5'))

        return FarmCoupling(self.water_users, self.farms_table, self.farm_idx, self.water_user_mask, self.transform)
...
class FarmCoupling(object):
    ...
    def __init__(self, water_users_lst, farm_table, farm_idx, water_user_mask, transform):
        ...
        self.applied_water_factor = np.zeros_like(self.farms_table.values) #pd.DataFrame(dtype=object).reindex_like(self.farms_table).fillna(0)

        self.array_supplemental_irrigation = np.zeros_like(self.water_user_mask)

        self._calculate_applied_water_factor() #TODO: make this function initialize self.applied_water_factor as a pandas object
    ...
    def _area_lat_long(self):
        """returns a a 2D array with cell areas of water_user_mask map calculated in hectares"""
    ...
    def retrieve_water_diversion_per_node(self, date):
        """Returns a vector and two matrices with the following information:
         ...
         ::
           vector ``num_nodes``: total daily water volume (m3/day) diverted form each of the n nodes from all users

           matrix ``num_nodes x num_water_users`` daily water volume (m3/day) diverted from node n for user m

           matrix ``num_nodes x num_water_users`` daily water rate (mm/day) diverted from node n for user m
        ...
        # Transforms volume of plant water use into diversion rate by dividing by land, irrigation efficiency and
        # applied water factor
        ...
        Xw_vol = Xw * Xlnd * 10.0  # conversion factor to transform dekastere (10,000 liters or 10 m3) to cubic meter
    ...
    def retrieve_supplemental_irrigation_map(self, array_land_use, irr_ag_ids, water_diversion_table):
        ...
        id ``irr_ag_ids`` resulting from spreading evenly in space water rates diverted by water users as provided in
        ``water_diversion_table``

        :param array_land_use: numpy array with land use codes
        :param irr_ag_ids: tuple with land use codes associated with irrigated ag
        :param water_diversion_table: table with water diversions in mm*ha/day (decastere/day)
        """
        ...
        for i, farm in enumerate(gen):
            ...
            self.array_supplemental_irrigation = np.where(np.isin(lu, irr_ag_ids) &
                                                          ...
                                                          applied_water / area_ag if m > 0  else 0.0,
    ...
    def _calculate_applied_water_factor(self):
        ...
        for i, farm in enumerate(self.farms_table.values[self.farm_idx]):
            print("calculating applied water factors for farm %i with name %s" %(farm.id, farm.name))
            try:
