...
from utils.check_farms import check_farms
import numpy as np
import json
...
class Farm(WaterUser):
    ...
    def __init__(self, fname=None, **kwargs):
        ...
        super(Farm, self).__init__(int(kwargs.get("id")), int(kwargs.get("source_id")), kwargs.get("name"))
    ...
    def lagrange_mults(self):
        ...
        av_scale_Water = self.ref_prices / (self.ref_et * self.ref_land) #* 1000 # * 1000 to convert $/liter to $/m3
    ...
    def _set_reference_observations(self, **kwargs):
        ...
        costs[:, 1] *= ((self.ref_et * self.ref_land)/(self.ref_prices * self.ref_yields))[cal_mask]
        ...
        def calibrate(solve_pmp_program=True, **kwargs):
            ...
            if solve_pmp_program:
                ...
                if opt_res.success:
                    print("Farm %s with id %i successfully calibrated" % (self.name, self.id))
    ...
    def simulate(self, **kwargs):
        ...
        costs[:, 1] *= (self.ref_et * self.ref_land)/(self.ref_prices * self.ref_yields)
