...
def make_flags(from_argv=False):
    ...
    argparser.add_argument(
     '--s1_choice', type=str, default='I would prefer to talk to <Speaker 1>',
     help='text next to speaker 1 radio button'
    )
    argparser.add_argument(
     '--s2_choice', type=str, default='I would prefer to talk to <Speaker 2>',
     help='text next to speaker 2 radio button'
    )
    argparser.add_argument(
     '--question', type=str, default='Who would you prefer to talk to for a long conversation?',
    ...
    argparser.add_argument(
     '--block_on_onboarding', type=bool, default=True,
    ...
    # make desired tasks randomly from scratch
        ...
        for model_0, model_1 in opt['model_comparisons']:
            if (model_0 not in conv_ids_by_model or model_1 not in conv_ids_by_model):
                ...
                raise Error("Please provide a list of tuples of valid models in --model_comparison")
...
def get_new_task_data(worker, tasks_per_hit):
    ...
    while (not task_queue.empty()) and tries < task_queue.qsize():
        try:
            ...
            break
        tries += 1
        if (next_task['task_specs']['internal_id'] not in completed_tasks and
                next_task['conversations'][0] not in seen_conversations and
                next_task['conversations'][1] not in seen_conversations):
...
def main(opt):
    ...
    #     with open('/private/home/margaretli/ParlAI/parlai/mturk/run_data/live/workers.txt', 'r') as workers:
    #         for l in workers:
    #             if ',' in l:
    #                 l = l.split(',')[0]
