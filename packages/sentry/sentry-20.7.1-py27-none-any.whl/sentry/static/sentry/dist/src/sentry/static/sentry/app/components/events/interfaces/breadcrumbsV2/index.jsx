...
var Breadcrumbs = /** @class */ (function (_super) {
    ...
    Breadcrumbs.prototype.expandCollapsedCrumbs = function () {
        this.setState(function (prevState) { return ({
            filteredBySearch: prevState.breadcrumbs,
        }); }, this.scrollToTheBottom);
    };
    Breadcrumbs.prototype.scrollToTheBottom = function () {
        var _a;
        var element = (_a = this.listRef) === null || _a === void 0 ? void 0 : _a.current;
        if (!element) {
            return;
        }
        element.scrollTo({
    ...
    Breadcrumbs.prototype.loadBreadcrumbs = function () {
        ...
        var virtualCrumb = this.getVirtualCrumb(breadcrumbs[0]);
        if (virtualCrumb) {
            breadcrumbs = __spread(breadcrumbs, [virtualCrumb]);
        }
        var tranformedCrumbs = transformCrumbs(breadcrumbs);
        var filterOptions = this.getFilterOptions(tranformedCrumbs);
        this.setState({
    ...
    Breadcrumbs.prototype.getCollapsedBreadcrumbs = function (breadcrumbs) {
    ...
    Breadcrumbs.prototype.getVirtualCrumb = function (breadcrumb) {
    ...
    Breadcrumbs.prototype.render = function () {
        ...
        {filteredBySearch.length > 0 ? (<List breadcrumbs={filteredBySearch} ref={this.listRef} event={event} orgId={orgId} onSwitchTimeFormat={this.handleSwitchTimeFormat} displayRelativeTime={displayRelativeTime} searchTerm={searchTerm}/>) : (<StyledEmptyMessage icon={<IconWarning size="xl"/>} action={<Button onClick={this.handleResetFilter} priority="primary">
                {t('Reset Filter')}
              </Button>}>
