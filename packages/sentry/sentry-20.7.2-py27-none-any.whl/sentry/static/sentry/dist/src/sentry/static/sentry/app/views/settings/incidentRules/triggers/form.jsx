...
var TriggerForm = /** @class */ (function (_super) {
    ...
    TriggerForm.prototype.render = function () {
        var _a = this.props, disabled = _a.disabled, error = _a.error, trigger = _a.trigger, isCritical = _a.isCritical, thresholdType = _a.thresholdType, fieldHelp = _a.fieldHelp, triggerLabel = _a.triggerLabel, onThresholdTypeChange = _a.onThresholdTypeChange;
        return (<Field label={triggerLabel} help={fieldHelp} required={isCritical} error={error && error.alertThreshold}>
        <ThresholdControl disabled={disabled} disableThresholdType={!isCritical} type={trigger.label} thresholdType={thresholdType} threshold={trigger.alertThreshold} onChange={this.handleChangeThreshold} onThresholdTypeChange={onThresholdTypeChange}/>
      </Field>);
    };
    return TriggerForm;
}(React.PureComponent));
    ...
    TriggerFormContainer.prototype.render = function () {
        ...
        {triggers.map(function (trigger, index) {
            ...
            return (<TriggerForm key={index} api={api} config={config} disabled={disabled} error={errors && errors.get(index)} trigger={trigger} thresholdType={thresholdType} resolveThreshold={resolveThreshold} organization={organization} projects={projects} triggerIndex={index} isCritical={isCritical} fieldHelp={tct('The threshold that will activate the [severity] status.', {
                ...
                </React.Fragment>} onChange={_this.handleChangeTrigger(index)} onThresholdTypeChange={onThresholdTypeChange}/>);
        })}
