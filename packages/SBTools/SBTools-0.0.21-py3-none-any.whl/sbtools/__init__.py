...
def sendmsg(msgtitle='',msgtext='',touser="@all"):
    url='http://wx.msgbox.xyz/sendmsg'
    data={"msgtitle":msgtitle,"msgtext":msgtext,"touser":touser}
...
def formatcookie(MyCok):
    return eval('{"'+MyCok.replace('=','":"').replace(';','","').replace('":"":"','==').replace(" ","")+'"}')

def writestr(Str,File="./temp.log"):
    with open(File, 'a') as File:
        File.write(Str+"\n")
        print ("写入完成！")

def randomagent(UserAgentList):
...
def genuid():
    return md5hex(str(''.join(random.sample(string.ascii_letters + string.digits,18)))+str(timestamp(1)))

def findstring(Str,Key):
    return re.compile(Key).findall(str(Str))

def linkstr(str1,str2,lstr):
    if str(str1) == "":
        return str(str2)
    else:
