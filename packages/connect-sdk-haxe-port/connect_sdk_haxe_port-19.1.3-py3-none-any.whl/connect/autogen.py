...
class connect_Flow(connect_Base):
    ...
    def prepareRequestAndOpenLogSection(self,model):
        ...
        marketplace = (assetRequest.marketplace.id if (((assetRequest is not None) and ((assetRequest.marketplace is not None)))) else (listing.contract.marketplace.id if (((listing is not None) and ((listing.contract.marketplace is not None)))) else (tierConfigRequest.configuration.marketplace.id if (((tierConfigRequest is not None) and ((tierConfigRequest.configuration.marketplace is not None)))) else (usageFile.marketplace.id if (((usageFile is not None) and ((usageFile.marketplace is not None)))) else "marketplace"))))
        product = (assetRequest.asset.product.id if (((assetRequest is not None) and ((assetRequest.asset.product is not None)))) else (listing.product.id if (((listing is not None) and ((listing.product is not None)))) else (tierConfigRequest.configuration.product.id if (((tierConfigRequest is not None) and ((tierConfigRequest.configuration.product is not None)))) else (usageFile.product.id if (((usageFile is not None) and ((usageFile.product is not None)))) else "product"))))
...
class connect_logger_PlainLoggerFormatter(connect_Base):
    ...
    def formatSection(self,level,sectionLevel,text):
        ...
        self.parseCurrentRequest(level,textStr)
    ...
    def parseCurrentRequest(self,level,text):
        textStr = Std.string(text)
        if textStr.startswith(connect_logger_PlainLoggerFormatter.REQUEST_PREFIX):
            request = HxOverrides.arrayGet(textStr.split("\""), 1)
...
class connect_models_TierConfigRequest(connect_models_IdModel):
    ...
    _hx_statics = ["list", "get", "prepareUpdateBody"]
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
    ...
    def update(self,params):
        if (params is None):
            ...
            hasModifiedFields = (len(python_Boot.fields(diff)) > 1)
            if hasModifiedFields:
                request = connect_Env.getTierApi().updateTierConfigRequest(self.id,connect_models_TierConfigRequest.prepareUpdateBody(diff))
                return connect_models_Model.parse(connect_models_TierConfigRequest,request)
            else:
                return self
        else:
    ...
    def prepareUpdateBody(diff):
        ...
        if hasTcParams:
            Reflect.setField(diff,"params",Reflect.field(Reflect.field(diff,"params"),"concat")(Reflect.field(Reflect.field(diff,"configuration"),"params")))
        if hasConfigParams:
            Reflect.setField(diff,"params",Reflect.field(Reflect.field(diff,"params"),"concat")(Reflect.field(Reflect.field(Reflect.field(diff,"configuration"),"configuration"),"params")))
        if hasConfiguration:
...
class connect_storage_StepStorage:
    ...
    __slots__ = ()
    _hx_statics = ["getStepFilename", "load", "loadRequestFromFile", "loadAllFromFile", "decodeData", "getRequestField", "loadRequestFromParam", "loadAllFromParam", "save", "deleteRequestFromFile", "deleteRequestFromObject", "encodeData", "saveInConnect", "saveInFile", "addRequestToObject", "removeStepData"]
