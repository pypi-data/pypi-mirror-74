...
class connect_Env(connect_Base):
    ...
    _hx_fields = []
    _hx_methods = []
    _hx_statics = ["initConfig", "loadConfig", "isConfigInitialized", "initLogger", "isLoggerInitialized", "initDefaultQuery", "isDefaultQueryInitialized", "getConfig", "getLogger", "getApiClient", "getFulfillmentApi", "getUsageApi", "getTierApi", "getGeneralApi", "getMarketplaceApi", "_reset", "_getDefaultQuery", "config", "logger", "defaultQuery", "apiClient", "fulfillmentApi", "usageApi", "tierApi", "generalApi", "marketplaceApi", "defaultDependencies", "dependencies", "init", "initDefaultDependencies", "createInstance"]
    _hx_interfaces = []
    _hx_super = connect_Base
    ...
    def initLogger(config):
        if (connect_Env.logger is None):
            connect_Env.logger = connect_logger_Logger(config)
        else:
            raise haxe_Exception.thrown("Logger instance is already initialized.")

    @staticmethod
    def isLoggerInitialized():
        return (connect_Env.logger is not None)
    ...
    def _reset(deps = None):
        ...
        connect_Env.dependencies = None
        connect_Env.init(deps)
    ...
    @staticmethod
    def init(deps = None):
        connect_Env.initDefaultDependencies()
        if (connect_Env.dependencies is None):
            connect_Env.dependencies = connect_util_Dictionary()
            if (deps is not None):
                keys = deps.keys()
                key = keys
                while key.hasNext():
                    key1 = key.next()
                    if connect_Env.defaultDependencies.exists(key1):
                        connect_Env.dependencies.setString(key1,deps.getString(key1))

    @staticmethod
    def initDefaultDependencies():
        if (connect_Env.defaultDependencies is None):
            connect_Env.defaultDependencies = connect_util_Dictionary().setString("IApiClient","connect.api.impl.ApiClientImpl").setString("IFulfillmentApi","connect.api.impl.FulfillmentApiImpl").setString("IUsageApi","connect.api.impl.UsageApiImpl").setString("ITierApi","connect.api.impl.TierApiImpl").setString("IGeneralApi","connect.api.impl.GeneralApiImpl").setString("IMarketplaceApi","connect.api.impl.MarketplaceApiImpl")

    @staticmethod
    def createInstance(interfaceName):
        connect_Env.init()
        className = (connect_Env.dependencies.getString(interfaceName) if (connect_Env.dependencies.exists(interfaceName)) else connect_Env.defaultDependencies.getString(interfaceName))
        classObj = Type.resolveClass(className)
        if (classObj is not None):
            return classObj(*[])
        else:
            raise haxe_Exception.thrown((("Cannot find class name \"" + ("null" if className is None else className)) + "\""))
...
class connect_Flow(connect_Base):
    ...
    __slots__ = ("filterFunc", "steps", "model", "originalModelStr", "data", "abortRequested", "abortMessage", "stepAttempt")
    _hx_fields = ["filterFunc", "steps", "model", "originalModelStr", "data", "abortRequested", "abortMessage", "stepAttempt"]
    _hx_methods = ["setup", "step", "getAssetRequest", "getListing", "getTierConfigRequest", "getUsageFile", "setData", "getData", "getDataKeys", "approveByTemplate", "approveByTile", "fail", "inquire", "pend", "_run", "getCurrentAttempt", "process", "processSetup", "loadStepDataIfStored", "getStepParam", "processStep", "prepareRequestAndOpenLogSection", "processAbortAndCloseLogSection", "abort", "logStepData", "getClassName"]
    _hx_statics = ["SKIP_MSG", "STEP_PARAM_ID", "STEP_PARAM_ID_TIER", "getExceptionMessage", "getFormattedRequest", "getFormattedData", "getDataTable"]
    _hx_interfaces = []
    _hx_super = connect_Base
    ...
    def step(self,description,func):
        _this = self.steps
        x = connect__Flow_Step(description,func)
        _this.append(x)
        return self
    ...
    def approveByTemplate(self,id):
        ...
        if (request is not None):
            connect_storage_StepStorage.removeStepData(request.id,self.getStepParam())
            request.update(None)
            request.approveByTemplate(id)
            self.abort("")
        elif (tcr is not None):
            connect_storage_StepStorage.removeStepData(tcr.id,self.getStepParam())
            tcr.update(None)
    ...
    def _run(self,_hx_list):
        _gthis = self
        connect_Env.getLogger().openSection(((("Running " + HxOverrides.stringOrNull(self.getClassName())) + " on ") + Std.string(connect_util_DateTime.now())))
        def _hx_local_5():
            def _hx_local_3(m):
                def _hx_local_2():
                    def _hx_local_1():
                        ...
                        if (Std.isOfType(_hx_local_0,connect_models_IdModel) or ((_hx_local_0 is None))):
                            ...
                            raise "Class cast error"
                        return _hx_local_0
                    return _gthis.filterFunc(_hx_local_1())
                return _hx_local_2()
            return connect_util_Collection._fromArray(list(filter(_hx_local_3,_hx_list.toArray()))) if (self.filterFunc is not None) else _hx_list
        filteredList = _hx_local_5()
        model = filteredList.iterator()
        while model.hasNext():
            model1 = model.next()
            def _hx_local_7():
                _hx_local_6 = model1
                if (Std.isOfType(_hx_local_6,connect_models_IdModel) or ((_hx_local_6 is None))):
                    _hx_local_6
                else:
                    raise "Class cast error"
                return _hx_local_6
            self.process(_hx_local_7())
    ...
    def process(self,model):
        _gthis = self
        if self.prepareRequestAndOpenLogSection(model):
            if self.processSetup():
                stepData = self.loadStepDataIfStored()
                self.stepAttempt = stepData.attempt
                _g = []
                _g1 = stepData.firstIndex
                _g2 = len(self.steps)
                while (_g1 < _g2):
                    i = _g1
                    _g1 = (_g1 + 1)
                    ...
                    _g.append(x)
                steps = _g
                def _hx_local_0(step,prev):
                    if (prev is not None):
                        return _gthis.processStep(step,prev.nextIndex,prev.lastRequestStr,prev.lastDataStr)
                    else:
                        return None
                Lambda.fold(steps,_hx_local_0,_hx_AnonObject({'nextIndex': stepData.firstIndex, 'lastRequestStr': "", 'lastDataStr': "{}"}))
            connect_Env.getLogger().closeSection()

    def processSetup(self):
        connect_Env.getLogger().openSection("Setup")
        try:
            self.setup()
        except BaseException as _g:
            None
            ...
            connect_Env.getLogger().writeCodeBlock(connect_logger_Logger.LEVEL_ERROR,exStr,"")
    ...
    def loadStepDataIfStored(self):
        stepData = connect_storage_StepStorage.load(self.model.id,self.getStepParam())
        self.data = stepData.data
        if (stepData.storage != connect_storage_StorageType.FailedStorage):
            connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,(((("Resuming request from step " + Std.string(((stepData.firstIndex + 1)))) + " with ") + Std.string(stepData.storage)) + "."))
        return stepData

    def getStepParam(self):
        assetRequest = self.getAssetRequest()
        tierConfigRequest = self.getTierConfigRequest()
        if (assetRequest is not None):
            return assetRequest.asset.getParamById(connect_Flow.STEP_PARAM_ID)
        elif (tierConfigRequest is not None):
            return tierConfigRequest.getParamById(connect_Flow.STEP_PARAM_ID_TIER)
        else:
            return None

    def processStep(self,step,index,lastRequestStr,lastDataStr):
        requestStr = connect_util_Util.beautifyObject(self.model.toObject(),connect_Env.getLogger().isCompact(),(connect_Env.getLogger().getLevel() != connect_logger_Logger.LEVEL_DEBUG))
        dataStr = Std.string(self.data)
        connect_Env.getLogger().openSection(((Std.string((index + 1)) + ". ") + HxOverrides.stringOrNull(step.description)))
        self.logStepData(connect_logger_Logger.LEVEL_INFO,requestStr,dataStr,lastRequestStr,lastDataStr)
        try:
            step.func(self)
        except BaseException as _g:
            None
            ex = haxe_Exception.caught(_g).unwrap()
            if (connect_Env.getLogger().getLevel() == connect_logger_Logger.LEVEL_ERROR):
                self.logStepData(connect_logger_Logger.LEVEL_ERROR,requestStr,dataStr,lastRequestStr,lastDataStr)
            exStr = connect_Flow.getExceptionMessage(ex)
            connect_Env.getLogger().writeCodeBlock(connect_logger_Logger.LEVEL_ERROR,exStr,"")
            if (self.getAssetRequest() is not None):
                self.getAssetRequest()._updateConversation((HxOverrides.stringOrNull(connect_Flow.SKIP_MSG) + ("null" if exStr is None else exStr)))
            self.abort()
        return self.processAbortAndCloseLogSection(index,requestStr,dataStr)

    def prepareRequestAndOpenLogSection(self,model):
        self.model = model
        self.originalModelStr = model.toString()
        assetRequest = self.getAssetRequest()
        listing = self.getListing()
        tierConfigRequest = self.getTierConfigRequest()
        usageFile = self.getUsageFile()
        provider = (assetRequest.asset.connection.provider.id if (((assetRequest is not None) and ((assetRequest.asset.connection.provider is not None)))) else (listing.provider.id if (((listing is not None) and ((listing.provider is not None)))) else (tierConfigRequest.configuration.connection.provider.id if (((tierConfigRequest is not None) and ((tierConfigRequest.configuration.connection.provider is not None)))) else (usageFile.provider.id if (((usageFile is not None) and ((usageFile.provider is not None)))) else "provider"))))
        hub = (assetRequest.asset.connection.hub.id if (((assetRequest is not None) and ((assetRequest.asset.connection.hub is not None)))) else (tierConfigRequest.configuration.connection.hub.id if (((tierConfigRequest is not None) and ((tierConfigRequest.configuration.connection.hub is not None)))) else "hub"))
        marketplace = (assetRequest.marketplace.id if (((assetRequest is not None) and ((assetRequest.marketplace is not None)))) else (listing.contract.marketplace.id if (((listing is not None) and ((listing.contract.marketplace is not None)))) else (tierConfigRequest.marketplace.id if (((tierConfigRequest is not None) and ((tierConfigRequest.marketplace is not None)))) else (usageFile.marketplace.id if (((usageFile is not None) and ((usageFile.marketplace is not None)))) else "marketplace"))))
        product = (assetRequest.asset.product.id if (((assetRequest is not None) and ((assetRequest.asset.product is not None)))) else (listing.product.id if (((listing is not None) and ((listing.product is not None)))) else (tierConfigRequest.product.id if (((tierConfigRequest is not None) and ((tierConfigRequest.product is not None)))) else (usageFile.product.id if (((usageFile is not None) and ((usageFile.product is not None)))) else "product"))))
        tierAccount = (assetRequest.asset.tiers.customer.id if (((assetRequest is not None) and ((assetRequest.asset.tiers.customer is not None)))) else (tierConfigRequest.configuration.account.id if (((tierConfigRequest is not None) and ((tierConfigRequest.configuration.account is not None)))) else "tier_account"))
        if ((assetRequest is not None) or ((tierConfigRequest is not None))):
            connect_Env.getLogger().setFilename(((((((((("" + ("null" if provider is None else provider)) + "/") + ("null" if hub is None else hub)) + "/") + ("null" if marketplace is None else marketplace)) + "/") + ("null" if product is None else product)) + "/") + ("null" if tierAccount is None else tierAccount)))
        elif ((listing is not None) or ((usageFile is not None))):
            connect_Env.getLogger().setFilename(((("usage/" + ("null" if provider is None else provider)) + "/") + ("null" if marketplace is None else marketplace)))
        connect_Env.getLogger().openSection(((("Processing request \"" + HxOverrides.stringOrNull(self.model.id)) + "\" on ") + Std.string(connect_util_DateTime.now())))
        if ((assetRequest is not None) and assetRequest.needsMigration()):
            connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,"Skipping request because it is pending migration.")
            connect_Env.getLogger().closeSection()
            return False
        else:
    ...
    def processAbortAndCloseLogSection(self,index,requestStr,dataStr):
        if self.abortRequested:
            if (self.abortMessage is None):
                connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,"Skipping request. Trying to save step data.")
                saveResult = connect_storage_StepStorage.save(self.model,connect_storage_StepData(index,self.data,connect_storage_StorageType.ConnectStorage,(self.stepAttempt + 1)),self.getStepParam(),Reflect.field(self.model,"update"))
                tmp = saveResult.index
                if (tmp == 0):
                    connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,"Step data saved in Connect.")
                elif (tmp == 1):
                    connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,"Step data saved locally.")
                elif (tmp == 2):
                    connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,"Step data could not be saved.")
                else:
                    pass
            elif (self.abortMessage != ""):
                connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,self.abortMessage)
            connect_Env.getLogger().closeSection()
            return None
        else:
            self.stepAttempt = 1
            connect_Env.getLogger().closeSection()
            return _hx_AnonObject({'nextIndex': (index + 1), 'lastRequestStr': requestStr, 'lastDataStr': dataStr})

    def abort(self,message = None):
        self.abortRequested = True
        self.abortMessage = message

    def logStepData(self,level,request,data,lastRequest,lastData):
        handler = connect_Env.getLogger().getHandlers().iterator()
        while handler.hasNext():
            handler1 = handler.next()
            _hx_list = connect_util_Collection().push(connect_Flow.getFormattedRequest(request,lastRequest,handler1.formatter)).push(connect_Flow.getFormattedData(data,lastData,self.data,handler1.formatter))
            connect_Env.getLogger()._writeToHandler(level,handler1.formatter.formatList(level,_hx_list),handler1)
    ...
    @staticmethod
    def getFormattedRequest(request,lastRequest,fmt):
        if (request != lastRequest):
            if (connect_Env.getLogger().getLevel() == connect_logger_Logger.LEVEL_DEBUG):
                ...
                requestStr = (connect_util_Util.beautifyObject(diff,connect_Env.getLogger().isCompact(),False) if ((diff is not None)) else request)
                requestTitle = ("Request (changes):" if ((diff is not None)) else "Request:")
                return (("" + ("null" if requestTitle is None else requestTitle)) + HxOverrides.stringOrNull(fmt.formatCodeBlock(connect_Env.getLogger().getLevel(),Std.string(requestStr),"json")))
            else:
    ...
    @staticmethod
    def getFormattedData(data,lastData,dataDict,fmt):
        if (data != "{}"):
            if (data != lastData):
                if (connect_Env.getLogger().getLevel() == connect_logger_Logger.LEVEL_DEBUG):
                    return ("Data:" + HxOverrides.stringOrNull(connect_Flow.getDataTable(dataDict,fmt)))
                else:
                    _g = []
                    key = dataDict.keys()
                    while key.hasNext():
    ...
    def getDataTable(data,fmt):
        ...
        return fmt.formatTable(connect_Env.getLogger().getLevel(),dataCol)
...
class connect__Flow_Step:
    _hx_class_name = "connect._Flow.Step"
    ...
    def _hx_empty_init(_hx_o):
        ...
        _hx_o.func = None
connect__Flow_Step._hx_class = connect__Flow_Step
_hx_classes["connect._Flow.Step"] = connect__Flow_Step
...
class connect_api_ConnectHelper:
    ...
    @staticmethod
    def get(resource,id = None,suffix = None,params = None,rqlParams = None):
        if (rqlParams is None):
            rqlParams = False
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("GET",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(),params,rqlParams))

    @staticmethod
    def put(resource,id,body):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("PUT",connect_api_ConnectHelper.parsePath(resource,id),connect_api_ConnectHelper.getHeaders(),None,None,body))

    @staticmethod
    def post(resource,id = None,suffix = None,body = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("POST",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(),None,None,body))

    @staticmethod
    def postFile(resource,id = None,suffix = None,fileArg = None,fileName = None,fileContents = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("POST",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(False),None,False,None,fileArg,fileName,fileContents))

    @staticmethod
    def delete(resource,id,suffix = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("DELETE",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders()))

    @staticmethod
    def connectSyncRequest(method,path,headers,params = None,rqlParams = None,data = None,fileArg = None,fileName = None,fileContent = None):
        if (rqlParams is None):
            rqlParams = False
        ...
        return connect_Env.getApiClient().syncRequest(method,url,headers,data,fileArg,fileName,fileContent,None)

    @staticmethod
    def parsePath(resource,id = None,suffix = None):
        return ((("null" if resource is None else resource) + HxOverrides.stringOrNull(((("/" + ("null" if id is None else id)) if ((id is not None)) else "")))) + HxOverrides.stringOrNull(((("/" + ("null" if suffix is None else suffix)) if ((suffix is not None)) else ""))))
...
connect_api_IFulfillmentApi._hx_class = connect_api_IFulfillmentApi
_hx_classes["connect.api.IFulfillmentApi"] = connect_api_IFulfillmentApi


class connect_api_IGeneralApi:
    _hx_class_name = "connect.api.IGeneralApi"
    __slots__ = ()
    _hx_methods = ["listAccounts", "createAccount", "getAccount", "listAccountUsers", "getAccountUser", "listConversations", "createConversation", "getConversation", "createConversationMessage", "listProducts", "getProduct", "listProductActions", "getProductAction", "getProductActionLink", "getProductConnections", "listProductItems", "getProductParameters", "getProductTemplates", "getProductVersions", "getProductVersion", "getProductVersionActions", "getProductVersionAction", "getProductVersionActionLink", "getProductVersionItems", "getProductVersionParameters", "getProductVersionTemplates", "listProductConfigurations", "setProductConfigurationParam", "listProductAgreements", "listProductMedia", "createProductMedia", "getProductMedia", "updateProductMedia", "deleteProductMedia", "listCategories", "getCategory"]
connect_api_IGeneralApi._hx_class = connect_api_IGeneralApi
_hx_classes["connect.api.IGeneralApi"] = connect_api_IGeneralApi


class connect_api_IMarketplaceApi:
    _hx_class_name = "connect.api.IMarketplaceApi"
    __slots__ = ()
...
connect_api_IMarketplaceApi._hx_class = connect_api_IMarketplaceApi
_hx_classes["connect.api.IMarketplaceApi"] = connect_api_IMarketplaceApi


class connect_api_ITierApi:
    _hx_class_name = "connect.api.ITierApi"
    __slots__ = ()
...
connect_api_ITierApi._hx_class = connect_api_ITierApi
_hx_classes["connect.api.ITierApi"] = connect_api_ITierApi


class connect_api_IUsageApi:
    _hx_class_name = "connect.api.IUsageApi"
    __slots__ = ()
...
connect_api_IUsageApi._hx_class = connect_api_IUsageApi
_hx_classes["connect.api.IUsageApi"] = connect_api_IUsageApi


class connect_api_Query(connect_Base):
    _hx_class_name = "connect.api.Query"
    __slots__ = ("in__", "out_", "limit_", "orderBy_", "offset_", "ordering_", "like_", "ilike_", "select_", "relOps")
    _hx_fields = ["in__", "out_", "limit_", "orderBy_", "offset_", "ordering_", "like_", "ilike_", "select_", "relOps"]
    _hx_methods = ["copy", "default_", "in_", "out", "limit", "orderBy", "offset", "ordering", "like", "ilike", "select", "equal", "notEqual", "greater", "greaterOrEqual", "lesser", "lesserOrEqual", "toString", "toPlain", "toObject", "toJson", "addRelOp"]
    ...
    def ordering(self,propertyList):
        self.ordering_ = list(propertyList)
        return self
...
class connect_api_impl_ApiClientImpl(connect_Base):
    ...
    def syncRequest(self,method,url,headers,body,fileArg,fileName,fileContent,certificate):
        ...
        level = (connect_logger_Logger.LEVEL_ERROR if (((response.status >= 400) or ((response.status == -1)))) else connect_logger_Logger.LEVEL_INFO)
        connect_api_impl_ApiClientImpl.logRequest(level,method,url,headers,body,response)
    ...
    @staticmethod
    def logRequest(level,method,url,headers,body,response):
        ...
        while handler.hasNext():
            ...
            connect_Env.getLogger()._writeToHandler(level,fmt.formatBlock(level,((("" + ("null" if firstMessage is None else firstMessage)) + "\n") + HxOverrides.stringOrNull(fmt.formatList(level,requestList)))),handler1)
    ...
    def maskHeaders(headers):
        masked = connect_util_Dictionary()
        key = headers.keys()
        while key.hasNext():
            key1 = key.next()
            if (key1 == "Authorization"):
                auth = Std.string(headers.get("Authorization"))
                parts = auth.split(":")
                join = None
                if (len(parts) > 1):
                    _this = parts[1:None]
                    join = ":".join([python_Boot.toString1(x1,'') for x1 in _this])
                else:
                    join = ""
                maskedAuth = ((((HxOverrides.stringOrNull((parts[0] if 0 < len(parts) else None)) + ":") + HxOverrides.stringOrNull(StringTools.lpad(HxString.substr(join,(len(join) - 4),None),"*",len(join)))) if ((len(parts) > 1)) else ("ApiKey " + HxOverrides.stringOrNull(StringTools.lpad(HxString.substr(auth,(len(auth) - 4),None),"*",(len(auth) - 7))))) if (auth.startswith("ApiKey ")) else StringTools.lpad(HxString.substr(auth,(len(auth) - 4),None),"*",len(auth)))
    ...
    def getFormattedData(data,title,fmt):
        ...
        if connect_util_Util.isJson(data):
            ...
            block = fmt.formatCodeBlock(connect_Env.getLogger().getLevel(),connect_util_Util.beautify(data,connect_Env.getLogger().isCompact(),(connect_Env.getLogger().getLevel() != connect_logger_Logger.LEVEL_DEBUG)),"json")
...
class connect_api_impl_FulfillmentApiImpl(connect_Base):
    _hx_class_name = "connect.api.impl.FulfillmentApiImpl"
    __slots__ = ()
    _hx_fields = []
    _hx_methods = ["listRequests", "getRequest", "createRequest", "updateRequest", "changeRequestStatus", "assignRequest", "renderTemplate", "listAssets", "getAsset", "getAssetRequests"]
    _hx_statics = ["REQUESTS_PATH", "TEMPLATES_PATH", "ASSETS_PATH"]
    _hx_interfaces = [connect_api_IFulfillmentApi]
    _hx_super = connect_Base
    ...
    def listRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH,None,None,filters)

    def getRequest(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH,id)

    def createRequest(self,body):
        return connect_api_ConnectHelper.post(connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH,None,None,body)

    def updateRequest(self,id,request):
        return connect_api_ConnectHelper.put(connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH,id,request)

    def changeRequestStatus(self,id,status,data):
        return connect_api_ConnectHelper.post(connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH,id,status,data)

    def assignRequest(self,id,assignee):
        return connect_api_ConnectHelper.post(connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH,id,("assign/" + ("null" if assignee is None else assignee)))

    def renderTemplate(self,id,request_id):
        return connect_api_ConnectHelper.get(connect_api_impl_FulfillmentApiImpl.TEMPLATES_PATH,id,"render",connect_api_Query().equal("request_id",request_id))

    def listAssets(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_FulfillmentApiImpl.ASSETS_PATH,None,None,filters,True)

    def getAsset(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_FulfillmentApiImpl.ASSETS_PATH,id)

    def getAssetRequests(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_FulfillmentApiImpl.ASSETS_PATH,id,"requests")

    @staticmethod
    def _hx_empty_init(_hx_o):        pass
connect_api_impl_FulfillmentApiImpl._hx_class = connect_api_impl_FulfillmentApiImpl
_hx_classes["connect.api.impl.FulfillmentApiImpl"] = connect_api_impl_FulfillmentApiImpl


class connect_api_impl_GeneralApiImpl(connect_Base):
    _hx_class_name = "connect.api.impl.GeneralApiImpl"
    __slots__ = ()
    _hx_fields = []
    _hx_methods = ["listAccounts", "createAccount", "getAccount", "listAccountUsers", "getAccountUser", "listConversations", "createConversation", "getConversation", "createConversationMessage", "listProducts", "getProduct", "listProductActions", "getProductAction", "getProductActionLink", "getProductConnections", "listProductItems", "getProductParameters", "getProductTemplates", "getProductVersions", "getProductVersion", "getProductVersionActions", "getProductVersionAction", "getProductVersionActionLink", "getProductVersionItems", "getProductVersionParameters", "getProductVersionTemplates", "listProductConfigurations", "setProductConfigurationParam", "listProductAgreements", "listProductMedia", "createProductMedia", "getProductMedia", "updateProductMedia", "deleteProductMedia", "listCategories", "getCategory"]
    _hx_statics = ["ACCOUNTS_PATH", "CONVERSATIONS_PATH", "PRODUCTS_PATH", "CATEGORIES_PATH"]
    _hx_interfaces = [connect_api_IGeneralApi]
    _hx_super = connect_Base
    ...
    def listAccounts(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.ACCOUNTS_PATH,None,None,filters)

    def createAccount(self):
        return connect_api_ConnectHelper.post(connect_api_impl_GeneralApiImpl.ACCOUNTS_PATH)

    def getAccount(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.ACCOUNTS_PATH,id)

    def listAccountUsers(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.ACCOUNTS_PATH,id,"users")

    def getAccountUser(self,id,userId):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.ACCOUNTS_PATH,id,("users/" + ("null" if userId is None else userId)))

    def listConversations(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.CONVERSATIONS_PATH,None,None,filters)

    def createConversation(self,data):
        return connect_api_ConnectHelper.post(connect_api_impl_GeneralApiImpl.CONVERSATIONS_PATH,None,None,data)

    def getConversation(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.CONVERSATIONS_PATH,id)

    def createConversationMessage(self,id,data):
        return connect_api_ConnectHelper.post(connect_api_impl_GeneralApiImpl.CONVERSATIONS_PATH,id,"messages",data)

    def listProducts(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,None,None,filters,True)

    def getProduct(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id)

    def listProductActions(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"actions",filters)

    def getProductAction(self,id,actionId):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,("actions/" + ("null" if actionId is None else actionId)))

    def getProductActionLink(self,id,actionId):
        response = python_lib_Json.loads(connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,(("actions/" + ("null" if actionId is None else actionId)) + "/actionLink")),**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon})))
        return response.link

    def getProductConnections(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"connections")

    def listProductItems(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"items",filters)

    def getProductParameters(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"parameters")

    def getProductTemplates(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"templates")

    def getProductVersions(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"versions")

    def getProductVersion(self,id,version):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,("versions/" + Std.string(version)))

    def getProductVersionActions(self,id,version):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,(("versions/" + Std.string(version)) + "/actions"))

    def getProductVersionAction(self,id,version,actionId):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,((("versions/" + Std.string(version)) + "/actions/") + ("null" if actionId is None else actionId)))

    def getProductVersionActionLink(self,id,version,actionId):
        response = python_lib_Json.loads(connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,(((("versions/" + Std.string(version)) + "/actions/") + ("null" if actionId is None else actionId)) + "/actionLink")),**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon})))
        return response.link

    def getProductVersionItems(self,id,version):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,(("versions/" + Std.string(version)) + "/items"))

    def getProductVersionParameters(self,id,version):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,(("versions/" + Std.string(version)) + "/parameters"))

    def getProductVersionTemplates(self,id,version):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,(("versions/" + Std.string(version)) + "/templates"))

    def listProductConfigurations(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"configurations",filters)

    def setProductConfigurationParam(self,id,param):
        return connect_api_ConnectHelper.post(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"configurations",param)

    def listProductAgreements(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"agreements",filters)

    def listProductMedia(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"media",filters)

    def createProductMedia(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,"media")

    def getProductMedia(self,id,mediaId):
        return connect_api_ConnectHelper.post(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,("media/" + ("null" if mediaId is None else mediaId)))

    def updateProductMedia(self,id,mediaId,media):
        return connect_api_ConnectHelper.put(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,((("" + ("null" if id is None else id)) + "/media/") + ("null" if mediaId is None else mediaId)),media)

    def deleteProductMedia(self,id,mediaId):
        return connect_api_ConnectHelper.delete(connect_api_impl_GeneralApiImpl.PRODUCTS_PATH,id,("media/" + ("null" if mediaId is None else mediaId)))

    def listCategories(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.CATEGORIES_PATH,None,None,filters)

    def getCategory(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_GeneralApiImpl.CATEGORIES_PATH,id)

    @staticmethod
    def _hx_empty_init(_hx_o):        pass
connect_api_impl_GeneralApiImpl._hx_class = connect_api_impl_GeneralApiImpl
_hx_classes["connect.api.impl.GeneralApiImpl"] = connect_api_impl_GeneralApiImpl


class connect_api_impl_MarketplaceApiImpl:
    _hx_class_name = "connect.api.impl.MarketplaceApiImpl"
    __slots__ = ()
    _hx_methods = ["listAgreements", "createAgreement", "getAgreement", "updateAgreement", "removeAgreement", "listAgreementVersions", "newAgreementVersion", "getAgreementVersion", "removeAgreementVersion", "listAgreementSubAgreements", "createAgreementSubAgreement", "listListings", "getListing", "putListing", "listListingRequests", "getListingRequest", "createListingRequest", "assignListingRequest", "unassignListingRequest", "changeListingRequestToDraft", "changeListingRequestToDeploying", "changeListingRequestToCompleted", "changeListingRequestToCanceled", "changeListingRequestToReviewing", "listMarketplaces", "createMarketplace", "getMarketplace", "updateMarketplace", "setMarketplaceIcon", "deleteMarketplace"]
    _hx_statics = ["AGREEMENTS_PATH", "LISTINGS_PATH", "LISTINGREQUESTS_PATH", "MARKETPLACES_PATH"]
    _hx_interfaces = [connect_api_IMarketplaceApi]

    def listAgreements(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,None,None,filters)

    def createAgreement(self,body):
        return connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,None,None,body)

    def getAgreement(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id)

    def updateAgreement(self,id,body):
        return connect_api_ConnectHelper.put(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,body)

    def removeAgreement(self,id):
        connect_api_ConnectHelper.delete(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id)

    def listAgreementVersions(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,"versions")

    def newAgreementVersion(self,id,body):
        return connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,"versions",body)

    def getAgreementVersion(self,id,version):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,("versions/" + ("null" if version is None else version)))

    def removeAgreementVersion(self,id,version):
        connect_api_ConnectHelper.delete(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,("versions/" + ("null" if version is None else version)))

    def listAgreementSubAgreements(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH)

    def createAgreementSubAgreement(self,id,body):
        return connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,id,connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH,body)

    def listListings(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.LISTINGS_PATH,None,None,filters)

    def getListing(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.LISTINGS_PATH,id)

    def putListing(self,id,body):
        return connect_api_ConnectHelper.put(connect_api_impl_MarketplaceApiImpl.LISTINGS_PATH,id,body)

    def listListingRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,None,None,filters)

    def getListingRequest(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id)

    def createListingRequest(self,body):
        return connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,None,None,body)

    def assignListingRequest(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"assign")

    def unassignListingRequest(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"unassign")

    def changeListingRequestToDraft(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"refine")

    def changeListingRequestToDeploying(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"deploy")

    def changeListingRequestToCompleted(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"complete")

    def changeListingRequestToCanceled(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"cancel")

    def changeListingRequestToReviewing(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH,id,"submit")

    def listMarketplaces(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH,None,None,filters)

    def createMarketplace(self,body):
        return connect_api_ConnectHelper.post(connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH,None,None,body)

    def getMarketplace(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH,id)

    def updateMarketplace(self,id,body):
        return connect_api_ConnectHelper.put(connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH,id,body)

    def setMarketplaceIcon(self,id,data):
        connect_api_ConnectHelper.postFile(connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH,id,"icon","name","marketplace.png",data)

    def deleteMarketplace(self,id):
        connect_api_ConnectHelper.delete(connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH,id)

    @staticmethod
    def _hx_empty_init(_hx_o):        pass
connect_api_impl_MarketplaceApiImpl._hx_class = connect_api_impl_MarketplaceApiImpl
_hx_classes["connect.api.impl.MarketplaceApiImpl"] = connect_api_impl_MarketplaceApiImpl


class connect_api_impl_TierApiImpl(connect_Base):
    _hx_class_name = "connect.api.impl.TierApiImpl"
    __slots__ = ()
    _hx_fields = []
    _hx_methods = ["listTierConfigRequests", "createTierConfigRequest", "getTierConfigRequest", "updateTierConfigRequest", "pendTierConfigRequest", "inquireTierConfigRequest", "approveTierConfigRequest", "failTierConfigRequest", "assignTierConfigRequest", "unassignTierConfigRequest", "listTierAccounts", "getTierAccount", "listTierConfigs", "getTierConfig"]
    _hx_statics = ["TCR_PATH", "TA_PATH", "TC_PATH"]
    _hx_interfaces = [connect_api_ITierApi]
    _hx_super = connect_Base
    ...
    def listTierConfigRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TCR_PATH,None,None,filters,True)

    def createTierConfigRequest(self,body):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,None,None,body)

    def getTierConfigRequest(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TCR_PATH,id)

    def updateTierConfigRequest(self,id,tcr):
        return connect_api_ConnectHelper.put(connect_api_impl_TierApiImpl.TCR_PATH,id,tcr)

    def pendTierConfigRequest(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"pend")

    def inquireTierConfigRequest(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"inquire")

    def approveTierConfigRequest(self,id,data):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"approve",data)

    def failTierConfigRequest(self,id,data):
        connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"fail",data)

    def assignTierConfigRequest(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"assign")

    def unassignTierConfigRequest(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"unassign")

    def listTierAccounts(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TA_PATH,None,None,filters)

    def getTierAccount(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TA_PATH,id)

    def listTierConfigs(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TC_PATH,None,None,filters,True)

    def getTierConfig(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TC_PATH,id)

    @staticmethod
    def _hx_empty_init(_hx_o):        pass
connect_api_impl_TierApiImpl._hx_class = connect_api_impl_TierApiImpl
_hx_classes["connect.api.impl.TierApiImpl"] = connect_api_impl_TierApiImpl


class connect_api_impl_UsageApiImpl(connect_Base):
    _hx_class_name = "connect.api.impl.UsageApiImpl"
    __slots__ = ()
    _hx_fields = []
    _hx_methods = ["listUsageFiles", "createUsageFile", "getUsageFile", "updateUsageFile", "deleteUsageFile", "uploadUsageFile", "submitUsageFileAction", "acceptUsageFileAction", "rejectUsageFileAction", "closeUsageFileAction", "getProductSpecificUsageFileTemplate", "uploadReconciliationFileFromProvider", "reprocessProcessedFile", "listUsageRecords", "getUsageRecord", "updateUsageRecord", "closeUsageRecord"]
    _hx_statics = ["USAGE_FILES_PATH", "USAGE_PRODUCTS_PATH", "USAGE_RECORDS_PATH"]
    _hx_interfaces = [connect_api_IUsageApi]
    _hx_super = connect_Base
    ...
    def listUsageFiles(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,None,None,filters)

    def createUsageFile(self,body):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,None,None,body)

    def getUsageFile(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id)

    def updateUsageFile(self,id,body):
        return connect_api_ConnectHelper.put(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,body)

    def deleteUsageFile(self,id):
        connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"delete")

    def uploadUsageFile(self,id,file):
        return connect_api_ConnectHelper.postFile(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"upload","usage_file","usage_file.xlsx",file)

    def submitUsageFileAction(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"submit")

    def acceptUsageFileAction(self,id,note):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"accept",haxe_format_JsonPrinter.print(_hx_AnonObject({'acceptance_note': note}),None,None))

    def rejectUsageFileAction(self,id,note):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"reject",haxe_format_JsonPrinter.print(_hx_AnonObject({'rejection_note': note}),None,None))

    def closeUsageFileAction(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"close")

    def getProductSpecificUsageFileTemplate(self,productId):
        return connect_api_ConnectHelper.get(connect_api_impl_UsageApiImpl.USAGE_PRODUCTS_PATH,productId,"template")

    def uploadReconciliationFileFromProvider(self,id,file):
        return connect_api_ConnectHelper.postFile(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"reconciliation","reconciliation_file","reconciliation_file.xlsx",file)

    def reprocessProcessedFile(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_FILES_PATH,id,"reprocess")

    def listUsageRecords(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_UsageApiImpl.USAGE_RECORDS_PATH,None,None,filters)

    def getUsageRecord(self,id):
        return connect_api_ConnectHelper.get(connect_api_impl_UsageApiImpl.USAGE_RECORDS_PATH,id)

    def updateUsageRecord(self,id,record):
        return connect_api_ConnectHelper.put(connect_api_impl_UsageApiImpl.USAGE_RECORDS_PATH,id,record)

    def closeUsageRecord(self,id,record):
        return connect_api_ConnectHelper.post(connect_api_impl_UsageApiImpl.USAGE_RECORDS_PATH,id,"close",record)

    @staticmethod
    def _hx_empty_init(_hx_o):        pass
connect_api_impl_UsageApiImpl._hx_class = connect_api_impl_UsageApiImpl
_hx_classes["connect.api.impl.UsageApiImpl"] = connect_api_impl_UsageApiImpl


class connect_logger_ILoggerWriter:
    _hx_class_name = "connect.logger.ILoggerWriter"
    __slots__ = ()
    _hx_methods = ["setFilename", "getFilename", "writeLine"]
...
class connect_logger_FileLoggerWriter(connect_Base):
    ...
    _hx_methods = ["setFilename", "getFilename", "writeLine", "getFile"]
    _hx_statics = []
    ...
    def writeLine(self,line):
        if (self.getFile() is not None):
            self.getFile().writeString((("null" if line is None else line) + "\n"))
            self.getFile().flush()
        try:
            _hx_str = Std.string(line)
            python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        except BaseException as _g:
            None
...
class connect_logger_ILoggerFormatter:
    ...
    __slots__ = ()
    _hx_methods = ["formatSection", "formatBlock", "formatCodeBlock", "formatList", "formatTable", "formatLine", "getFileExtension"]
...
class connect_logger_Logger(connect_Base):
    ...
    __slots__ = ("path", "level", "handlers", "sections", "maskedFields", "regexMaskingList", "compact")
    _hx_fields = ["path", "level", "handlers", "sections", "maskedFields", "regexMaskingList", "compact"]
    _hx_methods = ["getPath", "getLevel", "isCompact", "setFilename", "getFilename", "getHandlers", "openSection", "closeSection", "writeBlock", "writeCodeBlock", "writeList", "writeTable", "write", "getMaskedFields", "getRegExMaskingList", "log", "debug", "info", "notice", "warning", "error", "critical", "alert", "emergency", "_writeToHandler", "writeSections"]
    ...
    def __init__(self,config):
        ...
        self.sections = []
        self.maskedFields = config.maskedFields_
        self.regexMaskingList = config.regexMaskingList_
        if (self.maskedFields.indexOf("apiKey") == -1):
            self.maskedFields.push("apiKey")
    ...
    def setFilename(self,filename):
        fullname = ((HxOverrides.stringOrNull(self.path) + ("null" if filename is None else filename)) if (((self.path is not None) and ((filename is not None)))) else None)
        def _hx_local_0(o,last):
            if last:
                return o.writer.setFilename(((("" + ("null" if fullname is None else fullname)) + ".") + HxOverrides.stringOrNull(o.formatter.getFileExtension())))
            else:
                return False
    ...
    def getFilename(self):
        firstWriter = (self.handlers.get(0).writer if ((self.handlers.length() > 0)) else None)
        if (firstWriter is not None):
            filename = firstWriter.getFilename()
            fixedFilename = None
            if (filename is not None):
                _hx_str = self.path
                startIndex = None
                fixedFilename = (((filename.find(_hx_str) if ((startIndex is None)) else HxString.indexOfImpl(filename,_hx_str,startIndex))) == 0)
            else:
                fixedFilename = False
            fixedFilename1 = (HxString.substr(filename,len(filename),None) if fixedFilename else filename)
            return fixedFilename1
        else:
            return None
...
class connect_logger_LoggerConfig(connect_Base):
    ...
    __slots__ = ("path_", "level_", "handlers_", "maskedFields_", "compact_", "regexMaskingList_", "customHandlers")
    _hx_fields = ["path_", "level_", "handlers_", "maskedFields_", "compact_", "regexMaskingList_", "customHandlers"]
    _hx_methods = ["path", "level", "levelName", "handlers", "maskedFields", "compact", "regexMasks", "markdown"]
...
class connect_logger_MarkdownLoggerFormatter(connect_Base):
    ...
    def formatSection(self,level,sectionLevel,text):
        ...
        return ((("\n" + ("null" if prefix is None else prefix)) + ("null" if text is None else text)) + "\n")
    ...
    def formatCodeBlock(self,level,text,language):
        header = (("\n```" + ("null" if language is None else language)) + "\n")
        footer = "\n```\n"
        return ((("null" if header is None else header) + ("null" if text is None else text)) + ("null" if footer is None else footer))
    ...
    def formatList(self,level,_hx_list):
        if (_hx_list.length() > 0):
            ...
            while line.hasNext():
                line1 = line.next()
                _g.append(("* " + ("null" if line1 is None else line1)))
            formatted = _g
...
class connect_logger_PlainLoggerFormatter(connect_Base):
    ...
    _hx_methods = ["formatSection", "parseCurrentRequest", "formatPrefix", "formatBlock", "getPrefixedLines", "getUnprefixedLines", "formatCodeBlock", "formatList", "formatTable", "formatLine", "getFileExtension"]
    _hx_statics = ["NO_REQUEST", "REQUEST_PREFIX", "formatDate", "formatLevel", "removePrefix", "isPrefixed"]
    _hx_interfaces = [connect_logger_ILoggerFormatter]
    _hx_super = connect_Base
    ...
    def formatSection(self,level,sectionLevel,text):
        hashes = StringTools.rpad("","#",sectionLevel)
        prefix = ((("null" if hashes is None else hashes) + " ") if ((hashes != "")) else "")
        self.parseCurrentRequest(level,text)
        return ((("" + HxOverrides.stringOrNull(self.formatPrefix(level))) + ("null" if prefix is None else prefix)) + ("null" if text is None else text))

    def parseCurrentRequest(self,level,text):
        if text.startswith(connect_logger_PlainLoggerFormatter.REQUEST_PREFIX):
            request = HxOverrides.arrayGet(text.split("\""), 1)
            self.currentRequest = (request if ((request != "")) else connect_logger_PlainLoggerFormatter.NO_REQUEST)
    ...
    @staticmethod
    def isPrefixed(text):
        datePrefix = HxOverrides.arrayGet(text.split(" "), 0)
                        ...
                        if (_g3 == str):
                            ...
                            if (((className.find("connect.util.Collection") if ((startIndex is None)) else HxString.indexOfImpl(className,"connect.util.Collection",startIndex))) == 0):
                                ...
                                if (len(arr) > 0):
                                    ...
                                    setattr(obj,(("_hx_" + field3) if ((field3 in python_Boot.keywords)) else (("_hx_" + field3) if (((((len(field3) > 2) and ((ord(field3[0]) == 95))) and ((ord(field3[1]) == 95))) and ((ord(field3[(len(field3) - 1)]) != 95)))) else field3)),arr)
                            else:
                                startIndex2 = None
                                if (((className.find("connect.models.") if ((startIndex2 is None)) else HxString.indexOfImpl(className,"connect.models.",startIndex2))) == 0):
                                    ...
                                    if (len(python_Boot.fields(model)) != 0):
                                        ...
                                        setattr(obj,(("_hx_" + field4) if ((field4 in python_Boot.keywords)) else (("_hx_" + field4) if (((((len(field4) > 2) and ((ord(field4[0]) == 95))) and ((ord(field4[1]) == 95))) and ((ord(field4[(len(field4) - 1)]) != 95)))) else field4)),model)
                                else:
                                    field5 = connect_Inflection.toSnakeCase(field)
                                    setattr(obj,(("_hx_" + field5) if ((field5 in python_Boot.keywords)) else (("_hx_" + field5) if (((((len(field5) > 2) and ((ord(field5[0]) == 95))) and ((ord(field5[1]) == 95))) and ((ord(field5[(len(field5) - 1)]) != 95)))) else field5)),value)
                    else:
            ...
            if python_Boot.hasField(obj,snakeField):
                ...
                if (tmp == 4):
                    ...
                    if (_g2.params[0] == list):
                        ...
                        value2 = connect_models_Model._parseArray(classObj1,val)
                        ...
                        if (fieldClassName2 == "DateTime"):
                            value3 = connect_util_DateTime.fromString(val)
...
class connect_models_AssetRequest(connect_models_IdModel):
    ...
    _hx_methods = ["register", "update", "approveByTemplate", "approveByTile", "fail", "inquire", "pend", "assign", "needsMigration", "getConversation", "_updateConversation"]
    _hx_statics = ["list", "get"]
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
    ...
    def update(self,params):
        if (params is None):
            ...
            hasModifiedFields = (len(python_Boot.fields(diff)) > 1)
            if hasModifiedFields:
                request = connect_Env.getFulfillmentApi().updateRequest(self.id,haxe_format_JsonPrinter.print(diff,None,None))
                return connect_models_Model.parse(connect_models_AssetRequest,request)
            else:
                return self
        else:
    ...
    def pend(self):
        request = connect_Env.getFulfillmentApi().changeRequestStatus(self.id,"pend",haxe_format_JsonPrinter.print(_hx_AnonObject({}),None,None))
...
class connect_models_Item(connect_models_IdModel):
    ...
    __slots__ = ("mpn", "quantity", "oldQuantity", "renewal", "params", "status", "displayName", "globalId", "itemType", "period", "type", "name", "description")
    _hx_fields = ["mpn", "quantity", "oldQuantity", "renewal", "params", "status", "displayName", "globalId", "itemType", "period", "type", "name", "description"]
    _hx_methods = ["getParamById"]
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
...
class connect_models_Marketplace(connect_models_IdModel):
    ...
    __slots__ = ("name", "description", "activeContracts", "icon", "owner", "hubs", "zone", "countries", "sourcing")
    _hx_fields = ["name", "description", "activeContracts", "icon", "owner", "hubs", "zone", "countries", "sourcing"]
...
class connect_models_Param(connect_models_IdModel):
    ...
    __slots__ = ("name", "title", "description", "type", "hint", "placeholder", "value", "valueError", "valueChoice", "constraints", "shared", "valueChoices", "events", "scope", "phase", "marketplace")
    _hx_fields = ["name", "title", "description", "type", "hint", "placeholder", "value", "valueError", "valueChoice", "constraints", "shared", "valueChoices", "events", "scope", "phase", "marketplace"]
    _hx_methods = []
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
...
class connect_models_Product(connect_models_IdModel):
    ...
    _hx_methods = ["listActions", "getAction", "getActionLink", "getConnections", "listItems", "getItems", "getParameters", "getTemplates", "getVersions", "getVersion", "getVersionActions", "getVersionAction", "getVersionActionLink", "getVersionItems", "getVersionParameters", "getVersionTemplates", "listConfigurations", "setConfigurationParam", "listAgreements", "listMedia", "createMedia", "getMedia", "updateMedia", "deleteMedia"]
    _hx_statics = ["list", "get"]
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
    ...
    def getParameters(self):
        try:
            params = connect_Env.getGeneralApi().getProductParameters(self.id)
...
class connect_models_TierAccount(connect_models_IdModel):
    ...
    __slots__ = ("externalId", "name", "environment", "scopes", "contactInfo", "marketplace", "hub", "externalUid")
    _hx_fields = ["externalId", "name", "environment", "scopes", "contactInfo", "marketplace", "hub", "externalUid"]
...
class connect_models_TierConfigRequest(connect_models_IdModel):
    ...
    _hx_methods = ["register", "update", "approveByTemplate", "approveByTile", "fail", "inquire", "pend", "assign", "unassign", "getParamById"]
    _hx_statics = ["list", "get"]
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
    ...
    def update(self,params):
        if (params is None):
            ...
            hasModifiedFields = (len(python_Boot.fields(diff)) > 1)
            if hasModifiedFields:
                request = connect_Env.getTierApi().updateTierConfigRequest(self.id,haxe_format_JsonPrinter.print(self._toDiff(),None,None))
                return connect_models_Model.parse(connect_models_TierConfigRequest,request)
            else:
                return self
        else:
    ...
    def approveByTemplate(self,id):
        connect_Env.getTierApi().approveTierConfigRequest(self.id,haxe_format_JsonPrinter.print(_hx_AnonObject({'template_id': id}),None,None))

    def approveByTile(self,text):
        connect_Env.getTierApi().approveTierConfigRequest(self.id,haxe_format_JsonPrinter.print(_hx_AnonObject({'activation_tile': text}),None,None))
...
class connect_models_UsageFile(connect_models_IdModel):
    ...
    _hx_methods = ["register", "update", "delete", "uploadRecords", "upload", "submit", "accept", "reject", "close", "getTemplate", "getTemplateLink", "uploadReconciliation", "reprocess"]
    _hx_statics = ["list", "get", "APP", "CORE", "RELS1", "RELS2", "THEME", "CONTENT_TYPES", "STYLES", "WORKBOOK", "createSpreadsheet", "zipSheet", "parseSheet", "parseSheetStrings", "zipEntry", "getStrings", "getRowNames"]
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
    ...
    def uploadRecords(self,records):
        sheet = connect_models_UsageFile.createSpreadsheet(records.toArray(),self.id)
    ...
    @staticmethod
    def createSpreadsheet(records,fileId):
        sheet = []
        sheet.append(["s:record_id", "s:record_note", "s:item_search_criteria", "s:item_search_value", "s:amount", "s:quantity", "s:start_time_utc", "s:end_time_utc", "s:asset_search_criteria", "s:asset_search_value", "s:item_name", "s:item_mpn", "s:item_precision", "s:item_unit", "s:category_id", "s:asset_recon_id", "s:tier"])
        _g = 0
        _g1 = len(records)
        while (_g < _g1):
            i = _g
            ...
            x = (record.recordId if ((record.recordId is not None)) else ((("" + ("null" if fileId is None else fileId)) + "_record_") + Std.string(i)))
            x1 = (record.recordNote if ((record.recordNote is not None)) else "")
            x2 = (record.itemSearchCriteria if ((record.itemSearchCriteria is not None)) else "")
            x3 = (record.itemSearchValue if ((record.itemSearchValue is not None)) else "")
            x4 = (Std.string(record.amount) if ((record.amount is not None)) else "0")
            x5 = (Std.string(record.quantity) if ((record.quantity is not None)) else "0")
            x6 = None
            if (record.startTimeUtc is not None):
                _this = StringTools.replace(record.startTimeUtc.toString(),"T"," ")
                x6 = python_internal_ArrayImpl._get(_this.split("+"), 0)
            else:
                x6 = ""
            x7 = None
            if (record.endTimeUtc is not None):
                _this1 = StringTools.replace(record.endTimeUtc.toString(),"T"," ")
                x7 = python_internal_ArrayImpl._get(_this1.split("+"), 0)
            else:
                x7 = ""
            x8 = (record.assetSearchCriteria if ((record.assetSearchCriteria is not None)) else "")
            x9 = (record.assetSearchValue if ((record.assetSearchValue is not None)) else "")
            x10 = (record.itemName if ((record.itemName is not None)) else "")
            x11 = (record.itemMpn if ((record.itemMpn is not None)) else "")
            x12 = (record.itemPrecision if ((record.itemPrecision is not None)) else "")
            x13 = (record.itemUnit if ((record.itemUnit is not None)) else "")
            x14 = (record.categoryId if ((record.categoryId is not None)) else "generic_category")
            x15 = (record.assetReconId if ((record.assetReconId is not None)) else "")
            x16 = (Std.string(record.tier) if ((("s:" + Std.string(record.tier)) is not None)) else "")
            sheet.append([("s:" + ("null" if x is None else x)), ("s:" + ("null" if x1 is None else x1)), ("s:" + ("null" if x2 is None else x2)), ("s:" + ("null" if x3 is None else x3)), ("n:" + ("null" if x4 is None else x4)), ("n:" + ("null" if x5 is None else x5)), ("s:" + ("null" if x6 is None else x6)), ("s:" + ("null" if x7 is None else x7)), ("s:" + ("null" if x8 is None else x8)), ("s:" + ("null" if x9 is None else x9)), ("s:" + ("null" if x10 is None else x10)), ("s:" + ("null" if x11 is None else x11)), ("s:" + ("null" if x12 is None else x12)), ("s:" + ("null" if x13 is None else x13)), ("s:" + ("null" if x14 is None else x14)), ("s:" + ("null" if x15 is None else x15)), x16])
        return connect_models_UsageFile.zipSheet("usage_records",sheet)

    @staticmethod
    def zipSheet(name,sheet):
        entries = haxe_ds_List()
        entries.add(connect_models_UsageFile.zipEntry("_rels/.rels",connect_models_UsageFile.RELS1))
        entries.add(connect_models_UsageFile.zipEntry("docProps/app.xml",connect_models_UsageFile.APP))
        entries.add(connect_models_UsageFile.zipEntry("docProps/core.xml",StringTools.replace(connect_models_UsageFile.CORE,"%DATE%",connect_util_DateTime.now().toString())))
        entries.add(connect_models_UsageFile.zipEntry("xl/theme/theme1.xml",connect_models_UsageFile.THEME))
        entries.add(connect_models_UsageFile.zipEntry("xl/worksheets/sheet1.xml",connect_models_UsageFile.parseSheet(sheet)))
        entries.add(connect_models_UsageFile.zipEntry("xl/sharedStrings.xml",connect_models_UsageFile.parseSheetStrings(sheet)))
        entries.add(connect_models_UsageFile.zipEntry("xl/styles.xml",connect_models_UsageFile.STYLES))
        entries.add(connect_models_UsageFile.zipEntry("xl/workbook.xml",StringTools.replace(connect_models_UsageFile.WORKBOOK,"%NAME%",name)))
        entries.add(connect_models_UsageFile.zipEntry("xl/_rels/workbook.xml.rels",connect_models_UsageFile.RELS2))
        entries.add(connect_models_UsageFile.zipEntry("[Content_Types].xml",connect_models_UsageFile.CONTENT_TYPES))
    ...
    def parseSheet(sheet):
        strings = connect_models_UsageFile.getStrings(sheet)
        firstRowNames = connect_models_UsageFile.getRowNames((sheet[0] if 0 < len(sheet) else None),1)
        lastRowNames = connect_models_UsageFile.getRowNames(python_internal_ArrayImpl._get(sheet, (len(sheet) - 1)),len(sheet))
        buf_b = python_lib_io_StringIO()
        ...
        while (_g < _g1):
            r = _g
            _g = (_g + 1)
            rowNames = connect_models_UsageFile.getRowNames((sheet[r] if r >= 0 and r < len(sheet) else None),(r + 1))
    ...
    @staticmethod
    def getStrings(sheet):
        _g = []
        _g1 = 0
        while (_g1 < len(sheet)):
            row = (sheet[_g1] if _g1 >= 0 and _g1 < len(sheet) else None)
            _g1 = (_g1 + 1)
            _g2 = 0
            while (_g2 < len(row)):
...
class connect_models_UsageRecord(connect_models_Model):
    ...
    __slots__ = ("recordId", "recordNote", "itemSearchCriteria", "itemSearchValue", "amount", "quantity", "startTimeUtc", "endTimeUtc", "assetSearchCriteria", "assetSearchValue", "itemName", "itemMpn", "itemUnit", "itemPrecision", "categoryId", "assetReconId", "tier")
    _hx_fields = ["recordId", "recordNote", "itemSearchCriteria", "itemSearchValue", "amount", "quantity", "startTimeUtc", "endTimeUtc", "assetSearchCriteria", "assetSearchValue", "itemName", "itemMpn", "itemUnit", "itemPrecision", "categoryId", "assetReconId", "tier"]
    _hx_methods = []
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_Model
...
class connect_storage_StepData:
    ...
    def __init__(self,firstIndex,data,storage,attempt):
        ...
        if Std.isOfType(data,connect_util_Dictionary):
            def _hx_local_1():
                _hx_local_0 = data
                if (Std.isOfType(_hx_local_0,connect_util_Dictionary) or ((_hx_local_0 is None))):
                    _hx_local_0
                else:
                    raise "Class cast error"
                return _hx_local_0
            key = (_hx_local_1()).keys()
            while key.hasNext():
                key1 = key.next()
                value = Reflect.field(data,"get")(key1)
                className = (Type.getClassName(Type.getClass(value)) if (Std.isOfType(value,connect_models_Model)) else "")
                self.data.set(((("" + ("null" if key1 is None else key1)) + "::") + ("null" if className is None else className)),value)
        else:
            _g = 0
            _g1 = python_Boot.fields(data)
            while (_g < len(_g1)):
                field = (_g1[_g] if _g >= 0 and _g < len(_g1) else None)
                _g = (_g + 1)
                ...
                fieldClass = (Type.resolveClass(fieldClassName) if ((fieldClassName != "")) else None)
                value = Reflect.field(data,field)
                parsedValue = (connect_models_Model.parse(fieldClass,haxe_format_JsonPrinter.print(value,None,None)) if ((fieldClass is not None)) else value)
                self.data.set(fieldName,parsedValue)
...
class connect_storage_StepStorage:
    ...
    __slots__ = ()
    _hx_statics = ["load", "save", "removeStepData", "removeStepDataFromFile", "objWithRequestData", "objWithoutRequestData", "loadRequestFromParam", "loadRequestFromFile", "loadAllFromParam", "loadAllFromFile", "saveInConnect", "saveInFile", "getRequestField", "getDataFilename", "encodeData", "decodeData"]
    ...
    def save(request,stepData,param,updateFunc):
        connect_storage_StepStorage.removeStepDataFromFile(request.id)
        paramObj = connect_storage_StepStorage.objWithRequestData(connect_storage_StepStorage.loadAllFromParam(param),request.id,stepData)
        fileObj = connect_storage_StepStorage.objWithRequestData(connect_storage_StepStorage.loadAllFromFile(),request.id,stepData)
    ...
    def removeStepData(requestId,param):
        connect_storage_StepStorage.removeStepDataFromFile(requestId)
        ...
        if ((paramObj is not None) and python_Boot.hasField(paramObj,requestId)):
            param.value = connect_storage_StepStorage.encodeData(connect_storage_StepStorage.objWithoutRequestData(paramObj,requestId))
            return True
        return False

    @staticmethod
    def removeStepDataFromFile(requestId):
        fileObj = connect_storage_StepStorage.loadAllFromFile()
        if ((fileObj is not None) and python_Boot.hasField(fileObj,requestId)):
            fileData = connect_storage_StepStorage.objWithoutRequestData(fileObj,requestId)
            connect_storage_StepStorage.saveInFile(connect_storage_StepStorage.encodeData(fileData))

    @staticmethod
    def objWithRequestData(obj,requestId,stepData):
        fixedObj = (obj if ((obj is not None)) else _hx_AnonObject({}))
        value = _hx_AnonObject({'current_step': stepData.firstIndex, 'data': stepData.data.toObject(), 'attempt': stepData.attempt})
        setattr(fixedObj,(("_hx_" + requestId) if ((requestId in python_Boot.keywords)) else (("_hx_" + requestId) if (((((len(requestId) > 2) and ((ord(requestId[0]) == 95))) and ((ord(requestId[1]) == 95))) and ((ord(requestId[(len(requestId) - 1)]) != 95)))) else requestId)),value)
        return fixedObj

    @staticmethod
    def objWithoutRequestData(obj,requestId):
        fixedObj = (obj if ((obj is not None)) else _hx_AnonObject({}))
    ...
    @staticmethod
    def loadAllFromFile():
        dataFilename = connect_storage_StepStorage.getDataFilename()
        if (sys_FileSystem.exists(dataFilename) and (not sys_FileSystem.isDirectory(dataFilename))):
            return connect_storage_StepStorage.decodeData(sys_io_File.getContent(dataFilename))
        return None
    ...
    def saveInFile(data):
        dataFilename = connect_storage_StepStorage.getDataFilename()
        try:
            sys_io_File.saveContent(dataFilename,data)
            return True
        except BaseException as _g:
            None
            return False
    ...
    def getRequestField(object,requestId,storage):
        if python_Boot.hasField(object,requestId):
            ...
            return connect_storage_StepData(stepData.current_step,stepData.data,storage,stepData.attempt)
        return None
...
class connect_util_Blob(connect_Base):
    ...
    __slots__ = ("bytes",)
    _hx_fields = ["bytes"]
    _hx_methods = ["length", "toString", "_getBytes", "_toArray"]
...
class connect_util_Dictionary(connect_Base):
    ...
    __slots__ = ("map",)
    _hx_fields = ["map"]
    _hx_methods = ["clear", "copy", "exists", "get", "getBool", "getInt", "getFloat", "getString", "iterator", "keys", "remove", "set", "setBool", "setInt", "setFloat", "setString", "toString", "toObject"]
...
class connect_util_Diff:
    ...
    _hx_statics = ["applyArray", "swapArray", "parse", "parseArray", "compareArrays", "areEqual", "isSupported", "checkStructs", "mapToObject", "changeArrayToObject"]
    ...
    def __init__(self,first,second):
        ...
        def _hx_local_6(f):
            ...
            if (connect_util_Util.isStruct(a) and connect_util_Util.isStruct(b)):
                _this = _gthis.c
                value = connect_util_Diff(a,b)
                _this.h[f] = value
            elif (connect_util_Util.isArray(a) and connect_util_Util.isArray(b)):
                _this = _gthis.c
        ...
        def _hx_local_2(k):
            ...
            if Std.isOfType(change,list):
                if (Reflect.field(change,"length") == 2):
                    setattr(out,(("_hx_" + k) if ((k in python_Boot.keywords)) else (("_hx_" + k) if (((((len(k) > 2) and ((ord(k[0]) == 95))) and ((ord(k[1]) == 95))) and ((ord(k[(len(k) - 1)]) != 95)))) else k)),HxOverrides.arrayGet(change, 1))
                else:
                ...
                setattr(out,(("_hx_" + k) if ((k in python_Boot.keywords)) else (("_hx_" + k) if (((((len(k) > 2) and ((ord(k[0]) == 95))) and ((ord(k[1]) == 95))) and ((ord(k[(len(k) - 1)]) != 95)))) else k)),value)
        Lambda.iter(changedKeys,_hx_local_2)
        return out
    ...
    def swap(self):
        ...
        def _hx_local_0(k):
            ...
            if Std.isOfType(change,list):
                if (Reflect.field(change,"length") == 2):
                    changes.h[k] = [HxOverrides.arrayGet(change, 1), HxOverrides.arrayGet(change, 0)]
                else:
    ...
    def toObject(self):
        ...
        def _hx_local_0(key):
            ...
            elif (connect_util_Util.isArray(value) and ((Reflect.field(value,"length") == 3))):
                o = obj.c
    ...
    @staticmethod
    def parse(obj):
        additions = haxe_ds_StringMap()
        def _hx_local_0(f):
            value = Reflect.field(Reflect.field(obj,"a"),f)
            additions.h[f] = value
        Lambda.iter(python_Boot.fields(Reflect.field(obj,"a")),_hx_local_0)
        deletions = haxe_ds_StringMap()
        def _hx_local_1(f):
            value = Reflect.field(Reflect.field(obj,"d"),f)
            deletions.h[f] = value
        Lambda.iter(python_Boot.fields(Reflect.field(obj,"d")),_hx_local_1)
        changes = haxe_ds_StringMap()
        def _hx_local_2(f):
            change = Reflect.field(Reflect.field(obj,"c"),f)
            if Std.isOfType(change,list):
                if (Reflect.field(change,"length") == 2):
                    changes.h[f] = change
                else:
                    value = connect_util_Diff.parseArray(change)
                    changes.h[f] = value
            else:
                value = connect_util_Diff.parse(change)
                changes.h[f] = value
        Lambda.iter(python_Boot.fields(Reflect.field(obj,"c")),_hx_local_2)
        diff = Type.createEmptyInstance(connect_util_Diff)
        setattr(diff,(("_hx_" + "a") if (("a" in python_Boot.keywords)) else (("_hx_" + "a") if (((((len("a") > 2) and ((ord("a"[0]) == 95))) and ((ord("a"[1]) == 95))) and ((ord("a"[(len("a") - 1)]) != 95)))) else "a")),additions)
        setattr(diff,(("_hx_" + "d") if (("d" in python_Boot.keywords)) else (("_hx_" + "d") if (((((len("d") > 2) and ((ord("d"[0]) == 95))) and ((ord("d"[1]) == 95))) and ((ord("d"[(len("d") - 1)]) != 95)))) else "d")),deletions)
        setattr(diff,(("_hx_" + "c") if (("c" in python_Boot.keywords)) else (("_hx_" + "c") if (((((len("c") > 2) and ((ord("c"[0]) == 95))) and ((ord("c"[1]) == 95))) and ((ord("c"[(len("c") - 1)]) != 95)))) else "c")),changes)
    ...
    @staticmethod
    def parseArray(arr):
        additions = (arr[0] if 0 < len(arr) else None)
        deletions = (arr[1] if 1 < len(arr) else None)
        def _hx_local_0(el):
            i = (el[0] if 0 < len(el) else None)
            if (len(el) == 3):
                return [i, (el[1] if 1 < len(el) else None), (el[2] if 2 < len(el) else None)]
            elif Std.isOfType((el[1] if 1 < len(el) else None),list):
                return [i, connect_util_Diff.parseArray((el[1] if 1 < len(el) else None))]
            else:
                return [i, connect_util_Diff.parse((el[1] if 1 < len(el) else None))]
        changes = list(map(_hx_local_0,(arr[2] if 2 < len(arr) else None)))
    ...
    def compareArrays(first,second):
        ...
        while (_g_current < len(_g_array)):
            ...
            x2 = (None if (connect_util_Diff.areEqual(a,b)) else ([i1, connect_util_Diff(a,b)] if ((connect_util_Util.isStruct(a) and connect_util_Util.isStruct(b))) else ([i1, connect_util_Diff.compareArrays(a,b)] if ((connect_util_Util.isArray(a) and connect_util_Util.isArray(b))) else [i1, a, b])))
            _g.append(x2)
        changeList = _g
        _g = []
        _g1 = 0
    ...
    def isSupported(v):
        cls = Type.getClass(v)
        isSimple = (((cls is None) or ((Type.getClassName(cls) is None))) or Std.isOfType(v,str))
        if (not ((isSimple or connect_util_Util.isArray(v)))):
            return connect_util_Util.isStruct(v)
        else:
            return True

    @staticmethod
    def checkStructs(first,second):
        if ((not connect_util_Util.isStruct(first)) or (not connect_util_Util.isStruct(second))):
...
class connect_util_Util:
    ...
    __slots__ = ()
    _hx_statics = ["replaceStrSensitiveData", "beautify", "beautifyObject", "maskFields", "isJson", "isJsonObject", "isJsonArray", "isArray", "isStruct", "createObjectDiff", "getLines", "addIdsToObject", "compactArray"]
    ...
    @staticmethod
    def beautify(text,compact,masked):
        try:
            return connect_util_Util.beautifyObject(python_lib_Json.loads(text,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon}))),compact,masked)
        except BaseException as _g:
            None
            return connect_util_Util.replaceStrSensitiveData(text,connect_Env.getLogger().getRegExMaskingList())

    @staticmethod
    def beautifyObject(obj,compact,masked):
        if compact:
            if (Type.typeof(obj) == ValueType.TObject):
                ...
                return haxe_format_JsonPrinter.print(mapped,None,"  ")
        else:
            return haxe_format_JsonPrinter.print((connect_util_Util.maskFields(obj) if masked else obj),None,"  ")
        ...
        if (Type.typeof(obj) == ValueType.TObject):
            ...
            while (_g < len(_g1)):
                ...
                if (maskedFields.indexOf(fieldName) != -1):
                    if (Type.typeof(Reflect.field(obj,fieldName)) == ValueType.TObject):
                        if python_Boot.hasField(Reflect.field(obj,fieldName),"id"):
                            value = Reflect.field(Reflect.field(obj,fieldName),"id")
                        ...
                        setattr(obj,(("_hx_" + fieldName) if ((fieldName in python_Boot.keywords)) else (("_hx_" + fieldName) if (((((len(fieldName) > 2) and ((ord(fieldName[0]) == 95))) and ((ord(fieldName[1]) == 95))) and ((ord(fieldName[(len(fieldName) - 1)]) != 95)))) else fieldName)),"HIDDEN FIELD")
                elif (((Type.typeof(obj) == ValueType.TObject) or Std.isOfType(obj,connect_util_Collection)) or Std.isOfType(obj,list)):
                    value1 = connect_util_Util.maskFields(Reflect.field(obj,fieldName))
    ...
    @staticmethod
    def getLines(text):
        windowsReplaced = StringTools.replace(text,"\r\n","\n")
        ...
        def _hx_local_4(field):
            ...
            if python_Boot.hasField(original,field):
                ...
                if (connect_util_Util.isStruct(value) and connect_util_Util.isStruct(originalValue)):
                    ...
                    def _hx_local_3():
                        _hx_local_2 = originalValue
                        if (Std.isOfType(_hx_local_2,list) or ((_hx_local_2 is None))):
                            _hx_local_2
                        else:
                            raise "Class cast error"
                        return _hx_local_2
                    originalValueArr = _hx_local_3()
                    value1 = connect_util_Util.compactArray(valueArr,originalValueArr)
                    setattr(out,(("_hx_" + field) if ((field in python_Boot.keywords)) else (("_hx_" + field) if (((((len(field) > 2) and ((ord(field[0]) == 95))) and ((ord(field[1]) == 95))) and ((ord(field[(len(field) - 1)]) != 95)))) else field)),value1)
                else:
                    setattr(out,(("_hx_" + field) if ((field in python_Boot.keywords)) else (("_hx_" + field) if (((((len(field) > 2) and ((ord(field[0]) == 95))) and ((ord(field[1]) == 95))) and ((ord(field[(len(field) - 1)]) != 95)))) else field)),value)
            else:
                setattr(out,(("_hx_" + field) if ((field in python_Boot.keywords)) else (("_hx_" + field) if (((((len(field) > 2) and ((ord(field[0]) == 95))) and ((ord(field[1]) == 95))) and ((ord(field[(len(field) - 1)]) != 95)))) else field)),value)
        Lambda.iter(python_Boot.fields(object),_hx_local_4)
        return out
...
connect_Flow.STEP_PARAM_ID = "__sdk_processor_step"
connect_Flow.STEP_PARAM_ID_TIER = "__sdk_processor_step_tier"
connect_api_impl_FulfillmentApiImpl.REQUESTS_PATH = "requests"
connect_api_impl_FulfillmentApiImpl.TEMPLATES_PATH = "templates"
connect_api_impl_FulfillmentApiImpl.ASSETS_PATH = "assets"
connect_api_impl_GeneralApiImpl.ACCOUNTS_PATH = "accounts"
connect_api_impl_GeneralApiImpl.CONVERSATIONS_PATH = "conversations"
connect_api_impl_GeneralApiImpl.PRODUCTS_PATH = "products"
connect_api_impl_GeneralApiImpl.CATEGORIES_PATH = "categories"
connect_api_impl_MarketplaceApiImpl.AGREEMENTS_PATH = "agreements"
connect_api_impl_MarketplaceApiImpl.LISTINGS_PATH = "listings"
connect_api_impl_MarketplaceApiImpl.LISTINGREQUESTS_PATH = "listing-requests"
connect_api_impl_MarketplaceApiImpl.MARKETPLACES_PATH = "marketplaces"
connect_api_impl_TierApiImpl.TCR_PATH = "tier/config-requests"
connect_api_impl_TierApiImpl.TA_PATH = "tier/accounts"
connect_api_impl_TierApiImpl.TC_PATH = "tier/configs"
connect_api_impl_UsageApiImpl.USAGE_FILES_PATH = "usage/files"
connect_api_impl_UsageApiImpl.USAGE_PRODUCTS_PATH = "usage/products"
connect_api_impl_UsageApiImpl.USAGE_RECORDS_PATH = "usage/records"
