...
def ipsi_clayton(u_values, theta, is_log=False):
    ...
    """
    x = np.array(u_values, dtype=np.float64)
...
def log_polyg(lx_var, alpha_var, d_var):
    ...
    ).reshape((int(d_var), int(lx_var.shape[0])), order='F') + np.repeat(
        x, int(d_var)
...
def pdf_gumbel(u_values, theta, is_log=False):
    ...
    alpha = 1.0 / theta
    assert 0.0 < alpha, "pdf_gumbel: 0.0 < alpha: alpha = " + str(alpha) + \
                        ", theta = " + str(theta)
    assert alpha <= 1.0, "pdf_gumbel: alpha <= 1.0: alpha = " + str(alpha) + \
                         " theta = " + str(theta)
    lx = alpha * lnt
