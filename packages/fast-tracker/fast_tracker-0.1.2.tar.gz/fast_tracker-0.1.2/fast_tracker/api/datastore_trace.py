...
from fast_tracker.core.datastore_node import DatastoreNode
...
class DatastoreTrace(TimeTrace):
    ...
    :type parent: :class:`fast_tracker.api.time_trace.TimeTrace`

    Usage::

        >>> import fast_tracker.agent
        >>> with fast_tracker.agent.DatastoreTrace(
    ...
    def __init__(self, product, target, operation,
        ...
        if kwargs:
            if len(kwargs) > 1:
                raise TypeError("无效的关键字参数:", kwargs)
...
def DatastoreTraceWrapper(wrapped, product, target, operation):
    ...
    :rtype: :class:`fast_tracker.common.object_wrapper.FunctionWrapper`
    ...
    Usage::
        ...
        >>> import fast_tracker.agent
        >>> import time
        >>> timed_sleep = fast_tracker.agent.DatastoreTraceWrapper(time.sleep,
        ...        'time', None, 'sleep')
...
def datastore_trace(product, target, operation):
    ...
    Usage::
        ...
        >>> import fast_tracker.agent
        >>> import time
        >>> @fast_tracker.agent.datastore_trace('time', None, 'sleep')
...
def wrap_datastore_trace(module, object_path, product, target, operation):
    ...
    Usage::
        ...
        >>> import fast_tracker.agent
        >>> import time
        >>> fast_tracker.agent.wrap_datastore_trace(time, 'sleep', 'time', None,
        ...        'sleep')
