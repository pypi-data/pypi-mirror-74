...
class lagou():
        ...
        # PC端接口
            ...
            while True:
                ...
                if is_need_verify_code:
                    res = self.session.get(self.vcode_url.format(time.time()), headers=self.headers)
                ...
                res = self.session.post(self.login_url, data=data, headers=login_headers)
                res_json = res.json()
                # 登录成功
                if res_json['state'] == 1:
                    self.__verifyTicket()
                ...
                # 需要验证码
                elif res_json['state'] == 10010:
                    is_need_verify_code = True
                # 账号或密码错误
                elif res_json['state'] in [201, 203, 220, 240, 241, 400]:
    ...
    def __verifyTicket(self):
        res = self.session.get(self.ticket_url, headers=self.verify_headers, allow_redirects=False)
        url = res.next.url
        url = re.sub('http://', 'https://', url)
        self.verify_headers['Host'] = 'www.lagou.com'
        res = self.session.get(url, headers=self.verify_headers, allow_redirects=False)
    '''获得challenge参数'''
