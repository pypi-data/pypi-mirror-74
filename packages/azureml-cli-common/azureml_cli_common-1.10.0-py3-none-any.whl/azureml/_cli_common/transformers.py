...
def transform_model_profile(result_tuple):
    ...
    # hack to differentiate the new profiling workflow
    if verbose or 'requestedCpu' in result:
        return result
...
def table_transform_model_profile(result):
    ...
    # hack to detect old profiling workflow
    if 'requestedCpu' not in result:
        return OrderedDict(
            [('name', result['name']),
             ('createdTime', _convert_time(result['createdTime']).strftime(TABLE_OUTPUT_TIME_FORMAT)),
             ('state', result['state']),
