...
name
    Name of {kind}.
account
    Name of the Coiled account which the {kind} belongs to.
    If not provided, will use the ``coiled.cloud.account`` configuration
    value.
"""
...
class Cloud:
    ...
    async def _create_software_environment(
        self, name=None, conda=None, container=None, log_output=sys.stdout
    ):
...
def create_software_environment(
    ...
    with Cloud() as cloud:
        return cloud.create_software_environment(
            name=name, conda=conda, container=container, log_output=log_output
        )
...
@delete_docstring
def delete_software_environment(name, account=None):
    with Cloud(account=account) as cloud:
