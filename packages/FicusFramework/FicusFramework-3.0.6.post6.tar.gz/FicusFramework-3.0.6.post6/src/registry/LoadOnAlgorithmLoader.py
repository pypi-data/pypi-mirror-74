...
def build_param_def(conf_param_defs:list, is_input_param:bool) -> list:
    ...
    for conf_param_def in conf_param_defs:
        ...
        if "model" in conf_param_def and len(conf_param_def["model"]) > 0:
            param_def = {
                "type": conf_param_def["type"].upper(),
                "description": conf_param_def["description"],
                "matchTags": conf_param_def["matchTags"],
                "matchParams": conf_param_def["matchParams"],
            ...
            for conf_field in conf_param_def["model"]:
                meta_model_field = {
                    "fieldName": conf_field["fieldName"],
                    "description": conf_field["description"],
                    "dataType": conf_field["dataType"].upper(),
                    "primaryKey": conf_field["primaryKey"],
                    "maxLength": conf_field["maxLength"],
                    "minLength": conf_field["minLength"],
                    "defaultValue": conf_field["defaultValue"],
                    "multiple": False
                }
...
def build_data_algorithm(conf_algorithm:dict) -> dict:
    data_algorithm = {
        "code": conf_algorithm["code"],
        "name": conf_algorithm["name"],
        "description": conf_algorithm["description"],
        "actorName": conf_algorithm["actorName"],
        "handler": conf_algorithm["handler"],
        "author": conf_algorithm["author"],
        "version": conf_algorithm["version"],
        "categories": conf_algorithm["categories"],
        "shardable": conf_algorithm["shardable"],
        "inputParamDefs": list(),
        "paramDefs": list(),
        "enable": True,
        "createTime": time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    if "inputParamDefs" in conf_algorithm and len(conf_algorithm["inputParamDefs"]) > 0:
        for conf_input_param_def in conf_algorithm["inputParamDefs"]:
            input_param_def = {
                "code" : conf_input_param_def["code"],
                "dataType": conf_input_param_def["dataType"].upper(),
                "description": conf_input_param_def["description"],
                "required": conf_input_param_def["required"]
            }
...
def register_algorithm(conf_algorithm:dict, task_handler_names:dict):
    ...
    if "version" not in conf_algorithm or conf_algorithm["version"] == "":
        raise IllegalArgumentException(f"自注册算法组件失败,version为空")
    if conf_algorithm["version"] < 1:
        raise IllegalArgumentException(f"自注册算法组件失败,version小于1")

    #构建信息
    data_algorithm = build_data_algorithm(conf_algorithm)

    #增加/更新
    if DataAlgorithmClient.exists(data_algorithm["code"]):
        ...
        if data_algorithm["version"] > exists_algorithm["version"]:
            ...
            log.warning(f"更新的版本号小于或等于-原始数值:{exists_algorithm['code']}，不做算法组件的处理。")
    else:
