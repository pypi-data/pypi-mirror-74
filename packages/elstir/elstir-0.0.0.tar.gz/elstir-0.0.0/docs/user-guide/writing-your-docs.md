...
the `elstir.yml` configuration file.
...
!!! note
    ...
    `.foo.md` or `.bar/baz.md`) are ignored by elstir, which matches the
...
elstir to the built site unaltered. See [how to link to images and media]
...
`index.md`, which elstir will render to `index.html` when building the site.
...
directory. Therefore, elstir will allow you to name your index pages as
...
it is a README file. However, when elstir renders your site, the file will be
...
The [nav](configuration.md#nav) configuration setting in your `elstir.yml` file
...
elstir pages must be authored in [Markdown][md], a lightweight markup language
...
elstir uses the [Python-Markdown] library to render Markdown documents to HTML.
...
implementations, elstir includes support for extending the Markdown syntax with
Python-Markdown [extensions]. See the elstir' [markdown_extensions]
configuration setting for details on how to enable extensions.

elstir includes some extensions by default, which are highlighted below.
...
elstir allows you to interlink your documentation by using regular Markdown
[links]. However, there are a few additional benefits to formatting those links
specifically for elstir as outlined below.
...
When the elstir build runs, these Markdown links will automatically be
...
!!! warning
    ...
    are adjusted by elstir to ensure they are always relative to the page. Absolute
...
The [toc] extension is used by elstir to generate an ID for every header in your
...
can set in your `elstir.yml` configuration file to alter the default behavior:
...
syntax does not meets the author's needs. elstir does not limit Markdown in this
regard. However, as all raw HTML is ignored by the Markdown parser, elstir is
...
elstir includes support for both YAML and MultiMarkdown style meta-data (often
...
are passed by elstir to the page template. Therefore, if a theme includes
...
In addition to displaying information in a template, elstir includes support for
a few predefined meta-data keys which can alter the behavior of elstir for that
...
: The template to use with the current page.
    ...
    By default, elstir uses the `main.html` template of a theme to render
...
: The "title" to use for the document.
    ...
    elstir will attempt to determine the title of a document in the following
    ways, in order:
...
the YAML keys are case sensitive and elstir expects keys to be all lowercase.
...
parser encounters an error, then elstir does not recognize the section as
...
!!! note
    ...
    elstir does not support YAML style deliminators (`---` or `...`) for
    MultiMarkdown style meta-data. In fact, elstir relies on the the presence or
    ...
    meta-data, elstir does not attempt to parse the content as MultiMarkdown
    style meta-data.
