...
That's it! Behind the scenes, elstir will build your docs and use the
[ghp-import] tool to commit them to the `gh-pages` branch and push the
`gh-pages` branch to GitHub.

Use `elstir gh-deploy --help` to get a full list of options available for the
`gh-deploy` command.
...
`elstir gh-deploy` command from there:

```sh
cd ../orgname.github.io/
elstir gh-deploy --config-file ../my-project/elstir.yml --remote-branch master
```

Note that you need to explicitly point to the `elstir.yml` configuration file as
...
so that elstir will work with your custom domain. You need to add a `CNAME` file
to the root of your [docs_dir]. The file must contain a single bare domain or
subdomain on a single line (see elstir' own [CNAME file] as an example). You may
...
`docs_dir`, elstir will include the file in your built site and push it to your
...
[CNAME file]: https://github.com/elstir/elstir/blob/master/docs/CNAME
...
and Bazaar. Read the Docs supports elstir out-of-the-box. Follow the
...
!!! note
    ...
    to use the [Read the Docs theme][theme] which ships with elstir. The various
    themes which may be referenced in Read the Docs' documentation are Sphinx
    specific themes and will not work with elstir.

[rtd]: https://readthedocs.org/
[instructions]: https://docs.readthedocs.io/en/stable/intro/getting-started-with-elstir.html
...
documentation generated by elstir. While it would be impossible to document how
to upload the docs to every hosting provider out there, the following guidelines
should provide some general assistance.

When you build your site (using the `elstir build` command), all of the files
are written to the directory assigned to the [site_dir] configuration option
(defaults to `"site"`) in your `elstir.yaml` config file. Generally, you will
...
When elstir builds the documentation it will include a 404.html file in the
