...
from elstir import utils, theme, plugins
from elstir.config.base import Config, ValidationError
...
class Deprecated(BaseConfigOption):
    ...
    def pre_validation(self, config, key_name):
        ...
        warning = ('The configuration option {} has been deprecated and '
                   'will be removed in a future release of elstir.'
                   ''.format(key_name))
        ...
        if key_name == 'dev_addr' and host in ['0.0.0.0', '::']:
            self.warnings.append(
                ("The use of the IP address '{}' suggests a production environment "
                 "or the use of a proxy to connect to the elstir server. However, "
                 "the elstir' server is intended for local development purposes only. "
    ...
    def post_validation(self, config, key_name):
        ...
        if key_name == 'pages' and config['pages'] is not None:
            ...
            warning = ("The 'pages' configuration option has been deprecated and will "
                       "be removed in a future release of elstir. Use 'nav' instead.")
            self.warnings.append(warning)
