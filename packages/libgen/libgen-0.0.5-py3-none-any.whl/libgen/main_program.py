...
def main():
    ...
    args.add_argument("download", help="download a file", action='store_true')
    ...
    if res.download:
        if res.url:
            ...
            res = requests.get(f'http://gen.lib.rus.ec/json.php?ids={res.id}&fields=Author,Title, md5')
            if res:
                ...
                title = data["title"]
                md5 = data["md5"]
                link = "http://93.174.95.29/main/{md5}"
