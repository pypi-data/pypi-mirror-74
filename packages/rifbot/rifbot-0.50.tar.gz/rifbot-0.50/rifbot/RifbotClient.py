...
from .StrategyRunRepositoryClient import StrategyRunRepositoryClient
...
class RifbotClient:
    ...
    async def strategyRunRepository(self):
        if 'strategyArchive' in self.clients:
            return self.clients['strategyArchive']

        service = await self.service('strategyArchive')
        client = StrategyRunRepositoryClient(service, self)
        self.clients['strategyArchive'] = client
