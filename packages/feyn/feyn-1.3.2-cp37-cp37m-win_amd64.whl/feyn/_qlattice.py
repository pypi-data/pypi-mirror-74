...
class QLattice:
    ...
    def get_qgraph(self, registers: List[str], output: str, stypes: Dict[str,str]={}, max_depth: int = 5, specs=None):
        """
        ...
        for reg in registers:
            ...
            if reg == output:
                if stype == 'c':
                    raise ValueError("Categorical output not supported")
                pattern = f"out:*(*)->{stype}"
            else:
                pattern = f"in:*({stype})->i"
    ...
    def _generate(self, specs: List[str], registers: Dict[str, str], max_depth) -> Dict:
        req = requests.post("%s/generate" % self.url,
