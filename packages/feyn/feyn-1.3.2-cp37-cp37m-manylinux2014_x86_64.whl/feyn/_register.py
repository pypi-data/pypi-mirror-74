from typing import Hashable, List, Dict, Tuple, Union, cast
import requests
...
class RegisterCollection:
    ...
    def delete(self, name: str):
        ...
        """
        req = requests.delete(f"{self.url}/register/{name}", headers=self.headers)
    ...
    def _get_register_list(self) -> List[Register]:
        resp = requests.get(f"{self.url}/register", headers=self.headers)
