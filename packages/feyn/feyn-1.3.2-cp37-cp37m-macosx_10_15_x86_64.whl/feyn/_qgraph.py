...
def _distance_mod(a,b,modulo):
    """
    Return the distance between a and b, respecting that a and b live in a torus space if size modulo.

    The parameters a, b, and modulo must be vectors of the same dimension.

    Arguments:
        a - a vector in the torus space
        b - a vector in the torus space
        modulo - the size of the torus space
    """
    res = []
...
class QGraph:
    ...
    """
    def __init__(self, qlattice, specs: List[str], registers: Dict[str, str]):
    ...
    def _refresh(self):
        ...
        qgraph_json = self.qlattice._generate(self.specs, self.registers, max_depth)
    ...
    def _template(self):
        res = QGraph(self.qlattice, self.specs, self.registers)
        res._filters = self._filters.copy()

        return res

    def filter(self, qfilter:feyn.filters.QGraphFilter):
    ...
    def _algo_x(self):
        ...
        for g in self._graphs:
            ...
            if g.age > 200*np.exp(-cnt/(density/(10*0.693))) + 1:
                # Too old
                continue
    ...
    def algo_density(self):
        ...
        for g in self._graphs:
            ...
            if g.age > 100*np.exp(-cnt/(density/(7*0.693))):
                # Too old
                continue
    ...
    def fit(self, data, n_samples:Optional[int]=None, loss_function=_feyn.DEFAULT_LOSS, show:Optional[FittingShowType]="graph", threads:int=1, sample_weights=None) -> None:
        """
    ...
    def sort(self, data, loss_function=_feyn.DEFAULT_LOSS, threads:int=1):
        """
        ...
        Returns:
            QGraph -- The QGraph itself
        """
        return self.trainer.sort(self, data, loss_function, threads)
    ...
    def best(self, n=5):
        ...
        for g in self._graphs:
            ...
            too_close = [_dist1_mod(x,other,self.qlattice.width)<5 for other in locs]
            if True in too_close:
                # This graph has an endpoint within 5 x-units from another, better graph
                continue
